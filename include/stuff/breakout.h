...
struct arch {
  ...
  struct toc_entry *toc_entries;   /* the table of contents entries */
...
struct object {
  ...
  struct segment_command *seg_bitcode; /* the 32-bit bitcode segment command */
  ...
  struct linkedit_data_command *link_opt_hint_cmd; /* the linker optimization
                                                      hint command, if any */
  struct section **sections;           /* array of 32-bit section structs */
  struct section_64 **sections64;      /* array of 64-bit section structs */
  struct dyld_info_command *dyld_info; /* the LC_DYLD_INFO command,if any */
  struct linkedit_data_command *dyld_exports_trie;   /* the exports trie */
  struct linkedit_data_command *dyld_chained_fixups; /* the fixups */
  struct linkedit_data_command
...
__private_extern__ struct ofile *
breakout(char *filename, struct arch **archs, uint32_t *narchs,
         enum bool calculate_input_prebind_cksum);

__private_extern__ struct ofile *
breakout_mem(void *membuf, uint32_t length, char *filename, struct arch **archs,
             uint32_t *narchs, enum bool calculate_input_prebind_cksum);

__private_extern__ void free_archs(struct arch *archs, uint32_t narchs);

__private_extern__ void
writeout(struct arch *archs, uint32_t narchs, char *output, unsigned short mode,
         enum bool sort_toc, enum bool commons_in_toc,
         enum bool force_64bit_toc, enum bool library_warnings,
         enum bool deterministic_libraries, uint32_t *throttle);
...
writeout_to_mem(struct arch *archs, uint32_t narchs, char *filename,
                ...
                enum bool library_warning, enum bool deterministic_libraries,
...
void warning_arch(struct arch *arch, struct member *member, char *format, ...)
#ifdef __GNUC__
    __attribute__((format(printf, 3, 4)))
#endif
    ;

void error_arch(struct arch *arch, struct member *member, char *format, ...)
#ifdef __GNUC__
    __attribute__((format(printf, 3, 4)))
#endif
    ;

void fatal_arch(struct arch *arch, struct member *member, char *format, ...)
#ifdef __GNUC__
    __attribute__((format(printf, 3, 4)))
#endif
    ;
