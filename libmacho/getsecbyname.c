...
#define DECLARE_VAR(var, type) static type *var##_pointer = NULL
#define SETUP_VAR(var)                                                         \
  if (var##_pointer == NULL) {                                                 \
    _dyld_lookup_and_bind(STRINGIFY(_##var), (uint32_t *)&var##_pointer,       \
                          NULL);                                               \
  }
 ...
 */
const struct section *getsectbynamefromheader(struct mach_header *mhp,
                                              const char *segname,
                                              const char *sectname) {
 ...
 * Otherwise it returns zero.
 */
const struct section_64 *getsectbynamefromheader_64(struct mach_header_64 *mhp,
                                                    const char *segname,
                                                    const char *sectname) {
 ...
 */
const struct section *getsectbynamefromheaderwithswap(struct mach_header *mhp,
                                                      const char *segname,
                                                      const char *sectname,
                                                      int fSwap) {
  struct segment_command *sgp;
...
getsectbynamefromheaderwithswap_64(struct mach_header_64 *mhp,
...
const struct section_64 *getsectbyname(const char *segname,
                                       const char *sectname) {
 ...
 */
char *getsectdata(const char *segname, const char *sectname,
                  unsigned long *size) {
#ifndef __LP64__
...
uint8_t *getsectiondata(
  ...
  unsigned startOffset = (mh->magic == MH_MAGIC_64)
                             ? sizeof(struct mach_header_64)
                             : sizeof(struct mach_header);
  struct load_command *lc =
      (struct load_command *)((uint8_t *)mh + startOffset);
  uintptr_t slide = 0;
    ...
    if (lc->cmd == LC_SEGMENT_64) {
      ...
      if (strncmp(seg->segname, segname, 16) == 0) {
        const struct section_64 *sect =
            (struct section_64 *)((uint8_t *)seg +
                                  sizeof(struct segment_command_64));
        for (int j = 0; j < seg->nsects; ++j) {
          if ((strncmp(sect[j].sectname, sectname, 16) == 0) &&
              (strncmp(sect[j].segname, segname, 16) == 0)) {
            *size = sect[j].size;
      ...
      if (strncmp(seg->segname, segname, 16) == 0) {
        const struct section *sect =
            (struct section *)((uint8_t *)seg + sizeof(struct segment_command));
        for (int j = 0; j < seg->nsects; ++j) {
          if ((strncmp(sect[j].sectname, sectname, 16) == 0) &&
              (strncmp(sect[j].segname, segname, 16) == 0)) {
            *size = sect[j].size;
...
#ifndef __LP64__
uint8_t *getsegmentdata(const struct mach_header *mhp, const char *segname,
                        unsigned long *size) {
  struct segment_command *sgp;
  uintptr_t slide;
  uint32_t i;
 ...
 */
char *getsectdatafromheader(struct mach_header *mhp, const char *segname,
                            const char *sectname, uint32_t *size) {
  const struct section *sp;
 ...
 * Otherwise it returns zero for the pointer and the size.
 */
char *getsectdatafromheader_64(struct mach_header_64 *mhp, const char *segname,
 ...
 */
void *getsectdatafromFramework(const char *FrameworkName, const char *segname,
