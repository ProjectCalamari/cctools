...
static uint32_t address_func(uint32_t size, uint32_t headers_size);
#endif

static void count_func_symbols(struct nlist *symbols,
                               struct nlist_64 *symbols64, uint32_t nsymbols,
                               char *strings, uint32_t strsize);

static void load_func_symbols(struct nlist *symbols, struct nlist_64 *symbols64,
                              uint32_t nsymbols, char *strings,
                              uint32_t strsize, uint64_t vmaddr_slide);

static enum bool funcsymbol(uint8_t n_type, uint8_t n_sect, char *name);

static int valcmp(nltype *p1, nltype *p2);

static void count_N_SO_stabs(struct nlist *symbols, struct nlist_64 *symbols64,
                             uint32_t nsymbols, char *strings,
                             uint32_t strsize);

static void load_files(struct nlist *symbols, struct nlist_64 *symbols64,
...
good:
  ...
  if ((ofile.mh == NULL && ofile.mh64 == NULL) ||
      (ofile.mh_filetype != MH_EXECUTE && ofile.mh_filetype != MH_DYLIB &&
    ...
    if (lc->cmd == LC_SEGMENT) {
      ...
      for (j = 0; j < sg->nsects; j++) {
        if (strcmp(s->sectname, SECT_TEXT) == 0 &&
          ...
          textspace = (unsigned char *)ofile.object_addr + s->offset;
      ...
      for (j = 0; j < sg64->nsects; j++) {
        if (strcmp(s64->sectname, SECT_TEXT) == 0 &&
          ...
          textspace = (unsigned char *)ofile.object_addr + s64->offset;
      ...
      if (ofile_map(lib_name, &host_arch_flag, NULL, &lib_ofile, FALSE) ==
          FALSE)
        goto done1;
      ...
      for (j = 0; j < lib_ofile.mh->ncmds; j++) {
        if (lib_st == NULL && lib_lc->cmd == LC_SYMTAB) {
          ...
          count_func_symbols(
              (struct nlist *)(lib_ofile.object_addr + lib_st->symoff),
              lib_st->nsyms, lib_ofile.object_addr + lib_st->stroff,
              lib_st->strsize);
          break;
          ...
          for (k = 0; k < sg->nsects; k++) {
            if (strcmp(s->sectname, SECT_TEXT) == 0 &&
                strcmp(s->segname, SEG_TEXT) == 0) {
              shlib_text_ranges = reallocate(
                  shlib_text_ranges,
                  (nshlib_text_ranges + 1) * sizeof(struct shlib_text_range));
              shlib_text_ranges[nshlib_text_ranges].lowpc = s->addr;
              shlib_text_ranges[nshlib_text_ranges].highpc = s->addr + s->size;
              nshlib_text_ranges++;
            }
            s++;
          }
        }
        lib_lc = (struct load_command *)((char *)lib_lc + lib_lc->cmdsize);
      }
    done_and_unmap1:
  ...
  if (st != NULL) {
    ...
    load_files(symbols, symbols64, st->nsyms, ofile.object_addr + st->stroff,
               st->strsize);
  }
#ifdef notdef
  lc = ofile.load_commands;
  ...
  qsort(nl, nname, sizeof(nltype), (int (*)(const void *, const void *))valcmp);
#ifdef DEBUG
  if (debug & AOUTDEBUG) {
    for (i = 0; i < nname; i++) {
      printf("[getnfile] ");
...
#ifdef __OPENSTEP__
void get_rld_state_symbols(void) {
  ...
  for (i = 0; i < grld_nloaded_states; i++) {
    ...
    if (rld_load(stream, &(headers[i]), grld_loaded_state[i].object_filenames,
  ...
  for (i = 0; i < grld_nloaded_states; i++) {
    ...
    lc = (struct load_command *)(object_addr + sizeof(struct mach_header));
    for (j = 0; j < headers[i]->ncmds; j++) {
      if (st == NULL && lc->cmd == LC_SYMTAB) {
        st = (struct symtab_command *)lc;
        count_func_symbols((struct nlist *)(object_addr + st->symoff),
                           st->nsyms, object_addr + st->stroff, st->strsize);
      }
      lc = (struct load_command *)((char *)lc + lc->cmdsize);
    }
  }
  ...
  for (i = 0; i < grld_nloaded_states; i++) {
    ...
    for (j = 0; j < headers[i]->ncmds; j++) {
      if (st == NULL && lc->cmd == LC_SYMTAB) {
        st = (struct symtab_command *)lc;
        load_func_symbols((struct nlist *)(object_addr + st->symoff), st->nsyms,
                          object_addr + st->stroff, st->strsize, 0);
      }
      lc = (struct load_command *)((char *)lc + lc->cmdsize);
    }
  }
...
void get_dyld_state_symbols(void) {
  ...
  for (i = 0; i < image_count; i++) {
    ...
    for (j = 0; j < ncmds; j++) {
      if (st == NULL && lc->cmd == LC_SYMTAB) {
        st = (struct symtab_command *)lc;
        if (ofiles[i].mh != NULL) {
          symbols = (struct nlist *)(ofiles[i].object_addr + st->symoff);
          symbols64 = NULL;
        } else {
          symbols64 = (struct nlist_64 *)(ofiles[i].object_addr + st->symoff);
          symbols = NULL;
        }
        count_func_symbols(symbols, symbols64, st->nsyms,
                           ofiles[i].object_addr + st->stroff, st->strsize);
      }
...
void get_text_min_max(uint64_t *text_min, uint64_t *text_max) {
  uint32_t i, j, ncmds;
  struct load_command *lc;
  struct segment_command *sg;
  struct segment_command_64 *sg64;
