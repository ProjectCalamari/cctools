...
int main(int argc, const char *argv[]) {
  ...
  while (argc > 0) {
    if (read_options && *argv && '-' == **argv) {
      if (0 == strcmp("-o", *argv) || 0 == strcmp("-output", *argv)) {
        argv++;
        argc--;
        if (out_path) {
  ...
  if (0 == err) {
    for (uint32_t icmd = 0; icmd < lcmds->count; ++icmd) {
      if (lcmds->items[icmd]->cmd == LC_SYMTAB) {
        struct symtab_command *st = (struct symtab_command *)lcmds->items[icmd];

        struct nlist_64 *symbols = calloc(st->nsyms, sizeof(*symbols));
    ...
    if (LC_SEGMENT == lc.cmd) {
      ...
      if (lc.cmdsize < sizeof(struct segment_command)) {
        fprintf(stderr,
                ...
                gProgramName, fb->path, fb->mh.cputype, fb->mh.cpusubtype, icmd,
                lc.cmd, lc.cmdsize);
        lcmds_free(lcmds);
        return NULL;
      }
...
void usage(const char *__restrict format, ...) {
  ...
  fprintf(stderr, "usage: %s -output <output> <file>\n", basename);
