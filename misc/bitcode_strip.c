...
static void process(struct arch *archs, uint32_t narchs);

static enum bool check_object(struct arch *arch, struct member *member,
                              struct object *object);

static void strip_bitcode_segment(struct arch *arch, struct member *member,
                                  struct object *object);

static void leave_just_bitcode_segment(struct arch *arch, struct member *member,
                                       struct object *object);

static void strip_bitcode_from_load_commands(struct arch *arch,
                                             struct object *object,
                                             enum bool resign);

static void leave_only_bitcode_load_commands(struct arch *arch,
                                             struct object *object,
                                             enum bool keeping_plist);

static void make_ld_process_mh_object(struct arch *arch, struct member *member,
                                      struct object *object);

static void make_ld_r_object(struct arch *arch, struct object *object);

static void setup_symbolic_info_for_mh_object(struct arch *arch,
...
int main(int argc, char **argv, char **envp) {
  ...
  for (i = 1; i < argc; i++) {
    if (strcmp(argv[i], "-o") == 0) {
      ...
      if (input != NULL) {
        error("more than one input file specified (%s and %s)", argv[i], input);
        usage();
      }
      input = argv[i];
    }
  ...
  writeout(archs, narchs, output_file, 0777, TRUE, FALSE, FALSE, FALSE, FALSE,
           NULL);
 ...
 */
static void usage(void) {
  fprintf(stderr, "Usage: %s input [-r | -m | -l] [-keep_cs] -o output\n",
          progname);
  exit(EXIT_FAILURE);
}
    ...
    if (archs[i].type == OFILE_ARCHIVE) {
      for (j = 0; j < archs[i].nmembers; j++) {
        if (archs[i].members[j].type == OFILE_Mach_O) {
          ...
          if (check_object(archs + i, archs[i].members + j, object) == FALSE)
            return;
          if (rflag || mflag ||
              (object->mh_filetype != MH_OBJECT &&
               object->seg_bitcode == NULL && object->seg_bitcode64 == NULL))
            strip_bitcode_segment(archs + i, archs[i].members + j, object);
          else
            leave_just_bitcode_segment(archs + i, archs[i].members + j, object);
        }
      }
      /*
      ...
      for (j = 0; j < archs[i].nmembers; j++) {
        ...
        if (archs[i].members[j].member_long_name == TRUE) {
          size = rnd32(archs[i].members[j].member_name_size, sizeof(int64_t));
      ...
      if (check_object(archs + i, NULL, object) == FALSE)
        return;
      if (rflag || mflag ||
          (object->mh_filetype != MH_OBJECT && object->seg_bitcode == NULL &&
...
static enum bool check_object(struct arch *arch, struct member *member,
  ...
  if (object->mh_filetype != MH_OBJECT &&
    ...
    fatal_arch(arch, member,
               "can't be used on a file not built for "
    ...
    for (i = 0; i < mh_ncmds && first_bitcode_section_ordinal == 0; i++) {
      if (lc->cmd == LC_SEGMENT) {
        ...
        if (sg == object->seg_bitcode && sg->nsects > 0) {
          ...
          last_bitcode_section_ordinal = section_ordinal + sg->nsects;
        }
        ...
        if (sg64 == object->seg_bitcode64 && sg64->nsects > 0) {
          first_bitcode_section_ordinal = section_ordinal;
          last_bitcode_section_ordinal = section_ordinal + sg64->nsects;
        }
      ...
      if (object->seg_bitcode != NULL) {
        ...
        for (i = 0; i < object->st->nsyms; i++) {
          if ((symbols64[i].n_type & N_TYPE) == N_SECT &&
              symbols64[i].n_sect >= first_bitcode_section_ordinal &&
...
static void strip_bitcode_segment(struct arch *arch, struct member *member,
   ...
   */
  segalignexp = get_seg_align(object->mh, object->mh64, object->load_commands,
                              FALSE, object->object_size, arch->file_name);
  ...
  if (object->mh != NULL) {
    ...
    if (object->seg_bitcode64 != NULL) {
      ...
      object->seg_linkedit64->fileoff -= object->seg_bitcode64->filesize;
    }
    object->input_sym_info_size = (uint32_t)object->seg_linkedit64->filesize;
    ...
    if (has_bitcode && mflag) {
      if (object->seg_bitcode64->filesize >= segalign)
        ...
        bitcode_marker_size = (uint32_t)object->seg_bitcode64->filesize;
      object->output_new_content = allocate(bitcode_marker_size);
      memset(object->output_new_content, '\0', bitcode_marker_size);
      object->output_new_content_size = bitcode_marker_size;
  ...
  if (object->dyld_info != NULL) {
    ...
    if (object->dyld_info->export_size != 0)
      ...
      dyld_info_end =
          object->dyld_info->lazy_bind_off + object->dyld_info->lazy_bind_size;
    else if (object->dyld_info->weak_bind_size != 0)
      dyld_info_end =
          object->dyld_info->weak_bind_off + object->dyld_info->weak_bind_size;
  ...
  if (object->dyld_chained_fixups != NULL) {
    ...
    if (rflag) {
      ...
      /* we removed a segment, so reduce the segment count in the fixup chain
       * header */
      starts->seg_count -= 1;
    }
  ...
  if (object->code_sign_drs_cmd != NULL) {
    if (has_bitcode) {
       ...
       */
      if (object->mh != NULL)
        object->seg_linkedit->filesize -= object->code_sign_drs_cmd->datasize;
      else
        object->seg_linkedit64->filesize -= object->code_sign_drs_cmd->datasize;
  ...
  if (object->st != NULL && object->st->nsyms != 0) {
    if (object->mh != NULL) {
      ...
      object->output_symbols64 =
          (struct nlist_64 *)(object->object_addr + object->st->symoff);
      if (object->object_byte_sex != get_host_byte_sex())
        swap_nlist_64(object->output_symbols64, object->st->nsyms,
                      get_host_byte_sex());
  ...
  if (object->dyst != NULL) {
    ...
    if (object->dyst->ntoc != 0) {
      object->output_tocs =
          (struct dylib_table_of_contents *)(object->object_addr +
                                             object->dyst->tocoff);
    ...
    if (object->dyst->nmodtab != 0) {
      if (object->mh != NULL) {
        object->output_mods = (struct dylib_module *)(object->object_addr +
                                                      object->dyst->modtaboff);
        object->output_mods64 = NULL;
      } else {
        object->output_mods64 =
            (struct dylib_module_64 *)(object->object_addr +
                                       object->dyst->modtaboff);
    ...
    if (object->dyst->nextrefsyms != 0) {
      object->output_refs =
          (struct dylib_reference *)(object->object_addr +
  ...
  if (object->code_sig_cmd != NULL) {
    ...
    if (keep_cs_flag == FALSE && has_ld_sig == FALSE && has_bitcode) {
      ...
      if (end_of_string_table != 0)
        alignment_padding = object->code_sig_cmd->dataoff - end_of_string_table;
      else
        alignment_padding = 0;
      if (object->mh != NULL)
...
static void leave_just_bitcode_segment(struct arch *arch, struct member *member,
  ...
  if (object->mh_filetype == MH_OBJECT) {
    fatal_arch(arch, member,
               "Can't use the -l option on .o files "
  ...
  if (object->mh != NULL) {
          ...
          if (sg->nsects > 0) {
            ...
            for (j = 0; j < sg->nsects; j++) {
              if (strcmp(s->segname, SEG_TEXT) == 0 &&
                  strcmp(s->sectname, "__info_plist") == 0 && s->size != 0 &&
                  plist == NULL) {
                plist = s;
                text = sg;
              }
    ...
    if (start_offset == 0)
      start_offset = sizeof(struct mach_header) + object->mh->sizeofcmds;

    object->object_size -= (object->seg_linkedit->fileoff - start_offset);
    ...
    if (plist != NULL) {
      ...
      object->output_new_content = allocate(object->output_new_content_size);
      memcpy(object->output_new_content, object->object_addr + plist->offset,
             plist->size);
    ...
    if (start_offset == 0)
      start_offset = sizeof(struct mach_header_64) + object->mh64->sizeofcmds;

    object->object_size -= (object->seg_linkedit64->fileoff - start_offset);

    if (plist64 != NULL) {
      object->output_new_content =
          object->object_addr + object->seg_bitcode64->fileoff;
      object->output_new_content_size =
          (uint32_t)(object->seg_bitcode64->filesize + plist64->size);
      object->output_new_content = allocate(object->output_new_content_size);
      memcpy(object->output_new_content, object->object_addr + plist64->offset,
             plist64->size);
...
static void strip_bitcode_from_load_commands(struct arch *arch,
  ...
  if (mh_sizeofcmds > new_sizeofcmds)
    memset((char *)object->load_commands + new_sizeofcmds, '\0',
           (mh_sizeofcmds - new_sizeofcmds));
  if (object->mh != NULL) {
  ...
  if (resign == TRUE && object->code_sig_cmd != NULL) {
    int err =
        codedir_create_object(output_file, object->object_addr,
                              object->object_size, &object->output_codedir);
  ...
  for (i = 0; i < mh_ncmds; i++) {
    if (lc1->cmd == LC_SEGMENT) {
      ...
      if (strcmp(sg->segname, "__LLVM") != 0 &&
        ...
        if (keeping_plist == FALSE || strcmp(sg->segname, SEG_TEXT) != 0) {
          sg->vmaddr = 0;
          sg->vmsize = 0;
          sg->fileoff = 0;
          sg->filesize = 0;
        ...
        s = (struct section *)((char *)sg + sizeof(struct segment_command));
        for (j = 0; j < sg->nsects; j++) {
          if (keeping_plist == FALSE || strcmp(s->segname, SEG_TEXT) != 0 ||
      ...
      if (strcmp(sg64->segname, "__LLVM") != 0 &&
        ...
        if (keeping_plist == FALSE || strcmp(sg64->segname, SEG_TEXT) != 0) {
          sg64->vmaddr = 0;
          sg64->vmsize = 0;
          sg64->fileoff = 0;
          sg64->filesize = 0;
        ...
        s64 = (struct section_64 *)((char *)sg64 +
                                    sizeof(struct segment_command_64));
        for (j = 0; j < sg64->nsects; j++) {
          if (keeping_plist == FALSE || strcmp(s64->segname, SEG_TEXT) != 0 ||
              strcmp(s64->sectname, "__info_plist") != 0) {
            s64->addr = 0;
            s64->size = 0;
            s64->offset = 0;
      ...
      new_sizeofcmds += lc2->cmdsize;
      lc2 = (struct load_command *)((char *)lc2 + lc2->cmdsize);
    } else if (keep_cs_flag == TRUE || (lc1->cmd != LC_CODE_SIGNATURE &&
                                        lc1->cmd != LC_DYLIB_CODE_SIGN_DRS)) {
      if (lc1->cmd == LC_MAIN) {
...
static void setup_symbolic_info_for_mh_object(struct arch *arch,
  ...
  if (object->seg_linkedit != NULL || object->seg_linkedit64 != NULL)
    fatal_arch(arch, member,
               "malformed MH_OBJECT should not contain a "
               "link edit segment");
  if (object->dyst != NULL && object->dyst->nlocrel != 0)
    fatal_arch(arch, member,
               "malformed MH_OBJECT should not contain "
  ...
  if (object->st->nsyms != 0 && object->st->symoff < offset)
  ...
  if (object->dyst != NULL && object->dyst->nextrefsyms != 0)
    fatal_arch(arch, member,
               ...
               "table");
  if (object->st->strsize != 0 && object->st->stroff < offset)
  ...
  if (object->st != NULL && object->st->nsyms != 0) {
    if (object->mh != NULL) {
      object->input_sym_info_size += object->st->nsyms * sizeof(struct nlist);
      object->output_symbols =
          (struct nlist *)(object->object_addr + object->st->symoff);
      if (object->object_byte_sex != get_host_byte_sex())
        swap_nlist(object->output_symbols, object->st->nsyms,
