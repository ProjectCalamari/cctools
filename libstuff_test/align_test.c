...
static void mh_add_segment32(struct mach_header *mh,
                             struct load_command **lcmds, uint32_t vmaddr,
                             uint32_t nsects) {
  ...
  sg.nsects = nsects;
  sg.cmdsize =
      (sizeof(struct segment_command) + sizeof(struct section) * sg.nsects);

  mh_add_load_command(mh, lcmds, &sg, sizeof(sg));
}

static void mh_add_section32(struct mach_header *mh,
                             struct load_command **lcmds, uint32_t align) {
  struct section sc = {0};
  sc.align = align;
...
static void test_get_align_none(void) {
  ...
  struct mach_header mh32 = {
      MH_MAGIC, CPU_TYPE_ARM, CPU_SUBTYPE_ARM_V7, MH_CORE, 0, 0, 0,
  };
  struct mach_header_64 mh64 = {
      MH_MAGIC_64, CPU_TYPE_ARM64, CPU_SUBTYPE_ARM64E, MH_CORE, 0, 0, 0, 0,
  };
...
static void test_get_align_objects_32(void) {
  ...
  struct mach_header mh = {
      MH_MAGIC, CPU_TYPE_ARM, CPU_SUBTYPE_ARM_V7, MH_OBJECT, 0, 0, 0,
  };
  ...
  align =
      get_seg_align(&mh, NULL, lcmds, FALSE, sizeof(mh) + mh.sizeofcmds, "");
...
static void test_get_align_objects_64(void) {
  ...
  struct mach_header_64 mh = {
      MH_MAGIC_64, CPU_TYPE_ARM64, CPU_SUBTYPE_ARM64E, MH_OBJECT, 0, 0, 0,
  };
  struct load_command *lcmds = NULL;
  uint32_t align;
  ...
  align =
      get_seg_align(NULL, &mh, lcmds, FALSE, sizeof(mh) + mh.sizeofcmds, "");
...
static void test_get_align_dylibs_32(void) {
  // create an MH_DYLIB
  struct mach_header mh = {
      MH_MAGIC, CPU_TYPE_ARM, CPU_SUBTYPE_ARM_V7, MH_DYLIB, 0, 0, 0,
  };
  struct load_command *lcmds = NULL;
  uint32_t align;
...
static void test_get_align_dylibs_64(void) {
  // create an MH_DYLIB
  struct mach_header_64 mh = {
      MH_MAGIC_64, CPU_TYPE_ARM64, CPU_SUBTYPE_ARM64E, MH_DYLIB, 0, 0, 0,
  };
  struct load_command *lcmds = NULL;
  uint32_t align;
...
static int test_main(void) {
  ...
  if (!err)
    err = test_add("test get_seg_align with bad input", test_get_align_none);
  if (!err)
    err = test_add("test get_seg_align 32-bit objects",
                   test_get_align_objects_32);
  if (!err)
    err = test_add("test get_seg_align 64-bit objects",
                   test_get_align_objects_64);
  if (!err)
    err =
        test_add("test get_seg_align 32-bit dylibs", test_get_align_dylibs_32);
  if (!err)
    err =
        test_add("test get_seg_align 64-bit dylibs", test_get_align_dylibs_64);
