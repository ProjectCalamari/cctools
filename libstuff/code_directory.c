...
int codedir_is_linker_signed(const char *data, uint32_t size) {
#if 1
  // HACK: libcodedirectory.h is in both the macOS SDK in /usr/local/include,
  // and in the tool chain at /usr/include. but there is no way to control
  // clang's search path to look in the toolchain first. So, declare newer API
  // locally. Once this new header is in all SDKs we can remove this. Can't use
  // real function name because it returns an enum type and we cannot define the
  // enum here because it will cause a duplicate definition when the header is
  // updated.
  extern int my_libcd_is_blob_a_linker_signature(
      const uint8_t *buf, size_t buf_len,
      int *is_linker_signature) __asm("_libcd_is_blob_a_linker_signature");
  int ret;
  int linker_signed = 0;

  ret = my_libcd_is_blob_a_linker_signature((const uint8_t *)data, (size_t)size,
                                            &linker_signed);
  ...
  ret = libcd_is_blob_a_linker_signature((const uint8_t *)data, (size_t)size,
                                         &linker_signed);
  switch (ret) {
 ...
 */
struct codedir *codedir_create(const char *output_path, uint32_t filesize,
                               uint32_t ptralign, uint32_t platform,
                               uint32_t minos) {
  ...
  if (libcd_set_hash_types_for_platform_version(codedir->handle, (int)platform,
                                                (int)minos)) {
...
static int read_load_command(char *buffer, uint32_t buflen, uint32_t mh_size,
                             uint32_t sizeofcmds, int i, off_t lc_off,
                             const struct load_command *lc, const char *name,
                             void *out_buf, uint32_t out_size, bool swap,
                             swapfn_t swapfn) {
  if (!name)
    name = "unknown";
  ...
  if (mh_size + sizeofcmds < lc_off + out_size) {
    error("load command #%d (%s) end %llu extends past sizeofcmds: %u", i, name,
          lc_off + sizeof(lc), sizeofcmds);
    return -1;
  }
 ...
 */
int codedir_create_object(const char *output_path, char *buffer,
                          uint32_t buflen, struct codedir **out_codedir) {
  // read magic
  ...
  for (int i = 0; i < ncmds; ++i) {
    ...
    if (read_load_command(buffer, buflen, sizeofhdr, sizeofcmds, i, offset,
                          NULL, NULL, &lc, sizeof(lc), swap,
                          (swapfn_t)swap_load_command))
      return -1;
    ...
    case LC_SEGMENT: {
      ...
      if (read_load_command(buffer, buflen, sizeofhdr, sizeofcmds, i, offset,
                            &lc, "LC_SEGMENT", &sg, sizeof(sg), swap,
                            (swapfn_t)swap_segment_command))
        return -1;
    ...
    case LC_SEGMENT_64: {
      ...
      if (read_load_command(buffer, buflen, sizeofhdr, sizeofcmds, i, offset,
                            &lc, "LC_SEGMENT_64", &sg, sizeof(sg), swap,
    ...
    case LC_CODE_SIGNATURE: {
      if (read_load_command(buffer, buflen, sizeofhdr, sizeofcmds, i, offset,
                            &lc, "LC_CODE_SIGNATURE", &cs, sizeof(cs), swap,
    ...
    case LC_VERSION_MIN_TVOS: {
      ...
      if (read_load_command(buffer, buflen, sizeofhdr, sizeofcmds, i, offset,
                            &lc, lcname, &vm, sizeof(vm), swap,
                            (swapfn_t)swap_version_min_command))
        return -1;
      lcname = NULL;
    ...
    case LC_BUILD_VERSION: {
      ...
      if (read_load_command(buffer, buflen, sizeofhdr, sizeofcmds, i, offset,
                            &lc, "LC_BUILD_VERSION", &bv, sizeof(bv), swap,
  ...
  uint32_t filesize =
      (cs.dataoff ? cs.dataoff : (uint32_t)(le_fileoff + le_filesize));

  struct codedir *codedir =
      codedir_create(output_path, filesize, ptralign, platform, min_vers);
  if (!codedir) {
    return -1;
  }
 ...
 */
int codedir_write(const struct codedir *codedir, const char *input,
  ...
  if (sret != LIBCD_SERIALIZE_SUCCESS) {
    ...
    switch (sret) {
    case LIBCD_SERIALIZE_SUCCESS:
      s = "SUCCESS";
      break;
    case LIBCD_SERIALIZE_WRITE_ERROR:
      s = "WRITE_ERROR";
      break;
    case LIBCD_SERIALIZE_READ_PAGE_ERROR:
      s = "READ_PAGE_ERROR";
      break;
    case LIBCD_SERIALIZE_INVALID_THREAD_COUNT:
      s = "INVALID_THREAD_COUNT";
      break;
    case LIBCD_SERIALIZE_SIGNATURE_FAILURE:
      s = "SIGNATURE_FAILURE";
      break;
    case LIBCD_SERIALIZE_EMPTY:
      s = "EMPTY";
      break;
    case LIBCD_SERIALIZE_NO_MEM:
      s = "NO_MEM";
      break;
    }
 ...
 */
int codedir_write_object(const struct codedir *codedir, char *object_addr,
