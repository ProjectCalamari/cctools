...
extern void
get_dyld_bind_info(const uint8_t *start, /* inputs */
                   const uint8_t *end, const char **dylibs, uint32_t ndylibs,
                   struct segment_command **segs, uint32_t nsegs,
                   struct segment_command_64 **segs64, uint32_t nsegs64,
                   enum bool swapped, char *object_addr, uint64_t object_size,
...
extern void print_dyld_bind_info(struct dyld_bind_info *dbi, uint64_t ndbi);
...
extern const char *
get_dyld_bind_info_symbolname(uint64_t address, struct dyld_bind_info *dbi,
                              uint64_t ndbi, struct dyld_bind_info **dbi_index,
...
extern void get_dyld_chained_fixups(
    ...
    const uint8_t *end, const char **dylibs, uint32_t ndylibs,
    struct segment_command **segs, uint32_t nsegs,
    struct segment_command_64 **segs64, uint32_t nsegs64, enum bool swapped,
    char *object_addr, uint64_t object_size,
    struct dyld_bind_info **dbi, /* outputs */
    uint64_t *ndbi, enum chain_format_t *chain_format,
    enum chain_header_t header_type, enum bool print_errors);
 ...
 */
extern uint64_t get_chained_rebase_value(uint64_t chain_value,
                                         enum chain_format_t chain_format,
                                         uint64_t textbase);

extern void print_dyld_chained_fixups(struct load_command *load_commands,
