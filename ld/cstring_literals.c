 ...
 */
__private_extern__ void cstring_merge(struct cstring_data *data,
 ...
 * to be ordered.
 */
__private_extern__ void cstring_order(struct cstring_data *data,
                                      struct merged_section *ms) {
#ifndef RLD
  ...
  if (dead_strip == TRUE) {
    ...
    cstring_order_lines =
        allocate(sizeof(struct cstring_order_line) * (line_number - 1));
    data->cstring_load_order_data->order_line_buffer = buffer;
    data->cstring_load_order_data->cstring_order_lines = cstring_order_lines;
    data->cstring_load_order_data->ncstring_order_lines = (line_number - 1);
  }
  ...
  for (i = 0; i < ms->order_size; i++) {
    ...
    if (dead_strip == TRUE)
      cstring_order_lines[line_number - 1].output_offset = output_offset;
 ...
 * order file if any.
 */
__private_extern__ void cstring_reset_live(struct cstring_data *data,
                                           struct merged_section *ms) {
#ifndef RLD
  ...
  if (ms->order_filename != NULL) {
    ...
    cstring_order_lines = data->cstring_load_order_data->cstring_order_lines;
    ncstring_order_lines = data->cstring_load_order_data->ncstring_order_lines;
    for (i = 0; i < ncstring_order_lines; i++) {
      ...
      if (live) {
        ...
        get_cstring_from_sectorder(ms, &character_index, buffer, line_number,
                                   1);
 ...
 */
__private_extern__ void get_cstring_from_sectorder(struct merged_section *ms,
                                                   unsigned long *index,
                                                   char *buffer,
    ...
    if (ms->order_addr[i] == '\\') {
      if (i + 1 >= ms->order_size || ms->order_addr[i + 1] == '\0')
        fatal("format error in -sectorder file: %s line %lu "
              ...
              ms->order_filename, line_number, char_pos, ms->s.segname,
              ms->s.sectname);
      /* move past the '\\' */
      i++;
      char_pos++;
      if (ms->order_addr[i] >= '0' && ms->order_addr[i] <= '7') {
        ...
        case 'x':
          ...
          while (
              i + 1 < ms->order_size &&
              ((ms->order_addr[i + 1] >= '0' && ms->order_addr[i + 1] <= '9') ||
               (ms->order_addr[i + 1] >= 'a' && ms->order_addr[i + 1] <= 'f') ||
               (ms->order_addr[i + 1] >= 'A' &&
                ms->order_addr[i + 1] <= 'F'))) {
            if (k <= 8)
 ...
 */
__private_extern__ unsigned long lookup_cstring(char *cstring,
  ...
  if (data->hashtable == NULL) {
    data->hashtable =
        allocate(sizeof(struct cstring_bucket *) * CSTRING_HASHSIZE);
    memset(data->hashtable, '\0',
           sizeof(struct cstring_bucket *) * CSTRING_HASHSIZE);
  }
  ...
  for (p = &(data->cstring_blocks); *p; p = &(cstring_block->next)) {
    ...
    memset(cstring_block->cstrings + cstring_block->used + cstring_len, '\0',
           len - cstring_len);
 ...
 */
__private_extern__ void cstring_output(struct cstring_data *data,
                                       struct merged_section *ms) {
  ...
  for (p = &(data->cstring_blocks); *p;) {
    cstring_block = *p;
    memcpy(output_addr + offset, cstring_block->cstrings, cstring_block->used);
 ...
 * the data block itself.
 */
__private_extern__ void cstring_free(struct cstring_data *data) {
  unsigned long i;
 ...
 */
__private_extern__ void print_cstring_data(struct cstring_data *data,
                                           char *indent) {
  char *s;
  struct cstring_block **p, *cstring_block;
  /*
  ...
  */
  ...
  for (p = &(data->cstring_blocks); *p; p = &(cstring_block->next)) {
    ...
    if (cstring_block->full)
      ...
      print("%s\tfull FALSE\n", indent);
    print("%s\tnext 0x%x\n", indent, (unsigned int)(cstring_block->next));
    print("%s\tcstrings\n", indent);
 ...
 */
__private_extern__ void cstring_data_stats(struct cstring_data *data,
                                           struct merged_section *ms) {
  if (data == NULL)
    return;
