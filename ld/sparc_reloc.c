...
#endif /* SHLIB */
/*
 * This file contains the routines to do relocation for SPARC processor
 * architecture.
 */
#include <stdlib.h>
 ...
 */
__private_extern__ void sparc_reloc(char *contents,
  ...
  for (i = 0; i < section_map->s->nreloc; i++) {
    ...
    if (r_type == SPARC_RELOC_HI22 || r_type == SPARC_RELOC_LO10) {
      if (i + 1 < section_map->s->nreloc) {
        ...
        if ((pair_reloc->r_address & R_SCATTERED) != 0) {
          ...
          pair_reloc = NULL;
          pair_r_type = (enum reloc_type_sparc)spair_reloc->r_type;
          other_half = spair_reloc->r_address;
        } else {
          pair_r_type = (enum reloc_type_sparc)pair_reloc->r_type;
    ...
    if (r_extern) {
      ...
      if (((merged_symbol->nlist.n_type & N_TYPE) == N_UNDF) ||
        ...
        if (r_type == SPARC_RELOC_VANILLA) {
          ...
          case SPARC_RELOC_HI22:
            offset = ((instruction & 0x3fffff) << 10) | (other_half & 0x3ff);
            break;
          case SPARC_RELOC_LO10:
            offset = (instruction & 0x3ff) | ((other_half & 0x3fffff) << 10);
            break;
      ...
      if (r_symbolnum != R_ABS) {
        ...
        if (local_map->nfine_relocs == 0 &&
           ...
           * relocated.
           */
          if (r_type == SPARC_RELOC_VANILLA || r_type == SPARC_RELOC_SECTDIFF) {
            switch (r_length) {
            case 0: /* byte */
            ...
            case SPARC_RELOC_HI22_SECTDIFF:
              value = ((instruction & 0x3fffff) << 10) | (other_half & 0x3ff);
              break;
            case SPARC_RELOC_LO10:
            case SPARC_RELOC_LO10_SECTDIFF:
              value = (instruction & 0x3ff) | ((other_half & 0x3fffff) << 10);
              break;
            case SPARC_RELOC_WDISP22:
          ...
          if (r_type == SPARC_RELOC_VANILLA || r_type == SPARC_RELOC_SECTDIFF) {
            ...
            case SPARC_RELOC_HI22_SECTDIFF:
              ...
              instruction =
                  (instruction & 0xffc00000) | ((value >> 10) & 0x3fffff);
              break;
            case SPARC_RELOC_LO10:
            case SPARC_RELOC_LO10_SECTDIFF:
              other_half = (value >> 10) & 0x3fffff;
              instruction = (instruction & 0xfffffc00) | (value & 0x3ff);
              break;
            case SPARC_RELOC_WDISP22:
              if ((value & 0xff800000) != 0xff800000 &&
                ...
                error_with_cur_obj("relocation overflow "
                                   ...
                                   section_map->s->sectname);
              value >>= 2;
              instruction = (instruction & 0xffc00000) | (value & 0x003fffff);
              break;
            case SPARC_RELOC_WDISP30:
              value >>= 2;
              instruction = (instruction & 0xc0000000) | (value & 0x3fffffff);
              break;
            default:
     ...
     */
      ...
      case SPARC_RELOC_HI22_SECTDIFF:
        ...
        immediate += value;
        instruction =
            (instruction & 0xffc00000) | ((immediate >> 10) & 0x3fffff);
      ...
      case SPARC_RELOC_LO10:
        ...
        immediate += value;
        instruction = (instruction & 0xfffffc00) | (immediate & 0x3ff);
      ...
      case SPARC_RELOC_WDISP22:
        ...
        if ((immediate & 0xff800000) != 0xff800000 &&
          ...
          error_with_cur_obj("relocation overflow for relocation "
                             ...
                             section_map->s->sectname);
        immediate >>= 2;
        instruction = (instruction & 0xffc00000) | (immediate & 0x3fffff);
        break;
      case SPARC_RELOC_WDISP30:
        ...
        immediate += value;
        immediate >>= 2;
        instruction = (instruction & 0xc0000000) | (immediate & 0x3fffffff);
        break;
      default:
