 ...
 */
struct seg_addr_table *parse_default_seg_addr_table(char **seg_addr_table_name,
                                                    uint32_t *table_size) {
...
#endif
  return (parse_seg_addr_table(*seg_addr_table_name, "default",
                               "segment address table", table_size));
}
 ...
 */
struct seg_addr_table *parse_seg_addr_table(
  ...
  seg_addr_table_size++;
  new_seg_addr_table =
      allocate(sizeof(struct seg_addr_table) * seg_addr_table_size);
  k = 0;
  line = 1;
...
search_seg_addr_table(struct seg_addr_table *seg_addr_table,
                      char *install_name) {
 ...
 */
void process_seg_addr_table(char *file_name, FILE *out_fp, char *comment_prefix,
                            void (*processor)(struct seg_addr_table *entry,
                                              FILE *out_fp, void *cookie),
                            void *cookie) {
  int fd;
  struct stat stat_buf;
  ...
  file_addr = NULL;
  if (stat_buf.st_size > 0xFFFFFFFF) {
    fatal("File too large (%llu): %s", stat_buf.st_size, file_name);
  } else if (stat_buf.st_size == 0) {
  ...
  for (i = 0; i < file_size; /* no increment expression */) {
    ...
    if (file_addr[i] == '#') {
      if (strncmp(comment_prefix, file_addr + i + 1, comment_prefix_length) ==
          0) {
        i++;
        while (file_addr[i] != '\n')
          i++;
    ...
    if (endp == NULL)
      fatal("improper hexadecimal number on line %u in file: %s", line,
            file_name);
    i = (uint32_t)(endp - file_addr);
    if (i == file_size)
      fatal("missing library install name on line %u in file: %s", line,
            file_name);
    /*
     * Since we checked to see the file ends in a '\n' we can
     * be assured this won't run off the end of the file.
