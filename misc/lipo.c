...
static void process_replace_file(struct replace *replace);
static void check_archive(char *name, char *addr, uint64_t size,
                          cpu_type_t *cputype, cpu_subtype_t *cpusubtype);
static void check_extend_format_1(char *name, struct ar_hdr *ar_hdr,
                                  uint64_t size_left,
                                  uint32_t *member_name_size);
static uint32_t get_mh_filetype(char *addr, uint64_t size);
...
static int size_ar_name(char *ar_name);
static struct input_file *new_input(void);
static struct thin_file *new_thin(void);
static struct arch_flag *new_arch_flag(struct arch_flag **arch_flags,
                                       uint32_t *narch_flags);
static struct replace *new_replace(void);
static struct segalign *new_segalign(void);
static int cmp_qsort(const struct thin_file *thin1,
                     const struct thin_file *thin2);
static enum bool ispoweroftwo(uint32_t x);
static void check_arch(struct input_file *input, struct thin_file *thin);
...
int main(int argc, char *argv[], char *envp[]) {
    ...
    if (argv[a][0] == '-') {
      ...
      case 'a':
        if (strcmp(p, "arch") == 0 || strcmp(p, "a") == 0) {
          ...
          input = new_input();
          if (get_arch_from_flag(argv[a + 1], &(input->arch_flag)) == 0) {
            error("unknown architecture specification flag: %s "
                  "in specifying input file %s %s %s",
      ...
      case 'd':
        ...
        break;
      case 'e':
        if (strcmp(p, "extract") == 0 || strcmp(p, "extract_family") == 0 ||
            strcmp(p, "e") == 0) {
          extract_flag = TRUE;
          ...
          arch_flag = new_arch_flag(&extract_arch_flags, &nextract_arch_flags);
      ...
      case 'r':
        if (strcmp(p, "remove") == 0 || strcmp(p, "rem") == 0) {
          ...
          replace = new_replace();
          if (get_arch_from_flag(argv[a + 1], &(replace->arch_flag)) == 0) {
            error("unknown architecture specification flag: "
      ...
      case 's':
        if (strcmp(p, "segalign") == 0 || strcmp(p, "s") == 0) {
          ...
          if (get_arch_from_flag(argv[a + 1], &(segalign->arch_flag)) == 0) {
            error("unknown architecture specification flag: "
      ...
      case 'v':
        if (strcmp(p, "verify_arch") == 0) {
          ...
          verify_archs = (struct arch_flag *)allocate(sizeof(struct arch_flag) *
                                                      nverify_archs);
          for (i = 0; a < argc; a++, i++) {
            if (get_arch_from_flag(argv[a], verify_archs + i) == 0) {
  ...
  if (info_flag == FALSE && detailed_info_flag == FALSE &&
      brief_info_flag == FALSE && create_flag == FALSE && thin_flag == FALSE &&
      extract_flag == FALSE && remove_flag == FALSE && replace_flag == FALSE &&
      verify_flag == FALSE) {
  ...
  if (create_flag + thin_flag + extract_flag + remove_flag + replace_flag +
          info_flag + detailed_info_flag + brief_info_flag + verify_flag >
      1) {
  ...
  if (hideARM64_flag == TRUE && create_flag == FALSE && replace_flag == FALSE &&
      remove_flag == FALSE) {
  ...
  if (thin_flag) {
    ...
    for (i = 0; i < nthin_files; i++) {
      if (thin_files[i].cputype == thin_arch_flag.cputype &&
        ...
        if (__builtin_available(macOS 10.12, *)) {
          time_result = utimensat(AT_FDCWD, output_file, output_times, 0);
        } else {
    ...
    if (i == nthin_files)
      fatal("fat input file (%s) does not contain the specified "
            "architecture (%s) to thin it to",
            input->name, thin_arch_flag.name);
  }
  ...
  if (extract_flag) {
    ...
    for (i = 0; i < nextract_arch_flags; i++) {
      for (j = i + 1; j < nextract_arch_flags; j++) {
        if (extract_arch_flags[i].cputype == extract_arch_flags[j].cputype &&
    ...
    for (i = 0; i < nextract_arch_flags; i++) {
      ...
      for (j = 0; j < nthin_files; j++) {
        if (extract_arch_flags[i].cputype == thin_files[j].cputype &&
            ((extract_arch_flags[i].cpusubtype & ~CPU_SUBTYPE_MASK) ==
                 (thin_files[j].cpusubtype & ~CPU_SUBTYPE_MASK) ||
             extract_family_flag == TRUE)) {
  ...
  if (remove_flag) {
    ...
    for (i = 0; i < nremove_arch_flags; i++) {
      for (j = 0; j < nthin_files; j++) {
        if (remove_arch_flags[i].cputype == thin_files[j].cputype &&
            (remove_arch_flags[i].cpusubtype & ~CPU_SUBTYPE_MASK) ==
                (thin_files[j].cpusubtype & ~CPU_SUBTYPE_MASK)) {
  ...
  if (replace_flag) {
    ...
    for (i = 0; i < nreplaces; i++) {
      for (j = i + 1; j < nreplaces; j++) {
        if (replaces[i].arch_flag.cputype == replaces[j].arch_flag.cputype &&
    ...
    for (i = 0; i < nreplaces; i++) {
      ...
      for (j = 0; j < nthin_files; j++) {
        if (replaces[i].arch_flag.cputype == thin_files[j].cputype &&
            (replaces[i].arch_flag.cpusubtype & ~CPU_SUBTYPE_MASK) ==
                (thin_files[j].cpusubtype & ~CPU_SUBTYPE_MASK)) {
  ...
  if (brief_info_flag) {
    for (i = 0; i < nthin_files; i++) {
      const char *s = get_arch_name_if_known(thin_files[i].cputype,
  ...
  if (info_flag) {
    for (i = 0; i < ninput_files; i++) {
      if (input_files[i].fat_header != NULL) {
        printf("Architectures in the fat file: %s are: ", input_files[i].name);
  ...
  if (detailed_info_flag) {
    for (i = 0; i < ninput_files; i++) {
      if (input_files[i].fat_header != NULL) {
        ...
        printf("nfat_arch %u", input_files[i].raw_nfat_arch);
        if (input_files[i].fat_header->nfat_arch -
            input_files[i].raw_nfat_arch) {
          printf(" (+%u hidden)", input_files[i].fat_header->nfat_arch -
                                      input_files[i].raw_nfat_arch);
        }
        printf("\n");
        for (j = 0; j < input_files[i].fat_header->nfat_arch; j++) {
          ...
          if (input_files[i].fat_arches != NULL) {
            ...
            printf("\n");
            print_cputype(input_files[i].fat_arches64[j].cputype,
                          input_files[i].fat_arches64[j].cpusubtype);
  ...
  if (verify_flag == TRUE) {
    for (i = 0; i < nverify_archs; i++) {
      found = FALSE;
      for (j = 0; j < nthin_files; j++) {
        if (verify_archs[i].cputype == thin_files[j].cputype &&
...
static void create_fat(void) {
  ...
  for (i = 0; i < nsegaligns; i++) {
    for (j = i + 1; j < nsegaligns; j++) {
      if (segaligns[i].arch_flag.cputype == segaligns[j].arch_flag.cputype &&
  ...
  for (i = 0; i < nsegaligns; i++) {
    for (j = 0; j < nthin_files; j++) {
      if (segaligns[i].arch_flag.cputype == thin_files[j].cputype &&
          (segaligns[i].arch_flag.cpusubtype & ~CPU_SUBTYPE_MASK) ==
              (thin_files[j].cpusubtype & ~CPU_SUBTYPE_MASK)) {
        /*
         Since this program has to guess at alignments and guesses high when
         unsure this check shouldn't be used so the the correct alignment can be
         specified by the the user. if(thin_files[j].align > segaligns[i].align)
                                fatal("specified segment alignment: %d for "
                                      "architecture %s is less than the
         alignment: %d " "required from the input file", 1 <<
         segaligns[i].align, segaligns[i].arch_flag.name, 1 <<
         thin_files[j].align);
        */
  ...
  if (hideARM64_flag) {
    ...
    for (i = 0; i < nthin_files; i++) {
      if (MH_EXECUTE !=
          get_mh_filetype(thin_files[i].addr, thin_files[i].size)) {
  ...
  if ((fd = open(rename_file, O_WRONLY | O_CREAT | O_TRUNC, output_filemode)) ==
      -1)
  ...
  if (extract_family_flag != TRUE || nthin_files != 1) {
    ...
    if (write64(fd, &fat_header, sizeof(struct fat_header)) !=
      ...
      system_fatal("can't write fat header to output file: %s", rename_file);
#ifdef __LITTLE_ENDIAN__
    ...
    for (i = 0; i < nthin_files; i++) {
      ...
      if (fat64_flag == TRUE) {
        ...
        if (write64(fd, &fat_arch64, sizeof(struct fat_arch_64)) !=
          ...
          system_fatal("can't write fat arch to output file: %s", rename_file);
      } else {
#ifdef __LITTLE_ENDIAN__
...
static void process_input_file(struct input_file *input) {
  ...
  if (fstat(fd, &stat_buf2) == -1)
    system_fatal("can't stat input file: %s", input->name);
  if (stat_buf2.st_size != size || stat_buf2.st_mtime != stat_buf.st_mtime)
...
#endif /* __LITTLE_ENDIAN__ */
	{
             ...
             if (big_size > size)
               fatal("truncated or malformed fat file (fat_arch structs would "
                     "extend past the end of the file) %s",
                     input->name);
             input->fat_arches =
               ...
               for (j = i + 1; j < input->fat_header->nfat_arch; j++) {
                 if (input->fat_arches[i].cputype ==
                   ...
                   fatal("fat file %s contains two of the same architecture "
                         ...
                         input->name, input->fat_arches[i].cputype,
                         input->fat_arches[i].cpusubtype & ~CPU_SUBTYPE_MASK);
               }
             }
...
#endif /* __LITTLE_ENDIAN__ */
	{
             ...
             if (big_size > size)
               fatal("truncated or malformed fat file (fat_arch_64 structs "
                     "would extend past the end of the file) %s",
                     input->name);
             input->fat_arches64 =
             ...
             for (i = 0; i < input->fat_header->nfat_arch; i++) {
               for (j = i + 1; j < input->fat_header->nfat_arch; j++) {
                 if (input->fat_arches64[i].cputype ==
                   ...
                   fatal("fat file %s contains two of the same architecture "
                         "(cputype (%d) cpusubtype (%d))",
                         input->name, input->fat_arches64[i].cputype,
                         input->fat_arches64[i].cpusubtype & ~CPU_SUBTYPE_MASK);
               }
             }
  ...
  /* see if this file is Mach-O file for 32-bit architectures */
    ...
    lcp = (struct load_command *)((char *)mhp + sizeof(struct mach_header));
    ...
    if (MH_OBJECT == mhp->filetype)
      thin->align = guess_align(getpagesize(), MINSEGALIGN32, MAXSEGALIGN);
    else
      thin->align = get_seg_align(mhp, NULL, lcp, swapped, size, input->name);
  ...
  /* see if this file is Mach-O file for 64-bit architectures */
    ...
    lcp =
        (struct load_command *)((char *)mhp64 + sizeof(struct mach_header_64));
    ...
    if (MH_OBJECT == mhp64->filetype)
      thin->align = guess_align(getpagesize(), MINSEGALIGN64, MAXSEGALIGN);
    else
      thin->align = get_seg_align(NULL, mhp64, lcp, swapped, size, input->name);
...
#ifdef LTO_SUPPORT
      if (is_llvm_bitcode_from_memory(addr, (uint32_t)size, &input->arch_flag,
                                      NULL) != 0) {
        /* create a thin file struct for it */
        thin = new_thin();
        thin->name = input->name;
        ...
        if (fat64_flag == FALSE && size > UINT32_MAX)
          fatal("file too large to be in a fat file because the "
                ...
                "and the size (%llu) of %s exceeds that",
                size, input->name);
        thin->cputype = input->arch_flag.cputype;
        thin->cpusubtype = input->arch_flag.cpusubtype;
        thin->offset = 0;
...
#endif /* LTO_SUPPORT */
        fatal("can't figure out the architecture type of: %s", input->name);
    }
  }
}
...
static void process_replace_file(struct replace *replace) {
  ...
  if ((fd = open(replace->thin_file.name, O_RDONLY)) == -1)
    system_fatal("can't open replacement file: %s", replace->thin_file.name);
  if (fstat(fd, &stat_buf) == -1)
    system_fatal("can't stat replacement file: %s", replace->thin_file.name);
  size = stat_buf.st_size;
  /*
   * mmap() can't handle mapping regular files with zero size.  So this
   * is handled separately.
  ...
  if ((stat_buf.st_mode & S_IFREG) == S_IFREG && size == 0)
    ...
    addr = mmap(0, size, PROT_READ | PROT_WRITE, MAP_FILE | MAP_PRIVATE, fd, 0);
  if ((intptr_t)addr == -1)
    system_error("can't map replacement file: %s", replace->thin_file.name);
  close(fd);
  ...
  if (size >= sizeof(struct fat_header) &&
#ifdef __BIG_ENDIAN__
      (*((uint32_t *)addr) == FAT_MAGIC || *((uint32_t *)addr) == FAT_MAGIC_64))
#endif /* __BIG_ENDIAN__ */
#ifdef __LITTLE_ENDIAN__
           (*((uint32_t *)addr) == SWAP_INT(FAT_MAGIC) ||
            *((uint32_t *)addr) == SWAP_INT(FAT_MAGIC_64)))
  ...
  /* see if this file is Mach-O file for 32-bit architectures */
    ...
    if (MH_OBJECT == mhp->filetype)
      replace->thin_file.align =
          guess_align(getpagesize(), MINSEGALIGN32, MAXSEGALIGN);

    else
      replace->thin_file.align =
          get_seg_align(mhp, NULL, lcp, swapped, size, replace->thin_file.name);
  }
  /* see if this file is Mach-O file for 64-bit architectures */
  else if (size >= sizeof(struct mach_header_64) &&
           (*((uint32_t *)addr) == MH_MAGIC_64 ||
    ...
    if (MH_OBJECT == mhp64->filetype)
      replace->thin_file.align =
          guess_align(getpagesize(), MINSEGALIGN64, MAXSEGALIGN);
    else
      replace->thin_file.align = get_seg_align(NULL, mhp64, lcp, swapped, size,
                                               replace->thin_file.name);
  }
  /* see if this file is an archive file */
  else if (size >= SARMAG && strncmp(addr, ARMAG, SARMAG) == 0) {

    check_archive(replace->thin_file.name, addr, size, &cputype, &cpusubtype);
  ...
  if (replace->thin_file.cputype != replace->arch_flag.cputype ||
    ...
    fatal("specified architecture: %s for replacement file: %s does "
          "not match the file's architecture",
          replace->arch_flag.name, replace->thin_file.name);
}
...
static void check_archive(char *name, char *addr, uint64_t size,
  ...
  *cpusubtype = 0;
  ...
  while (size > offset) {
      ...
      if (magic == FAT_MAGIC || magic == FAT_MAGIC_64)
        ...
        if (magic == SWAP_INT(FAT_MAGIC) || magic == SWAP_INT(FAT_MAGIC_64))
          ...
          fatal("archive member %s(%.*s) is a fat file (not "
                ...
                name, (int)i, ar_name);
      if ((size - ar_name_size) - offset >= sizeof(struct mach_header) &&
          (magic == MH_MAGIC || magic == SWAP_INT(MH_MAGIC))) {
        memcpy(&mh, addr + offset + ar_name_size, sizeof(struct mach_header));
        ...
        if (*cputype == 0) {
          ...
          *cpusubtype = mh.cpusubtype;
          ...
          fatal("archive member %s(%.*s) cputype (%d) and "
                ...
                " (%d) (all members must match)",
                name, (int)i, ar_name, mh.cputype,
                mh.cpusubtype & ~CPU_SUBTYPE_MASK, *cputype,
                (*cpusubtype) & ~CPU_SUBTYPE_MASK);
        } else {
          if (mh.cputype == CPU_TYPE_ARM && *cpusubtype != mh.cpusubtype)
            fatal("archive member %s(%.*s) cputype (%d) and "
                  "cpusubtype (%d) does not match previous "
                  "archive members cputype (%d) and cpusubtype"
                  " (%d) (all members must match)",
        ...
        if (*cputype == 0) {
          ...
          *cpusubtype = mh64.cpusubtype;
          ...
          fatal("archive member %s(%.*s) cputype (%d) and "
                ...
                "archive members cputype (%d) and cpusubtype"
                " (%d) (all members must match)",
                name, (int)i, ar_name, mh64.cputype,
                mh64.cpusubtype & ~CPU_SUBTYPE_MASK, *cputype,
                (*cpusubtype) & ~CPU_SUBTYPE_MASK);
        } else {
...
#ifdef LTO_SUPPORT
          if (is_llvm_bitcode_from_memory(ar_addr, ar_size, &arch_flag, NULL) !=
              0) {
            if (*cputype == 0) {
              ...
              *cpusubtype = arch_flag.cpusubtype;
              ...
              fatal("archive member %s(%.*s) cputype (%d) "
                    ...
                    "match)",
                    name, (int)i, ar_name, arch_flag.cputype,
                    arch_flag.cpusubtype & ~CPU_SUBTYPE_MASK, *cputype,
                    (*cpusubtype) & ~CPU_SUBTYPE_MASK);
            }
          }
#endif /* LTO_SUPPORT */
    ...
    offset += rnd64(strtoul(ar_hdr->ar_size, NULL, 10), sizeof(short));
  }
}
/*
  ...
  *member_name_size = 0;
  ...
  if (isdigit(*p) == 0)
    fatal("archive: %s malformed (ar_name: %.*s for archive extend "
          "format #1 starts with non-digit)",
  ...
  if (ar_name_size == UINT_MAX && errno == ERANGE)
    fatal("archive: %s malformed (size in ar_name: %.*s for archive "
          "extend format #1 overflows uint32_t)",
          name, (int)sizeof(ar_hdr->ar_name), ar_hdr->ar_name);
  while (*endp == ' ' && *endp != '\0')
    endp++;
  if (*endp != '\0')
  ...
  if (detailed_info_flag) {
    if ((cputype == CPU_TYPE_X86_64 || cputype == CPU_TYPE_POWERPC64) &&
      ...
      printf("    capabilities 0x%x\n",
             (unsigned int)((rawsubtype & CPU_SUBTYPE_MASK) >> 24));
  }
}
...
static struct input_file *new_input(void) {
  ...
  input_files =
      reallocate(input_files, (ninput_files + 1) * sizeof(struct input_file));
...
static struct thin_file *new_thin(void) {
  ...
  thin_files =
      reallocate(thin_files, (nthin_files + 1) * sizeof(struct thin_file));
...
static struct arch_flag *new_arch_flag(struct arch_flag **arch_flags,
  ...
  *arch_flags =
      reallocate(*arch_flags, (*narch_flags + 1) * sizeof(struct arch_flag));
...
static struct replace *new_replace(void) {
  ...
  replaces = reallocate(replaces, (nreplaces + 1) * sizeof(struct replace));
...
static struct segalign *new_segalign(void) {
  ...
  segaligns = reallocate(segaligns, (nsegaligns + 1) * sizeof(struct segalign));
...
static void check_arch(struct input_file *input, struct thin_file *thin) {
  if (input->arch_flag.cputype != thin->cputype)
    fatal("specifed architecture type (%s) for file (%s) does "
          ...
          input->arch_flag.name, input->name, thin->cputype,
          thin->cpusubtype & ~CPU_SUBTYPE_MASK, input->arch_flag.cputype,
...
static void usage(void) {
  fprintf(stderr,
          ...
          "    -replace <arch_type> <file_name> [-replace <arch_type> "
          "<file_name> ...]\n"
          "    -thin <arch_type>\n"
          "    -verify_arch <arch_type> ...\n"
