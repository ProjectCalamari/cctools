 ...
 */
__private_extern__ void mod_section_merge(struct mod_term_data *data,
  ...
  if (s->size % 4 != 0) {
    error_with_cur_obj("module %s function pointer section (%.16s,"
                       "%.16s) size is not a multiple of 4 bytes",
                       type_name, s->segname, s->sectname);
    return;
  }
  if (s->nreloc != s->size / 4) {
    error_with_cur_obj(
        "module %s function pointer section (%.16s,"
  ...
  for (i = 0; i < s->nreloc; i++) {
    ...
    if (r_address % 4 != 0) {
      error_with_cur_obj(
          ...
          (unsigned int)r_address, i, type_name, s->segname, s->sectname);
      continue;
    }
    if (r_length != 2) {
      error_with_cur_obj(
          ...
          "%ld in module %s function pointer section (%.16s,%.16s) "
          "is not 2 (long)",
          (unsigned int)r_length, i, type_name, s->segname, s->sectname);
      continue;
    }
    if (r_pcrel != 0) {
      error_with_cur_obj(
          ...
          "%ld in module %s function pointer section (%.16s,%.16s) "
          "is not 0 (FALSE)",
          (unsigned int)r_pcrel, i, type_name, s->segname, s->sectname);
      continue;
    }
    if (r_type != 0) {
      error_with_cur_obj(
          "r_type (0x%x) field of relocation entry "
          "%ld in module %s function pointer section (%.16s,%.16s) "
          "is not 0 (VANILLA)",
          (unsigned int)r_type, i, type_name, s->segname, s->sectname);
      continue;
    }
  }
    ...
    if ((ms->s.flags & SECTION_TYPE) == S_MOD_INIT_FUNC_POINTERS) {
      if (dylib_ms_init != NULL && dylib_ms_init != ms)
        error("multiple module initialization function pointer "
              ...
              "allowed in MH_DYLIB output",
              s->segname, s->sectname, dylib_ms_init->s.segname,
              dylib_ms_init->s.sectname);
      dylib_ms_init = ms;
      ...
      if (ninit > 0xffff)
        error("too many module initialization function pointer in "
              ...
              "is %d)",
              s->segname, s->sectname, 0xffff);
    } else {
      if (dylib_ms_term != NULL && dylib_ms_term != ms)
        error("multiple module termination function pointer "
              "sections (%.16s,%.16s) and (%.16s,%.16s) only one "
              "allowed in MH_DYLIB output",
              s->segname, s->sectname, dylib_ms_term->s.segname,
              dylib_ms_term->s.sectname);
      dylib_ms_term = ms;
  ...
  if (s->align > 2)
    warning_with_cur_obj(
        "aligment greater than 2 (2^2, 4 bytes) for "
        "module %s function pointer section (%.16s,%.16s) (will leave "
        "holes with zeroes for pointers)",
        type_name, s->segname, s->sectname);
    ...
    if (redo_live == FALSE) {
      ...
      for (i = 0; i < npointers; i++) {
        if (fine_relocs[i].live == TRUE) {
          ...
          cur_obj->file_name, ms->s.segname, ms->s.sectname, (unsigned
          int)r_address);
          */
          fine_relocs[i].output_offset = data->output_offset;
          data->output_offset += 4;
 ...
 */
__private_extern__ void mod_section_order(struct mod_term_data *data,
                                          struct merged_section *ms) {
#ifndef RLD
  kern_return_t r;
  char *type_name;
  ...
  error("section ordering for module %s function pointer sections not "
        "allowed (-sectorder %s %s %s ignored)",
        type_name, ms->s.segname, ms->s.sectname, ms->order_filename);
  /*
  ...
  if ((r = vm_deallocate(mach_task_self(), (vm_address_t)ms->order_addr,
    ...
    mach_fatal(r,
               "can't vm_deallocate() memory for -sectorder file: "
 ...
 * TRUE.
 */
__private_extern__ void mod_section_reset_live(struct mod_term_data *data,
                                               struct merged_section *ms) {
#ifndef RLD
 ...
 */
__private_extern__ void mod_section_free(struct mod_term_data *data) {
  data->output_offset = 0;
}
