...
// can't include "stuff/breakout.h" because it also declares `arch` and other
// structs used here
extern struct ofile *breakout(const char *filename, void **archs,
                              uint32_t *narchs, enum bool);
...
struct cmd_flags {
  ...
  uint32_t nLdirs;                 /* number of -Ldir arguments */
  char **ldflags;                  /* other ld(1) flags to pass */
  uint32_t nldflags;               /* number of ld(1) flags for above */
  enum bool verbose;               /* print exec(2) commands run */
  struct arch_flag arch_only_flag; /* the -arch_only flag if specified */
...
char *standard_dirs[] = {"/lib/", "/usr/lib/", "/usr/local/lib/", NULL};
...
static char *file_name_from_l_flag(char *l_flag);
static char *search_for_file(char *base_name);
static char *search_paths_for_lname(const char *lname_argument);
static char *search_path_for_lname(const char *dir, const char *lname_argument);
static void add_member(struct ofile *ofile);
static void free_archs(void);
static void create_library(char *output, struct ofile *ofile);
static enum byte_sex get_target_byte_sex(struct arch *arch,
                                         enum byte_sex host_byte_sex);
static char *put_toc_member(char *p, struct arch *arch,
                            enum byte_sex host_byte_sex,
                            enum byte_sex target_byte_sex);
static void create_dynamic_shared_library(char *output);
static void create_dynamic_shared_library_cleanup(int sig);
static void make_table_of_contents(struct arch *arch, char *output);
#ifdef LTO_SUPPORT
static void save_lto_member_toc_info(struct member *member, void *mod);
#endif /* LTO_SUPPORT */
static int toc_name_qsort(const struct toc *toc1, const struct toc *toc2);
static int toc_index1_qsort(const struct toc *toc1, const struct toc *toc2);
static enum bool toc_symbol(struct nlist *symbol, struct section **sections);
static enum bool toc_symbol_64(struct nlist_64 *symbol64,
                               struct section_64 **sections64);
static enum bool toc(uint32_t n_strx, uint8_t n_type, uint64_t n_value,
                     enum bool attr_no_toc);
static enum bool check_sort_tocs(struct arch *arch, char *output,
                                 enum bool library_warnings);
static void warn_duplicate_member_names(void);
static int member_name_qsort(const struct member *member1,
                             const struct member *member2);
static int member_offset_qsort(const struct member *member1,
                               const struct member *member2);
static void warn_member(struct arch *arch, struct member *member,
                        const char *format, ...)
    __attribute__((format(printf, 3, 4)));
static void ld_trace_archive(const char *path);
static void ld_trace_close(void);
static void ld_trace_append(const char *format, ...)
...
static void output_flush(char *library, uint64_t library_size, int fd,
                         uint64_t offset, uint64_t size);
static void final_output_flush(char *library, int fd);
#ifdef DEBUG
static void print_block_list(void);
#endif /* DEBUG */
static struct block *get_block(void);
static void remove_block(struct block *block);
static uint64_t trnc64(uint64_t v, uint64_t r);
...
int main(int argc, char **argv, char **envp) {
    ...
    if (argv[i][0] == '-') {
      ...
      if (strcmp(argv[i], "-o") == 0) {
        ...
        if (cmd_flags.arch_only_flag.name != NULL) {
          ...
          if (get_arch_from_flag(argv[i + 1], &cmd_flags.arch_only_flag) == 0) {
        ...
        if (dirname != NULL) {
          ...
          // If open fails, fallback and try again assuming there is a dirname
          // too
        ...
        if (stat_buf.st_size != 0) {
          ...
          if ((intptr_t)addr == -1)
            system_error("can't map file list file: %s", filelist);
        } else {
        ...
        if (addr[stat_buf.st_size - 1] != '\n')
          nfiles++;
        p = allocate((strlen(dirname) + 1) * nfiles + stat_buf.st_size);
        cmd_flags.files =
            reallocate(cmd_flags.files, sizeof(char *) * (maxfiles + nfiles));
        cmd_flags.filelist = reallocate(cmd_flags.filelist,
                                        sizeof(char *) * (maxfiles + nfiles));
        memset(cmd_flags.filelist + maxfiles, '\0', sizeof(char *) * nfiles);
        maxfiles += nfiles;
        ...
        next_root = argv[i + 1];
        cmd_flags.ldflags = reallocate(
            cmd_flags.ldflags, sizeof(char *) * (cmd_flags.nldflags + 2));
        ...
        cmd_flags.ldflags = reallocate(
            cmd_flags.ldflags, sizeof(char *) * (cmd_flags.nldflags + 4));
        cmd_flags.ldflags[cmd_flags.nldflags++] = argv[i];
        cmd_flags.ldflags[cmd_flags.nldflags++] = argv[i + 1];
        ...
        i += 3;
                 ...
                 strcmp(argv[i], "-init") == 0 || strcmp(argv[i], "-U") == 0 ||
                 strcmp(argv[i], "-Y") == 0 ||
        ...
        i += 1;
                 ...
                 strcmp(argv[i], "-Sn") == 0 || strcmp(argv[i], "-Si") == 0 ||
                 strcmp(argv[i], "-Sp") == 0 || strcmp(argv[i], "-S") == 0 ||
                 ...
                 strcmp(argv[i], "-ObjC") == 0 || strcmp(argv[i], "-M") == 0 ||
                 strcmp(argv[i], "-t") == 0 ||
        ...
        if (cmd_flags.ranlib == TRUE) {
          ...
          cmd_flags.ldflags = reallocate(
              cmd_flags.ldflags, sizeof(char *) * (cmd_flags.nldflags + 1));
          cmd_flags.ldflags[cmd_flags.nldflags++] = argv[i];
        }
        ...
        if (argv[i][1] == 'L' && argv[i][2] == '\0') {
          ...
          cmd_flags.Ldirs =
              realloc(cmd_flags.Ldirs, sizeof(char *) * (cmd_flags.nLdirs + 1));
        ...
        for (j = 1; argv[i][j] != '\0'; j++) {
          ...
          case 'v':
            if (cmd_flags.ranlib == TRUE) {
              error("unknown option character `%c' in: %s", argv[i][j],
                    argv[i]);
              usage();
            }
          ...
          case 'D':
            ...
            break;
          default:
            error("unknown option character `%c' in: %s", argv[i][j], argv[i]);
            usage();
          }
        }
      }
  ...
  if (next_root != NULL) {
    for (i = 0; standard_dirs[i] != NULL; i++) {
      p = allocate(strlen(next_root) + strlen(standard_dirs[i]) + 1);
      strcpy(p, next_root);
    ...
    for (i = 0; i < cmd_flags.nLdirs; i++) {
      ...
      if (cmd_flags.Ldirs[i][2] == '/') {
        ...
        if (access(p, F_OK) != -1) {
          free(p);
          p = makestr("-L", next_root, cmd_flags.Ldirs[i] + 2, NULL);
          cmd_flags.Ldirs[i] = p;
        } else {
          free(p);
        }
...
static void process(void) {
  ...
  for (i = 0; i < cmd_flags.nfiles; i++) {
    if (strncmp(cmd_flags.files[i], "-l", 2) == 0 ||
      ...
      if (file_name != NULL) {
        ...
        if (ofile_map(file_name, NULL, NULL, ofiles + i, TRUE) == FALSE)
          continue;
        if (gDepInfo)
      ...
      if (ofile_map(cmd_flags.files[i], NULL, NULL, ofiles + i, TRUE) == FALSE)
        continue;
      if (gDepInfo) {
    ...
    if (ofiles[i].file_type == OFILE_FAT) {
      ...
      do {
        if (ofiles[i].arch_type == OFILE_ARCHIVE) {
          if (cmd_flags.ld_trace_archives == TRUE &&
              cmd_flags.dynamic == FALSE && ld_trace_archive_printed == FALSE) {
          ...
          if ((flag = ofile_first_member(ofiles + i)) == TRUE) {
            if (ofiles[i].member_ar_hdr != NULL &&
                strncmp(ofiles[i].member_name, SYMDEF, sizeof(SYMDEF) - 1) == 0)
            ...
            while (flag == TRUE) {
              ...
              if (ofiles[i].mh != NULL || ofiles[i].mh64 != NULL ||
                ...
                error("for architecture: %s file: %s(%.*s) "
                      ...
                      (int)ofiles[i].member_name_size, ofiles[i].member_name);
              }
              flag = ofile_next_member(ofiles + i);
            }
          }
      ...
      /* log the archive */
      if (cmd_flags.ld_trace_archives == TRUE && cmd_flags.dynamic == FALSE &&
          ld_trace_archive_printed == FALSE) {
        ld_trace_archive(ofiles[i].file_name);
        ld_trace_archive_printed = TRUE;
      }
      ...
      if ((flag = ofile_first_member(ofiles + i)) == TRUE) {
        ...
        while (flag == TRUE) {
          ...
          if (ofiles[i].member_type == OFILE_FAT) {
            ...
            do {
              if (ofiles[i].mh != NULL || ofiles[i].mh64 != NULL ||
                  ofiles[i].lto != NULL || cmd_flags.ranlib == TRUE) {
                add_member(ofiles + i);
              } else {
                /*
                ...
                error("file: %s(%.*s) for architecture: %s "
                      ...
                      "a library)",
                      cmd_flags.files[i], (int)ofiles[i].member_name_size,
                      ofiles[i].member_name, ofiles[i].arch_flag.name);
              }

            } while (ofile_next_arch(ofiles + i) == TRUE);
          } else if (ofiles[i].mh != NULL || ofiles[i].mh64 != NULL ||
#ifdef LTO_SUPPORT
                     ofiles[i].lto != NULL ||
#endif /* LTO_SUPPORT */
                     cmd_flags.ranlib == TRUE) {
          ...
          for (k = 0; k < archs[j].nmembers; k++) {
            if (archs[j].members[k].mh == NULL &&
              ...
              error("library member: %s(%.*s) is not an "
                    ...
                    cmd_flags.files[i],
                    (int)archs[j].members[k].input_base_name_size,
  ...
  // if any -ref args are used in -static mode, build extra .o file that lists
  // them
  if ((cmd_flags.nreflibs != 0) || (cmd_flags.nreffw != 0)) {
    if (cmd_flags.dynamic == FALSE)
      fatal("reference libraries are not supported by this open-source build");
  }
 ...
 */
static char *search_path_for_lname(const char *dir,
...
static void add_member(struct ofile *ofile) {
  ...
  if ((ofile->member_ar_hdr == NULL) &&
      (strcmp(ofile->file_name, ALWAYS_LOAD_MEMBER_NAME) != 0)) {
  ...
  if (ofile->mh != NULL || ofile->mh64 != NULL) {
    if (ofile->mh_cputype == 0) {
      if (ofile->member_ar_hdr != NULL) {
        ...
        error("file: %s cputype is zero (a reserved value)", ofile->file_name);
      return;
    }
    /*
  ...
  if (ofile->mh != NULL || ofile->mh64 != NULL
    ...
    only_cpusubtype = (cmd_flags.arch_only_flag.cpusubtype & ~CPU_SUBTYPE_MASK);
    ...
    if (cmd_flags.arch_only_flag.name != NULL) {
      if (only_cputype != member_cputype)
        return;
      if (only_cputype == CPU_TYPE_ARM || only_cputype == CPU_TYPE_ARM64_32 ||
    ...
    for (; i < narchs; i++) {
      ...
      arch_cpusubtype = (archs[i].arch_flag.cpusubtype & ~CPU_SUBTYPE_MASK);

      if (arch_cputype == member_cputype) {
        if ((arch_cputype == CPU_TYPE_ARM || arch_cputype == CPU_TYPE_ARM64 ||
  ...
  if (narchs == 1 && archs[0].arch_flag.cputype == 0) {
    ...
    if (ofile->mh != NULL || ofile->mh64 != NULL) {
      if (ofile->mh_cputype == CPU_TYPE_ARM ||
        ...
        archs[narchs].arch_flag.name = (char *)get_arch_name_from_types(
            ofile->mh_cputype, (ofile->mh_cpusubtype & ~CPU_SUBTYPE_MASK));
  ...
  if (arch->arch_flag.cputype == 0 &&
    ...
    if (family_arch_flag != NULL) {
      ...
      arch->arch_flag.cpusubtype = (ofile->mh_cpusubtype & ~CPU_SUBTYPE_MASK);
    }
  }
#ifdef LTO_SUPPORT
  if (arch->arch_flag.cputype == 0 && (ofile->lto != NULL)) {
    family_arch_flag = get_arch_family_from_cputype(ofile->lto_cputype);
    if (family_arch_flag != NULL) {
      arch->arch_flag = *family_arch_flag;
    } else {
      ...
      if (arch->arch_flag.name != NULL)
        sprintf(arch->arch_flag.name, "cputype %u cpusubtype %u",
                ofile->lto_cputype, ofile->lto_cpusubtype & ~CPU_SUBTYPE_MASK);
  ...
  arch->members =
      reallocate(arch->members, sizeof(struct member) * (arch->nmembers + 1));
  ...
  if (ofile->member_ar_hdr == NULL) {
    ...
    if (cmd_flags.use_long_names == TRUE) {
      ...
      ar_name_size =
          (uint32_t)(rnd(member->input_base_name_size, 8) +
                     (rnd(sizeof(struct ar_hdr), 8) - sizeof(struct ar_hdr)));
      sprintf(ar_name_buf, "%s%-*lu", AR_EFMT1,
              (int)(sizeof(member->ar_hdr.ar_name) - (sizeof(AR_EFMT1) - 1)),
    ...
    sprintf((char *)(&member->ar_hdr) + sizeof(member->ar_hdr.ar_name),
            "%-*ld%-*u%-*u%-*o%-*ld%-*s", (int)sizeof(member->ar_hdr.ar_date),
            (long int)stat_buf.st_mtime, (int)sizeof(member->ar_hdr.ar_uid),
            (unsigned short)stat_buf.st_uid, (int)sizeof(member->ar_hdr.ar_gid),
            (unsigned short)stat_buf.st_gid,
            (int)sizeof(member->ar_hdr.ar_mode), (unsigned int)stat_buf.st_mode,
            (int)sizeof(member->ar_hdr.ar_size), (long)size + ar_name_size,
            (int)sizeof(member->ar_hdr.ar_fmag), ARFMAG);
  } else {
    /*
    ...
    member->input_member_offset = ofile->member_offset - sizeof(struct ar_hdr);
    ...
    if (cmd_flags.use_long_names == TRUE) {
      ...
      if (ofile->member_name != ofile->member_ar_hdr->ar_name) {
        ...
        for (ar_name_size = member->member_name_size; ar_name_size > 1;
             ar_name_size--) {
        ...
        member->member_name_size = size_ar_name(&member->ar_hdr);
        ar_name_size =
            (uint32_t)(rnd(ofile->member_name_size, 8) +
                       (rnd(sizeof(struct ar_hdr), 8) - sizeof(struct ar_hdr)));
        member->output_long_name = TRUE;
        sprintf(ar_name_buf, "%s%-*lu", AR_EFMT1,
                (int)(sizeof(member->ar_hdr.ar_name) - (sizeof(AR_EFMT1) - 1)),
      ...
      if (ofile->member_name != ofile->member_ar_hdr->ar_name) {
        for (j = 0;
             j < sizeof(member->ar_hdr.ar_name) &&
             j < ofile->member_name_size && ofile->member_name[j] != '\0';
             j++)
     ...
     */
    sprintf(ar_size_buf, "%-*ld", (int)sizeof(member->ar_hdr.ar_size),
            (long)size + ar_name_size);
    memcpy(member->ar_hdr.ar_size, ar_size_buf, sizeof(member->ar_hdr.ar_size));
  }
...
static void create_library(char *output, struct ofile *ofile) {
  ...
  if (narchs == 0) {
    if (cmd_flags.ranlib == TRUE) {
      if (cmd_flags.q == FALSE)
        warning("empty library: %s (no table of contents added)", output);
      return;
    } else {
      if (cmd_flags.dynamic == FALSE || cmd_flags.no_files_ok == FALSE) {
        if (cmd_flags.arch_only_flag.name != NULL)
   ...
   */
  if (cmd_flags.ranlib == TRUE && get_unix_standard_mode() == TRUE &&
  ...
  if (cmd_flags.ranlib == TRUE && narchs == 1 && ofile != NULL &&
      ofile->toc_addr != NULL && ofile->toc_bad == FALSE &&
     ...
     * in place.
     */
    if (strncmp(ofile->toc_ar_hdr->ar_name, AR_EFMT1, sizeof(AR_EFMT1) - 1) ==
        0) {
      /*
  ...
  if (write_in_place == TRUE) {
     ...
     */
    time_offsets[0] =
        SARMAG + ((char *)&toc_ar_hdr.ar_date - (char *)&toc_ar_hdr);
      ...
      if (archs[0].using_64toc == FALSE) {
        for (i = 0; i < archs[0].toc_nranlibs; i++) {
          for (j = 0; j < archs[0].nmembers; j++) {
            if (archs[0].members[j].offset == archs[0].toc_ranlibs[i].ran_off) {
              archs[0].toc_ranlibs[i].ran_off =
                  (uint32_t)archs[0].members[j].input_member_offset;
              break;
            }
          }
        }
    ...
    if (snprintf(tempfile, templen, "%s%s", output, suffix) != (templen - 1)) {
    ...
    if (narchs > 1) {
      ...
      if (cmd_flags.fat64 == TRUE) {
        ...
        fat_arch64 =
            (struct fat_arch_64 *)(library + sizeof(struct fat_header));
        fat_arch = NULL;
      } else {
    ...
    for (i = 0; i < narchs; i++) {
       ...
       */
      time_offsets[i] =
          (p - library) + ((char *)&toc_ar_hdr.ar_date - (char *)&toc_ar_hdr);
      ...
      for (j = 0; j < arch->nmembers; j++) {
        flush_start = p;
        memcpy(p, (char *)&(arch->members[j].ar_hdr), sizeof(struct ar_hdr));
        ...
        memcpy(p, arch->members[j].object_addr, arch->members[j].object_size);
#ifdef VM_SYNC_DEACTIVATE
        vm_msync(mach_task_self(), (vm_address_t)arch->members[j].object_addr,
                 (vm_size_t)arch->members[j].object_size, VM_SYNC_DEACTIVATE);
   ...
   */
  if (fstat(fd, &stat_buf) == -1) {
    system_fatal("can't stat output file: %s", tempfile ? tempfile : output);
    return;
  }
  if (cmd_flags.D == TRUE || zero_ar_date == TRUE)
    toc_mtime = 0;
   ...
   */
  sprintf((char *)(&toc_ar_hdr), "%-*s%-*ld", (int)sizeof(toc_ar_hdr.ar_name),
          SYMDEF, (int)sizeof(toc_ar_hdr.ar_date), (long int)toc_mtime);
  for (i = 0; i < narchs; i++) {
...
static enum byte_sex get_target_byte_sex(struct arch *arch,
  ...
  for (i = 0; i < arch->nmembers && target_byte_sex == UNKNOWN_BYTE_SEX; i++) {
...
static char *put_toc_member(char *p, struct arch *arch,
  ...
  if (arch->using_64toc == FALSE) {
    ...
    if (target_byte_sex != host_byte_sex)
      swap_ranlib_64(arch->toc_ranlibs64, arch->toc_nranlibs, target_byte_sex);
    if (arch->toc_nranlibs != 0)
...
static void output_flush(char *library, uint64_t library_size, int fd,
  ...
  if (offset + size > library_size)
    fatal("internal error: output_flush(offset = %llu, size = %llu) "
          "out of range for library_size = %llu",
          offset, size, library_size);
  ...
  if (before != NULL && before->offset + before->size == offset) {
    ...
    if (after != NULL && offset + size == after->offset) {
      ...
      if (after->written_size == 0)
        write_size =
            trnc64(after->offset + after->size - write_offset, host_pagesize);
      else
        write_size =
            trnc64(after->written_offset - write_offset, host_pagesize);
      if (write_size != 0) {
   ...
   */
    ...
    if (after->written_size == 0)
      ...
      write_size = trnc64(after->written_offset - write_offset, host_pagesize);
    if (write_size != 0) {
  ...
  if (write_size != 0) {
    ...
    if (write64(fd, library + write_offset, write_size) != (ssize_t)write_size)
      system_fatal("can't write to output file");
    if ((r = vm_deallocate(mach_task_self(),
                           (vm_address_t)(library + write_offset),
 ...
 */
static void remove_block(struct block *block) { free(block); }
...
static void create_dynamic_shared_library(char *output) {
  ...
  if (cmd_flags.arch_only_flag.name != NULL) {
    family_arch_flag =
        get_arch_family_from_cputype(cmd_flags.arch_only_flag.cputype);
    if (family_arch_flag != NULL) {
  ...
  for (i = 0; i < narchs || (i == 0 && narchs == 0); i++) {
    ...
    if (sdkroot && sdkroot[0] == '/') {
      ...
      int res = asprintf(&sdk_dylib1o_path, "%s/usr/lib/dylib1.o", sdkroot);
  ...
  if (narchs > 1) {
    ...
    for (i = 0; i < narchs; i++) {
      p = makestr(cmd_flags.output, ".libtool.", archs[i].arch_flag.name, NULL);
      if (unlink(p) == -1) {
...
static void create_dynamic_shared_library_cleanup(int sig) {
  ...
  for (i = 0; i < narchs; i++) {
    (void)unlink(
        makestr(cmd_flags.output, ".libtool.", archs[i].arch_flag.name, NULL));
  }
  exit(EXIT_FAILURE);
}
...
static void make_table_of_contents(struct arch *arch, char *output) {
  ...
  for (i = 0; i < arch->nmembers; i++) {
    ...
    if (member->mh != NULL || member->mh64 != NULL) {
      ...
      if (member->mh != NULL)
        ...
        member->sections64 = allocate(nsects * sizeof(struct section_64 *));
      nsects = 0;
      lc = member->load_commands;
      for (j = 0; j < ncmds; j++) {
        if (lc->cmd == LC_SEGMENT) {
          sg = (struct segment_command *)lc;
          section =
              (struct section *)((char *)sg + sizeof(struct segment_command));
          for (k = 0; k < sg->nsects; k++) {
            member->sections[nsects++] = section++;
          }
        } else if (lc->cmd == LC_SEGMENT_64) {
          sg64 = (struct segment_command_64 *)lc;
          section64 = (struct section_64 *)((char *)sg64 +
      ...
      if (member->st != NULL && member->st->nsyms != 0) {
        if (member->mh != NULL) {
          symbols = (struct nlist *)(member->object_addr + member->st->symoff);
          if (member->object_byte_sex != get_host_byte_sex())
            swap_nlist(symbols, member->st->nsyms, get_host_byte_sex());
        } else {
          symbols64 =
              (struct nlist_64 *)(member->object_addr + member->st->symoff);
          if (member->object_byte_sex != get_host_byte_sex())
            swap_nlist_64(symbols64, member->st->nsyms, get_host_byte_sex());
        }
        ...
        for (j = 0; j < member->st->nsyms; j++) {
          ...
          if ((n_type & N_TYPE) == N_SECT) {
            if (n_sect == NO_SECT) {
              warn_member(arch, member,
                          ...
                          "n_sect field given its type (N_SECT))",
                          j);
              errors++;
              continue;
            }
          ...
          if (member->mh != NULL)
            is_toc_symbol = toc_symbol(symbols + j, member->sections);
          else
            is_toc_symbol = toc_symbol_64(symbols64 + j, member->sections64);
      ...
      if (member->st != NULL && member->st->nsyms != 0) {
        ...
        for (j = 0; j < member->st->nsyms; j++) {
          ...
          if (member->mh != NULL)
            ...
            is_toc_symbol = toc_symbol_64(symbols64 + j, member->sections64);
          if (is_toc_symbol == TRUE) {
            strcpy(arch->toc_strings + s, strings + n_strx);
        ...
        if (member->object_byte_sex != get_host_byte_sex()) {
          if (member->mh != NULL)
            swap_nlist(symbols, member->st->nsyms, member->object_byte_sex);
          else
   ...
   *	the strings
   */
  arch->toc_size = (uint32_t)(sizeof(struct ar_hdr) + sizeof(uint32_t) +
                              arch->toc_nranlibs * sizeof(struct ranlib) +
                              sizeof(uint32_t) + arch->toc_strsize);
  ...
  if (cmd_flags.toc64 == TRUE)
    ...
    for (i = 0; i < arch->nmembers; i++) {
      if (arch->members[i].offset + SARMAG + arch->toc_size > UINT32_MAX) {
        arch->using_64toc = TRUE;
        break;
      }
    }
  ...
  if (arch->using_64toc) {
    if (cmd_flags.use_long_names == FALSE && cmd_flags.L_or_T_specified == TRUE)
    ...
    arch->toc_ranlibs64 =
        allocate(sizeof(struct ranlib_64) * arch->toc_nranlibs);
    /*
     ...
     *   the strings
     */
    arch->toc_size = (uint32_t)(sizeof(struct ar_hdr) + sizeof(uint64_t) +
                                arch->toc_nranlibs * sizeof(struct ranlib_64) +
                                sizeof(uint64_t) + arch->toc_strsize);
  ...
  sprintf((char *)(&arch->toc_ar_hdr), "%-*s%-*ld%-*u%-*u%-*o%-*ld",
          (int)sizeof(arch->toc_ar_hdr.ar_name), ar_name,
          (int)sizeof(arch->toc_ar_hdr.ar_date), toc_time,
          (int)sizeof(arch->toc_ar_hdr.ar_uid), (unsigned short)toc_uid,
          (int)sizeof(arch->toc_ar_hdr.ar_gid), (unsigned short)toc_gid,
          (int)sizeof(arch->toc_ar_hdr.ar_mode), (unsigned int)toc_mode,
 ...
 */
static enum bool toc_symbol(struct nlist *symbol, struct section **sections) {
  return (toc(symbol->n_un.n_strx, symbol->n_type, symbol->n_value,
...
static enum bool toc_symbol_64(struct nlist_64 *symbol64,
                               struct section_64 **sections64) {
  return (toc(symbol64->n_un.n_strx, symbol64->n_type, symbol64->n_value,
...
static enum bool toc(uint32_t n_strx, uint8_t n_type, uint64_t n_value,
  ...
  if ((n_type & N_TYPE) == N_UNDF && n_value != 0 && cmd_flags.c == FALSE)
    return (FALSE);
  ...
  for (i = 0; i < arch->toc_nranlibs - 1; i++) {
    if (strcmp(arch->tocs[i].name, arch->tocs[i + 1].name) == 0) {
      ...
      if ((int)(arch->tocs[i].index1) > 0) {
        ...
        warn_member(arch, member, "defines symbol: %s", arch->tocs[i].name);
        arch->tocs[i].index1 = -(arch->tocs[i].index1);
      }
      if ((int)(arch->tocs[i + 1].index1) > 0) {
        member = arch->members + arch->tocs[i + 1].index1 - 1;
        warn_member(arch, member, "defines symbol: %s", arch->tocs[i + 1].name);
        arch->tocs[i + 1].index1 = -(arch->tocs[i + 1].index1);
      }
    }
  }
  ...
  for (i = 0; i < narchs; i++) {
    ...
    for (j = 0; j < archs[i].nmembers - 1; j++) {
      ...
      if (strncmp(archs[i].members[j].member_name,
                  archs[i].members[j + 1].member_name, len) == 0) {
        if (strcmp(archs[i].members[j].member_name, ALWAYS_LOAD_MEMBER_NAME) ==
            0)
          continue;
        fprintf(stderr, "%s: warning ", progname);
        if (narchs > 1)
          fprintf(stderr, "for architecture: %s ", archs[i].arch_flag.name);
        fprintf(stderr,
        ...
        // rdar://110498050 (Rename duplicate archive member names so dsymutil
        // and lldb and find correct .o file) lldb and dysmutil use the
        // filename/mod-time from debug notes to file the .o file in a static
        // library Duplicate file names mean that could be ambiguous.  To
        // disambiguate, we make a synthetic mod-time for files that have a
        // duplicate name.
        ++dupCount;
        // ar_data is a 12 byte field that needs to filled with spaces after the
        // number
