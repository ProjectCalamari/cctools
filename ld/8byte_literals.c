 ...
 */
__private_extern__ void literal8_merge(struct literal8_data *data,
  ...
  if (redo_live == FALSE) {
    ...
    for (i = 0; i < nliteral8s; i++) {
      ...
      fine_relocs[i].output_offset = lookup_literal8(literal8s[i], data, ms);
    }
 ...
 * to be ordered.
 */
__private_extern__ void literal8_order(struct literal8_data *data,
                                       struct merged_section *ms) {
#ifndef RLD
  ...
  if (dead_strip == TRUE) {
    ...
    literal8_order_lines =
        allocate(sizeof(struct literal8_order_line) * (line_number - 1));
    data->literal8_load_order_data->literal8_order_lines = literal8_order_lines;
  }
  ...
  while (i < ms->order_size) {
    if (get_hex_from_sectorder(ms, &i, &(literal8.long0), line_number) ==
      ...
      if (get_hex_from_sectorder(ms, &i, &(literal8.long1), line_number) ==
        ...
        if (dead_strip == TRUE) {
          literal8_order_lines[nliteral8_order_lines].literal8 = literal8;
          literal8_order_lines[nliteral8_order_lines].line_number = line_number;
          literal8_order_lines[nliteral8_order_lines].output_offset =
              output_offset;
 ...
 * order file if any.
 */
__private_extern__ void literal8_reset_live(struct literal8_data *data,
                                            struct merged_section *ms) {
#ifndef RLD
  ...
  if (ms->order_filename != NULL) {
    literal8_order_lines = data->literal8_load_order_data->literal8_order_lines;
    nliteral8_order_lines =
        data->literal8_load_order_data->nliteral8_order_lines;
 ...
 */
__private_extern__ enum bool get_hex_from_sectorder(struct merged_section *ms,
                                                    unsigned long *index,
                                                    unsigned long *value,
  ...
  while (i < ms->order_size &&
         (ms->order_addr[i] == ' ' || ms->order_addr[i] == '\t'))
    i++;
 ...
 * section (ms->s.align).
 */
__private_extern__ unsigned long lookup_literal8(struct literal8 literal8,
 ...
 * data block itself.
 */
__private_extern__ void literal8_output(struct literal8_data *data,
                                        struct merged_section *ms) {
  ...
  for (p = &(data->literal8_blocks); *p;) {
    literal8_block = *p;
    for (i = 0; i < literal8_block->used; i++) {
      memcpy(output_addr + offset, literal8_block->literal8s + i,
 ...
 * data block itself.
 */
__private_extern__ void literal8_free(struct literal8_data *data) {
 ...
 */
__private_extern__ void print_literal8_data(struct literal8_data *data,
                                            char *indent) {
  unsigned long i;
  struct literal8_block **p, *literal8_block;
  ...
  for (p = &(data->literal8_blocks); *p; p = &(literal8_block->next)) {
    ...
    print("%s\tnext 0x%x\n", indent, (unsigned int)(literal8_block->next));
    print("%s\tliteral8s\n", indent);
    for (i = 0; i < literal8_block->used; i++) {
 ...
 */
__private_extern__ void literal8_data_stats(struct literal8_data *data,
                                            struct merged_section *ms) {
  if (data == NULL)
    return;
