 ...
 */
#ifdef __DYNAMIC__
extern long kld_load_basefile(const char *base_filename);

/* Note: this takes only one object file name */
extern long kld_load(struct mach_header **header_addr,
                     const char *object_filename, const char *output_filename);

extern long kld_load_from_memory(struct mach_header **header_addr,
...
extern long kld_load_basefile_from_memory(const char *base_filename,
                                          char *base_addr, long base_size);

extern long kld_unload_all(long deallocate_sets);

extern long kld_lookup(const char *symbol_name, unsigned long *value);

extern long kld_forget_symbol(const char *symbol_name);

extern void kld_address_func(unsigned long (*func)(unsigned long size,
                                                   unsigned long headers_size));

#define KLD_STRIP_ALL 0x00000000
#define KLD_STRIP_NONE 0x00000001

extern void kld_set_link_options(unsigned long link_options);
