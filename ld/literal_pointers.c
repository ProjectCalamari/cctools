...
static unsigned long lookup_literal_pointer(struct merged_symbol *merged_symbol,
...
literal_pointer_order_line(unsigned long *line_start, unsigned long line_number,
                           struct literal_pointer_data *data,
                           struct merged_section *ms, char *buffer);
#endif /* !defined(RLD) */

static void count_reloc(struct merged_section *ms, enum bool new,
                        unsigned long r_extern, enum bool defined);
 ...
 * fine_relocs.
 */
__private_extern__ void literal_pointer_merge(struct literal_pointer_data *data,
                                              struct merged_section *ms,
                                              struct section *s,
  ...
  for (i = s->nreloc - 1; i >= 0; i--) {
    ...
    if (r_address % 4 != 0) {
      error_with_cur_obj(
          "r_address (0x%x) field of relocation entry "
    ...
    if (r_length != 2) {
      error_with_cur_obj(
          "r_length (0x%x) field of relocation entry "
    ...
    if (r_pcrel != 0) {
      error_with_cur_obj(
          "r_pcrel (0x%x) field of relocation entry "
    ...
    if (r_extern) {
      ...
      if (merged_symbol->nlist.n_type == (N_EXT | N_UNDF)) {
        ...
        merged_section_offset =
            fine_reloc_output_offset(literal_map, input_section_offset);
        /*
      ...
      if (r_symbolnum == R_ABS) {
        error_with_cur_obj(
            "r_symbolnum (0x%x) field of relocation "
      ...
      if (r_scattered == 0) {
        ...
        if (section_value < literal_s->addr ||
            section_value > literal_s->addr + literal_s->size) {
          error_with_cur_obj(
              "literal pointer (0x%x) in section "
              ...
              "it should",
              (unsigned int)section_value, s->segname, s->sectname,
              (unsigned int)(s->addr + r_address), literal_s->segname,
              literal_s->sectname, (unsigned int)r_symbolnum, i);
          continue;
        }
        if ((literal_ms->s.flags & SECTION_TYPE) != S_CSTRING_LITERALS &&
            (literal_ms->s.flags & SECTION_TYPE) != S_4BYTE_LITERALS &&
        ...
        if ((literal_ms->s.flags & SECTION_TYPE) != S_CSTRING_LITERALS &&
            ...
            (literal_ms->s.flags & SECTION_TYPE) != S_8BYTE_LITERALS) {
          error_with_cur_obj(
              "r_value field (0x%x) in relocation "
              "entry %ld in literal pointer section (%.16s,%.16s) "
              "refers to section (%.16s,%.16s) which is not a "
              "literal section",
              (unsigned int)r_value, i, s->segname, s->sectname,
    ...
    if (redo_live == FALSE) {
      ...
      if (fine_relocs[r_address / 4].live == TRUE) {
         ...
         */
        ms->relocated = TRUE;
        fine_relocs[r_address / 4].output_offset = lookup_literal_pointer(
            merged_symbol, literal_ms, merged_section_offset, offset, data, ms,
            &new);
        count_reloc(ms, new, r_extern, defined);
      } else {
 ...
 */
__private_extern__ void literal_pointer_order(struct literal_pointer_data *data,
                                              struct merged_section *ms) {
#ifndef RLD
  ...
  for (i = 0; i < ms->order_size; i++) {
    if (ms->order_addr[i] == '\0') {
      fatal("format error in -sectorder file: %s line %lu character "
            ...
            "character \'\\0\' found)",
            ms->order_filename, line_number, line_length, ms->s.segname,
            ms->s.sectname);
    }
    if (ms->order_addr[i] == '\n') {
      ms->order_addr[i] = '\0';
  ...
  if (dead_strip == TRUE) {
    ...
    order_lines =
        allocate(sizeof(struct literal_pointer_order_line) * (line_number - 1));
    data->literal_pointer_load_order_data->order_line_buffer = buffer;
  ...
  for (i = 0; i < ms->order_size; i++) {
    ...
    output_offset =
        literal_pointer_order_line(&i, line_number, data, ms, buffer);
    if (dead_strip == TRUE)
...
literal_pointer_order_line(unsigned long *line_start, unsigned long line_number,
  ...
  if (literal_ms == NULL) {
    ...
    case S_CSTRING_LITERALS:
      get_cstring_from_sectorder(ms, &i, buffer, line_number, char_pos);
      merged_section_offset =
          lookup_cstring(buffer, literal_ms->literal_data, literal_ms);
      output_offset = lookup_literal_pointer(
          NULL, literal_ms, merged_section_offset, 0, data, ms, &new);
    ...
    case S_4BYTE_LITERALS:
      if (get_hex_from_sectorder(ms, &i, &(literal4.long0), line_number) ==
          TRUE) {
        merged_section_offset =
            lookup_literal4(literal4, literal_ms->literal_data, literal_ms);
        output_offset = lookup_literal_pointer(
            NULL, literal_ms, merged_section_offset, 0, data, ms, &new);
        count_reloc(ms, new, 0, FALSE);
      } else {
    ...
    case S_8BYTE_LITERALS:
      if (get_hex_from_sectorder(ms, &i, &(literal8.long0), line_number) ==
          TRUE) {
        if (get_hex_from_sectorder(ms, &i, &(literal8.long1), line_number) ==
            TRUE) {
          merged_section_offset =
              lookup_literal8(literal8, literal_ms->literal_data, literal_ms);
          output_offset = lookup_literal_pointer(
              NULL, literal_ms, merged_section_offset, 0, data, ms, &new);
          count_reloc(ms, new, 0, FALSE);
        } else {
...
literal_pointer_reset_live(struct literal_pointer_data *data,
                           struct merged_section *ms) {
#ifndef RLD
  ...
  if (ms->order_filename != NULL) {
    ...
    for (i = 0; i < norder_lines; i++) {
       ...
       */
      live = is_literal_output_offset_live(ms, order_lines[i].output_offset);
    ...
    free(data->literal_pointer_load_order_data->literal_pointer_order_lines);
  ...
  *new = FALSE;
  if (data->hashtable == NULL) {
    ...
    memset(data->hashtable, '\0',
           sizeof(struct literal_pointer_bucket *) * LITERAL_POINTER_HASHSIZE);
  }
#if defined(DEBUG) && defined(PROBE_COUNT)
  data->nprobes++;
#endif
  hashval = ((long)merged_symbol + (long)literal_ms + merged_section_offset +
             offset) %
            LITERAL_POINTER_HASHSIZE;
  for (bp = data->hashtable[hashval]; bp; bp = bp->next) {
    ...
    if (bp->literal_pointer->merged_symbol == merged_symbol &&
        ...
        bp->literal_pointer->merged_section_offset == merged_section_offset &&
  ...
  for (p = &(data->literal_pointer_blocks); *p;
    ...
    if (literal_pointer_block->used != LITERAL_POINTER_BLOCK_SIZE) {
      ...
      bp->output_offset = output_offset + literal_pointer_block->used * 4;
 ...
 */
__private_extern__ void
literal_pointer_output(struct literal_pointer_data *data,
                       struct merged_section *ms) {
  unsigned long i;
  long *output_pointer;
  struct literal_pointer_block **p, *literal_pointer_block;
  ...
  for (p = &(data->literal_pointer_blocks); *p;
    ...
    for (i = 0; i < literal_pointer_block->used; i++) {
      if (literal_pointers[i].merged_symbol != NULL) {
        ...
        *output_pointer = literal_pointers[i].literal_ms->s.addr +
  ...
  if (save_reloc || output_for_dyld) {
    if (output_for_dyld) {
      extreloc = (struct relocation_info
                      *)(output_addr +
    ...
    for (p = &(data->literal_pointer_blocks); *p;
      ...
      for (i = 0; i < literal_pointer_block->used; i++) {
        ...
        if (literal_pointers[i].merged_symbol != NULL) {
          ...
          r->r_symbolnum =
              merged_symbol_output_index(literal_pointers[i].merged_symbol);
          r->r_pcrel = 0;
          r->r_length = 2;
          r->r_extern = 1;
          r->r_type = 0;
         ...
         */
          ...
          reloc->r_symbolnum = literal_pointers[i].literal_ms->output_sectnum;
          reloc->r_pcrel = 0;
          reloc->r_length = 2;
          reloc->r_extern = 0;
          reloc->r_type = 0;
         ...
         */
          ...
          sreloc->r_value = literal_pointers[i].literal_ms->s.addr +
                            literal_pointers[i].merged_section_offset;
          reloc++;
          sreloc++;
        }
    ...
    if (output_for_dyld) {
      ...
      if (host_byte_sex != target_byte_sex)
        swap_relocation_info(
            (struct relocation_info *)(output_addr + ms->s.reloff),
            ms->s.nreloc, target_byte_sex);
      output_flush(ms->s.reloff, ms->s.nreloc * sizeof(struct relocation_info));
    }
  }
#endif /* !defined(RLD) */
 ...
 */
__private_extern__ void
literal_pointer_free(struct literal_pointer_data *data) {
  unsigned long i;
 ...
 */
__private_extern__ void
print_literal_pointer_data(struct literal_pointer_data *data, char *indent) {
  unsigned long i;
  struct literal_pointer_block **p, *literal_pointer_block;
  struct literal_pointer *literal_pointers;
    ...
    for (i = 0; i < literal_pointer_block->used; i++) {
      if (literal_pointers[i].merged_symbol != NULL) {
        ...
        print("%s    section (%.16s,%.16s) section_offset %lu "
              "offset %lu\n",
              indent, literal_pointers[i].literal_ms->s.segname,
 ...
 */
__private_extern__ void
literal_pointer_data_stats(struct literal_pointer_data *data,
                           struct merged_section *ms) {
  if (data == NULL)
    return;
