...
static int test_main(void) {
  ...
  if (!err)
    err = test_add("test allocate", test_allocate);
  if (!err)
    err = test_add("test reallocate", test_reallocate);
  if (!err)
    err = test_add("test savestr", test_savestr);
  if (!err)
    err = test_add("test makestr", test_makestr);
