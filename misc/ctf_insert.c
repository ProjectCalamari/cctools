...
static void ctf_insert(struct arch *arch, struct member *member,
                       struct object *object);

static void add_ctf_section(struct arch *arch, char *arch_name, uint32_t offset,
  ...
  for (i = 1; i < argc; i++) {
    if (strcmp(argv[i], "-o") == 0) {
      ...
      if (i + 2 == argc) {
        ...
        arch_ctfs =
            reallocate(arch_ctfs, (narch_ctfs + 1) * sizeof(struct arch_ctf));
        ...
        if (stat_buf.st_size != 0) {
          contents = mmap(0, stat_buf.st_size, PROT_READ | PROT_WRITE,
      ...
      if (input != NULL) {
        error("more than one input file file: %s specified", input);
        usage();
      }
      input = argv[i];
    }
  ...
  for (i = 0; i < narch_ctfs; i++) {
    if (arch_ctfs[i].arch_found == FALSE)
      fatal("input file: %s does not contain a matching architecture "
            "for specified '-arch %s %s' option",
            input, arch_ctfs[i].arch_flag.name, arch_ctfs[i].filename);
  }
...
static void process(struct arch *archs, uint32_t narchs) {
  ...
  for (i = 0; i < narchs; i++) {
    if (archs[i].type == OFILE_Mach_O)
      ...
      fatal_arch(archs + i, NULL,
                 "file type not valid input for "
...
static void ctf_insert(struct arch *arch, struct member *member,
   ...
   */
  segalign = get_seg_align(object->mh, object->mh64, object->load_commands,
   ...
   */
  if ((flags & MH_DYLDLINK) == MH_DYLDLINK || object->dyld_info != NULL ||
    ...
    fatal_arch(arch, member,
               "file is input for the dynamic linker so "
   ...
   */
  if (object->nnote > 0)
    fatal_arch(arch, member, "file contains LC_NOTE, which is not supported: ");
  ...
  if (object->split_info_cmd) {
    if (!(object->split_info_cmd->dataoff &&
      ...
      fatal_arch(arch, member,
                 "file for input contains split-seg "
   ...
   */
  if (object->dyst != NULL &&
      (object->dyst->ntoc != 0 || object->dyst->nmodtab != 0 ||
       object->dyst->nmodtab != 0 || object->dyst->nextrefsyms != 0 ||
  ...
  if (i >= narch_ctfs) {
    fatal_arch(arch, member,
               "no matching -arch option for this slice "
               "of file: ");
    return;
  }
   ...
   * moved.
   */
  add_ctf_section(arch, arch_ctfs[i].arch_flag.name, offset, addr,
                  arch_ctfs[i].size);
...
static void add_ctf_section(struct arch *arch, char *arch_name, uint32_t offset,
  ...
  if (arch->object->mh != NULL) {
    ...
    added_header_size = sizeof(struct segment_command) + sizeof(struct section);
  } else {
  ...
  for (i = 0; i < ncmds; i++) {
    if (lc->cmd == LC_SEGMENT) {
      ...
      if (strcmp(sg->segname, "__LINKEDIT") == 0)
        linkedit_offset =
            (uint32_t)((char *)lc - (char *)arch->object->load_commands);
      ...
      if (sg->nsects != 0) {
        ...
        if (sg->filesize != 0 && sg->fileoff != 0 && sg->fileoff < low_fileoff)
  ...
  if (arch->object->mh != NULL) {
    newsizeofcmds += (sizeof(struct segment_command) + sizeof(struct section));
  } else {
    newsizeofcmds +=
        (sizeof(struct segment_command_64) + sizeof(struct section_64));
  }
   ...
   */
  if (arch->object->mh != NULL) {
    sg_CTF = (struct segment_command *)&((char *)buffer)[written];
    memset(sg_CTF, '\0', sizeof(struct segment_command));
    sg_CTF->cmd = LC_SEGMENT;
    sg_CTF->cmdsize = sizeof(struct segment_command) + sizeof(struct section);
    ...
    s_ctf = (struct section *)((char *)sg_CTF + sizeof(struct segment_command));
    ...
    s_ctf->reserved1 = 0;
    s_ctf->reserved2 = 0;
    written += (sizeof(struct segment_command) + sizeof(struct section));
  } else {
    sg64_CTF = (struct segment_command_64 *)&((char *)buffer)[written];
    ...
    s64_ctf = (struct section_64 *)((char *)sg64_CTF +
                                    sizeof(struct segment_command_64));
    ...
    s64_ctf->reserved1 = 0;
    s64_ctf->reserved2 = 0;
    written += (sizeof(struct segment_command_64) + sizeof(struct section_64));
  }
