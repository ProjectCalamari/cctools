...
__private_extern__ char *progname =
    NULL;                 /* name of the program for error messages (argv[0]) */
...
/*
 * This is set if the object is an executable (MH_EXECUTE) that is for use with
 * the dynamic linker.
 */
...
static void strip_file(char *input_file, struct arch_flag *arch_flags,
                       uint32_t narch_flags, enum bool all_archs,
                       enum bool no_options);

static void strip_arch(struct arch *archs, uint32_t narchs,
                       struct arch_flag *arch_flags, uint32_t narch_flags,
                       enum bool all_archs);

static void strip_object(struct arch *arch, struct member *member,
                         struct object *object);

static uint32_t get_starting_syminfo_offset(struct object *object);

static void check_object_relocs(struct arch *arch, struct member *member,
                                struct object *object, char *segname,
                                char *sectname, uint64_t sectsize,
                                char *contents, struct relocation_info *relocs,
                                uint32_t nreloc, struct nlist *symbols,
                                struct nlist_64 *symbols64, uint32_t nsyms,
                                char *strings, int32_t *missing_reloc_symbols,
                                enum byte_sex host_byte_sex);

static void check_indirect_symtab(struct arch *arch, struct member *member,
                                  struct object *object, uint32_t nitems,
                                  uint32_t reserved1, uint32_t section_type,
                                  char *contents, struct nlist *symbols,
                                  struct nlist_64 *symbols64, uint32_t nsyms,
                                  char *strings, int32_t *missing_reloc_symbols,
...
#ifndef NMEDIT
static enum bool strip_symtab(struct arch *arch, struct member *member,
                              struct object *object,
                              struct dylib_table_of_contents *tocs,
                              uint32_t ntoc, struct dylib_module *mods,
                              struct dylib_module_64 *mods64, uint32_t nmodtab,
                              struct dylib_reference *refs,
                              uint32_t nextrefsyms, uint32_t *p_swift_version,
                              enum bool *nlist_outofsync_with_dyldinfo);

#ifdef TRIE_SUPPORT
static int prune(const char *name);
#endif /* TRIE_SUPPORT */

static void make_ld_r_object(struct arch *arch, struct member *member,
                             struct object *object);

static void strip_LC_UUID_commands(struct arch *arch, struct member *member,
                                   struct object *object);

static void strip_LC_SEGMENT_SPLIT_INFO_command(struct arch *arch,
                                                struct member *member,
                                                struct object *object);

static void strip_LC_ATOM_INFO_command(struct arch *arch, struct member *member,
                                       struct object *object);

#ifndef NMEDIT
static void strip_LC_CODE_SIGNATURE_commands(struct arch *arch,
                                             struct member *member,
                                             struct object *object);
#endif /* !(NMEDIT) */

static enum bool private_extern_reference_by_module(
    uint32_t symbol_index, struct dylib_reference *refs, uint32_t nextrefsyms);

static enum bool symbol_pointer_used(uint32_t symbol_index,
                                     uint32_t *indirectsyms,
                                     uint32_t nindirectsyms);

static int cmp_qsort_strx_map(const struct strx_map *a,
                              const struct strx_map *b);

static int cmp_bsearch_strx_map(const uint32_t *old_strx,
                                const struct strx_map *strx_map);

static int cmp_qsort_undef_map(const struct undef_map *sym1,
                               const struct undef_map *sym2);

static int cmp_qsort_undef_map_64(const struct undef_map64 *sym1,
                                  const struct undef_map64 *sym2);
#endif /* !defined(NMEDIT) */

#ifdef NMEDIT
static enum bool edit_symtab(
    struct arch *arch, struct member *member, struct object *object,
    struct nlist *symbols, struct nlist_64 *symbols64, uint32_t nsyms,
    char *strings, uint32_t strsize, struct dylib_table_of_contents *tocs,
    uint32_t ntoc, struct dylib_module *mods, struct dylib_module_64 *mods64,
    uint32_t nmodtab, struct dylib_reference *refs, uint32_t nextrefsyms);
#endif /* NMEDIT */

#ifndef NMEDIT
static void setup_debug_filenames(char *dfile);

static int cmp_qsort_filename(const char **name1, const char **name2);

static int cmp_bsearch_filename(const char *name1, const char **name2);
#endif /* NMEDIT */
...
static int cmp_qsort_global(const struct nlist **sym1,
                            const struct nlist **sym2);

static int cmp_qsort_global_64(const struct nlist_64 **sym1,
                               const struct nlist_64 **sym2);

static int cmp_bsearch_global_stab(const char *name, const struct nlist **sym);

static int cmp_bsearch_global_stab_64(const char *name,
                                      const struct nlist_64 **sym);

static int cmp_bsearch_global(const char *name, const struct nlist **sym);

static int cmp_bsearch_global_64(const char *name, const struct nlist_64 **sym);
#endif /* NMEDIT */
...
int main(int argc, char *argv[], char *envp[]) {
      ...
      if (strcmp(argv[i], "-o") == 0) {
        ...
        if (strcmp("all", argv[i + 1]) == 0) {
          ...
          for (j = 0; j < narch_flags; j++) {
            if (arch_flags[j].cputype == arch_flags[narch_flags].cputype &&
                (arch_flags[j].cpusubtype & ~CPU_SUBTYPE_MASK) ==
                    (arch_flags[narch_flags].cpusubtype & ~CPU_SUBTYPE_MASK) &&
                strcmp(arch_flags[j].name, arch_flags[narch_flags].name) == 0)
              break;
          }
          if (j == narch_flags)
            narch_flags++;
  ...
  if (Rfile) {
    ...
    if (sfile) {
      for (j = 0; j < nremove_symbols; j++) {
        sp = bsearch(remove_symbols[j].name, save_symbols, nsave_symbols,
  ...
  for (i = 1; i < argc; i++) {
    if (args_left && argv[i][0] == '-') {
      if (argv[i][1] == '\0')
        args_left = 0;
      else if (strcmp(argv[i], "-o") == 0 || strcmp(argv[i], "-s") == 0 ||
               strcmp(argv[i], "-R") == 0 ||
#ifndef NMEDIT
               strcmp(argv[i], "-d") == 0 ||
#endif /* !defined(NMEDIT) */
...
static void strip_file(char *input_file, struct arch_flag *arch_flags,
  ...
  if (output_file != NULL) {
    writeout(archs, narchs, output_file, stat_buf.st_mode & 0777, TRUE, FALSE,
#ifdef NMEDIT
             FALSE,
#else
             toc64flag,
     ...
     */
    if (unix_standard_mode == TRUE && access(input_file, W_OK) == -1) {
    ...
    if (rename_file != NULL) {
      if (rename(output_file, rename_file) == -1)
        system_error("can't move temporary file: %s to file: %s", output_file,
                     rename_file);
      free(rename_file);
    } else {
...
static void strip_arch(struct arch *archs, uint32_t narchs,
  ...
  for (i = 0; i < narchs; i++) {
    ...
    if (all_archs == TRUE) {
      ...
      family = FALSE;
      if (narch_flags == 1) {
        family_arch_flag = get_arch_family_from_cputype(arch_flags[0].cputype);
        if (family_arch_flag != NULL)
          family =
              (enum bool)((family_arch_flag->cpusubtype & ~CPU_SUBTYPE_MASK) ==
    ...
    if (archs[i].type == OFILE_ARCHIVE) {
      ...
      for (j = 0; j < archs[i].nmembers; j++) {
        ...
        if (archs[i].members[j].member_long_name == TRUE) {
          size = rnd32(archs[i].members[j].member_name_size, 8) +
                 (rnd32(sizeof(struct ar_hdr), 8) - sizeof(struct ar_hdr));
          archs[i].toc_long_name = TRUE;
        }
        if (archs[i].members[j].object != NULL) {
          size += rnd(archs[i].members[j].object->object_size -
      ...
      if (deterministic_archives == TRUE) {
        sprintf(date_buf, "%-*ld", (int)sizeof(h.ar_date), (unsigned long)0);
        /*
         * This has to be done by hand because sprintf puts a
         * null at the end of the buffer.
         */
  ...
  if (all_archs == FALSE && narch_flags != 0) {
    for (i = 0; i < narch_flags; i++) {
      if (arch_flag_processed[i] == FALSE)
        error("file: %s does not contain architecture: %s", archs[0].file_name,
              arch_flags[i].name);
    }
...
static void strip_object(struct arch *arch, struct member *member,
  ...
  if ((object->st == NULL || object->st->nsyms == 0)
    ...
    warning_arch(arch, member, "input object file already stripped: ");
  }
  ...
  if (object->st != NULL) {
    ...
    if (object->mh != NULL) {
      ...
      symbols = NULL;
      symbols64 = (struct nlist_64 *)(object->object_addr + object->st->symoff);
      if (object->object_byte_sex != host_byte_sex)
  ...
  if (object->mh_filetype == MH_DYLIB && (flags & MH_PREBOUND) != MH_PREBOUND) {
  ...
  if (object->mh_filetype == MH_DYLIB) {
    tocs = (struct dylib_table_of_contents *)(object->object_addr +
                                              object->dyst->tocoff);
    ntoc = object->dyst->ntoc;
    nmodtab = object->dyst->nmodtab;
    if (object->mh != NULL) {
      mods = (struct dylib_module *)(object->object_addr +
                                     object->dyst->modtaboff);
      if (object->object_byte_sex != host_byte_sex)
        swap_dylib_module(mods, nmodtab, host_byte_sex);
      mods64 = NULL;
    } else {
      mods = NULL;
      mods64 = (struct dylib_module_64 *)(object->object_addr +
                                          object->dyst->modtaboff);
      if (object->object_byte_sex != host_byte_sex)
        swap_dylib_module_64(mods64, nmodtab, host_byte_sex);
    }
    refs = (struct dylib_reference *)(object->object_addr +
                                      object->dyst->extrefsymoff);
      ...
      if (object->mh != NULL) {
        ...
        object->object_size -=
            (object->seg_linkedit64->fileoff -
             (sizeof(struct mach_header_64) + object->mh64->sizeofcmds));
        /*
         * Set the file offset to the link edit information to be
         * right after the load commands.
         */
  ...
  if (object->dyst != NULL && object->dyst->nindirectsyms != 0) {
    ...
    indirectsyms =
        (uint32_t *)(object->object_addr + object->dyst->indirectsymoff);
    if (object->object_byte_sex != host_byte_sex)
      swap_indirect_symbols(indirectsyms, nindirectsyms, host_byte_sex);
  } else {
    indirectsyms = NULL;
    nindirectsyms = 0;
  }
  ...
  if (object->mh != NULL)
    object->input_sym_info_size = nsyms * sizeof(struct nlist) + strsize;
  else
    object->input_sym_info_size = nsyms * sizeof(struct nlist_64) + strsize;
#ifndef NMEDIT
  if (object->mh != NULL)
    flags = object->mh->flags;
  else
  ...
  if (Nflag && (mh_flags & MH_DYLDLINK) == MH_DYLDLINK &&
      object->mh_filetype != MH_KEXT_BUNDLE && object->dyst != NULL &&
...
#endif /* !defined(NMEDIT) */
  {
#ifdef NMEDIT
    if (object->st != NULL) {
      if (edit_symtab(arch, member, object, symbols, symbols64, nsyms, strings,
                      strsize, tocs, ntoc, mods, mods64, nmodtab, refs,
                      nextrefsyms) == FALSE)
        return;
    }
#else  /* !defined(NMEDIT) */
    if (object->st != NULL) {
      if (strip_symtab(arch, member, object, tocs, ntoc, mods, mods64, nmodtab,
                       refs, nextrefsyms, &swift_version,
                       &nlist_outofsync_with_dyldinfo) == FALSE)
        return;
    }
    if (no_uuid == TRUE)
    ...
    if (object->dyld_info != NULL) {
      ...
      if (object->dyld_info->export_size != 0)
        dyld_info_end =
            object->dyld_info->export_off + object->dyld_info->export_size;
        ...
        dyld_info_end =
            object->dyld_info->bind_off + object->dyld_info->bind_size;
      else if (object->dyld_info->rebase_size != 0)
        dyld_info_end =
            object->dyld_info->rebase_off + object->dyld_info->rebase_size;
#ifndef NMEDIT
      /*
      ...
      object->input_sym_info_size +=
          object->dyld_info->rebase_size + object->dyld_info->bind_size +
          object->dyld_info->weak_bind_size +
          object->dyld_info->lazy_bind_size + object->dyld_info->export_size;
#ifndef NMEDIT
      /*
       * When stripping out the section contents to create a
       * dynamic library stub we only keep the dyld export data,
...
#endif /* !(NMEDIT) */
      {
        object->output_sym_info_size += object->dyld_chained_fixups->datasize;
    ...
    if (object->dyld_exports_trie != NULL) {
      ...
      object->output_sym_info_size += object->dyld_exports_trie->datasize;
    ...
    if (object->function_variants != NULL) {
      object->input_sym_info_size += object->function_variants->datasize;
      object->output_sym_info_size += object->function_variants->datasize;
    ...
    if (object->function_variant_fixups != NULL) {
      object->input_sym_info_size += object->function_variant_fixups->datasize;
      object->output_sym_info_size += object->function_variant_fixups->datasize;
    ...
    if (object->split_info_cmd != NULL) {
...
#endif /* !(NMEDIT) */
      {
        ...
        object->output_split_info_data_size = object->split_info_cmd->datasize;
    ...
    if (object->nlazyLoads != 0) {
      for (uint32_t i = 0; i < object->nlazyLoads; ++i) {
        ...
        object->lazyLoads[i].data =
            (uint8_t *)object->object_addr + object->lazyLoads[i].lc->dataoff;
    ...
    if (object->func_starts_info_cmd != NULL) {
...
#endif /* !(NMEDIT) */
      {
        ...
        object->output_sym_info_size += object->func_starts_info_cmd->datasize;
      }
      object->input_sym_info_size += object->func_starts_info_cmd->datasize;
    }
    ...
    if (object->data_in_code_cmd != NULL) {
...
#endif /* !(NMEDIT) */
      {
        ...
        object->output_sym_info_size += object->data_in_code_cmd->datasize;
      }
      object->input_sym_info_size += object->data_in_code_cmd->datasize;
    }
    ...
    if (object->atom_info_cmd != NULL) {
...
#endif /* !(NMEDIT) */
      {
        ...
        object->output_atom_info_data_size = object->atom_info_cmd->datasize;
    ...
    if (object->code_sign_drs_cmd != NULL) {
      ...
      object->input_sym_info_size += object->code_sign_drs_cmd->datasize;
      object->output_sym_info_size += object->code_sign_drs_cmd->datasize;
    }
    ...
    if (object->link_opt_hint_cmd != NULL) {
      ...
      object->input_sym_info_size += object->link_opt_hint_cmd->datasize;
      object->output_sym_info_size += object->link_opt_hint_cmd->datasize;
    }
    ...
    if (object->st != NULL) {
      if (object->mh != NULL) {
        ...
        object->output_sym_info_size += new_nsyms * sizeof(struct nlist);
      } else {
        object->input_sym_info_size += nsyms * sizeof(struct nlist_64);
        object->output_symbols64 = new_symbols64;
        object->output_sym_info_size += new_nsyms * sizeof(struct nlist_64);
      }
...
#endif /* !(NMEDIT) */
      {
        object->output_code_sig_data =
            object->object_addr + object->code_sig_cmd->dataoff;
        object->output_code_sig_data_size = object->code_sig_cmd->datasize;
      }
      object->input_sym_info_size = rnd32(object->input_sym_info_size, 16);
...
#endif /* !(NMEDIT) */
      {
        object->output_sym_info_size = rnd32(object->output_sym_info_size, 16);
    ...
    if (object->dyst != NULL) {
...
#ifdef NMEDIT
          if (object->mh != NULL)
            swap_dylib_module(new_mods, new_nmodtab, object->object_byte_sex);
          else
            swap_dylib_module_64(new_mods64, new_nmodtab,
                                 object->object_byte_sex);
#else
          if (object->mh != NULL)
            swap_dylib_module(mods, nmodtab, object->object_byte_sex);
          else
            swap_dylib_module_64(mods64, nmodtab, object->object_byte_sex);
#endif
      ...
      if (object->dyst->nlocrel != 0) {
        object->output_loc_relocs =
            (struct relocation_info *)(object->object_addr +
                                       object->dyst->locreloff);
#ifndef NMEDIT
        /*
         * When stripping out the section contents to create a
         * dynamic library stub the relocation info also gets
      ...
      if (object->hints_cmd != NULL) {
        if (object->hints_cmd->nhints != 0) {
          object->output_hints =
              (struct twolevel_hint *)(object->object_addr +
      ...
      if (object->dyst->nextrel != 0) {
        object->output_ext_relocs =
            (struct relocation_info *)(object->object_addr +
                                       object->dyst->extreloff);
#ifndef NMEDIT
        /*
         * When stripping out the section contents to create a
         * dynamic library stub the relocation info also gets
      ...
      if (object->dyst->ntoc != 0) {
        ...
        offset += object->dyst->ntoc * sizeof(struct dylib_table_of_contents);
      } else
      ...
      if (object->dyst->nmodtab != 0) {
        ...
        if (object->mh != NULL)
          ...
          offset += object->dyst->nmodtab * sizeof(struct dylib_module_64);
      } else
        object->dyst->modtaboff = 0;

      if (object->dyst->nextrefsyms != 0) {
        object->dyst->extrefsymoff = offset;
        offset += object->dyst->nextrefsyms * sizeof(struct dylib_reference);
      } else
      ...
      if (object->st != NULL) {
        ...
        if (new_strsize != 0) {
          if (object->mh != NULL)
            object->st->stroff =
                object->st->symoff + new_nsyms * sizeof(struct nlist);
          else
            object->st->stroff =
                object->st->symoff + new_nsyms * sizeof(struct nlist_64);
        } else
          object->st->stroff = 0;
  ...
  if (object->seg_linkedit != NULL) {
    ...
    object->seg_linkedit64->vmsize =
        (object->seg_linkedit64->filesize + 0x3FFF) & (-0x4000);
  }
    ...
    if (lc->cmd == LC_SEGMENT &&
      ...
      for (j = 0; j < sg->nsects; j++) {
        if (s->nreloc != 0) {
          ...
          relocs = (struct relocation_info *)(object->object_addr + s->reloff);
          if (object->object_byte_sex != host_byte_sex)
          ...
          check_object_relocs(arch, member, object, s->segname, s->sectname,
                              s->size, contents, relocs, s->nreloc, symbols,
                              symbols64, nsyms, strings, &missing_reloc_symbols,
                              host_byte_sex);
          if (object->object_byte_sex != host_byte_sex)
            swap_relocation_info(relocs, s->nreloc, object->object_byte_sex);
        }
        s++;
      }
    } else if (lc->cmd == LC_SEGMENT_64 &&
      ...
      for (j = 0; j < sg64->nsects; j++) {
        if (s64->nreloc != 0) {
          ...
          relocs =
              (struct relocation_info *)(object->object_addr + s64->reloff);
          if (object->object_byte_sex != host_byte_sex)
            swap_relocation_info(relocs, s64->nreloc, host_byte_sex);
          ...
          check_object_relocs(arch, member, object, s64->segname, s64->sectname,
                              s64->size, contents, relocs, s64->nreloc, symbols,
                              symbols64, nsyms, strings, &missing_reloc_symbols,
                              host_byte_sex);
          if (object->object_byte_sex != host_byte_sex)
            swap_relocation_info(relocs, s64->nreloc, object->object_byte_sex);
        }
        s64++;
      }
    }
  ...
  if (object->dyst != NULL && object->dyst->nextrel != 0) {
    ...
    if (object->object_byte_sex != host_byte_sex)
      swap_relocation_info(relocs, object->dyst->nextrel, host_byte_sex);

    for (i = 0; i < object->dyst->nextrel; i++) {
      if ((relocs[i].r_address & R_SCATTERED) == 0 && relocs[i].r_extern == 1) {
        if (relocs[i].r_symbolnum > nsyms) {
          fatal_arch(arch, member,
                     "bad r_symbolnum for external "
                     "relocation entry %d in: ",
                     i);
        }
        if (saves[relocs[i].r_symbolnum] == 0) {
          if (missing_reloc_symbols == 0) {
            error_arch(arch, member,
                       "symbols "
          ...
          if (object->mh != NULL) {
            fprintf(stderr, "%s\n",
                    strings + symbols[relocs[i].r_symbolnum].n_un.n_strx);
          } else {
            fprintf(stderr, "%s\n",
                    strings + symbols64[relocs[i].r_symbolnum].n_un.n_strx);
          }
          saves[relocs[i].r_symbolnum] = -1;
        }
        if (saves[relocs[i].r_symbolnum] != -1) {
          relocs[i].r_symbolnum = saves[relocs[i].r_symbolnum] - 1;
        }
      } else {
        fatal_arch(arch, member,
    ...
    for (i = 0; i < ncmds; i++) {
      if (lc->cmd == LC_SEGMENT &&
        ...
        for (j = 0; j < sg->nsects; j++) {
          ...
          contents = object->object_addr + s->offset;
          check_indirect_symtab(arch, member, object, nitems, s->reserved1,
                                section_type, contents, symbols, symbols64,
        ...
        for (j = 0; j < sg64->nsects; j++) {
          ...
          contents = object->object_addr + s64->offset;
          check_indirect_symtab(arch, member, object, nitems, s64->reserved1,
                                section_type, contents, symbols, symbols64,
                                nsyms, strings, &missing_reloc_symbols,
                                swift_version, host_byte_sex);
          s64++;
  ...
  if (object->code_sig_cmd != NULL) {
    ...
    if (codedir_is_linker_signed(dataaddr, datasize)) {
      codedir_create_object(output_file ? output_file : input_leaf_name,
                            object->object_addr, orgCodeSigOffset,
...
static uint32_t get_starting_syminfo_offset(struct object *object) {
  ...
  if (object->seg_linkedit != NULL || object->seg_linkedit64 != NULL) {
    ...
    if (object->atom_info_cmd != NULL && object->atom_info_cmd->datasize != 0 &&
        object->atom_info_cmd->dataoff < offset)
    ...
    if (object->dyst != NULL && object->dyst->ntoc != 0 &&
        object->dyst->tocoff < offset)
...
static void check_object_relocs(struct arch *arch, struct member *member,
  ...
  for (k = 0; k < nreloc; k++) {
    if ((relocs[k].r_address & R_SCATTERED) == 0 && relocs[k].r_extern == 1) {
      if (relocs[k].r_symbolnum > nsyms) {
        fatal_arch(arch, member,
                   "bad r_symbolnum for relocation "
                   "entry %d in section (%.16s,%.16s) in: ",
                   k, segname, sectname);
      }
      if (object->mh != NULL) {
       ...
       */
      if (object->mh != NULL)
        n_ext = new_symbols[saves[relocs[k].r_symbolnum] - 1].n_type & N_EXT;
      else
        n_ext = new_symbols64[saves[relocs[k].r_symbolnum] - 1].n_type & N_EXT;
      if (n_ext != N_EXT && (object->mh_cputype & CPU_ARCH_ABI64) == 0 &&
        ...
        if (object->mh != NULL) {
          value = *(uint32_t *)(contents + relocs[k].r_address);
          if (object->object_byte_sex != host_byte_sex)
           ...
           */
          if (relocs[k].r_type != 0 || relocs[k].r_length != 2) {
            fatal_arch(arch, member,
                       "don't have "
                       ...
                       "in: ",
                       k, segname, sectname, strings + n_strx);
          }
          value += n_value;
          if (object->object_byte_sex != host_byte_sex)
            value = SWAP_INT(value);
          *(uint32_t *)(contents + relocs[k].r_address) = value;
        } else {
           ...
           * that won't need a scattered relocation entry.
           */
          if (relocs[k].r_type != 0 || relocs[k].r_length != 3) {
            fatal_arch(arch, member,
                       "don't have "
                       "code to convert external relocation "
                       "entry %d in section (%.16s,%.16s) "
...
#endif /* NMEDIT */
      if (relocs[k].r_extern == 1 && saves[relocs[k].r_symbolnum] != -1) {
...
static void check_indirect_symtab(struct arch *arch, struct member *member,
  ...
  for (k = 0; k < nitems; k++) {
    ...
    if (index == INDIRECT_SYMBOL_LOCAL || index == INDIRECT_SYMBOL_ABS ||
        index == (INDIRECT_SYMBOL_LOCAL | INDIRECT_SYMBOL_ABS))
      continue;
    if (index > nsyms)
      fatal_arch(arch, member,
                 "indirect symbol table entry %d (past "
                 "the end of the symbol table) in: ",
                 reserved1 + k);
#ifdef NMEDIT
...
#endif
    {
      ...
      if ((n_type & N_TYPE) != N_UNDF && (n_type & N_TYPE) != N_PBUD &&
          section_type == S_NON_LAZY_SYMBOL_POINTERS) {
        object->output_indirect_symtab[reserved1 + k] = INDIRECT_SYMBOL_LOCAL;
        if ((n_type & N_TYPE) == N_ABS)
          object->output_indirect_symtab[reserved1 + k] |= INDIRECT_SYMBOL_ABS;
        made_local = TRUE;
        /*
        ...
        object->output_indirect_symtab[reserved1 + k] = saves[index] - 1;
               ...
               (Tflag && swift_version != 0 &&
                (mh_flags & MH_DYLDLINK) == MH_DYLDLINK && n_strx != 0 &&
...
static void swap_objc_image_info(struct objc_image_info *o,
...
static enum bool strip_symtab(struct arch *arch, struct member *member,
  ...
  *nlist_outofsync_with_dyldinfo = FALSE;
  ...
  if (object->mh_filetype == MH_OBJECT && (Sflag || xflag)) {
     ...
     * object.
     */
    if (has_dwarf == TRUE || object->mh_cputype == CPU_TYPE_ARM ||
    ...
    for (i = 0; i < nsects; i++) {
      if (object->mh != NULL) {
        if ((strcmp(sections[i]->segname, "__DATA") == 0 ||
            ...
            strncmp(sections[i]->sectname, "__objc_imageinfo", 16) == 0 &&
            sections[i]->size >= sizeof(struct objc_image_info)) {
          p_objc_image_info = object->object_addr + sections[i]->offset;
          break;
        }
      } else {
        if ((strcmp(sections64[i]->segname, "__DATA") == 0 ||
            ...
            strncmp(sections64[i]->sectname, "__objc_imageinfo", 16) == 0 &&
            sections64[i]->size >= sizeof(struct objc_image_info)) {
          p_objc_image_info = object->object_addr + sections64[i]->offset;
          break;
        }
      }
    }
  ...
  *p_swift_version = swift_version;
  ...
  for (i = 0; i < nsyms; i++) {
      ...
      if (n_value != 0) {
        if (n_value > strsize) {
          error_arch(arch, member,
                     ...
                     "indirect symbol table entry %d in: ",
                     i);
          return (FALSE);
        }
      }
    ...
    if ((n_type & N_EXT) == 0) { /* local symbol */
        ...
        if (n_type & N_STAB) { /* debug symbol */
          if (dfile && n_type == N_SO) {
            if (n_strx != 0) {
              ...
              pp = bsearch(
                  basename, debug_filenames, ndebug_filenames, sizeof(char *),
                  (int (*)(const void *, const void *))cmp_bsearch_filename);
              /*
          ...
          if (xflag == 0 && (Sflag || Xflag)) {
            if (Xflag == 0 || (n_strx != 0 && strings[n_strx] != 'L')) {
              /*
          ...
          if ((n_type & N_PEXT) == N_PEXT) {
            if (saves[i] == 0 && private_extern_reference_by_module(
       ...
       */
        ...
        if (saves[i] == 0 && sfile) {
          sp =
              bsearch(strings + n_strx, save_symbols, nsave_symbols,
                      sizeof(struct symbol_list),
                      (int (*)(const void *, const void *))symbol_list_bsearch);
          if (sp != NULL) {
            if (sp->sym == NULL) {
        ...
        if (saves[i] == 0 &&
            private_extern_reference_by_module(i, refs, nextrefsyms) == TRUE) {
          if (n_strx != 0)
            strx_map[strx_count++].old_strx = n_strx;
          new_nlocalsym++;
          new_nsyms++;
       ...
       */
      if (Rfile &&
          (object->mh != NULL || object->mh64->cputype != CPU_TYPE_X86_64 ||
           object->mh64->filetype != MH_OBJECT)) {
        sp = bsearch(strings + n_strx, remove_symbols, nremove_symbols,
                     sizeof(struct symbol_list),
                     (int (*)(const void *, const void *))symbol_list_bsearch);
        if (sp != NULL) {
          if ((n_type & N_TYPE) == N_UNDF || (n_type & N_TYPE) == N_PBUD) {
      ...
      if (Aflag && (n_type & N_TYPE) == N_ABS &&
          (n_value != 0 ||
           (n_strx != 0 && strncmp(strings + n_strx, ".objc_class_name_",
      ...
      if (saves[i] == 0 && (uflag || default_dyld_executable) &&
          ((((n_type & N_TYPE) == N_UNDF) && n_value == 0) ||
      ...
      if (saves[i] == 0 && nflag && (n_type & N_TYPE) == N_SECT) {
        if (n_strx != 0) {
          len = strlen(strings + n_strx) + 1;
          new_ext_strsize += len;
        }
      ...
      if (saves[i] == 0 && sfile) {
        sp = bsearch(strings + n_strx, save_symbols, nsave_symbols,
                     sizeof(struct symbol_list),
                     (int (*)(const void *, const void *))symbol_list_bsearch);
        if (sp != NULL) {
          if (sp->sym != NULL) {
       ...
       */
      if (object->mh != NULL && saves[i] == 0 && (n_type & N_TYPE) == N_SECT &&
          (s_flags & SECTION_TYPE) == S_COALESCED &&
          symbol_pointer_used(i, indirectsyms, nindirectsyms) == TRUE) {
        if (n_strx != 0) {
          len = strlen(strings + n_strx) + 1;
      ...
      if (saves[i] == 0 && (n_type & N_TYPE) == N_SECT &&
          (n_desc & N_WEAK_DEF) != 0) {
        if (n_strx != 0) {
          len = strlen(strings + n_strx) + 1;
          new_ext_strsize += len;
      ...
      if (saves[i] == 0 &&
          ((Xflag || Sflag || xflag) || ((rflag || default_dyld_executable) &&
      ...
      if (saves[i] == 0 &&
          ((object->mh == NULL && object->mh64->cputype == CPU_TYPE_X86_64 &&
            object->mh64->filetype == MH_OBJECT) ||
           (object->mh64 == NULL && object->mh->cputype == CPU_TYPE_I386 &&
   ...
   */
  if (!strip_all_nlists && (new_nlocalsym == 0 && nindirectsyms != 0)) {
  ...
  if (object->mh != NULL) {
    ...
    new_symbols = NULL;
    new_symbols64 =
        (struct nlist_64 *)allocate(new_nsyms * sizeof(struct nlist_64));
  }
  if (object->mh != NULL)
  ...
  if (hack_5614542 == TRUE) {
    ...
    if (object->mh != NULL)
      new_symbols[inew_syms].n_un.n_strx = (uint32_t)(q - new_strings);
    else
      new_symbols64[inew_syms].n_un.n_strx = (uint32_t)(q - new_strings);
    q += strlen(q) + 1;
    inew_syms++;
  }
  ...
  for (i = 0; i < nsyms; i++) {
    if (saves[i]) {
      ...
      if ((n_type & N_EXT) == 0) {
        ...
        if (n_strx != 0) {
          struct strx_map *map = bsearch(
              &n_strx, strx_map, strx_uniqcount, sizeof(*strx_map),
              (int (*)(const void *, const void *))cmp_bsearch_strx_map);
          if (map != NULL) {
            if (map->new_strx == 0) {
            ...
            if (object->mh != NULL)
              new_symbols[inew_syms].n_un.n_strx = map->new_strx;
            else
              new_symbols64[inew_syms].n_un.n_strx = map->new_strx;
          } else {
            error_arch(arch, member,
                       "n_strx %d is not in the "
  ...
  for (i = 0; i < nsyms; i++) {
    if (saves[i]) {
      ...
      if ((n_type & N_EXT) == N_EXT &&
          ((n_type & N_TYPE) != N_UNDF && (n_type & N_TYPE) != N_PBUD)) {
        if (object->mh != NULL)
          new_symbols[inew_syms] = symbols[i];
        else
          new_symbols64[inew_syms] = symbols64[i];
        ...
        if (n_strx != 0) {
          ...
          if (object->mh != NULL)
            ...
            new_symbols64[inew_syms].n_un.n_strx = (uint32_t)(p - new_strings);
          p += strlen(p) + 1;
        }
        if ((n_type & N_TYPE) == N_INDR) {
          if (n_value != 0) {
  ...
  if (object->mh != NULL) {
    undef_map =
        (struct undef_map *)allocate(new_nundefsym * sizeof(struct undef_map));
    undef_map64 = NULL;
  } else {
    undef_map = NULL;
  ...
  for (i = 0; i < nmodtab; i++) {
    ...
    if (iextdefsym > nsyms) {
      error_arch(arch, member,
                 ...
                 "symbols of module table entry %d in: ",
                 i);
      return (FALSE);
    }
    ...
    if (irefsym + nrefsym > nextrefsyms) {
      error_arch(arch, member,
                 ...
                 "entries of module table entry %d in: ",
                 i);
      return (FALSE);
    }
  ...
  for (i = 0; i < ntoc; i++) {
    if (tocs[i].symbol_index >= nsyms) {
      error_arch(arch, member,
                 ...
                 "contents table entry %d in: ",
                 i);
      return (FALSE);
    }
    if (saves[tocs[i].symbol_index] != 0 && changes[tocs[i].symbol_index] == 0)
      new_ntoc++;
  }
   ...
   * done by default on executables, unless any of -S/-x/-X/-T flags are
   * specified without any options that specifically remove global symbols. If
   * we are stripping nlist with the -N flag we must leave the export trie as
   * is.
   */
  if (!strip_all_nlists &&
      ((dyld_executable && !(Sflag || xflag || Xflag || Tflag)) || Rfile ||
       sfile)) {
    if (object->dyld_info != NULL && object->dyld_info->export_size != 0) {
      const char *error_string;
      uint32_t trie_new_size;

      error_string = prune_trie(
          (uint8_t *)(object->object_addr + object->dyld_info->export_off),
          object->dyld_info->export_size, prune, &trie_new_size);
      if (error_string != NULL) {
        error_arch(arch, member,
                   "LC_DYLD_INFO exports trie prune: %s in: ", error_string);
        return (FALSE);
      }
    }
    ...
    if (object->dyld_exports_trie != NULL &&
      ...
      error_string = prune_trie(
          (uint8_t *)(object->object_addr + object->dyld_exports_trie->dataoff),
          object->dyld_exports_trie->datasize, prune, &trie_new_size);
      if (error_string != NULL) {
        error_arch(arch, member,
                   "LC_DYLD_EXPORTS_TRIE trie prune: %s in: ", error_string);
        return (FALSE);
      }
    }
  }
...
static int prune(const char *name) {
  ...
  for (i = 0; i < new_nextdefsym; i++) {
    if (new_symbols != NULL) {
      if (strcmp(name, new_strings +
                           new_symbols[inew_nextdefsym + i].n_un.n_strx) == 0)
        return (0);
    } else {
      if (strcmp(name, new_strings +
                           new_symbols64[inew_nextdefsym + i].n_un.n_strx) == 0)
        return (0);
    }
  }
  return (1);
...
static void make_ld_r_object(struct arch *arch, struct member *member,
  ...
  if (object->object_byte_sex != host_byte_sex) {
    if (object->mh != NULL) {
      if (swap_object_headers(object->mh, object->load_commands) == FALSE)
        fatal("internal error: swap_object_headers() failed");
      swap_nlist(symbols, nsyms, object->object_byte_sex);
    } else {
      if (swap_object_headers(object->mh64, object->load_commands) == FALSE)
        fatal("internal error: swap_object_headers() failed");
      swap_nlist_64(symbols64, nsyms, object->object_byte_sex);
    }
    swap_indirect_symbols(indirectsyms, nindirectsyms, object->object_byte_sex);
  }
   ...
   */
  if (ld_r_narchs != 1 || ld_r_archs->type != OFILE_Mach_O ||
  ...
  if (ld_r_archs->object->mh != NULL) {
    symbols = (struct nlist *)(ld_r_archs->object->object_addr +
    ...
    symbols = NULL;
    symbols64 = (struct nlist_64 *)(ld_r_archs->object->object_addr +
                                    ld_r_archs->object->st->symoff);
    if (ld_r_archs->object->object_byte_sex != host_byte_sex)
      swap_nlist_64(symbols64, nsyms, host_byte_sex);
  }
  strings = ld_r_archs->object->object_addr + ld_r_archs->object->st->stroff;
  ...
  if (ld_r_archs->object->dyst != NULL &&
    ...
    indirectsyms = (uint32_t *)(ld_r_archs->object->object_addr +
  ...
  if (ld_r_archs->object->mh != NULL)
    ld_r_archs->object->input_sym_info_size =
        nsyms * sizeof(struct nlist) + strsize;
  else
    ld_r_archs->object->input_sym_info_size =
        nsyms * sizeof(struct nlist_64) + strsize;
...
static void strip_LC_CODE_SIGNATURE_commands(struct arch *arch,
   ...
   */
  if (object->mh != NULL)
    object->seg_linkedit->fileoff = sizeof(struct mach_header) + sizeofcmds;
  else
    object->seg_linkedit64->fileoff =
...
static enum bool edit_symtab(
    ...
    if (lc->cmd == LC_SEGMENT) {
      ...
      for (j = 0; j < sg->nsects; j++) {
        if (strcmp(s->segname, SEG_DATA) == 0 &&
            strcmp(s->sectname, SECT_DATA) == 0 && data_n_sect == NO_SECT) {
          data_n_sect = n_sect;
          break;
        }
        n_sect++;
      ...
      for (j = 0; j < sg64->nsects; j++) {
        if (strcmp(s64->segname, SEG_DATA) == 0 &&
            strcmp(s64->sectname, SECT_DATA) == 0 && data_n_sect == NO_SECT) {
          data_n_sect = n_sect;
          break;
        }
        n_sect++;
    ...
    if (n_type & N_EXT) {
      if ((n_type & N_TYPE) != N_UNDF && (n_type & N_TYPE) != N_PBUD) {
        if ((n_type & N_TYPE) == N_SECT) {
          ...
          if (((s_flags & SECTION_TYPE) == S_COALESCED) && pflag == FALSE &&
              object->mh_filetype != MH_OBJECT) {
            ...
            new_strsize += len;
            sp = bsearch(
                strings + n_strx, remove_symbols, nremove_symbols,
                sizeof(struct symbol_list),
                (int (*)(const void *, const void *))symbol_list_bsearch);
            if (sp != NULL) {
              if (sp->sym != NULL) {
             ...
             */
            sp = bsearch(
                strings + n_strx, save_symbols, nsave_symbols,
                sizeof(struct symbol_list),
                (int (*)(const void *, const void *))symbol_list_bsearch);
            if (sp != NULL) {
              if (sp->sym != NULL) {
        ...
        if (sp != NULL) {
          ...
          if (Aflag && n_type == (N_EXT | N_ABS) &&
              (n_value != 0 ||
               (n_strx != 0 &&
                strncmp(strings + n_strx, ".objc_class_name_",
                        sizeof(".objc_class_name_") - 1) == 0))) {
            /* this remains a global defined symbol */
            new_nextdefsym++;
            new_ext_strsize += len;
          ...
          change_symbol:
            if ((n_type & N_TYPE) != N_INDR) {
              nmedits[i] = TRUE;
              if (object->mh != NULL)
                changed_globals[nchanged_globals++] = symbols + i;
              else
                changed_globals64[nchanged_globals++] = symbols64 + i;
              if (pflag) {
                /* this remains a global defined symbol */
                new_nextdefsym++;
                new_ext_strsize += len;
  ...
  for (i = 0; i < nsyms; i++) {
    ...
    if (n_type == N_SO)
      ...
      if (object->mh != NULL) {
        global_symbol =
            bsearch(global_name, changed_globals, nchanged_globals,
                    sizeof(struct nlist *),
                    (int (*)(const void *, const void *))cmp_bsearch_global);
        ...
        global_symbol64 =
            bsearch(global_name, changed_globals64, nchanged_globals,
                    sizeof(struct nlist_64 *),
                    (int (*)(const void *, const void *))cmp_bsearch_global_64);
    ...
    } else if (!dwarf_debug_map && (n_type == N_GSYM || n_type == N_FUN) &&
      ...
      if (object->mh != NULL) {
        global_symbol = bsearch(
            global_name, changed_globals, nchanged_globals,
            sizeof(struct nlist *),
            (int (*)(const void *, const void *))cmp_bsearch_global_stab);
        ...
        global_symbol64 = bsearch(
            global_name, changed_globals64, nchanged_globals,
            sizeof(struct nlist_64 *),
            (int (*)(const void *, const void *))cmp_bsearch_global_stab_64);
        global_symbol = NULL;
        if (global_symbol64 != NULL) {
          global_symbol_found = TRUE;
      ...
      if (global_symbol_found == TRUE) {
        if (n_type == N_GSYM) {
          ...
          if (j + 1 + n_strx >= strsize || global_name[j + 1] != 'G') {
            error_arch(arch, member,
          ...
          if (j + 1 + n_strx >= strsize || global_name[j + 1] == 'F') {
            global_name[j + 1] = 'f';
          }
        }
      }
  ...
  if (object->mh_filetype == MH_DYLIB && nmodtab != 0) {
    ...
    for (i = 0; i < nmodtab; i++) {
      if (object->mh != NULL) {
        ...
        new_mods[i].objc_module_info_addr = mods[i].objc_module_info_addr;
        new_mods[i].objc_module_info_size = mods[i].objc_module_info_size;
      } else {
        strcpy(p, strings + mods64[i].module_name);
        ...
        new_mods64[i].objc_module_info_addr = mods64[i].objc_module_info_addr;
        new_mods64[i].objc_module_info_size = mods64[i].objc_module_info_size;
      }
    }
    ...
    for (i = 0; i < nextrefsyms; i++) {
      if (nmedits[refs[i].isym] == TRUE) {
        if (refs[i].flags == REFERENCE_FLAG_DEFINED)
          new_refs[i].flags = REFERENCE_FLAG_PRIVATE_DEFINED;
        else if (refs[i].flags == REFERENCE_FLAG_UNDEFINED_NON_LAZY)
          new_refs[i].flags = REFERENCE_FLAG_PRIVATE_UNDEFINED_NON_LAZY;
        else if (refs[i].flags == REFERENCE_FLAG_UNDEFINED_LAZY)
          new_refs[i].flags = REFERENCE_FLAG_PRIVATE_UNDEFINED_LAZY;
        else
...
static int cmp_bsearch_global_64(const char *name,
                                 const struct nlist_64 **sym) {
  return (strcmp(name, global_strings + (*sym)->n_un.n_strx));
}
#endif /* defined(NMEDIT) */
