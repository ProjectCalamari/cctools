...
#define read_16(p)                                                             \
  (lnd->little_endian ? ((p)[1] << 8 | (p)[0]) : ((p)[0] << 8 | (p)[1]))
#define read_32(p)                                                             \
  (lnd->little_endian ? ((p)[3] << 24 | (p)[2] << 16 | (p)[1] << 8 | (p)[0])   \
                      : ((p)[0] << 24 | (p)[1] << 16 | (p)[2] << 8 | (p)[3]))
#define read_64(p)                                                             \
  (lnd->little_endian ? ((uint64_t)(p)[7] << 56 | (uint64_t)(p)[6] << 48 |     \
...
static void skip_leb128(struct line_reader_data *leb) {
...
static uint64_t read_uleb128(struct line_reader_data *leb) {
  uint64_t result = 0;
  int bit = 0;
...
/* Read a SLEB128 into a 64-bit word.  Return 0 on overflow or error
   ...
   be OK...  */
static int64_t read_sleb128(struct line_reader_data *leb) {
...
char *line_file(struct line_reader_data *lnd, uint64_t n) {
...
static void init_state(struct line_info *s) {
  s->file = 1;
  s->line = 1;
  s->col = 0;
  s->pc = 0;
...
int line_at_eof(struct line_reader_data *lnd) { return lnd->cpos == lnd->end; }

static bool next_state(struct line_reader_data *lnd) {
  ...
  for (;;) {
    ...
    if (op >= lnd->opcode_base) {
      ...
      lnd->cur.pc += (op / lnd->line_range * lnd->minimum_instruction_length);
      return true;
    } else
      switch (op) {
      case DW_LNS_extended_op: {
        ...
        case DW_LNE_define_file: {
          const uint8_t **filenames;
          filenames = realloc(lnd->filenames,
