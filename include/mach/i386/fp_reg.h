...
typedef struct {
  uint64_t lo64, hi64;
} __attribute__((packed)) reg128_t;
typedef struct {
  reg128_t lo128, hi128;
} __attribute__((packed)) reg256_t;
typedef struct {
  reg256_t lo256, hi256;
} __attribute__((packed)) reg512_t;
 ...
 */
#define FPC_IE                                                                 \
  0x0001 /* enable invalid operation                                           \
            exception */
#define FPC_IM FPC_IE
#define FPC_DE                                                                 \
  0x0002 /* enable denormalized operation                                      \
            exception */
