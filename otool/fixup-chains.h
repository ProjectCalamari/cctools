...
// #define LC_DYLD_CHAINED_FIXUPS 0x80000034 // used with linkedit_data_command,
// payload is dyld_chained_fixups_header
...
struct dyld_chained_starts_in_image {
  uint32_t seg_count;
  uint32_t seg_info_offset[1]; // each entry is offset into this struct for that
                               // segment
...
struct dyld_chained_starts_in_segment {
  ...
  uint32_t max_valid_pointer; // for 32-bit OS, any value beyond this is not a
                              // pointer
  uint16_t page_count;        // how many pages are in array
  uint16_t page_start[1]; // each entry is offset in each page of first element
                          // in chain or DYLD_CHAINED_PTR_START_NONE if no
                          // fixups on page
  // uint16_t    chain_starts[1];    // some 32-bit formats may require multiple
  // starts per page. for those, if high bit is set in page_starts[], then it is
  // index into chain_starts[] which is a list of starts the last of which has
  // the high bit set
};
...
enum {
  ...
  DYLD_CHAINED_PTR_ARM64E_USERLAND24 =
      12, // stride 8, unauth target is vm offset, 24-bit bind
};
...
struct dyld_chained_ptr_arm64e_bind {
  uint64_t ordinal : 16, zero : 16, addend : 19,
      next : 11, // 8-byte stride
      bind : 1,  // == 1
      auth : 1;  // == 0
};
...
// DYLD_CHAINED_PTR_32
// Note: for DYLD_CHAINED_PTR_32 some non-pointer values are co-opted into the
// chain as out of range rebases.  If an entry in the chain is >
// max_valid_pointer, then it is not a pointer.  To restore the value, subtract
// off the bias, which is (64MB+max_valid_pointer)/2.
