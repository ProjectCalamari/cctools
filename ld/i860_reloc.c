 ...
 */
__private_extern__ void i860_reloc(char *contents,
  ...
  for (i = 0; i < section_map->s->nreloc; i++) {
        ...
        if (r_type == I860_RELOC_VANILLA) {
          ...
          case I860_RELOC_HIGHADJ:
            if (other_half & 0x8000)
              offset =
                  ((instruction & 0xffff) << 16) + (other_half | 0xffff0000);
            else
              offset = ((instruction & 0xffff) << 16) + other_half;
            break;
          ...
          case I860_RELOC_SPLIT0:
            offset = ((instruction >> 5) & 0xf800) | (instruction & 0x7ff);
            if ((offset & 0x8000) != 0)
              offset |= 0xffff0000;
            /*
          ...
          case I860_RELOC_SPLIT1:
            offset = ((instruction >> 5) & 0xf800) | (instruction & 0x7fe);
            if ((offset & 0x8000) != 0)
              offset |= 0xffff0000;
            break;
          case I860_RELOC_SPLIT2:
            offset = ((instruction >> 5) & 0xf800) | (instruction & 0x7fc);
      ...
      if (r_symbolnum != R_ABS) {
        ...
        if (local_map->nfine_relocs == 0 &&
          ...
          fatal("internal error, in i860_reloc() section (%.16s,"
                "%.16s) has fine relocs",
                local_map->s->segname, local_map->s->sectname);
        }
      }
    }
    if (r_pcrel) {
     ...
     * generic_reloc() first.
     */
    if (r_type == I860_RELOC_VANILLA || r_type == I860_RELOC_SECTDIFF) {
      switch (r_length) {
      case 0: /* byte */
     ...
     */
      ...
      case I860_RELOC_HIGHADJ:
        ...
        if (immediate & 0x8000)
          instruction =
              (instruction & 0xffff0000) | (((immediate >> 16) + 1) & 0xffff);
        else
          instruction =
      ...
      case I860_RELOC_LOW1:
        ...
        if ((immediate & 0x8000) != 0)
          immediate |= 0xffff0000;
        immediate += value;
        instruction = (instruction & 0xffff0001) | (immediate & 0xfffe);
        break;
      case I860_RELOC_LOW2:
      ...
      case I860_RELOC_LOW3:
        ...
        if ((immediate & 0x8000) != 0)
          immediate |= 0xffff0000;
        immediate += value;
        instruction = (instruction & 0xffff0007) | (immediate & 0xfff8);
        break;
      case I860_RELOC_LOW4:
        ...
        if ((immediate & 0x8000) != 0)
          immediate |= 0xffff0000;
        immediate += value;
        instruction = (instruction & 0xffff000f) | (immediate & 0xfff0);
        break;
      case I860_RELOC_SPLIT0:
        immediate = ((instruction >> 5) & 0xf800) | (instruction & 0x7ff);
        if ((immediate & 0x8000) != 0)
          immediate |= 0xffff0000;
        /*
        ...
        instruction = (instruction & 0xffe0f800) | ((immediate & 0xf800) << 5) |
                      (immediate & 0x7ff);
        break;
      case I860_RELOC_SPLIT1:
        immediate = ((instruction >> 5) & 0xf800) | (instruction & 0x7fe);
        if ((immediate & 0x8000) != 0)
          immediate |= 0xffff0000;
        immediate += value;
        instruction = (instruction & 0xffe0f801) | ((immediate & 0xf800) << 5) |
                      (immediate & 0x7fe);
        break;
      case I860_RELOC_SPLIT2:
        immediate = ((instruction >> 5) & 0xf800) | (instruction & 0x7fc);
        if ((immediate & 0x8000) != 0)
          immediate |= 0xffff0000;
        immediate += value;
        instruction = (instruction & 0xffe0f803) | ((immediate & 0xf800) << 5) |
                      (immediate & 0x7FC);
        break;
      case I860_RELOC_BRADDR:
