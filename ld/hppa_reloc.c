 ...
 */
__private_extern__ void hppa_reloc(char *contents,
  ...
  for (i = 0; i < section_map->s->nreloc; i++) {
    ...
    if (r_type == HPPA_RELOC_PAIR) {
      error_with_cur_obj("stray relocation HPPA_RELOC_PAIR entry "
                         "(%lu) in section (%.16s,%.16s)",
                         i, section_map->s->segname, section_map->s->sectname);
      continue;
    }
    /*
    ...
    if (r_type == HPPA_RELOC_HI21 || r_type == HPPA_RELOC_LO14 ||
      ...
      if (i + 1 < section_map->s->nreloc) {
        ...
        if ((pair_reloc->r_address & R_SCATTERED) != 0) {
          spair_reloc = (struct scattered_relocation_info *)pair_reloc;
          pair_reloc = NULL;
      ...
      if (r_scattered != 1) {
        error_with_cur_obj("relocation entry (%lu) in section "
                           ...
                           "relocation entry not scattered type",
                           i, section_map->s->segname,
                           section_map->s->sectname);
        continue;
      }
    ...
    if (r_extern) {
      ...
      if (r_symbolnum >= cur_obj->symtab->nsyms) {
        error_with_cur_obj(
            ...
            "range",
            r_symbolnum, i, section_map->s->segname, section_map->s->sectname);
        return;
      }
      symbolnum = r_symbolnum;
      undefined_map =
          bsearch(&symbolnum, cur_obj->undefined_maps, cur_obj->nundefineds,
      ...
      if (undefined_map != NULL) {
        ...
        if ((nlists[symbolnum].n_type & N_EXT) != N_EXT) {
          error_with_cur_obj("r_symbolnum (%lu) field of external"
                             ...
                             "refers to a non-external symbol",
                             symbolnum, i, section_map->s->segname,
                             section_map->s->sectname);
          return;
        }
        /*
        ...
        if ((nlists[symbolnum].n_type & N_TYPE) == N_SECT &&
            (cur_obj->section_maps[nlists[symbolnum].n_sect - 1].s->flags &
             SECTION_TYPE) == S_COALESCED) {
          merged_symbol =
              lookup_symbol(strings + nlists[symbolnum].n_un.n_strx);
          if (merged_symbol->name_len == 0) {
            fatal("internal error, in hppa_reloc() failed to "
                  "lookup coalesced symbol %s",
                  strings + nlists[symbolnum].n_un.n_strx);
          }
        } else {
          if ((nlists[symbolnum].n_type & N_EXT) != N_EXT ||
          ...
          fatal("internal error, in hppa_reloc() symbol index %lu"
                " in above file not in undefined map",
                symbolnum);
        }
      }
      if ((merged_symbol->nlist.n_type & N_TYPE) == N_SECT &&
          (get_output_section(merged_symbol->nlist.n_sect)->flags &
           SECTION_TYPE) == S_COALESCED) {
        if (((merged_symbol->nlist.n_type & N_PEXT) == N_PEXT &&
             keep_private_externs == FALSE) ||
            dynamic == FALSE || (output_for_dyld && has_dynamic_linker_command))
          force_extern_reloc = FALSE;
        else
      ...
      if (((merged_symbol->nlist.n_type & N_TYPE) == N_UNDF) ||
        ...
        if (r_type == HPPA_RELOC_VANILLA) {
          ...
          case HPPA_RELOC_LO14:
            ...
            offset = low_sign_ext(offset, 14) + (other_half << 11);
            break;
          case HPPA_RELOC_BR17:
          case HPPA_RELOC_BL17:
            /* assemble low part */
            offset = assemble_17((instruction & 0x1f0000) >> 16,
                                 (instruction & 0x1ffc) >> 2, instruction & 1);
            /* sign extend it  */
          ...
          case HPPA_RELOC_JBSR:
            ...
            if ((merged_symbol->nlist.n_type & N_TYPE) == N_SECT) {
              other_half =
                  value - output_sections[merged_symbol->nlist.n_sect]->s.addr +
                  offset;
      ...
      if ((merged_symbol->nlist.n_type & N_TYPE) == N_SECT)
        output_sections[merged_symbol->nlist.n_sect]->referenced = TRUE;
    } else {
      /*
      ...
      if (r_symbolnum != R_ABS) {
        ...
        if (r_type == HPPA_RELOC_SECTDIFF ||
            r_type == HPPA_RELOC_HI21_SECTDIFF ||
            r_type == HPPA_RELOC_LO14_SECTDIFF) {
          pair_local_map = &(cur_obj->section_maps[pair_r_symbolnum - 1]);
          pair_local_map->output_section->referenced = TRUE;
          if (pair_local_map->s->flags & S_ATTR_DEBUG) {
            error_with_cur_obj(
                "illegal reference to debug "
                "section, from non-debug section (%.16s,%.16s) "
                "via relocation entry (%lu) to section (%.16s,"
                "%.16s)",
                section_map->s->segname, section_map->s->sectname, i,
                pair_local_map->s->segname, pair_local_map->s->sectname);
            return;
          }
        }
        if (local_map->nfine_relocs == 0 &&
            (pair_local_map == NULL || pair_local_map->nfine_relocs == 0)) {
          if (r_type == HPPA_RELOC_SECTDIFF ||
              r_type == HPPA_RELOC_HI21_SECTDIFF ||
              r_type == HPPA_RELOC_LO14_SECTDIFF) {
           ...
           * relocated.
           */
          if (r_type == HPPA_RELOC_VANILLA || r_type == HPPA_RELOC_SECTDIFF) {
            switch (r_length) {
            case 0: /* byte */
            ...
            case HPPA_RELOC_HI21_SECTDIFF:
              value = (assemble_21(instruction & 0x1fffff) << 11) +
                      sign_ext(other_half, 14);
              break;
            case HPPA_RELOC_LO14:
            case HPPA_RELOC_LO14_SECTDIFF:
              value = instruction & 0x3fff;
              value = low_sign_ext(value, 14) + (other_half << 11);
              break;
            case HPPA_RELOC_BR17:
            case HPPA_RELOC_BL17:
              value = assemble_17((instruction & 0x1f0000) >> 16,
                                  (instruction & 0x1ffc) >> 2, instruction & 1);
            ...
            case HPPA_RELOC_JBSR:
              ...
              if (r_scattered == 0)
                other_half = fine_reloc_output_offset(local_map, other_half);
              else {
                other_half =
                    fine_reloc_output_offset(local_map,
                                             r_value - local_map->s->addr) +
                    sign_ext(other_half, 24) - (r_value - local_map->s->addr);
                other_half = sign_unext(other_half, 24);
              }
              break;
            default:
          ...
          if (r_type == HPPA_RELOC_SECTDIFF ||
            ...
            if (local_map->nfine_relocs != 0) {
              ...
              legal_reference(section_map, r_address, local_map,
                              r_value - local_map->s->addr + offset, i, TRUE);
              value = fine_reloc_output_address(
                  local_map, r_value - local_map->s->addr,
                  local_map->output_section->s.addr);
            } else {
              value = local_map->output_section->s.addr + local_map->offset +
                      r_value - local_map->s->addr;
            }
            ...
            if (pair_local_map->nfine_relocs != 0) {
               ...
               * respect to indirect sections.
               */
              legal_reference(section_map, r_address, pair_local_map,
                              pair_r_value - pair_local_map->s->addr, i, TRUE);
              value -= fine_reloc_output_address(
                  pair_local_map, pair_r_value - pair_local_map->s->addr,
                  pair_local_map->output_section->s.addr);
            } else {
              value -= pair_local_map->output_section->s.addr +
                       pair_local_map->offset + pair_r_value -
                       pair_local_map->s->addr;
            }
             ...
             */
            legal_reference(section_map, r_address, local_map,
                            r_value - local_map->s->addr + offset, i, FALSE);
            value = fine_reloc_output_address(
                local_map, r_value - local_map->s->addr,
                local_map->output_section->s.addr);
            value -= output_pc;
          ...
          if (r_type == HPPA_RELOC_VANILLA || r_type == HPPA_RELOC_SECTDIFF) {
            switch (r_length) {
            case 0: /* byte */
              if ((value & 0xffffff00) && ((value & 0xffffff80) != 0xffffff80))
              ...
              break;
            case 1: /* word (2 byte) */
              if ((value & 0xffff0000) && ((value & 0xffff8000) != 0xffff8000))
                error_with_cur_obj("relocation for entry "
                                   "%lu in section (%.16s,%.16s) does not "
                                   "fit in 2 bytes",
                                   i, section_map->s->segname,
            ...
            case HPPA_RELOC_HI21_SECTDIFF:
              ...
              instruction =
                  (instruction & 0xffe00000) | dis_assemble_21(hi21 >> 11);
              break;
            case HPPA_RELOC_LO14:
            case HPPA_RELOC_LO14_SECTDIFF:
              other_half = hi21 >> 11;
              lo14 = low_sign_unext(lo14, 14);
              instruction = (instruction & 0xffffc000) | (lo14 & 0x3fff);
              break;
            case HPPA_RELOC_BR17:
              other_half = hi21 >> 11;
              if ((lo14 & 3) != 0)
              ...
              instruction =
                  (instruction & 0xffe0e002) | (w1 << 16) | (w2 << 2) | w;
              break;
            case HPPA_RELOC_BL17:
              value -= 8;
              if ((value & 3) != 0)
            ...
            case HPPA_RELOC_JBSR:
              if (section_map->nfine_relocs == 0)
                value -= section_map->output_section->s.addr +
                         section_map->offset + r_address + 8;
              else
                value -= section_map->output_section->s.addr +
                         fine_reloc_output_offset(section_map, r_address) + 8;
              if (save_reloc == 0 &&
    ...
    if (r_type == HPPA_RELOC_VANILLA || r_type == HPPA_RELOC_SECTDIFF) {
      ...
      case 0: /* byte */
        ...
        if ((value & 0xffffff00) && ((value & 0xffffff80) != 0xffffff80))
          error_with_cur_obj("relocation for entry %lu in section"
                             " (%.16s,%.16s) does not fit in 1 byte",
                             i, section_map->s->segname,
                             section_map->s->sectname);
        set_byte((char *)(contents + r_address), value);
        break;
      case 1: /* word (2 byte) */
        ...
        if ((value & 0xffff0000) && ((value & 0xffff8000) != 0xffff8000))
          error_with_cur_obj("relocation for entry %lu in section"
                             " (%.16s,%.16s) does not fit in 2 bytes",
                             i, section_map->s->segname,
                             section_map->s->sectname);
        set_short((short *)(contents + r_address), value);
        break;
     ...
     */
      ...
      case HPPA_RELOC_HI21_SECTDIFF:
        ...
        calc_hppa_HILO(value + r_value - pair_r_value,
                       immediate - r_value + pair_r_value, &hi21, &lo14);
        instruction = (instruction & 0xffe00000) | dis_assemble_21(hi21 >> 11);
        other_half = lo14 & 0x3fff;
        break;
      ...
      case HPPA_RELOC_HI21:
        ...
        if (r_extern)
          ...
          calc_hppa_HILO(value + r_value, immediate - r_value, &hi21, &lo14);
        else
      ...
      case HPPA_RELOC_BR17:
        immediate = assemble_17((instruction & 0x1f0000) >> 16,
                                (instruction & 0x1ffc) >> 2, instruction & 1);
        immediate = (sign_ext(immediate, 17) << 2) + (other_half << 11);
        if (r_extern)
          calc_hppa_HILO(value, immediate, &hi21, &lo14);
        else if (r_scattered)
          calc_hppa_HILO(value + r_value, immediate - r_value, &hi21, &lo14);
        ...
        other_half = hi21 >> 11;
        dis_assemble_17(lo14, &w1, &w2, &w);
        instruction = (instruction & 0xffe0e002) | (w1 << 16) | (w2 << 2) | w;
        break;
      case HPPA_RELOC_BL17:
        immediate = assemble_17((instruction & 0x1f0000) >> 16,
                                (instruction & 0x1ffc) >> 2, instruction & 1);
      ...
      case HPPA_RELOC_JBSR:
        if (r_extern)
          ...
          value += local_map->s->addr + sign_ext(other_half, 24);
          other_half = local_map->offset + sign_ext(other_half, 24);
        }
        if (section_map->nfine_relocs == 0)
          value += -(section_map->output_section->s.addr + section_map->offset +
                     r_address + 8);
        else
          value += -(section_map->output_section->s.addr +
                     fine_reloc_output_offset(section_map, r_address) + 8);
  ...
  update_reloc:;
    ...
    if (save_reloc || output_for_dyld) {
      if (r_extern) {
         ...
         */
        if (prebinding == TRUE &&
            (section_map->s->flags & SECTION_TYPE) == S_LAZY_SYMBOL_POINTERS) {
         ...
         */
          ...
          reloc->r_symbolnum = merged_symbol_output_index(merged_symbol);
        }
         ...
         */
          ...
          if (local_map->nfine_relocs == 0) {
            reloc->r_symbolnum = local_map->output_section->output_sectnum;
          } else {
            reloc->r_symbolnum = fine_reloc_output_sectnum(
                local_map, r_value - local_map->s->addr);
          }
        }
      } else {
        ...
        if (output_for_dyld) {
          ...
          if (local_map->nfine_relocs == 0)
            ...
            sreloc->r_value = fine_reloc_output_address(
                local_map, r_value - local_map->s->addr,
                local_map->output_section->s.addr);
        }
      }
      ...
      if (r_scattered == 0) {
        if (section_map->nfine_relocs == 0) {
          ...
          reloc->r_address = fine_reloc_output_offset(section_map, r_address);
        }
      } else {
        if (section_map->nfine_relocs == 0) {
          sreloc->r_address += section_map->offset;
        } else {
          sreloc->r_address = fine_reloc_output_offset(section_map, r_address);
        }
      }
      /*
      ...
      if (pair_r_type == HPPA_RELOC_PAIR) {
        if (pair_reloc != NULL)
          ...
          if (r_type == HPPA_RELOC_SECTDIFF ||
             ...
             */
            if (pair_local_map->nfine_relocs == 0)
              spair_reloc->r_value += -pair_local_map->s->addr +
                                      (pair_local_map->output_section->s.addr +
                                       pair_local_map->offset);
            else
              spair_reloc->r_value = fine_reloc_output_address(
                  pair_local_map, pair_r_value - pair_local_map->s->addr,
                  pair_local_map->output_section->s.addr);
