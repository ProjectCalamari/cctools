...
static LLVMDisasmContextRef (*create)(const char *, void *, int,
                                      LLVMOpInfoCallback,
                                      LLVMSymbolLookupCallback) = NULL;
static LLVMDisasmContextRef (*createCPU)(const char *, const char *, void *,
                                         int, LLVMOpInfoCallback,
                                         LLVMSymbolLookupCallback) = NULL;
...
static void load_llvm_disasm(void) {
  ...
  if (tried_to_load_llvm == 0) {
    ...
    if (create == NULL || dispose == NULL || disasm == NULL) {
 ...
 */
__private_extern__ LLVMDisasmContextRef
llvm_create_disasm(const char *TripleName, const char *CPU, void *DisInfo,
  ...
  if (*CPU != '\0' && createCPU != NULL)
    DC = createCPU(TripleName, CPU, DisInfo, TagType, GetOpInfo, SymbolLookUp);
  else
 ...
 */
__private_extern__ void llvm_disasm_dispose(LLVMDisasmContextRef DC) {
  if (dispose != NULL)
    dispose(DC);
}
 ...
 */
__private_extern__ size_t llvm_disasm_instruction(LLVMDisasmContextRef DC,
                                                  uint8_t *Bytes,
                                                  uint64_t BytesSize,
 ...
 */
__private_extern__ int llvm_disasm_set_options(LLVMDisasmContextRef DC,
                                               uint64_t Options) {
 ...
 */
__private_extern__ const char *llvm_disasm_version_string(void) {
  load_llvm_disasm();
  if (llvm_handle == NULL)
    return (NULL);
  if (version == NULL)
 ...
 */
__private_extern__ int llvm_disasm_new_getopinfo_abi(void) {
