...
static void get_segments(const struct ofile *ofile, struct segentry **o_segs,
                         uint32_t *o_nseg, struct sectentry **o_sects,
                         uint32_t *o_nsect);
static void get_symbols(const struct ofile *ofile, struct symbol **o_syms,
                        uint32_t *o_nsym);
...
int main(int argc, char **argv) {
  ...
  while (argc > 0) {
    if (read_options && *argv && '-' == **argv) {
      if (0 == strcmp("-h", *argv) || 0 == strcmp("-help", *argv)) {
        usage();
        ...
        if (!*argv) {
          warning("one start address must be specified for %s", option);
          usage();
        }
  ...
  if (NULL != archs->object->mh) {
    if (MH_PRELOAD != archs->object->mh->filetype) {
      fatal("input file: %s must be an MH_PRELOAD file type", archs->file_name);
    }
  } else {
      ...
      for (uint32_t j = 0; j < sg->nsects; ++j) {
        ...
        if (0 != strcmp(sg->segname, sc->segname)) {
          warning("input file %s: %s section %s points to different "
                  "section name: %s\n",
                  ofile->file_name, sg->segname, sc->sectname, sc->segname);
        }
...
static int find_symbol(const struct ofile *ofile, const struct symbol *sym,
  ...
  if (0 == strcmp(sym->name, name) && 0 == (sym->n_type & N_STAB)) {
    ...
    if (sectord == 0) {
      fatal("input file %s: symbol %s is not in any section", ofile->file_name,
            name);
    }
    if (sectord > g_nsect) {
      fatal("input file %s: symbol %s has a bad section ordinal "
            "(%d > %d)",
            ofile->file_name, name, sectord, g_nsect);
    }
...
static void find_data_symbols(const struct ofile *ofile) {
  ...
  for (uint32_t i = 0; i < g_nsym; ++i) {
    find_symbol(ofile, &g_syms[i], g_flags.data_lma_sym, &g_flags.data_lma_loc);
  ...
  if (!g_flags.data_lma_loc || !g_flags.data_size_loc) {
    if (!g_flags.data_lma_loc)
      error("input file %s: symbol not found: %s", ofile->file_name,
            g_flags.data_lma_sym);
    if (!g_flags.data_size_loc)
      error("input file %s: symbol not found: %s", ofile->file_name,
            g_flags.data_size_sym);
    exit(EXIT_FAILURE);
  }
}
...
void process(const struct ofile *ofile, const char *output) {
  ...
  for (uint32_t i = 0; i < g_nsect; ++i) {
    ...
    if ((sect->addr + startadj) % (1 << sect->align)) {
      error("input file: %s section %.16s,%.16s addr "
            ...
            sect->addr + startadj, sect->align, 1 << sect->align);
    }
  ...
  if (!buf)
    fatal("Can't allocate buffer for output file (size = %lu)", output_size);
  ...
  for (uint32_t i = 0; i < g_nseg; ++i) {
    ...
    if (!g_flags.dry_run) {
       ...
       */
      if (size) {
        memcpy(buf + seg->out_fileoff, ofile->file_addr + seg->fileoff, size);
      }
    }
  }
  ...
  if (!g_flags.dry_run) {
    ...
    if (!tempname)
      fatal("Can't allocate buffer for output filename (size = %lu)", tempsize);
