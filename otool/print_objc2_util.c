...
void indent_reset(struct indent *indent) {
  indent->level = 0;
  indent->widths[indent->level] = 0;
}

void indent_push(struct indent *indent, uint32_t width) {
  indent->level += 1;
  if (indent->level < MAXINDENT)
    indent->widths[indent->level] = width;
}

void indent_pop(struct indent *indent) {
  if (indent->level)
    indent->level -= 1;
}
 ...
 */
void print_field_scalar(struct indent *indent, const char *label,
                        const char *fmt, ...) {
  /* print the label */
 ...
 */
void print_field_label(struct indent *indent, const char *label, ...) {
  va_list ap;
  int width = 0;
   ...
   */
  label_width = (indent->level < MAXINDENT ? indent->widths[indent->level] : 0);
#else
  /*
 ...
 */
void warn_about_zerofill(const char *segname, const char *sectname,
