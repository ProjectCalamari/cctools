...
static void pqinsert(Edge *edge);
static Edge *pqremove(void);
static void pqdelete(Edge *edge);
static void upheap(int k);
static void downheap(int k);

static void do_what(void) {
  struct file *afile;
...
static char *find_file(uint64_t pc) {
  struct file *afile;
  static int flag = 0;
...
static int printp(nltype *node, FILE *f) {
  if (f == callt) {
...
static void print_node(int node) {
  nltype *nlp;
  char *file;
...
static Edge *find_edge(int v1, int v2, int w) {
  Edge *edge;
...
static Edge *make_edge(int v1, int v2, int w) {
  Edge *edge;
...
static int compare_file(struct file *x, struct file *y) {
...
static int compare_nl(nltype *x, nltype *y) {
  if (x->ncall < y->ncall)
    return (-1);
...
static int compare_nl_order(nltype *x, nltype *y) {
  if (x->order > y->order)
    return (-1);
...
static void enum_arcs(void (*proc)(arctype *arc, arctype *backarc)) {
  nltype *nlp;
  arctype *arcp, *backarc;

  for (nlp = nl; nlp < npe; nlp++) {
    for (arcp = nlp->children; arcp; arcp = arcp->arc_childlist) {
      for (backarc = arcp->arc_childp->children; backarc;
...
static void most_edges_proc(arctype *arc, arctype *backarc) { most_edges++; }

static void enum_edges(int v, int (*proc_e1)(Edge *), int (*proc_e2)(Edge *)) {
  Edge *edge;
  Edge *next1, *next2;
...
static int make_edge_e1(Edge *edge) {
  if (edge->pqp) {
    pqdelete(edge);
...
static int make_edge_e2(Edge *edge) {
  if (edge->pqp) {
    pqdelete(edge);
...
static int find_edge_e1(Edge *edge) {
  if (edge->pqp) {
    pqdelete(edge);
...
static int find_edge_e2(Edge *edge) {
  if (edge->pqp) {
    pqdelete(edge);
...
static int pqinsert_e1(Edge *edge) {
  if (!edge->pqp)
    pqinsert(edge);
  return (0);
}
...
static void pqinsert_proc(arctype *arc, arctype *backarc) {
  pqinsert(make_edge(arc->arc_parentp - nl, arc->arc_childp - nl,
...
#ifdef notdef
static void ugh(void) {}

static voids print_pq(void) {
  int i;
  Edge *edge;
