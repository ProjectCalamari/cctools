 ...
 */
__private_extern__ void arm_reloc(char *contents,
  ...
  for (i = reloc_index; i < section_map->s->nreloc; i++) {
    ...
    if (r_extern) {
      ...
      if (r_symbolnum == R_ABS) {
        ...
        if (local_map->nfine_relocs == 0 &&
           ...
           * relocated.
           */
          if (r_type == ARM_RELOC_VANILLA || r_type == ARM_RELOC_SECTDIFF ||
              r_type == ARM_RELOC_LOCAL_SECTDIFF) {
            switch (r_length) {
            case 0: /* byte */
          ...
          if (r_type == ARM_RELOC_VANILLA ||
            ...
            case ARM_RELOC_BR24:
               ...
               * optimized away.
               */
              if (((fine_reloc_arm(local_map, r_value - local_map->s->addr) ==
                    TRUE) ||
                   (fine_reloc_local(local_map, r_value - local_map->s->addr) ==
                    TRUE)) &&
               ...
               * already is one).
               */
              if ((fine_reloc_thumb(local_map, r_value - local_map->s->addr) ==
                   TRUE) ||
                  ((instruction & 0xfe000000) == 0xfa000000)) {
                /*
                 ...
                 */
                instruction = (0xfa000000 | ((value & 0x2) << 23));
                /*
              ...
              if ((value & 0xfe000000) != 0xfe000000 &&
                ...
                error_with_cur_obj("relocation overflow "
                                   ...
                                   section_map->s->sectname);
              instruction =
                  (instruction & 0xff000000) | ((value >> 2) & 0x00ffffff);
              break;
            case ARM_THUMB_RELOC_BR22:
              /* The pc added will be +4 from the pc */
              value -= 4;
              ...
              instruction = (instruction & 0xf800f800) |
                            (value & 0x7ff000) >> 12 | (value & 0xffe) << 15;
              break;
            default:
    ...
    if (r_type == ARM_RELOC_VANILLA || r_type == ARM_RELOC_LOCAL_SECTDIFF ||
        r_type == ARM_RELOC_SECTDIFF) {
      /*
     ...
     */
      ...
      case ARM_RELOC_BR24:
         ...
         */
        if ((r_extern == TRUE) && (relocated_extern_arm_symbol == TRUE) &&
            ((instruction & 0xfe000000) == 0xfa000000))
          instruction = 0xeb000000;
        /*
        ...
        if ((immediate & 0xfe000000) != 0xfe000000 &&
          ...
          error_with_cur_obj("relocation overflow for relocation "
                             ...
                             section_map->s->sectname);
        instruction =
            (instruction & 0xff000000) | (immediate & 0x03ffffff) >> 2;
        break;
      case ARM_THUMB_RELOC_BR22:
        /*
         ...
         */
        if ((instruction & 0xf800f800) == 0xe800f000 && (r_address & 0x2))
          immediate -= 2;
        immediate += value;
        /*
         ...
         */
        if ((r_extern == TRUE) && (relocated_extern_thumb_symbol == TRUE) &&
            ((instruction & 0xf800f800) == 0xe800f000))
          instruction |= 0x10000000;
        /*
         ...
         * instruction[0] must be zero for a BLX.
         */
        if ((instruction & 0xf800f800) == 0xe800f000 && (immediate & 0x2) != 0)
          immediate += 2;
        if ((immediate & 0xffc00000) != 0xffc00000 &&
          ...
          error_with_cur_obj("relocation overflow for relocation "
                             ...
                             section_map->s->sectname);
        instruction = (instruction & 0xf800f800) |
                      (immediate & 0x7ff000) >> 12 | (immediate & 0xffe) << 15;
        break;
