...
static void process(struct ofile *ofile, char *arch_name, void *cookie);
...
static void output_base_relocs(char *out);
...
int main(int argc, char **argv, char **envp) {
  ...
  ofile_process(input, NULL, 0, FALSE, FALSE, FALSE, FALSE, process, NULL);
  if (errors != 0)
    return (EXIT_FAILURE);
...
static void process(struct ofile *ofile, char *arch_name, void *cookie) {
  ...
  for (i = 0; i < ncmds; i++) {
    if (lc->cmd == LC_SEGMENT) {
      ...
      for (j = 0; j < sg->nsects; j++) {
        relocs = (struct relocation_info *)(ofile->object_addr + s[j].reloff);
        if (swapped)
      ...
      for (j = 0; j < sg64->nsects; j++) {
        relocs = (struct relocation_info *)(ofile->object_addr + s64[j].reloff);
        if (swapped)
          swap_relocation_info(relocs, s64[j].nreloc, host_byte_sex);
        if (ofile->mh_cputype == CPU_TYPE_X86_64)
          gather_base_reloc_info(s64[j].addr, relocs, s64[j].nreloc,
  ...
  if (dyst != NULL && dyst->nlocrel != 0) {
    relocs = (struct relocation_info *)(ofile->object_addr + dyst->locreloff);
    if (swapped)
      swap_relocation_info(relocs, dyst->nlocrel, host_byte_sex);
    if (ofile->mh_cputype == CPU_TYPE_I386)
      gather_base_reloc_info(first_addr, relocs, dyst->nlocrel, CPU_TYPE_I386,
                             2, GENERIC_RELOC_VANILLA, IMAGE_REL_BASED_HIGHLOW);
    if (ofile->mh_cputype == CPU_TYPE_ARM)
      gather_base_reloc_info(first_addr, relocs, dyst->nlocrel, CPU_TYPE_ARM, 2,
                             GENERIC_RELOC_VANILLA, IMAGE_REL_BASED_HIGHLOW);
    else if (ofile->mh_cputype == CPU_TYPE_X86_64)
      gather_base_reloc_info(first_addr, relocs, dyst->nlocrel, CPU_TYPE_X86_64,
                             3, X86_64_RELOC_UNSIGNED, IMAGE_REL_BASED_DIR64);
    else
      fatal("unsupported cputype %d", ofile->mh_cputype);
  }
