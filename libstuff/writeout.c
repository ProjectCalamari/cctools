...
static void copy_new_symbol_info(char *p, uint32_t *size,
...
static void make_table_of_contents(struct arch *archs, char *output,
                                   time_t toc_time, enum bool sort_toc,
                                   enum bool commons_in_toc,
                                   enum bool force_64bit_toc,
                                   enum bool library_warnings,
                                   enum bool deterministic);

static enum bool toc_symbol(struct nlist *symbol, enum bool commons_in_toc,
...
static enum bool toc(uint32_t n_strx, uint8_t n_type, uint64_t n_value,
                     enum bool commons_in_toc, enum bool attr_no_toc);

static int toc_entry_name_qsort(const struct toc_entry *toc1,
                                const struct toc_entry *toc2);

static int toc_entry_index_qsort(const struct toc_entry *toc1,
                                 const struct toc_entry *toc2);

static enum bool check_sort_toc_entries(struct arch *arch, char *output,
                                        enum bool library_warnings);
 ...
 */
__private_extern__ void writeout(struct arch *archs, uint32_t narchs,
                                 char *output, unsigned short mode,
                                 enum bool sort_toc, enum bool commons_in_toc,
                                 enum bool force_64bit_toc,
                                 enum bool library_warnings,
                                 enum bool deterministic, uint32_t *throttle) {
  uint32_t fsync;
  int fd;
#ifndef __OPENSTEP__
  ...
  writeout_to_mem(archs, narchs, output, (void **)&file, &file_size, sort_toc,
                  commons_in_toc, force_64bit_toc, library_warnings,
                  deterministic, &seen_archive);
...
#define WRITE_SIZE (32 * 1024)
    ...
    if ((r = host_info(my_mach_host_self, HOST_SCHED_INFO, (host_info_t)(&info),
                       &count)) != KERN_SUCCESS) {
...
#endif
    if (time_result) {
      system_fatal("can't set the modifiy times in output file: %s", output);
      goto cleanup;
    }
  }
cleanup:
  if ((r = vm_deallocate(mach_task_self(), (vm_address_t)file, file_size)) !=
      KERN_SUCCESS) {
...
writeout_to_mem(struct arch *archs, uint32_t narchs, char *filename,
                void **outputbuf, uint64_t *length, enum bool sort_toc,
                enum bool commons_in_toc, enum bool force_64bit_toc,
                enum bool library_warnings, enum bool deterministic,
                enum bool *seen_archive) {
  ...
  *seen_archive = FALSE;
   ...
   * architecture.
   */
  if (narchs > 1 || archs[0].fat_arch != NULL || archs[0].fat_arch64 != NULL) {
    ...
    if (archs[i].type == OFILE_ARCHIVE) {
      *seen_archive = TRUE;
      ...
      if (archs[i].object->output_codedir) {
        size += codedir_datasize_delta(archs[i].object->output_codedir);
      }
#endif /* CODEDIRECTORY_SUPPORT */
      file_size += size;
      if (archs[i].fat_arch64 != NULL)
   ...
   * zero bytes.
   */
  if ((r = vm_allocate(mach_task_self(), (vm_address_t *)&file, file_size,
                       TRUE)) != KERN_SUCCESS)
    mach_fatal(r,
  ...
  if (narchs > 1 || archs[0].fat_arch != NULL || archs[0].fat_arch64 != NULL) {
    ...
    if (archs[0].fat_arch64 != NULL) {
      ...
      fat_arch64 = (struct fat_arch_64 *)(file + sizeof(struct fat_header));
      fat_arch = NULL;
    } else {
  ...
  for (i = 0; i < narchs; i++) {
    ...
    if (archs[i].type == OFILE_ARCHIVE) {
      *seen_archive = TRUE;
      ...
      for (j = 0; j < archs[i].nmembers && target_byte_sex == UNKNOWN_BYTE_SEX;
           j++) {
        if (archs[i].members[j].type == OFILE_Mach_O)
          target_byte_sex = archs[i].members[j].object->object_byte_sex;
      }
      ...
      if (archs[i].using_64toc == FALSE) {
        ...
        if (archs[i].ntocs * sizeof(struct ranlib) > 0xFFFFFFFF) {
          error("file too large to create because the library "
                ...
                archs[i].ntocs * sizeof(struct ranlib), archs[i].file_name);
          return;
        }
        ...
        if (target_byte_sex != host_byte_sex)
          swap_ranlib(archs[i].toc_ranlibs, (uint32_t)archs[i].ntocs,
                      target_byte_sex);
      ...
      for (j = 0; j < archs[i].nmembers; j++) {
        memcpy(p, (char *)(archs[i].members[j].ar_hdr), sizeof(struct ar_hdr));
        p += sizeof(struct ar_hdr);
            ...
            if (archs[i].members[j].object->mh != NULL) {
              ...
              if (swap_object_headers(
                ...
                fatal("internal error: "
                      "swap_object_headers() failed");
              if (archs[i].members[j].object->output_nsymbols != 0)
                swap_nlist_64(archs[i].members[j].object->output_symbols64,
                              archs[i].members[j].object->output_nsymbols,
                              archs[i].members[j].object->object_byte_sex);
            }
          }
          if (archs[i].members[j].object->output_sym_info_size == 0 &&
              archs[i].members[j].object->input_sym_info_size == 0) {
            size = archs[i].members[j].object->object_size;
            memcpy(p, archs[i].members[j].object->object_addr, size);
          } else {
            size = archs[i].members[j].object->object_size -
      ...
      if (archs[i].object->mh_filetype == MH_DYLIB) {
        ...
        for (index = i - 1; timestamp == 0 && index >= 0; index--) {
          if (archs[index].type == OFILE_Mach_O &&
              ...
              archs[index].object->mh_cputype == archs[i].object->mh_cputype) {
            ...
            swapped = archs[index].object->object_byte_sex != host_byte_sex;
            if (swapped)
        ...
        for (j = 0; j < ncmds; j++) {
          if (lcp->cmd == LC_ID_DYLIB) {
            dlp = (struct dylib_command *)lcp;
            if (archs[i].dont_update_LC_ID_DYLIB_timestamp == FALSE)
      ...
      if (archs[i].object->output_sym_info_size == 0 &&
        ...
        size =
            archs[i].object->object_size - archs[i].object->input_sym_info_size;
        memcpy(p, archs[i].object->object_addr, size);
        ...
        copy_new_symbol_info(p, &size, &dyst, archs[i].object->dyst, &hints_cmd,
                             archs[i].object->hints_cmd, archs[i].object);
      }
#ifdef CODEDIRECTORY_SUPPORT
      /*
...
static void make_table_of_contents(struct arch *arch, char *output,
  ...
  for (i = 0; i < arch->nmembers; i++) {
    ...
    if (member->type == OFILE_Mach_O) {
      ...
      if (object->mh != NULL) {
        object->sections = allocate(nsects * sizeof(struct section *));
        object->sections64 = NULL;
      } else {
        object->sections = NULL;
        object->sections64 = allocate(nsects * sizeof(struct section_64 *));
      }
      nsects = 0;
      lc = object->load_commands;
      for (j = 0; j < ncmds; j++) {
      ...
      if (object->output_sym_info_size == 0) {
        ...
        if (object->st != NULL && object->st->nsyms != 0) {
          if (object->mh != NULL) {
            symbols =
                (struct nlist *)(object->object_addr + object->st->symoff);
            if (object->object_byte_sex != get_host_byte_sex())
              swap_nlist(symbols, object->st->nsyms, get_host_byte_sex());
          } else {
            symbols64 =
                (struct nlist_64 *)(object->object_addr + object->st->symoff);
            if (object->object_byte_sex != get_host_byte_sex())
              swap_nlist_64(symbols64, object->st->nsyms, get_host_byte_sex());
          }
      ...
      for (j = 0; j < nsymbols; j++) {
        if (object->mh != NULL) {
          if (toc_symbol(symbols + j, commons_in_toc, object->sections) ==
              TRUE) {
            arch->ntocs++;
            arch->toc_strsize += strlen(strings + symbols[j].n_un.n_strx) + 1;
          }
        } else {
          if (toc_symbol_64(symbols64 + j, commons_in_toc,
                            object->sections64) == TRUE) {
            arch->ntocs++;
            arch->toc_strsize += strlen(strings + symbols64[j].n_un.n_strx) + 1;
          }
        }
      }
    }
      ...
      for (j = 0; j < nsymbols; j++) {
        if (lto_toc_symbol(member->lto, j, commons_in_toc) == TRUE) {
          arch->ntocs++;
          arch->toc_strsize += strlen(lto_symbol_name(member->lto, j)) + 1;
        }
      }
    }
#endif /* LTO_SUPPORT */
    ...
    if (member->type == OFILE_Mach_O) {
      ...
      for (j = 0; j < nsymbols; j++) {
        if (object->mh != NULL) {
          ...
          if (toc_symbol(symbols + j, commons_in_toc, object->sections) ==
              TRUE) {
            strcpy(arch->toc_strings + s, strings + symbols[j].n_un.n_strx);
            arch->toc_entries[r].symbol_name = arch->toc_strings + s;
            arch->toc_entries[r].member_index = i + 1;
            r++;
            s += strlen(strings + symbols[j].n_un.n_strx) + 1;
          }
        } else {
          if ((uint32_t)symbols64[j].n_un.n_strx > strings_size)
            continue;
          if (toc_symbol_64(symbols64 + j, commons_in_toc,
                            object->sections64) == TRUE) {
            strcpy(arch->toc_strings + s, strings + symbols64[j].n_un.n_strx);
            arch->toc_entries[r].symbol_name = arch->toc_strings + s;
            arch->toc_entries[r].member_index = i + 1;
            r++;
      ...
      if (object->output_sym_info_size == 0) {
        if (object->object_byte_sex != get_host_byte_sex()) {
          if (object->mh != NULL)
            ...
            swap_nlist_64(symbols64, nsymbols, object->object_byte_sex);
        }
      }
    }
#ifdef LTO_SUPPORT
      ...
      for (j = 0; j < nsymbols; j++) {
        if (lto_toc_symbol(member->lto, j, commons_in_toc) == TRUE) {
          strcpy(arch->toc_strings + s, lto_symbol_name(member->lto, j));
          arch->toc_entries[r].symbol_name = arch->toc_strings + s;
          arch->toc_entries[r].member_index = i + 1;
          r++;
  ...
  if (sort_toc == TRUE) {
    ...
    if (sorted == FALSE) {
      qsort(arch->toc_entries, arch->ntocs, sizeof(struct toc_entry),
            (int (*)(const void *, const void *))toc_entry_index_qsort);
    }
  } else {
    sorted = FALSE;
  }
  ...
  if (arch->using_64toc) {
    ...
    arch->toc_ranlibs64 = allocate(sizeof(struct ranlib_64) * arch->ntocs);
    /*
  ...
  for (i = 0; i < arch->ntocs; i++) {
    if (arch->using_64toc) {
      ...
      arch->toc_ranlibs[i].ran_un.ran_strx =
          (uint32_t)(arch->toc_entries[i].symbol_name - arch->toc_strings);
      arch->toc_ranlibs[i].ran_off =
          (uint32_t)arch->members[arch->toc_entries[i].member_index - 1].offset;
    }
  }
  ...
  sprintf((char *)(&arch->toc_ar_hdr), "%-*s%-*ld%-*u%-*u%-*o%-*ld",
          ...
          (int)sizeof(arch->toc_ar_hdr.ar_uid), (unsigned short)uid,
          (int)sizeof(arch->toc_ar_hdr.ar_gid), (unsigned short)gid,
...
static enum bool toc(uint32_t n_strx, uint8_t n_type, uint64_t n_value,
  ...
  if ((n_type & N_TYPE) == N_UNDF && n_value != 0 && commons_in_toc == FALSE)
    return (FALSE);
  ...
  for (i = 0; i < arch->ntocs - 1; i++) {
    if (strcmp(arch->toc_entries[i].symbol_name,
      ...
      if (arch->toc_entries[i].member_index > 0) {
        member = arch->members + arch->toc_entries[i].member_index - 1;
      ...
      if (arch->toc_entries[i + 1].member_index > 0) {
        member = arch->members + arch->toc_entries[i + 1].member_index - 1;
        warn_member(arch, member, "defines symbol: %s",
 ...
 */
__private_extern__ void reset_load_command_pointers(struct object *object) {
  struct load_command *lc;
  uint32_t i, ncmds;
  ...
  for (i = 0; i < ncmds; i++) {
    ...
    case LC_FUNCTION_STARTS:
      object->func_starts_info_cmd = (struct linkedit_data_command *)lc;
      break;
    case LC_DATA_IN_CODE:
      object->data_in_code_cmd = (struct linkedit_data_command *)lc;
      break;
    case LC_ATOM_INFO:
      object->atom_info_cmd = (struct linkedit_data_command *)lc;
      break;
    case LC_DYLIB_CODE_SIGN_DRS:
      object->code_sign_drs_cmd = (struct linkedit_data_command *)lc;
      break;
    case LC_LINKER_OPTIMIZATION_HINT:
      object->link_opt_hint_cmd = (struct linkedit_data_command *)lc;
      break;
    ...
    case LC_ENCRYPTION_INFO:
      object->encryption_info_command = (struct encryption_info_command *)lc;
      break;
