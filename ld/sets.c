 ...
 */
__private_extern__ void new_set(void) {
  long i;

  if (cur_set + 2 > nsets) {
    sets = reallocate(sets, (nsets + NSETS_INCREMENT) * sizeof(struct set));
 ...
 */
__private_extern__ void new_archive_or_fat(char *file_name, char *file_addr,
                                           unsigned long file_size) {
  sets[cur_set].archives =
      reallocate(sets[cur_set].archives,
                 (sets[cur_set].narchives + 1) * sizeof(struct archive));

  sets[cur_set].archives[sets[cur_set].narchives].file_name =
      allocate(strlen(file_name) + 1);
  strcpy(sets[cur_set].archives[sets[cur_set].narchives].file_name, file_name);
 ...
 */
__private_extern__ void clean_archives_and_fats(void) {
#ifndef SA_RLD
  unsigned long i;
  kern_return_t r;
 ...
 */
__private_extern__ void remove_set(void) {
  if (cur_set >= 0) {
 ...
 * initial state.
 */
__private_extern__ void free_sets(void) {
  long i;
