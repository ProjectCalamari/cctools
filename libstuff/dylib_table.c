 ...
 */
struct dylib_table *
parse_dylib_table(char *file_name, /* file name of dylib table file */
                  ...
                  char *argument)  /* -dylib_file argument or "dylib table" */
{
  ...
  if ((fd = open(file_name, O_RDONLY, 0)) == -1)
    system_fatal("Can't open: %s for %s %s", file_name, flag, argument);
  if (fstat(fd, &stat_buf) == -1)
    system_fatal("Can't stat file: %s for %s %s", file_name, flag, argument);
  ...
  if (stat_buf.st_size > 0xFFFFFFFF) {
    fatal("File too large (%llu): %s for %s %s", stat_buf.st_size, file_name,
          flag, argument);
    ...
    if ((intptr_t)file_addr == -1)
      system_error("can't map file: %s for %s %s", file_name, flag, argument);
  }
  close(fd);
  ...
  if (file_addr[file_size - 1] != '\n')
    fatal("file: %s for %s %s does not end in new line", file_name, flag,
          argument);
  ...
  new_dylib_table_size++;
  new_dylib_table = allocate(sizeof(struct dylib_table) * new_dylib_table_size);
  k = 0;
  for (j = 0; j < file_size; /* no increment expression */) {
    ...
    new_dylib_table[k].seg1addr = (uint32_t)strtoul(file_addr + j, &endp, 16);
    if (endp == NULL)
      fatal("improper hexadecimal number on line %u in "
            "file: %s for %s %s",
            j, file_name, flag, argument);
 ...
 */
struct dylib_table *parse_default_dylib_table(char **file_name) {
  size_t i;
  FILE *fp;
 ...
 * is returned.
 */
struct dylib_table *search_dylib_table(struct dylib_table *dylib_table,
                                       char *name) {
 ...
 * NULL.
 */
char *guess_dylib_install_name(char *name) {
  uint32_t i;
  char *guess;
  struct stat stat_buf;
  ...
  for (i = 0; versions[i] != NULL; i++) {
    guess =
        makestr(SYSTEM_LIBRARY, "/Frameworks/JavaVM.framework/Libraries/lib",
  ...
  for (i = 0; versions[i] != NULL; i++) {
    guess = makestr(SYSTEM_LIBRARY, "Printers/", name, ".", versions[i],
                    ".dylib", NULL);
    if (stat(guess, &stat_buf) != -1)
      return (guess);
    free(guess);
  ...
  for (i = 0; versions[i] != NULL; i++) {
    ...
    guess = makestr("/Local/Library/Frameworks/", name, ".framework/Versions/",
                    versions[i], "/", name, NULL);
#else
    guess = makestr("/MacOSX/Library/Frameworks/", name, ".framework/Versions/",
                    versions[i], "/", name, NULL);
#endif
    if (stat(guess, &stat_buf) != -1)
      return (guess);
  ...
  for (i = 0; versions[i] != NULL; i++) {
    guess = makestr("/usr/lib/", name, ".", versions[i], ".dylib", NULL);
    if (stat(guess, &stat_buf) != -1)
      return (guess);
    free(guess);
  }
  ...
  for (i = 0; versions[i] != NULL; i++) {
    guess = makestr("/usr/lib/java/", name, ".", versions[i], ".dylib", NULL);
    if (stat(guess, &stat_buf) != -1)
      return (guess);
    free(guess);
  }
  ...
  for (i = 0; versions[i] != NULL; i++) {
    guess = makestr("/usr/local/lib/", name, ".", versions[i], ".dylib", NULL);
    if (stat(guess, &stat_buf) != -1)
      return (guess);
    free(guess);
  }
  ...
  for (i = 0; versions[i] != NULL; i++) {
    guess = makestr(SYSTEM_LIBRARY,
                    "/Frameworks/CarbonCore.framework/"
                    "Versions/",
                    versions[i], "/Support/", name, ".dylib", NULL);
    if (stat(guess, &stat_buf) != -1)
      return (guess);
    free(guess);
  }
  for (i = 0; versions[i] != NULL; i++) {
    guess = makestr(SYSTEM_LIBRARY,
                    ...
                    "Versions/",
                    versions[i], "/Libraries/", name, ".dylib", NULL);
    if (stat(guess, &stat_buf) != -1)
      return (guess);
    free(guess);
  }
  for (i = 0; versions[i] != NULL; i++) {
    guess = makestr(SYSTEM_LIBRARY,
                    ...
                    "Versions/",
                    versions[i], "/Libraries/", name, ".", versions[i],
                    ".dylib", NULL);
    if (stat(guess, &stat_buf) != -1)
      return (guess);
    free(guess);
  ...
  for (i = 0; versions[i] != NULL; i++) {
    guess = makestr(SYSTEM_LIBRARY,
                    "/Frameworks/Carbon.framework/"
                    "Versions/",
                    versions[i], "/Resources//", name, ".qtx", NULL);
    if (stat(guess, &stat_buf) != -1)
      return (guess);
    free(guess);
  }
  for (i = 0; versions[i] != NULL; i++) {
    guess = makestr(SYSTEM_LIBRARY,
                    "/Frameworks/CoreGraphics.framework/"
                    "Versions/",
                    versions[i], "/Resources/", name, ".", versions[i], ".qtx",
                    NULL);
    if (stat(guess, &stat_buf) != -1)
      return (guess);
    free(guess);
