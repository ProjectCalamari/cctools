...
__private_extern__ enum bool trace = FALSE; /* print stages of link-editing */
__private_extern__ enum bool save_reloc =
    FALSE; /* save relocation information */
__private_extern__ enum bool output_for_dyld =
    FALSE; /* produce output for use with dyld */
__private_extern__ enum bool bind_at_load = FALSE; /* mark the output for dyld
                                                      to be bound when loaded */
__private_extern__ enum bool no_fix_prebinding =
    FALSE; /* mark the output for dyld to never
              run fix_prebinding */
__private_extern__ enum bool load_map = FALSE; /* print a load map */
__private_extern__ enum bool define_comldsyms =
    TRUE; /* define common and link-editor defined
             symbol reguardless of file type */
#ifndef RLD
static enum bool dflag_specified = FALSE; /* the -d flag has been specified */
#endif                                    /* !defined(RLD) */
__private_extern__ enum bool seglinkedit =
    FALSE; /* create the link edit segment */
#ifndef RLD
static enum bool seglinkedit_specified = FALSE; /* if either -seglinkedit or */
/*  -noseglinkedit was specified */
#endif                                        /* !defined(RLD) */
__private_extern__ enum bool whyload = FALSE; /* print why archive members are
                                                 loaded */
#ifndef RLD
static enum bool whatsloaded = FALSE; /* print which object files are loaded*/
#endif                                /* !defined(RLD) */
__private_extern__ enum bool flush = TRUE; /* Use the output_flush routine to
                                              flush output file by pages */
__private_extern__ enum bool sectorder_detail =
    FALSE; /* print sectorder warnings in detail */
__private_extern__ enum bool nowarnings = FALSE; /* suppress warnings */
__private_extern__ enum bool no_arch_warnings =
    FALSE; /* suppress wrong arch warnings */
__private_extern__ enum bool arch_errors_fatal =
    FALSE; /* cause wrong arch errors to be fatal*/
__private_extern__ enum bool archive_ObjC =
    FALSE; /* objective-C archive semantics */
__private_extern__ enum bool archive_all =
    FALSE; /* always load everything in archives */
__private_extern__ enum bool keep_private_externs =
    FALSE; /* don't turn private externs into
              non-external symbols */
/* TRUE if -dynamic is specified, FALSE if -static is specified */
__private_extern__ enum bool dynamic = TRUE;
#ifndef RLD
...
static enum bool unprebound_library(char *dylib_install_name,
...
int main(int argc, char *argv[], char *envp[]) {
  ...
  for (i = 1; i < argc; i++) {
    if (*argv[i] != '-') {
      ...
      case 'L':
        ...
        search_dirs =
            reallocate(search_dirs, (nsearch_dirs + 1) * sizeof(char *));
        search_dirs[nsearch_dirs++] = &(p[1]);
        if (stat(&(p[1]), &stat_buf) == -1)
          warning("-L: directory name (%s) does not exist", &(p[1]));
        break;
      case 'Z':
        if (p[1] != '\0')
          goto unknown_flag;
        ...
        if (strcmp(p, "preload") == 0 || p[1] == '\0') {
          ...
          if (*endp != '\0')
            fatal("size for -pagezero_size %s not a proper "
                  ...
                  argv[i + 1]);
          if (pagezero_size == 0)
            fatal("size for -pagezero_size %s must not be zero", argv[i + 1]);
          i += 1;
        } else if (strcmp(p, "prebind") == 0) {
          if (prebinding_flag_specified == TRUE && prebinding == FALSE)
      ...
      case 'f':
        if (p[1] == '\0')
          ...
          if (namespace_specified == TRUE && twolevel_namespace == TRUE)
      ...
      case 'F':
        ...
        framework_dirs =
            reallocate(framework_dirs, (nframework_dirs + 1) * sizeof(char *));
        framework_dirs[nframework_dirs++] = &(p[1]);
        if (stat(&(p[1]), &stat_buf) == -1)
          warning("-F: directory name (%s) does not exist", &(p[1]));
        break;
      ...
      case 'r':
        if (strcmp(p, "read_only_relocs") == 0) {
          ...
          if (strcmp(argv[i], "error") == 0)
            ...
            fatal("-read_only_relocs: unknown argument: %s", argv[i]);
            new_read_only_reloc_flag = READ_ONLY_RELOC_ERROR;
          }
      ...
      case 'd':
        if (strcmp(p, "d") == 0) {
          ...
          if (get_version_number("-dylib_current_version", argv[i + 1],
                                 &dylib_current_version) == FALSE)
            cleanup();
          ...
          if (i + 1 >= argc)
            fatal("-dylib_compatibility_version: argument "
                  "missing");
          if (get_version_number("-dylib_compatibility_version", argv[i + 1],
                                 &dylib_compatibility_version) == FALSE)
            cleanup();
          ...
          if (file_name == NULL || file_name[1] == '\0' || argv[i][0] == ':')
            fatal("-dylib_file argument: %s must have a ':' "
                  "between its file names",
                  argv[i]);
          dylib_files =
              reallocate(dylib_files, (ndylib_files + 1) * sizeof(char *));
          dylib_files[ndylib_files++] = argv[i];
        } else if (strcmp(p, "dylinker") == 0) {
          if (filetype_specified == TRUE && filetype != MH_DYLINKER)
            fatal("more than one output filetype specified");
          filetype_specified = TRUE;
      ...
      case 'n':
        if (strcmp(p, "noflush") == 0) {
          ...
          if (prebinding_flag_specified == TRUE && prebinding == TRUE)
            fatal("both -prebind and -noprebind can't "
                  "be specified");
          prebinding_flag_specified = TRUE;
          prebinding = FALSE;
      ...
      case 's':
        ...
        /* create a section from the contents of a file
          ...
          if (sect_spec->contents_filename != NULL)
            fatal("section (%s,%s) multiply specified with a "
                  "%s option",
                  argv[i + 1], argv[i + 2], argv[i]);
          if ((fd = open(argv[i + 3], O_RDONLY, 0)) == -1)
            system_fatal("Can't open: %s for %s %s %s", argv[i + 3], argv[i],
                         argv[i + 1], argv[i + 2]);
          if (fstat(fd, &stat_buf) == -1)
            system_fatal("Can't stat file: %s for %s %s %s", argv[i + 3],
          ...
          if (stat_buf.st_size != 0) {
            if ((r = map_fd((int)fd, (vm_offset_t)0,
                ...
                KERN_SUCCESS)
              mach_fatal(r, "can't map file: %s for %s %s %s", argv[i + 3],
                         argv[i], argv[i + 1], argv[i + 2]);
          } else {
        ...
        /* specify that section object symbols are to be created
          ...
          if (sect_object_symbols.specified &&
              (strcmp(sect_object_symbols.segname, argv[i + 1]) != 0 ||
               strcmp(sect_object_symbols.sectname, argv[i + 2]) != 0))
        ...
        /* layout a section in the order the symbols appear in file
          ...
          if (strcmp(argv[i], "error") == 0)
            ...
            fatal("-sect_diff_relocs: unknown argument: %s", argv[i]);
            new_sect_diff_reloc_flag = SECT_DIFF_RELOC_SUPPRESS;
          }
        ...
        /* specify the address (in hex) of a segment
          ...
          if (seg_spec->addr_specified == TRUE)
            fatal("address of segment %s multiply specified", argv[i + 1]);
        ...
        /* specify the protection for a segment
          ...
          if (*endp != '\0')
            fatal("bad character: '%c' in maximum protection: "
                  "%s for segment %s",
                  *endp, argv[i + 2], argv[i + 1]);
          seg_spec->initprot = getprot(argv[i + 3], &endp);
          if (*endp != '\0')
            fatal("bad character: '%c' in initial protection: "
                  "%s for segment %s",
                  *endp, argv[i + 3], argv[i + 1]);
          if (check_max_init_prot(seg_spec->maxprot, seg_spec->initprot) ==
              FALSE)
        ...
        /* specify the segment alignment as a hexadecimal power of 2
          ...
          if (segalign > MAXSEGALIGN)
            fatal("argument to -segalign: %lx (hex) must equal "
                  "to or less than %x (hex)",
                  segalign, (unsigned int)MAXSEGALIGN);
        ...
        /* specify a sub_umbrella
          ...
          if (i + 1 >= argc)
            fatal("%s: argument missing", argv[i]);
          sub_umbrellas =
              reallocate(sub_umbrellas, (nsub_umbrellas + 1) * sizeof(char *));
        ...
        /* specify a sub_library
          ...
          if (i + 1 >= argc)
            fatal("%s: argument missing", argv[i]);
          sub_librarys =
              reallocate(sub_librarys, (nsub_librarys + 1) * sizeof(char *));
          sub_librarys[nsub_librarys++] = argv[i + 1];
          i += 1;
        }
        /* -single_module for MH_DYLIB output */
        else if (strcmp(p, "single_module") == 0) {
          if (moduletype_specified == TRUE && multi_module_dylib == TRUE)
      ...
      case 't':
        ...
        if (strcmp(p, "twolevel_namespace") == 0) {
          if (namespace_specified == TRUE && twolevel_namespace == FALSE)
      ...
      case 'a':
        if (strcmp(p, "all_load") == 0)
          ...
          if (arch_flag.name != NULL && strcmp(arch_flag.name, argv[i]) != 0)
        ...
        /* specify an allowable client of this subframework
          ...
          if (i + 1 >= argc)
            fatal("%s: argument missing", argv[i]);
          allowable_clients = reallocate(
              allowable_clients, (nallowable_clients + 1) * sizeof(char *));
        ...
        if (strcmp(p, "client_name") == 0) {
          ...
          if (i + 1 >= argc)
            fatal("-compatibility_version: argument "
                  "missing");
          if (get_version_number("-compatibility_version", argv[i + 1],
                                 &dylib_compatibility_version) == FALSE)
            cleanup();
          if (dylib_compatibility_version == 0)
          ...
          if (get_version_number("-current_version", argv[i + 1],
                                 &dylib_current_version) == FALSE)
            cleanup();
          if (dylib_current_version == 0)
      ...
      case 'i':
        if (strcmp(p, "image_base") == 0) {
          ...
          if (indr_symbol_name == NULL || indr_symbol_name[1] == '\0' ||
              *symbol_name == ':')
      ...
      case 'm':
        if (strcmp(p, "multiply_defined") == 0) {
          ...
          if (strcmp(argv[i], "error") == 0)
            ...
            fatal("-multiply_defined: unknown argument: %s", argv[i]);
            new_multiply_defined_flag = MULTIPLY_DEFINED_WARNING;
          }
          ...
          if (strcmp(argv[i], "error") == 0)
            new_multiply_defined_unused_flag = MULTIPLY_DEFINED_ERROR;
          else if (strcmp(argv[i], "warning") == 0)
            new_multiply_defined_unused_flag = MULTIPLY_DEFINED_WARNING;
          else if (strcmp(argv[i], "suppress") == 0)
            new_multiply_defined_unused_flag = MULTIPLY_DEFINED_SUPPRESS;
          else {
          ...
          if (multiply_defined_unused_flag_specified == TRUE &&
              new_multiply_defined_unused_flag != multiply_defined_unused_flag)
            fatal("more than one value specified for "
                  "-multiply_defined_unused");
          multiply_defined_unused_flag_specified = TRUE;
          multiply_defined_unused_flag = new_multiply_defined_unused_flag;
          break;
        }
        /* -multi_module for MH_DYLIB output */
        else if (strcmp(p, "multi_module") == 0) {
          if (moduletype_specified == TRUE && multi_module_dylib == FALSE)
            fatal("can't specify both -single_module and "
                  "-multi_module");
          moduletype_specified = TRUE;
      ...
      case 'u':
        ...
        /* specify this dynamic library as a subframework
          ...
          if (remove_symbols != NULL)
            fatal("%s: multiply specified", argv[i]);
          setup_symbol_list(argv[i + 1], &remove_symbols, &nremove_symbols);
      ...
      case 'e':
        if (strcmp(p, "execute") == 0) {
          ...
          if (save_symbols != NULL)
            fatal("%s: multiply specified", argv[i]);
          setup_symbol_list(argv[i + 1], &save_symbols, &nsave_symbols);
      ...
      case 'U':
        ...
        if (++i >= argc)
          fatal("-U: argument missing");
        undef_syms = reallocate(undef_syms, (nundef_syms + 1) * sizeof(char *));
        ...
        if (strcmp(p, "w") == 0)
          ...
          if (++i >= argc)
            fatal("-weak_reference_mismatches: "
                  "argument missing");
          if (strcmp(argv[i], "error") == 0)
            new_weak_reference_mismatches = WEAK_REFS_MISMATCH_ERROR;
          else if (strcmp(argv[i], "weak") == 0)
            new_weak_reference_mismatches = WEAK_REFS_MISMATCH_WEAK;
          else if (strcmp(argv[i], "non-weak") == 0)
            new_weak_reference_mismatches = WEAK_REFS_MISMATCH_NON_WEAK;
          else {
          ...
          if (weak_reference_mismatches_specified == TRUE &&
              new_weak_reference_mismatches != weak_reference_mismatches)
            fatal("more than one value specified for "
                  "-weak_reference_mismatches");
          weak_reference_mismatches_specified = TRUE;
          weak_reference_mismatches = new_weak_reference_mismatches;
          break;
      ...
      case 'y':
        ...
        trace_syms = reallocate(trace_syms, (ntrace_syms + 1) * sizeof(char *));
        ...
        if (strcmp(p, "NEXTSTEP-deployment-target") == 0) {
          if (i + 1 >= argc)
            fatal("-NEXTSTEP-deployment-target: argument "
                  "missing");
          if (dynamic_specified == TRUE || static_specified == TRUE)
  ...
  if (next_root != NULL) {
    ...
    for (i = 0; standard_framework_dirs[i] != NULL; i++) {
      p = allocate(strlen(next_root) + strlen(standard_framework_dirs[i]) + 1);
      strcpy(p, next_root);
  ...
  if (getenv("LD_TRACE_BUNDLE_LOADER") != NULL && bundle_loader != NULL)
   ...
   */
  else if (getenv("LD_PREBIND") != NULL && save_reloc == FALSE &&
           filetype != MH_FVMLIB) {
    if (prebinding_flag_specified == TRUE && prebinding == FALSE) {
   ...
   */
  if (getenv("LD_TWOLEVEL_NAMESPACE") != NULL && namespace_specified == FALSE &&
  ...
  if (filetype == MH_DYLIB) {
    ...
    if (segs_read_only_addr_specified &&
      ...
      fatal("can't used -read_only_relocs %s with format produced "
            ...
            read_only_reloc_flag == READ_ONLY_RELOC_WARNING ? "warning"
                                                            : "suppress");
    if (segs_read_write_addr_specified && !segs_read_only_addr_specified)
    ...
    if (seg_addr_table_name != NULL ||
      ...
      if (seg_addr_table_name != NULL && env_seg_addr_table_name != NULL &&
      ...
      if (env_seg_addr_table_name != NULL) {
        seg_addr_table_name = env_seg_addr_table_name;
        seg_addr_table =
            parse_seg_addr_table(seg_addr_table_name, "LD_SEG_ADDR_TABLE",
                                 "environment variable", &table_size);
      } else
        seg_addr_table =
            parse_seg_addr_table(seg_addr_table_name, "-seg_addr_table",
                                 seg_addr_table_name, &table_size);
      if (seg_addr_table_filename != NULL)
        seg_addr_table_entry =
            search_seg_addr_table(seg_addr_table, seg_addr_table_filename);
      else
        seg_addr_table_entry =
            search_seg_addr_table(seg_addr_table, dylib_install_name);
      if (seg_addr_table_entry != NULL) {
        if (seg_addr_table_entry->split == TRUE) {
          if (read_only_reloc_flag != READ_ONLY_RELOC_ERROR) {
            warning("-read_only_relocs %s ignored, when using "
                    ...
                    read_only_reloc_flag == READ_ONLY_RELOC_WARNING
                        ? "warning"
                        : "suppress",
                    env_seg_addr_table_name != NULL ? "LD_SEG_ADDR_TABLE"
                                                    : "-seg_addr_table",
                    seg_addr_table_name, seg_addr_table_entry->line);
            read_only_reloc_flag = READ_ONLY_RELOC_ERROR;
          }
          if (seg1addr_specified) {
            warning("-seg1addr 0x%x ignored, using "
                    ...
                    (unsigned int)seg_addr_table_entry->segs_read_only_addr,
                    (unsigned int)seg_addr_table_entry->segs_read_write_addr,
                    env_seg_addr_table_name != NULL ? "LD_SEG_ADDR_TABLE"
                                                    : "-seg_addr_table",
                    seg_addr_table_name, seg_addr_table_entry->line);
          }
          ...
          seg1addr = 0;
          segs_read_only_addr_specified = TRUE;
          segs_read_only_addr = seg_addr_table_entry->segs_read_only_addr;
          segs_read_write_addr_specified = TRUE;
          segs_read_write_addr = seg_addr_table_entry->segs_read_write_addr;
          if (segs_read_only_addr == 0 && segs_read_write_addr == 0) {
          ...
          if (seg1addr_specified &&
            ...
            warning("-seg1addr 0x%x ignored, using "
                    ...
                    "table: %s %s line %u",
                    (unsigned int)seg1addr,
                    (unsigned int)seg_addr_table_entry->seg1addr,
                    env_seg_addr_table_name != NULL ? "LD_SEG_ADDR_TABLE"
                                                    : "-seg_addr_table",
                    seg_addr_table_name, seg_addr_table_entry->line);
          }
        ...
        warning("%s %s not found in segment address table %s %s",
                seg_addr_table_filename != NULL ? "-seg_addr_table_filename"
                                                : "-dylib_install_name",
                seg_addr_table_filename != NULL ? seg_addr_table_filename
                                                : dylib_install_name,
                env_seg_addr_table_name != NULL ? "LD_SEG_ADDR_TABLE"
    ...
    if (sub_framework == FALSE && dylib_install_name != NULL) {
      umbrella_framework_name =
          guess_short_name(dylib_install_name, &is_framework, &has_suffix);
  ...
  if (macosx_deployment_target.major >= 4) {
    if (filetype != MH_DYLIB) {
      ...
      if (!((arch_flag.name != NULL) &&
        ...
        if (prebinding_via_LD_PREBIND == FALSE &&
            prebinding_flag_specified == TRUE && prebinding == TRUE) {
     ...
     */
       ...
       */
      if (seg_addr_table_entry == NULL &&
          unprebound_library(dylib_install_name, seg_addr_table_filename) ==
              TRUE) {
        if (prebinding_flag_specified == TRUE && prebinding == TRUE) {
          warning("-prebind ignored because -install_name %s "
                  "listed in LD_UNPREBOUND_LIBRARIES environment "
                  "variable file: %s",
                  dylib_install_name, getenv("LD_UNPREBOUND_LIBRARIES"));
        }
        prebinding = FALSE;
      } else {
        /*
        ...
        if (seg_addr_table_entry == NULL &&
          ...
          if (seg1addr_specified) {
            warning("-seg1addr 0x%x ignored, using "
                    ...
                    "set",
                    (unsigned int)seg1addr, 0, (unsigned int)arch_rw_addr);
          }
          seg1addr_specified = FALSE;
          seg1addr = 0;
          segs_read_only_addr_specified = TRUE;
  ...
  if (filetype == MH_BUNDLE) {
    ...
    if (client_name != NULL)
      fatal("-client_name %s flag can only be used with -bundle", client_name);
    if (bundle_loader != NULL)
  ...
  if (filetype != MH_DYLIB) {
    ...
    if (init_name != NULL)
      warning("flag: -init %s ignored (-dylib was not specified", init_name);
  }
  ...
  if (seglinkedit_specified == FALSE) {
    if (filetype == MH_EXECUTE || filetype == MH_BUNDLE ||
        filetype == MH_FVMLIB || filetype == MH_DYLIB ||
        filetype == MH_DYLINKER)
      seglinkedit = TRUE;
    else
      seglinkedit = FALSE;
  } else {
    if (seglinkedit && (filetype != MH_EXECUTE && filetype != MH_BUNDLE &&
                        filetype != MH_FVMLIB && filetype != MH_DYLIB &&
                        filetype != MH_DYLINKER))
  ...
  for (i = 1; i < argc; i++) {
    if (*argv[i] != '-') {
        ...
        if (strcmp(p, "filelist") == 0) {
          ...
          if (stat_buf.st_size != 0) {
            if ((r = map_fd((int)fd, (vm_offset_t)0, (vm_offset_t *)&(addr),
                            (boolean_t)TRUE, (vm_size_t)stat_buf.st_size)) !=
                KERN_SUCCESS)
              mach_fatal(r, "can't map file list file: %s", filelist);
          } else {
          ...
          for (j = 0; j < stat_buf.st_size; j++) {
            if (addr[j] == '\n') {
              addr[j] = '\0';
              if (*dirname != '\0') {
                file_name = mkstr(dirname, "/", file_name, NULL);
              }
              pass1(file_name, FALSE, FALSE, FALSE, FALSE, FALSE);
              objects_specified++;
              file_name = addr + j + 1;
            }
          }
      ...
      case 'm':
        ...
        break;
      case 'u':
        if (strcmp(p, "undefined") == 0 || strcmp(p, "umbrella") == 0 ||
      ...
      case 'a':
        if (strcmp(p, "all_load") == 0 || strcmp(p, "arch_multiple") == 0 ||
      ...
      case 'p':
        ...
        break;
      case 's':
        if (strcmp(p, "sectcreate") == 0 || strcmp(p, "segcreate") == 0) {
          sections_created++;
          i += 3;
        } else if (strcmp(p, "sectalign") == 0 || strcmp(p, "segprot") == 0 ||
          ...
          i += 2;
        else if (strcmp(p, "seg1addr") == 0 || strcmp(p, "stack_addr") == 0 ||
                 strcmp(p, "stack_size") == 0 || strcmp(p, "segalign") == 0 ||
                 ...
                 strcmp(p, "sub_library") == 0 || strcmp(p, "syslibroot") == 0)
          i++;
        break;
      case 'r':
        if (strcmp(p, "r") == 0 || strcmp(p, "run_init_lazily") == 0)
          break;
        i++;
        break;
      case 'o':
      ...
      case 'e':
        ...
        break;
      case 'd':
        if (strcmp(p, "d") == 0 || strcmp(p, "dylib") == 0 ||
            strcmp(p, "dylinker") == 0 || strcmp(p, "dynamic") == 0 ||
      ...
      case 'w':
        if (strcmp(p, "weak_reference_mismatches") == 0)
          ...
          /* path searched abbrevated file name */
          pass1(argv[i] + sizeof("weak"), TRUE, FALSE, FALSE, FALSE, TRUE);
          objects_specified++;
        } else if (strcmp(p, "weak_framework") == 0) {
          if (dynamic == FALSE)
...
static enum bool unprebound_library(char *dylib_install_name,
  ...
  if (stat_buf.st_size != 0) {
    if ((r = map_fd((int)fd, (vm_offset_t)0, (vm_offset_t *)&file_addr,
        ...
        KERN_SUCCESS)
      mach_fatal(r,
                 "can't map file: %s for LD_UNPREBOUND_LIBRARIES "
                 "environment variable",
                 file_name);
  } else
 ...
 */
static void ld_exit(int exit_value) { exit(exit_value); }
 ...
 */
__private_extern__ void *reallocate(void *p, unsigned long size) {
  if (p == NULL)
 ...
 */
__private_extern__ char *savestr(const char *s) {
  long len;
  char *r;
...
static void print_architecture_banner(void) {
