 ...
 */
__private_extern__ void literal4_merge(struct literal4_data *data,
  ...
  if (redo_live == FALSE) {
    ...
    for (i = 0; i < nliteral4s; i++) {
      ...
      fine_relocs[i].output_offset = lookup_literal4(literal4s[i], data, ms);
    }
 ...
 * to be ordered.
 */
__private_extern__ void literal4_order(struct literal4_data *data,
                                       struct merged_section *ms) {
#ifndef RLD
  ...
  if (dead_strip == TRUE) {
    ...
    literal4_order_lines =
        allocate(sizeof(struct literal4_order_line) * (line_number - 1));
    data->literal4_load_order_data->literal4_order_lines = literal4_order_lines;
  }
  ...
  while (i < ms->order_size) {
    if (get_hex_from_sectorder(ms, &i, &(literal4.long0), line_number) ==
      ...
      if (dead_strip == TRUE) {
        literal4_order_lines[nliteral4_order_lines].literal4 = literal4;
        literal4_order_lines[nliteral4_order_lines].line_number = line_number;
 ...
 * order file if any.
 */
__private_extern__ void literal4_reset_live(struct literal4_data *data,
                                            struct merged_section *ms) {
#ifndef RLD
  ...
  if (ms->order_filename != NULL) {
    literal4_order_lines = data->literal4_load_order_data->literal4_order_lines;
    nliteral4_order_lines =
        data->literal4_load_order_data->nliteral4_order_lines;
    for (i = 0; i < nliteral4_order_lines; i++) {
      ...
      if (live) {
        (void)lookup_literal4(literal4_order_lines[i].literal4, data, ms);
      } else {
 ...
 * section (ms->s.align).
 */
__private_extern__ unsigned long lookup_literal4(struct literal4 literal4,
 ...
 * data block itself.
 */
__private_extern__ void literal4_output(struct literal4_data *data,
                                        struct merged_section *ms) {
  ...
  for (p = &(data->literal4_blocks); *p;) {
    literal4_block = *p;
    for (i = 0; i < literal4_block->used; i++) {
      memcpy(output_addr + offset, literal4_block->literal4s + i,
 ...
 * data block itself.
 */
__private_extern__ void literal4_free(struct literal4_data *data) {
 ...
 */
__private_extern__ void print_literal4_data(struct literal4_data *data,
                                            char *indent) {
  unsigned long i;
  struct literal4_block **p, *literal4_block;
  ...
  for (p = &(data->literal4_blocks); *p; p = &(literal4_block->next)) {
    ...
    print("%s\tnext 0x%x\n", indent, (unsigned int)(literal4_block->next));
    print("%s\tliteral4s\n", indent);
    for (i = 0; i < literal4_block->used; i++) {
      print("%s\t    0x%08x\n", indent,
 ...
 */
__private_extern__ void literal4_data_stats(struct literal4_data *data,
                                            struct merged_section *ms) {
  if (data == NULL)
    return;
