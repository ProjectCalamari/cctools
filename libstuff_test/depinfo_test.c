...
static void test_depinfo_dump(void) {
  ...
  if (!err) {
    ...
    if (depinfo_b)
      depinfo_free(depinfo_b);
    depinfo_b =
        depinfo_read("/tmp/depinfo.data", DI_READ_LOG | DI_READ_NORETVAL);
...
static int test_main(void) {
  ...
  if (!err)
    err = test_add("test depinfo_alloc", test_depinfo_alloc);
  if (!err)
    err = test_add("test depinfo_add", test_depinfo_add);
  if (!err)
    err = test_add("test depinfo_get", test_depinfo_get);
  if (!err)
    err = test_add("test depinfo_sort", test_depinfo_sort);
  if (!err)
    err = test_add("test depinfo_write_read", test_depinfo_write_read);
  if (!err)
    err = test_add("test depinfo_dump", test_depinfo_dump);
