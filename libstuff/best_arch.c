 ...
 * will always succeed.
 */
static int32_t internal_cpusubtype_findbestarch(cpu_type_t cputype,
  ...
  case CPU_TYPE_VEO:
    ...
    case CPU_SUBTYPE_VEO_3:
      for (i = 0; i < nfat_archs; i++) {
        ...
        if (fat_cputype != cputype)
          continue;
        if ((fat_cpusubtype & ~CPU_SUBTYPE_MASK) == CPU_SUBTYPE_VEO_2)
          return (i);
      }
    case CPU_SUBTYPE_VEO_4:
      for (i = 0; i < nfat_archs; i++) {
        ...
        if (fat_cputype != cputype)
          continue;
        if ((fat_cpusubtype & ~CPU_SUBTYPE_MASK) == CPU_SUBTYPE_VEO_3)
          return (i);
      }
      for (i = 0; i < nfat_archs; i++) {
        if (fat_archs64 != NULL) {
  ...
  case CPU_TYPE_ARM:
    ...
    if (cpusubtype == CPU_SUBTYPE_ARM_ALL ||
      ...
      for (i = 0; i < nfat_archs; i++) {
        ...
        if (fat_cputype == cputype && fat_cpusubtype == CPU_SUBTYPE_ARM_V7S)
          return (i);
      }
    }
    if (cpusubtype == CPU_SUBTYPE_ARM_ALL ||
      ...
      for (i = 0; i < nfat_archs; i++) {
        ...
        if (fat_cputype == cputype && fat_cpusubtype == CPU_SUBTYPE_ARM_V7F)
          return (i);
      }
    }
    if (cpusubtype == CPU_SUBTYPE_ARM_ALL ||
      ...
      for (i = 0; i < nfat_archs; i++) {
        ...
        if (fat_cputype == cputype && fat_cpusubtype == CPU_SUBTYPE_ARM_V7)
          return (i);
      }
    }
    if (cpusubtype == CPU_SUBTYPE_ARM_ALL || cpusubtype == CPU_SUBTYPE_ARM_V7 ||
      ...
      for (i = 0; i < nfat_archs; i++) {
        ...
        if (fat_cputype == cputype && fat_cpusubtype == CPU_SUBTYPE_ARM_V6)
          return (i);
      }
    }
    if (cpusubtype == CPU_SUBTYPE_ARM_ALL || cpusubtype == CPU_SUBTYPE_ARM_V6 ||
      ...
      for (i = 0; i < nfat_archs; i++) {
        ...
        if (fat_cputype == cputype && fat_cpusubtype == CPU_SUBTYPE_ARM_V5TEJ)
          return (i);
      }
    }
    if (cpusubtype == CPU_SUBTYPE_ARM_ALL ||
      ...
      for (i = 0; i < nfat_archs; i++) {
        ...
        if (fat_cputype == cputype && fat_cpusubtype == CPU_SUBTYPE_ARM_XSCALE)
          return (i);
      }
    }
    for (i = 0; i < nfat_archs; i++) {
      ...
      if (fat_cputype == cputype && fat_cpusubtype == CPU_SUBTYPE_ARM_V4T)
        return (i);
    }
    for (i = 0; i < nfat_archs; i++) {
      if (fat_archs64 != NULL) {
      ...
      if (fat_cputype == cputype && fat_cpusubtype == CPU_SUBTYPE_ARM_V6M)
        return (i);
    }
    for (i = 0; i < nfat_archs; i++) {
      if (fat_archs64 != NULL) {
      ...
      if (fat_cputype == cputype && fat_cpusubtype == CPU_SUBTYPE_ARM_V7M)
        return (i);
    }
    for (i = 0; i < nfat_archs; i++) {
      if (fat_archs64 != NULL) {
      ...
      if (fat_cputype == cputype && fat_cpusubtype == CPU_SUBTYPE_ARM_V7EM)
        return (i);
    }
    for (i = 0; i < nfat_archs; i++) {
      if (fat_archs64 != NULL) {
      ...
      if (fat_cputype == cputype && fat_cpusubtype == CPU_SUBTYPE_ARM_V8M_MAIN)
        return (i);
    }
    for (i = 0; i < nfat_archs; i++) {
      if (fat_archs64 != NULL) {
      ...
      if (fat_cputype == cputype && fat_cpusubtype == CPU_SUBTYPE_ARM_V8M_BASE)
        return (i);
    }
    for (i = 0; i < nfat_archs; i++) {
      if (fat_archs64 != NULL) {
    ...
    for (i = 0; i < nfat_archs; i++) {
      ...
      if (fat_cputype == cputype && fat_cpusubtype == CPU_SUBTYPE_ARM_ALL)
        return (i);
    }
  ...
  case CPU_TYPE_ARM64:
    for (i = 0; i < nfat_archs; i++) {
      ...
      if (fat_cputype != cputype)
        continue;
      if ((fat_cpusubtype & ~CPU_SUBTYPE_MASK) == CPU_SUBTYPE_ARM64_ALL)
        return (i);
    }
    break;
 ...
 * always succeed.
 */
__private_extern__ struct fat_arch_64 *
cpusubtype_findbestarch_64(cpu_type_t cputype, cpu_subtype_t cpusubtype,
                           struct fat_arch_64 *fat_archs64,
                           uint32_t nfat_archs) {
  int32_t i;

  i = internal_cpusubtype_findbestarch(cputype, cpusubtype, NULL, fat_archs64,
                                       nfat_archs);
  if (i == -1)
    return (NULL);
 ...
 */
__private_extern__ struct fat_arch *
cpusubtype_findbestarch(cpu_type_t cputype, cpu_subtype_t cpusubtype,
                        struct fat_arch *fat_archs, uint32_t nfat_archs) {
  int32_t i;

  i = internal_cpusubtype_findbestarch(cputype, cpusubtype, fat_archs, NULL,
                                       nfat_archs);
  if (i == -1)
    return (NULL);
  return (fat_archs + i);
 ...
 */
__private_extern__ cpu_subtype_t cpusubtype_combine(cpu_type_t cputype,
...
cpusubtype_execute(cpu_type_t host_cputype,
