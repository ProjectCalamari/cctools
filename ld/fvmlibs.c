 ...
 */
__private_extern__ void merge_fvmlibs(void) {
   ...
   */
    ...
    for (i = 0; i < mh->ncmds; i++) {
      if (lc->cmd == LC_SEGMENT) {
        sg = (struct segment_command *)lc;
        if (sg->flags == SG_FVMLIB)
          add_fvmlib_segment(sg, (char *)fl + fl->fvmlib.name.offset);
      }
      lc = (struct load_command *)((char *)lc + lc->cmdsize);
    }
  }
 ...
 */
__private_extern__ void print_load_fvmlibs_list(void) {
  struct merged_fvmlib **p, *mfl;
 ...
 */
__private_extern__ void print_fvmlib_segments(void) {
  struct merged_segment **p, *msg;
  unsigned long flags;
