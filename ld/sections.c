...
struct archive_name {
  ...
  struct object_name *object_names; /* names of the archive members */
  unsigned long nobject_names;      /* number of archive members */
};
...
struct load_symbol {
  ...
  unsigned long
      index_length; /* if archive_name is NULL the this is index into the */
                    ...
                    /*  which is an archive member name that may have */
                    /*  been truncated. */
  struct load_order *load_order; /* the load order for the above triple names */
  struct load_symbol
      *other_names;         /* other load symbols for the same symbol_name */
  struct load_symbol *next; /* next hash table pointer */
};
...
static void layout_ordered_section(struct merged_section *ms);
static void create_name_arrays(void);
static struct archive_name *create_archive_name(char *archive_name);
static void create_object_name(struct object_name **object_names,
                               unsigned long *nobject_names, char *object_name,
                               unsigned long index_length, char *archive_name);
static void free_name_arrays(void);
static void create_load_symbol_hash_table(unsigned long nsection_symbols,
                                          struct merged_section *ms);
static void free_load_symbol_hash_table(void);
static void create_load_symbol_hash_table_for_object(
    char *archive_name, char *object_name, unsigned long index_length,
    struct load_order *load_orders, unsigned long nload_orders,
    struct merged_section *ms);
static struct load_order *
lookup_load_order(char *archive_name, char *object_name, char *symbol_name,
                  struct merged_section *ms, unsigned long line_number);
static char *trim(char *name);
static struct section_map *lookup_section_map(char *archive_name,
                                              char *object_name);
static int qsort_load_order_names(const struct load_order *load_order1,
...
static int qsort_load_order_input_offset(const struct load_order *load_order1,
                                         const struct load_order *load_order2);
static int qsort_archive_names(const struct archive_name *archive_name1,
                               const struct archive_name *archive_name2);
static int bsearch_archive_names(const char *name,
                                 const struct archive_name *archive_name);
static int qsort_object_names(const struct object_name *object_name1,
                              const struct object_name *object_name2);
static int bsearch_object_names(const char *name,
                                const struct object_name *object_name);
static int qsort_fine_reloc_input_offset(const struct fine_reloc *fine_reloc1,
                                         const struct fine_reloc *fine_reloc2);
static int
qsort_order_load_map_orders(const struct order_load_map *order_load_map1,
                            const struct order_load_map *order_load_map2);
static void create_order_load_maps(struct merged_section *ms,
                                   unsigned long norder_load_maps);
#ifdef DEBUG
static void print_symbol_name_from_order_load_maps(struct section_map *map,
                                                   unsigned long value);
#endif /* DEBUG */
static void resize_live_section(struct merged_section *ms);
static void count_relocs(struct section_map *map,
                         struct relocation_info *relocs, unsigned long *nlocrel,
                         unsigned long *nextrel);
#endif /* !defined(RLD) */
static void scatter_copy(struct section_map *map, char *contents);
#ifndef RLD
static void reloc_output_for_dyld(struct section_map *map,
                                  struct relocation_info *relocs,
                                  struct relocation_info *output_locrel,
                                  struct relocation_info *output_extrel,
                                  unsigned long *nlocrel,
                                  unsigned long *nextrel);
static enum bool is_merged_section_read_only(struct merged_section *key);
static unsigned long scatter_copy_relocs(struct section_map *map,
                                         struct relocation_info *relocs,
                                         struct relocation_info *output_relocs);
static double calculate_time_used(struct timeval *start, struct timeval *end);
static void build_references(void);
static void print_references(void);
static void setup_references_in_section(struct merged_section *ms);
static void setup_references(struct section_map *map, struct object_file *obj);
static void setup_reference(struct live_ref *ref, struct object_file *obj,
                            struct fine_reloc *self_fine_reloc);
static void mark_all_fine_relocs_live_in_section(struct merged_section *ms);
/*
...
#ifdef DEBUG
char *walk_references_operation_names[] = {"MARK_LIVE", "SEARCH_FOR_LIVE",
                                           "CHECK_FOR_LIVE_TOUCH"};
#endif /* DEBUG */
static void walk_references_in_section(enum walk_references_operation operation,
                                       struct merged_section *ms);
static enum bool walk_references(enum walk_references_operation operation,
                                 struct fine_reloc *fine_reloc,
                                 struct section_map *map,
                                 struct object_file *obj);
static enum bool ref_operation(enum walk_references_operation operation,
                               struct ref *ref, struct object_file *obj);

#endif /* !defined(RLD) */
#ifdef DEBUG
static void print_load_symbol_hash_table(void);
#endif /* DEBUG */
 ...
 */
__private_extern__ void merge_sections(void) {
  unsigned long i;
  struct section *s;
  struct merged_section *ms;
  ...
  for (i = 0; i < cur_obj->nsection_maps; i++) {
     ...
     * accounting for it size, alignment and number of relocation
     * * entries as none of that info will be in the output file. Also set
     * output_uuid_info.emit to TRUE since we have seen an input file with a
     * debug section.
     */
    ...
    default:
      fatal("internal error: merge_section() called "
            ...
            "%.16s)",
            (unsigned int)(ms->s.flags & SECTION_TYPE), ms->s.segname,
            ms->s.sectname);
      break;
    }
#ifndef RLD
     ...
     */
    if (filetype == MH_DYLIB && strcmp(s->segname, SEG_OBJC) == 0 &&
        strcmp(s->sectname, SECT_OBJC_MODULES) == 0) {
      if ((ms->s.flags & SECTION_TYPE) != S_REGULAR)
        error_with_cur_obj(
            "for MH_DYLIB output files section "
 ...
 */
__private_extern__ struct merged_section *
create_merged_section(struct section *s) {
  ...
  while (*p) {
    ...
    if (strncmp(msg->sg.segname, s->segname, sizeof(s->segname)) == 0) {
      ...
      while (*q) {
        ms = *q;
        if (strncmp(ms->s.sectname, s->sectname, sizeof(s->sectname)) == 0) {
          if ((ms->s.flags & SECTION_TYPE) != (s->flags & SECTION_TYPE)) {
          ...
          if ((ms->s.flags & SECTION_TYPE) == S_SYMBOL_STUBS &&
            ...
            error_with_cur_obj("section's (%.16s,%.16s) sizeof "
                               ...
                               "sizeof stub %u",
                               s->segname, s->sectname, s->reserved2,
                               ms->s.reserved2);
            return (NULL);
          }
          return (ms);
      ...
      *q = allocate(sizeof(struct merged_section));
      ...
      if ((ms->s.flags & SECTION_TYPE) == S_CSTRING_LITERALS) {
        ...
        memset(ms->literal_data, '\0', sizeof(struct mod_term_data));
 ...
 */
__private_extern__ struct merged_segment *lookup_merged_segment(char *segname) {
  struct merged_segment **p, *msg;
 ...
 */
__private_extern__ struct merged_section *
lookup_merged_section(char *segname, char *sectname) {
  struct merged_segment **p, *msg;
  ...
  while (*p) {
    ...
    if (strncmp(msg->sg.segname, segname, sizeof(msg->sg.segname)) == 0) {
      ...
      while (*q) {
        ms = *q;
        if (strncmp(ms->s.sectname, sectname, sizeof(ms->s.sectname)) == 0) {
          return (ms);
        }
        q = &(ms->next);
      }
 ...
 * matched.
 */
__private_extern__ void remove_debug_segments(void) {
 ...
 * literals.
 */
__private_extern__ void merge_literal_sections(enum bool redo_live) {
  unsigned long i, j;
    ...
    for (i = 0; i < object_list->used; i++) {
      ...
      for (j = 0; j < cur_obj->nsection_maps; j++) {
        ...
        if ((ms->s.flags & SECTION_TYPE) == S_CSTRING_LITERALS ||
            ...
            (ms->s.flags & SECTION_TYPE) == S_MOD_TERM_FUNC_POINTERS ||
            (ms->s.flags & SECTION_TYPE) == S_COALESCED)
          (*ms->literal_merge)(ms->literal_data, ms, cur_obj->section_maps[j].s,
                               &(cur_obj->section_maps[j]), redo_live);
      }
    }
  }
 ...
 * specified.
 */
__private_extern__ void layout_ordered_sections(void) {
...
static void layout_ordered_section(struct merged_section *ms) {
      ...
      for (j = 0; j < cur_obj->nsection_maps; j++) {
        ...
        if (cur_obj->cur_section_map != NULL) {
          error_with_cur_obj("can't use -sectorder or -dead_strip"
                             ...
                             "named (%.16s,%.16s))",
                             cur_obj->cur_section_map - cur_obj->section_maps +
                                 1,
                             j + 1, ms->s.segname, ms->s.sectname);
          return;
        }
        ...
        for (k = 0; k < cur_obj->symtab->nsyms; k++) {
          if (object_symbols[k].n_sect == nsect &&
            ...
            nload_orders++;
            if (object_symbols[k].n_value == cur_obj->section_maps[j].s->addr)
              start_section = TRUE;
          }
        }
         ...
         * map and count.
         */
        load_orders = allocate(sizeof(struct load_order) * nload_orders);
        memset(load_orders, '\0', sizeof(struct load_order) * nload_orders);
        ...
        if (start_section == FALSE) {
          ...
          load_orders[l].value = cur_obj->section_maps[j].s->addr;
          l++;
        }
        for (k = 0; k < cur_obj->symtab->nsyms; k++) {
          if (object_symbols[k].n_sect == nsect &&
            ...
            load_orders[l].value = object_symbols[k].n_value;
...
#ifdef DEBUG
        if (debug & (1 << 14))
          print_load_order(load_orders, nload_orders, ms, cur_obj,
                           "names and values");
#endif /* DEBUG */
         ...
         */
        qsort(load_orders, nload_orders, sizeof(struct load_order),
        ...
        for (l = 0; l < nload_orders - 1; l++) {
          ...
          load_orders[l].input_size =
              load_orders[l + 1].value - load_orders[l].value;
        }
        load_orders[l].input_offset =
            load_orders[l].value - cur_obj->section_maps[j].s->addr;
        load_orders[l].input_size = cur_obj->section_maps[j].s->addr +
         ...
         */
        qsort(load_orders, nload_orders, sizeof(struct load_order),
              (int (*)(const void *, const void *))qsort_load_order_names);
#ifdef DEBUG
        if (debug & (1 << 16))
          print_load_order(load_orders, nload_orders, ms, cur_obj,
                           "sorted by name");
  ...
  for (i = 0; i < ms->order_size;) {
    ...
    parse_order_line(line, &archive_name, &object_name, &symbol_name, ms,
                     line_number);

    load_order = lookup_load_order(archive_name, object_name, symbol_name, ms,
                                   line_number);
    if (load_order != NULL) {
      if (load_order->order != 0) {
        if (archive_name == NULL) {
          if (*object_name == '\0') {
            warning("multiple specification of symbol: %s in "
                    ...
                    "section (%.16s,%.16s)",
                    symbol_name, ms->order_filename, line_number, ms->s.segname,
                    ms->s.sectname);
          } else {
            warning("multiple specification of %s:%s in "
                    "-sectorder file: %s line %lu for "
                    "section (%.16s,%.16s)",
                    object_name, symbol_name, ms->order_filename, line_number,
                    ms->s.segname, ms->s.sectname);
          }
        } else
          warning("multiple specification of %s:%s:%s in "
                  "-sectorder file: %s line %lu for "
                  "section (%.16s,%.16s)",
                  archive_name, object_name, symbol_name, ms->order_filename,
        ...
        output_offset = align_to_input_mod(
            output_offset, load_order->input_offset, ms->s.align);
      ...
      if (strncmp(symbol_name, ".section_offset",
        ...
        if (*endp != '\0')
          ...
          error("bad offset value (0x%x) of .section_offset in "
                ...
                "offset 0x%x)",
                (unsigned int)offset, ms->order_filename, line_number,
                ms->s.segname, ms->s.sectname, (unsigned int)output_offset);
        else
      ...
      if (strncmp(symbol_name, ".section_align",
        ...
        if (*endp != '\0')
          ...
          error("bad align value (%lu) of .section_align in "
                ...
                "than %d)",
                align, ms->order_filename, line_number, ms->s.segname,
                ms->s.sectname, MAXSECTALIGN);
        else
          output_offset = rnd(output_offset, 1 << align);
      } else if (strcmp(symbol_name, ".section_all") == 0) {
        section_map = lookup_section_map(archive_name, object_name);
        if (section_map != NULL) {
          section_map->no_load_order = TRUE;
          section_map->order = order++;
          output_offset = rnd(output_offset, (1 << section_map->s->align));
   ...
   * unnecessarily).
   */
  if ((r = vm_deallocate(mach_task_self(), (vm_address_t)ms->order_addr,
                         ms->order_size)) != KERN_SUCCESS)
    mach_fatal(r,
               "can't vm_deallocate() memory for -sectorder "
               "file: %s for section (%.16s,%.16s)",
               ms->order_filename, ms->s.segname, ms->s.sectname);
  ms->order_addr = NULL;
  ...
  for (q = &objects; *q; q = &(object_list->next)) {
    ...
    for (i = 0; i < object_list->used; i++) {
...
#ifdef DEBUG
      if (debug & (1 << 17))
        print_load_order(cur_obj->cur_section_map->load_orders,
                         cur_obj->cur_section_map->nload_orders, ms, cur_obj,
                         "file orders assigned");
#endif /* DEBUG */
      ...
      qsort(load_order, n, sizeof(struct load_order),
            (int (*)(const void *, const void *))qsort_load_order_input_offset);
      ...
      if (any_order == FALSE &&
         ...
         */
        if (dead_strip == FALSE ||
            (cur_obj->cur_section_map->s->flags & S_ATTR_NO_DEAD_STRIP) ==
          ...
          output_offset =
              rnd(output_offset, (1 << cur_obj->cur_section_map->s->align));
          cur_obj->cur_section_map->offset = output_offset;
          output_offset += cur_obj->cur_section_map->s->size;

          if (sectorder_detail == TRUE && ms->order_filename != NULL) {
            ...
            for (j = 0; j < n; j++) {
              if (cur_obj->ar_hdr == NULL) {
                if (nowarnings == FALSE)
                  print("%s:%s\n", cur_obj->file_name, load_order[j].name);
              } else {
                if (nowarnings == FALSE)
                  print("%s:%.*s:%s\n", cur_obj->file_name,
                        (int)cur_obj->ar_name_size, cur_obj->ar_name,
                        load_order[j].name);
              }
            }
          }
      ...
      for (j = 0; j < n; j++) {
        if (load_order[j].order == 0) {
          ...
          output_offset = align_to_input_mod(
              output_offset, load_order[j].input_offset, ms->s.align);
          ...
          if (cur_obj->cur_section_map->no_load_order == TRUE &&
            ...
            if (cur_obj->ar_hdr == NULL) {
              error("specification for both %s:%s "
                    ...
                    "(not allowed)",
                    cur_obj->file_name, ".section_all", cur_obj->file_name,
                    load_order[j].name, ms->order_filename, ms->s.segname,
                    ms->s.sectname);
            } else {
              error("specification for both "
                    ...
                    cur_obj->ar_name, ".section_all", cur_obj->file_name,
                    (int)cur_obj->ar_name_size, cur_obj->ar_name,
                    load_order[j].name, ms->order_filename, ms->s.segname,
                    ms->s.sectname);
            }
  ...
  for (q = &objects; *q; q = &(object_list->next)) {
    ...
    for (i = 0; i < object_list->used; i++) {
      ...
      if (cur_obj->cur_section_map->no_load_order == TRUE ||
        ...
        fine_relocs[0].output_offset = cur_obj->cur_section_map->offset;
        if (dead_strip == TRUE) {
      ...
      for (j = 0; j < n; j++) {
        fine_relocs[j].input_offset = load_orders[j].input_offset;
        fine_relocs[j].output_offset = load_orders[j].output_offset;
        if (dead_strip == TRUE) {
          if ((start_section == TRUE || j != 0) &&
              object_symbols[load_orders[j].index].n_type & N_EXT) {
            merged_symbol =
                lookup_symbol(object_strings +
                              object_symbols[load_orders[j].index].n_un.n_strx);
       ...
       * use them.
       */
      qsort(fine_relocs, n, sizeof(struct fine_reloc),
            (int (*)(const void *, const void *))qsort_fine_reloc_input_offset);
      ...
      if (dead_strip == TRUE) {
        qsort(
            load_orders, n, sizeof(struct load_order),
            (int (*)(const void *, const void *))qsort_load_order_input_offset);
        for (j = 0; j < n; j++)
 ...
 */
__private_extern__ unsigned long align_to_input_mod(unsigned long output_offset,
...
is_literal_output_offset_live(struct merged_section *ms,
 ...
 * is set to "".
 */
__private_extern__ void parse_order_line(char *line, char **archive_name,
                                         char **object_name, char **symbol_name,
                                         struct merged_section *ms,
...
static void create_name_arrays(void) {
  ...
  for (p = &objects; *p; p = &(object_list->next)) {
    ...
    for (i = 0; i < object_list->used; i++) {
      ...
      if (cur_obj->ar_hdr != NULL) {
        ...
        create_object_name(&(ar->object_names), &(ar->nobject_names), ar_name,
                           strlen(ar_name), cur_obj->file_name);
      } else {
        ...
        create_object_name(&object_names, &nobject_names, cur_obj->file_name, j,
                           NULL);
      }
    }
  }
  ...
  if (narchive_names != 0) {
    archive_names =
        reallocate(archive_names, sizeof(struct archive_name) * narchive_names);
    qsort(archive_names, narchive_names, sizeof(struct archive_name),
    ...
    for (i = 0; i < narchive_names; i++) {
      archive_names[i].object_names = reallocate(
          ...
          sizeof(struct object_name) * archive_names[i].nobject_names);
      qsort(archive_names[i].object_names, archive_names[i].nobject_names,
  ...
  if (nobject_names != nobjects)
    object_names =
        reallocate(object_names, sizeof(struct object_name) * nobject_names);
  qsort(object_names, nobject_names, sizeof(struct object_name),
        (int (*)(const void *, const void *))qsort_object_names);
}
...
#ifdef notdef
        /*
         * Since the 4.4bsd extened format #1 could be used for long member
         * names this warning is now always printed again.
         */
...
static void create_load_symbol_hash_table(unsigned long nsection_symbols,
  ...
  if (load_symbol_hashtable == NULL)
    load_symbol_hashtable =
        allocate(sizeof(struct load_symbol *) * LOAD_SYMBOL_HASHTABLE_SIZE);
  memset(load_symbol_hashtable, '\0',
         sizeof(struct load_symbol *) * LOAD_SYMBOL_HASHTABLE_SIZE);

  /* set up the load_symbols */
  if (nsection_symbols > load_symbols_size) {
    load_symbols_size = nsection_symbols;
    load_symbols = reallocate(load_symbols,
                              sizeof(struct load_symbol) * load_symbols_size);
  }
  memset(load_symbols, '\0', sizeof(struct load_symbol) * load_symbols_size);
  load_symbols_used = 0;

  for (i = 0; i < narchive_names; i++) {
    for (j = 0; j < archive_names[i].nobject_names; j++) {
      if (archive_names[i].object_names[j].object_file->cur_section_map != NULL)
        create_load_symbol_hash_table_for_object(
            ...
            archive_names[i]
                .object_names[j]
                .object_file->cur_section_map->load_orders,
            archive_names[i]
                .object_names[j]
                .object_file->cur_section_map->nload_orders,
            ms);
    }
  }
  ...
  for (j = 0; j < nobject_names; j++) {
    if (object_names[j].object_file->cur_section_map != NULL)
      create_load_symbol_hash_table_for_object(
          NULL, object_names[j].object_name, object_names[j].index_length,
          object_names[j].object_file->cur_section_map->load_orders,
          object_names[j].object_file->cur_section_map->nload_orders, ms);
  }
}
...
static void create_load_symbol_hash_table_for_object(
  ...
  for (i = 0; i < nload_orders; i++) {
    ...
    hash_index =
        hash_string(load_orders[i].name, NULL) % LOAD_SYMBOL_HASHTABLE_SIZE;
    for (hash_load_symbol = load_symbol_hashtable[hash_index];
         hash_load_symbol != NULL; hash_load_symbol = hash_load_symbol->next) {
      if (strcmp(load_orders[i].name, hash_load_symbol->symbol_name) == 0)
        break;
    }
        ...
        if (archive_name != NULL) {
          if (strcmp(other_name->object_name, object_name) == 0 &&
            ...
            warning("symbol appears more than once in the same "
                    ...
                    other_name->archive_name, other_name->object_name,
          ...
          if (strcmp(other_name->object_name, object_name) == 0 &&
            ...
            warning("symbol appears more than once in the same "
                    ...
                    "using a -sectorder option",
                    other_name->object_name, other_name->symbol_name);
            break;
          }
        }
      }
...
lookup_load_order(char *archive_name, char *object_name, char *symbol_name,
  ...
  if (archive_name != NULL) {
    a = bsearch(archive_name, archive_names, narchive_names,
                ...
                (int (*)(const void *, const void *))bsearch_archive_names);
    if (a == NULL)
      goto no_exact_match;
    o = a->object_names;
    n = a->nobject_names;
  ...
  l = bsearch(symbol_name, l, n, sizeof(struct load_order),
              (int (*)(const void *, const void *))bsearch_load_order_names);
  if (l == NULL)
    goto no_exact_match;
  return (l);
...
no_exact_match:
  ...
  /* find this symbol's place in the hash table */
  hash_index = hash_string(symbol_name, NULL) % LOAD_SYMBOL_HASHTABLE_SIZE;
  for (hash_load_symbol = load_symbol_hashtable[hash_index];
       hash_load_symbol != NULL; hash_load_symbol = hash_load_symbol->next) {
  ...
  number_of_matches = 0;
  first_match = NULL;
  for (other_name = hash_load_symbol; other_name != NULL;
       other_name = other_name->other_names) {
  ...
  for (other_name = hash_load_symbol; other_name != NULL;
    ...
    if (other_name->load_order->order == 0) {
      if (other_name->archive_name != NULL) {
        ...
        if (strcmp(base_name,
                   other_name->object_name + other_name->index_length) == 0) {
          if (first_match == NULL)
            first_match = other_name;
          number_of_matches++;
        }
        ...
        if (archive_name != NULL) {
          ...
          if (other_name->archive_name != NULL)
            warning("ambiguous specification of %s:%s in "
                    ...
                    "section (%.16s,%.16s) using %s:%s:%s",
                    object_name, symbol_name, ms->order_filename, line_number,
                    ms->s.segname, ms->s.sectname, other_name->archive_name,
                    other_name->object_name, other_name->symbol_name);
          else
            warning("ambiguous specification of %s:%s in "
                    "-sectorder file: %s line %lu for "
                    "section (%.16s,%.16s) using %s:%s",
                    object_name, symbol_name, ms->order_filename, line_number,
                    ms->s.segname, ms->s.sectname, other_name->object_name,
                    other_name->symbol_name);
        }
      }
      break;
  ...
  if (sectorder_detail == TRUE) {
    for (other_name = hash_load_symbol; other_name != NULL;
         other_name = other_name->other_names) {
      if (other_name->load_order->order == 0 && first_match != other_name) {
        if (archive_name != NULL) {
          if (other_name->archive_name != NULL)
          ...
          if (other_name->archive_name != NULL)
            warning("specification %s:%s ambiguous with "
                    "%s:%s:%s",
                    object_name, symbol_name, other_name->archive_name,
                    other_name->object_name, other_name->symbol_name);
          else
            warning("specification %s:%s ambiguous with "
                    "%s:%s",
                    object_name, symbol_name, other_name->object_name,
                    other_name->symbol_name);
        }
      }
    }
...
qsort_load_order_values(const struct load_order *load_order1,
                        const struct load_order *load_order2) {
  /*
...
static int qsort_archive_names(const struct archive_name *archive_name1,
                               const struct archive_name *archive_name2) {
  return (strcmp(archive_name1->archive_name, archive_name2->archive_name));
}
...
static int qsort_object_names(const struct object_name *object_name1,
                              const struct object_name *object_name2) {
  return (strcmp(object_name1->object_name, object_name2->object_name));
}
...
static void create_order_load_maps(struct merged_section *ms,
  ...
  order_load_maps = allocate(sizeof(struct order_load_map) * norder_load_maps);
    ...
    for (j = 0; j < archive_names[i].nobject_names; j++) {
      ...
      for (m = 0; m < cur_obj->nsection_maps; m++) {
        ...
        */
        ...
        for (k = 0; k < n; k++) {
          order_load_maps[l].archive_name = archive_names[i].archive_name;
  ...
  qsort(order_load_maps, norder_load_maps, sizeof(struct order_load_map),
 ...
 */
__private_extern__ void resize_live_sections(void) {
  struct merged_segment **p, *msg;
  struct merged_section **content, **zerofill, *ms;
...
static void resize_live_section(struct merged_section *ms) {
  ...
  for (i = 0; i < n; i++) {
    if (order_load_maps[i].load_order->fine_reloc->live == FALSE) {
      if (ms->order_filename != NULL && sectorder_detail == TRUE &&
        ...
        warning("specification of symbol: %s in -sectorder file: "
                ...
                "(dead stripped)",
                order_load_maps[i].load_order->name, ms->order_filename,
                order_load_maps[i].load_order->line_number, ms->s.segname,
                ms->s.sectname);
      continue;
    }
    ...
    output_offset = align_to_input_mod(output_offset, load_order->input_offset,
                                       ms->s.align);
    load_order->output_offset = output_offset;
 ...
 */
__private_extern__ void relayout_relocs(void) {
    ...
    for (i = 0; i < object_list->used; i++) {
      ...
      for (j = 0; j < cur_obj->nsection_maps; j++) {
        if (cur_obj->section_maps[j].s->flags & S_ATTR_DEBUG)
          continue;
        section_type = cur_obj->section_maps[j].s->flags & SECTION_TYPE;
        if (section_type == S_REGULAR ||
          ...
          relocs =
              (struct relocation_info *)(cur_obj->obj_addr + map->s->reloff);
  ...
  for (i = 0; i < map->s->nreloc; i++) {
    ...
    if (r_extern) {
      if (r_symbolnum >= cur_obj->symtab->nsyms)
        return;
      undefined_map =
          bsearch(&r_symbolnum, cur_obj->undefined_maps, cur_obj->nundefineds,
      ...
      if (undefined_map != NULL)
        ...
        nlists = (struct nlist *)(cur_obj->obj_addr + cur_obj->symtab->symoff);
        ...
        if ((nlists[r_symbolnum].n_type & N_TYPE) == N_SECT &&
            (cur_obj->section_maps[nlists[r_symbolnum].n_sect - 1].s->flags &
             SECTION_TYPE) == S_COALESCED) {
          merged_symbol =
              lookup_symbol(strings + nlists[r_symbolnum].n_un.n_strx);
          if (merged_symbol->name_len == 0) {
            fatal("internal error, in count_relocs() failed to "
                  "lookup coalesced symbol %s",
      ...
      if ((merged_symbol->nlist.n_type & N_TYPE) == N_INDR &&
        ...
        merged_symbol = (struct merged_symbol *)merged_symbol->nlist.n_value;
    ...
    if (r_extern == 0) {
      ...
      if (r_symbolnum != NO_SECT &&
          ...
          local_map->absolute_indirect_defineds == TRUE && r_pcrel == 1) {
        /*
        ...
        memcpy(fake_contents, cur_obj->obj_addr + map->s->offset + r_address,
               1 << r_length);
        /* fake up relocation entries using just the ones for item*/
        fake_relocs[0] = reloc;
        if ((reloc.r_address & R_SCATTERED) != 0) {
          sreloc = (struct scattered_relocation_info *)(&fake_relocs[0]);
          sreloc->r_address = 0;
        } else
        ...
        if (arch_flag.cputype == CPU_TYPE_MC680x0)
          generic_reloc(fake_contents, fake_relocs, &fake_map, FALSE, NULL, 0);
        else if (arch_flag.cputype == CPU_TYPE_I386)
          generic_reloc(fake_contents, fake_relocs, &fake_map, TRUE, NULL, 0);
        else if (arch_flag.cputype == CPU_TYPE_POWERPC ||
                 arch_flag.cputype == CPU_TYPE_VEO)
          ppc_reloc(fake_contents, fake_relocs, &fake_map, NULL, 0);
      ...
      pic = (enum bool)(reloc_is_sectdiff(arch_flag.cputype, r_type) ||
                        (r_pcrel == 1 && r_symbolnum != NO_SECT));
    } else
      pic = (enum bool)(r_pcrel == 1 &&
                        (merged_symbol->nlist.n_type & N_TYPE) == N_SECT);
    /*
     * For output_for_dyld PPC_RELOC_JBSR and HPPA_RELOC_JBSR's are
     * never put out.
     */
    if ((arch_flag.cputype == CPU_TYPE_POWERPC && r_type == PPC_RELOC_JBSR) ||
        (arch_flag.cputype == CPU_TYPE_HPPA && r_type == HPPA_RELOC_JBSR)) {
      i += pair;
      continue;
    }
 ...
 */
__private_extern__ void output_literal_sections(void) {
  struct merged_segment **p, *msg;
  struct merged_section **content, *ms;
 ...
 */
__private_extern__ void output_sections_from_files(void) {
  struct merged_segment **p, *msg;
  struct merged_section **content, *ms;
#ifdef DEBUG
  kern_return_t r;
  ...
  while (*p) {
    ...
    while (*content) {
      ...
      if (ms->contents_filename != NULL) {
        memcpy(output_addr + ms->s.offset, ms->file_addr, ms->file_size);
        /*
...
#ifdef DEBUG
        if ((r = vm_deallocate(mach_task_self(), (vm_address_t)ms->file_addr,
                               ms->file_size)) != KERN_SUCCESS)
          mach_fatal(r,
                     "can't vm_deallocate() memory for file: "
                     "%s used to create section (%.16s,%.16s)",
                     ms->contents_filename, ms->s.segname, ms->s.sectname);
        ms->file_addr = NULL;
#endif /* DEBUG */
      }
      content = &(ms->next);
 ...
 */
__private_extern__ void output_section(struct section_map *map) {
  char *contents;
  struct relocation_info *relocs;
#ifndef RLD
  ...
  if (map->nfine_relocs != 0)
    ...
    if (arch_flag.cputype == CPU_TYPE_I386 &&
      ...
      contents =
          output_addr + map->output_section->s.offset + map->flush_offset;
  ...
  if (output_for_dyld) {
    ...
    memcpy(relocs, cur_obj->obj_addr + map->s->reloff,
    ...
    relocs = (struct relocation_info *)(cur_obj->obj_addr + map->s->reloff);
  }
  if (cur_obj->swapped && map->input_relocs_already_swapped == FALSE) {
  ...
  if (output_for_dyld) {
    ...
    if ((map->s->flags & SECTION_TYPE) == S_REGULAR ||
      ...
      output_locrel = (struct relocation_info
                           *)(output_addr +
                              output_dysymtab_info.dysymtab_command.locreloff +
                              map->ilocrel * sizeof(struct relocation_info));
      output_extrel = (struct relocation_info
                           *)(output_addr +
                              output_dysymtab_info.dysymtab_command.extreloff +
     ...
     */
    reloc_output_for_dyld(map, relocs, output_locrel, output_extrel, &nlocrel,
                          &nextrel);
    /*
 ...
 */
static enum bool
is_pass2_merged_symbol_coalesced(struct merged_symbol *merged_symbol) {
  unsigned long i;
  ...
  for (i = 0; i < merged_symbol->definition_object->nsection_maps; i++) {
    if (merged_symbol->nlist.n_sect ==
        merged_symbol->definition_object->section_maps[i]
            .output_section->output_sectnum)
      if ((merged_symbol->definition_object->section_maps[i]
               .output_section->s.flags &
           SECTION_TYPE) == S_COALESCED)
        return (TRUE);
  }
  return (FALSE);
...
pass2_nsect_merged_symbol_section_type(struct merged_symbol *merged_symbol) {
  unsigned long i;

  if (merged_symbol == NULL || (merged_symbol->nlist.n_type & N_TYPE) != N_SECT)
...
static void scatter_copy(struct section_map *map, char *contents) {
  ...
  if ((map->s->flags & SECTION_TYPE) == S_NON_LAZY_SYMBOL_POINTERS ||
    ...
    for (i = 0; i < map->nfine_relocs - 1; i++) {
      if (map->fine_relocs[i].use_contents == TRUE &&
        ...
        if (map->fine_relocs[i].indirect_defined == TRUE ||
            ...
            (prebinding == TRUE && (index != INDIRECT_SYMBOL_LOCAL &&
                                    index != INDIRECT_SYMBOL_ABS))) {
          if (is_pass2_merged_symbol_coalesced(
                  map->fine_relocs[i].merged_symbol) == TRUE) {
            value = map->fine_relocs[i].merged_symbol->nlist.n_value;
            if ((map->fine_relocs[i].merged_symbol->nlist.n_desc &
                 N_ARM_THUMB_DEF))
              value |= 1;
          } else if (map->fine_relocs[i].local_symbol == FALSE) {
            undefined_map =
                bsearch(&index, cur_obj->undefined_maps, cur_obj->nundefineds,
                        sizeof(struct undefined_map),
                        (int (*)(const void *, const void *))undef_bsearch);
            if (undefined_map == NULL) {
              merged_symbol =
                  lookup_symbol(strings + nlists[index].n_un.n_strx);
              if (merged_symbol->name_len == 0)
            ...
            if (nlists[index].n_sect == NO_SECT)
              ...
              if (section_map->nfine_relocs == 0)
                value = nlists[index].n_value - section_map->s->addr +
          ...
          if (index == INDIRECT_SYMBOL_LOCAL) {
            memcpy(&value, contents + map->fine_relocs[i].input_offset, 4);
            if (cur_obj->swapped)
              value = SWAP_LONG(value);
            for (j = 0; j < cur_obj->nsection_maps; j++) {
            ...
            if (section_map->nfine_relocs == 0)
              value = value - section_map->s->addr +
    ...
    if (map->fine_relocs[i].use_contents == TRUE &&
      ...
      index = indirect_symtab[map->s->reserved1 +
                              (map->fine_relocs[i].input_offset / 4)];
      if (map->fine_relocs[i].indirect_defined == TRUE ||
          is_pass2_merged_symbol_coalesced(map->fine_relocs[i].merged_symbol) ==
              TRUE ||
          (prebinding == TRUE &&
           (index != INDIRECT_SYMBOL_LOCAL && index != INDIRECT_SYMBOL_ABS))) {
        if (is_pass2_merged_symbol_coalesced(
                map->fine_relocs[i].merged_symbol) == TRUE) {
          value = map->fine_relocs[i].merged_symbol->nlist.n_value;
          if ((map->fine_relocs[i].merged_symbol->nlist.n_desc &
          ...
          undefined_map =
              bsearch(&index, cur_obj->undefined_maps, cur_obj->nundefineds,
                      ...
                      (int (*)(const void *, const void *))undef_bsearch);
          if (undefined_map == NULL) {
            merged_symbol = lookup_symbol(strings + nlists[index].n_un.n_strx);
            if (merged_symbol->name_len == 0)
              fatal("interal error, scatter_copy() failed"
                    " in looking up external symbol");
          } else
   ...
   */
           ...
           prebinding == TRUE && arch_flag.cputype == CPU_TYPE_I386 &&
...
static void reloc_output_for_dyld(struct section_map *map,
  ...
  struct scattered_relocation_info *sreloc;
  unsigned long r_address, r_extern, r_type, r_scattered, r_pcrel, r_symbolnum,
      r_value;
  ...
  *nextrel = 0;
  ...
  if (segs_read_only_addr_specified == TRUE)
    addr_adjust = map->output_section->s.addr - segs_read_write_addr;
  else
    addr_adjust = map->output_section->s.addr - merged_segments->sg.vmaddr;
  /*
 ...
 */
__private_extern__ enum bool
is_merged_symbol_coalesced(struct merged_symbol *merged_symbol) {
  struct merged_segment **p, *msg;
  struct merged_section **q, *ms;
...
scatter_copy_relocs(struct section_map *map, struct relocation_info *relocs,
                    struct relocation_info *output_relocs) {
  ...
  for (i = 0; i < map->s->nreloc; i++) {
    ...
    if (fine_reloc_offset_in_output_for_output_offset(map, r_address)) {
      ...
      memcpy(output_relocs + nreloc, reloc, sizeof(struct relocation_info));
      nreloc++;
      if (reloc_has_pair(arch_flag.cputype, r_type)) {
 ...
 */
__private_extern__ void nop_pure_instruction_scattered_sections(void) {
  struct merged_segment **p, *msg;
  struct merged_section **content, *ms;
  char *contents;
 ...
 */
__private_extern__ void flush_scatter_copied_sections(void) {
  struct merged_segment **p, *msg;
  struct merged_section **content, *ms;
 ...
 */
__private_extern__ void live_marking(void) {
  struct merged_symbol *merged_symbol;
  enum bool found;
   ...
   */
  if (filetype != MH_FVMLIB && filetype != MH_DYLIB && filetype != MH_BUNDLE) {
    if (entry_point_name != NULL) {
       ...
       */
      found = FALSE;
      for (msg = merged_segments; msg != NULL && found == FALSE;
           msg = msg->next) {
        for (ms = msg->content_sections; ms != NULL && found == FALSE;
             ms = ms->next) {
          for (i = 0; i < ms->norder_load_maps && found == FALSE; i++) {
            if (ms->order_load_maps[i].size != 0) {
              ...
              fine_reloc = fine_reloc_for_input_offset(
                  ms->order_load_maps[i].section_map, input_offset);
              fine_reloc->live = TRUE;
              ...
              if (((debug & (1 << 25)) || (debug & (1 << 26)))) {
                ...
                if (ms->order_load_maps[i].archive_name != NULL)
                  print("%s(%s):", ms->order_load_maps[i].archive_name,
                        ms->order_load_maps[i].object_name);
                else
                  print("%s:", ms->order_load_maps[i].object_name);
                print("(%.16s,%.16s):0x%x:", ms->s.segname, ms->s.sectname,
...
static double calculate_time_used(struct timeval *start, struct timeval *end) {
  ...
  if (end->tv_usec >= start->tv_usec)
    ...
    time_used +=
        -1.0 + ((double)(1000000 + end->tv_usec - start->tv_usec) / 1000000.0);
  return (time_used);
}
        ...
        for (k = 0; k < map->nfine_relocs; k++) {
          ...
          if (fine_reloc->refs != NULL) {
            ...
            if (fine_reloc->merged_symbol != NULL)
              ...
              print_symbol_name_from_order_load_maps(
                  map, map->s->addr + fine_reloc->input_offset);
            print("\n");
          }
          for (ref = fine_reloc->refs; ref != NULL; ref = ref->next) {
            if (ref->merged_symbol != NULL)
              ...
              print_symbol_name_from_order_load_maps(
                  ref->map, ref->map->s->addr + ref->fine_reloc->input_offset);
              printf("\n");
            }
          }
        }
...
static void setup_references(struct section_map *map, struct object_file *obj) {
  ...
  for (i = 0; i < map->s->nreloc; i++) {
    ...
    if (arch_flag.cputype == CPU_TYPE_POWERPC ||
      ...
      fatal("-dead_strip not supported with cputype (%d)", arch_flag.cputype);
    else
...
static void setup_reference(struct live_ref *ref, struct object_file *obj,
  ...
  if (ref->ref_type == LIVE_REF_VALUE) {
    ...
    ref_fine_reloc =
        fine_reloc_for_input_offset(local_map, ref->value - local_map->s->addr);
#ifdef DEBUG
    if (debug & (1 << 27)) {
      print("      ref ");
      print("(%.16s,%.16s):0x%x", local_map->s->segname, local_map->s->sectname,
            (unsigned int)(ref_fine_reloc->input_offset));
      print_symbol_name_from_order_load_maps(
          local_map, local_map->s->addr + ref_fine_reloc->input_offset);
      printf("\n");
    }
#endif /* DEBUG */
  ...
  for (refs = self_fine_reloc->refs; refs != NULL; refs = refs->next) {
    if (r.fine_reloc == refs->fine_reloc && r.map == refs->map &&
        r.obj == refs->obj && r.merged_symbol == refs->merged_symbol)
      return;
  }
 ...
 */
__private_extern__ struct fine_reloc *
get_fine_reloc_for_merged_symbol(struct merged_symbol *merged_symbol,
                                 struct section_map **local_map) {
      ...
      for (j = 0; j < map->nfine_relocs; j++) {
        if (operation == MARK_LIVE) {
          if (fine_relocs[j].live == TRUE &&
            ...
            walk_references(MARK_LIVE, fine_relocs + j, map, obj);
          }
          ...
          if (fine_relocs[j].searched_for_live_refs == FALSE ||
              operation == CHECK_FOR_LIVE_TOUCH) {
            found_live = walk_references(operation, fine_relocs + j, map, obj);
            /*
            ...
            if (found_live == TRUE) {
              ...
              if (debug & (1 << 25)) {
                ...
                print("fine_relocs[%lu].input_offset = "
                      ...
                      j, (unsigned int)(fine_relocs[j].input_offset));
              }
#endif /* DEBUG */
              fine_relocs[j].live = TRUE;
          ...
          fatal("internal error: walk_references_in_section() "
                "called with unknown operation (%d)",
                operation);
        }
      }
    }
  ...
  if (ref->merged_symbol == NULL) {
    ...
    if ((((debug & (1 << 25)) || (debug & (1 << 26))) &&
      ...
      print("** In ref_operation(%s) ",
            walk_references_operation_names[operation]);
      print_obj_name(obj);
      print("(%.16s,%.16s):0x%x", ref->map->s->segname, ref->map->s->sectname,
            (unsigned int)(ref->fine_reloc->input_offset));
      if (ref->fine_reloc->merged_symbol != NULL) {
        print(":%s", ref->fine_reloc->merged_symbol->nlist.n_un.n_name);
      } else {
        print_symbol_name_from_order_load_maps(
            ref->map, ref->map->s->addr + ref->fine_reloc->input_offset);
      }
    ...
    if (operation == MARK_LIVE) {
      ...
      if (ref->fine_reloc->refs_marked_live == FALSE) {
        walk_references(MARK_LIVE, ref->fine_reloc, ref->map, ref->obj);
      }
    } else if (operation == SEARCH_FOR_LIVE) {
      if (ref->fine_reloc->live == TRUE)
        ...
        if (ref->fine_reloc->searched_for_live_refs == FALSE) {
          found_live = walk_references(SEARCH_FOR_LIVE, ref->fine_reloc,
                                       ref->map, ref->obj);
          if (found_live == TRUE)
            return (TRUE);
        }
    ...
    if (operation == MARK_LIVE) {
      ...
      if ((ref->merged_symbol->nlist.n_type & N_TYPE) == N_INDR) {
        indr_merged_symbol =
            (struct merged_symbol *)ref->merged_symbol->nlist.n_value;
    ...
    if (ref->merged_symbol->defined_in_dylib == TRUE) {
      if (operation == MARK_LIVE) {
        ...
        /* need to add code to search through a dylib's references for live
         * symbols */
        ;
      }
      ...
      if ((ref->merged_symbol->nlist.n_type & N_TYPE) == N_SECT) {
        ...
        local_map =
            &(ref->merged_symbol->definition_object->section_maps[n_sect - 1]);
      } else
        local_map = NULL;
      if (ref_fine_reloc != NULL) {
        ...
        if (operation == MARK_LIVE) {
          ...
          if (ref_fine_reloc->refs_marked_live == FALSE) {
            walk_references(MARK_LIVE, ref_fine_reloc, local_map,
          ...
          if (ref_fine_reloc->searched_for_live_refs == FALSE) {
            found_live =
                walk_references(SEARCH_FOR_LIVE, ref_fine_reloc, local_map,
                                ref->merged_symbol->definition_object);
            if (found_live == TRUE)
              return (TRUE);
          }
 ...
 */
__private_extern__ unsigned long
r_symbolnum_from_r_value(unsigned long r_value, struct object_file *obj) {
  unsigned i, r_symbolnum;

  r_symbolnum = 0;
  for (i = 0; i < obj->nsection_maps; i++) {
    if (r_value >= obj->section_maps[i].s->addr &&
        r_value < obj->section_maps[i].s->addr + obj->section_maps[i].s->size) {
      r_symbolnum = i + 1;
      break;
    }
  }
  if (r_symbolnum == 0) {
    ...
    for (i = 0; i < obj->nsection_maps; i++) {
      if (r_value ==
          obj->section_maps[i].s->addr + obj->section_maps[i].s->size) {
        r_symbolnum = i + 1;
        break;
      }
    }
 ...
 */
__private_extern__ void reset_merged_sections(void) {
   ...
   */
  for (msg = original_merged_segments; msg != NULL;
  ...
  if (msg != NULL && merged_segments->content_sections != NULL) {
    ...
    while (ms != NULL) {
      if (strncmp(ms->s.segname, msg->sg.segname, sizeof(msg->sg.segname)) ==
          0) {
 ...
 * is reset to zero.
 */
__private_extern__ void zero_merged_sections_sizes(void) {
  struct merged_segment **p, *msg;
  struct merged_section **q, *ms;
 ...
 */
__private_extern__ void remove_merged_sections(void) {
 ...
 */
__private_extern__ void print_merged_sections(char *string) {
  struct merged_segment *msg;
  struct merged_section *ms;
 ...
 * For tuning..
 */
__private_extern__ void print_merged_section_stats(void) {
  struct merged_segment *msg;
  struct merged_section *ms;
 ...
 * For debugging.
 */
__private_extern__ void print_load_order(struct load_order *load_order,
  ...
  print("Load order 0x%x %lu entries for (%.16s,%.16s) of ",
        (unsigned int)load_order, nload_order, ms->s.segname, ms->s.sectname);
  print_obj_name(object_file);
  print("(%s)\n", string);
  for (i = 0; i < nload_order; i++) {
    print("entry[%lu]\n", i);
    print("           name %s\n",
          load_order[i].name == NULL ? "null" : load_order[i].name);
 ...
 * For debugging.
 */
__private_extern__ void print_name_arrays(void) {
  unsigned long i, j;
  ...
  for (i = 0; i < narchive_names; i++) {
    ...
    for (j = 0; j < archive_names[i].nobject_names; j++) {
      ...
      print("\tobject file 0x%x ",
            (unsigned int)(archive_names[i].object_names[j].object_file));
  ...
  for (j = 0; j < nobject_names; j++) {
    ...
    print("\tobject file 0x%x ", (unsigned int)(object_names[j].object_file));
...
static void print_load_symbol_hash_table(void) {
  ...
  for (i = 0; i < LOAD_SYMBOL_HASHTABLE_SIZE; i++) {
    if (load_symbol_hashtable[i] != NULL)
      print("[%lu]\n", i);
    for (load_symbol = load_symbol_hashtable[i]; load_symbol != NULL;
      ...
      if (load_symbol->archive_name != NULL) {
        print("    (%s:%s:%s) length %lu\n", load_symbol->archive_name,
              load_symbol->object_name, load_symbol->symbol_name,
              load_symbol->index_length);
      } else {
        print("    (%s:%s) index %lu\n", load_symbol->object_name,
              load_symbol->symbol_name, load_symbol->index_length);
      }
      ...
      for (other_name = load_symbol->other_names; other_name != NULL;
           other_name = other_name->other_names) {
        print("other name\n");
        if (other_name->archive_name != NULL) {
          ...
          print("    (%s:%s) index %lu\n", other_name->object_name,
                other_name->symbol_name, other_name->index_length);
        }
