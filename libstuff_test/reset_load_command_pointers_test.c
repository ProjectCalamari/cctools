...
static void mh_add_buffer(struct mach_header *mh, struct load_command **lcmds,
                          void *q, uint32_t size) {
...
static void mh_add_load_command(struct mach_header *mh,
                                struct load_command **lcmds, void *lc,
                                uint32_t size) {
...
static uint32_t random32(void) { return arc4random(); }
...
static struct linkedit_data_command random_ledata_cmd(uint32_t cmd) {
...
static void test_reset_load_command_pointers(int is64, enum index_type except) {
  ...
  if (is64) {
    if (except != index_type_seg_linkedit) {
      ...
      seg_linkedit64.cmdsize =
          (sizeof(struct segment_command_64) +
           sizeof(struct section_64) * seg_linkedit64.nsects);
    ...
    if (except != index_type_seg_bitcode) {
      ...
      seg_bitcode64.cmdsize =
          (sizeof(struct segment_command_64) +
           sizeof(struct section_64) * seg_bitcode64.nsects);
    ...
    if (except != index_type_seg_linkedit) {
      ...
      seg_linkedit32.cmdsize = (sizeof(struct segment_command) +
                                sizeof(struct section) * seg_linkedit32.nsects);
    ...
    if (except != index_type_seg_bitcode) {
      ...
      seg_bitcode32.cmdsize = (sizeof(struct segment_command) +
                               sizeof(struct section) * seg_bitcode32.nsects);
  ...
  if (except != index_type_tlhint) {
    ...
    mh_add_load_command(mh_void, &obj->load_commands, &tlhint, sizeof(tlhint));
  }
  ...
  struct linkedit_data_command csdr = random_ledata_cmd(LC_DYLIB_CODE_SIGN_DRS);
  ...
  if (except != index_type_ldhint)
    mh_add_load_command(mh_void, &obj->load_commands, &ldhint, sizeof(ldhint));
  ...
  if (except != index_type_encinfo) {
    if (is64) {
      ...
      enc64.pad = random32();
      mh_add_load_command(mh_void, &obj->load_commands, &enc64, sizeof(enc64));
    } else {
      ...
      mh_add_load_command(mh_void, &obj->load_commands, &enc32, sizeof(enc32));
    }
  }
  ...
  check_loadcmd(index_type_symtab, "symtab", &st, obj->st, sizeof(st), except);
  check_loadcmd(index_type_dysymtab, "dysymtab", &dyst, obj->dyst, sizeof(dyst),
                except);
  ...
  check_loadcmd(index_type_dice, "data in code", &dice, obj->data_in_code_cmd,
                sizeof(dice), except);
  check_loadcmd(index_type_dice, "atom info", &rm, obj->atom_info_cmd,
                sizeof(rm), except);
  ...
  if (is64) {
    check_null("obj->encryption_info_command", obj->encryption_info_command);
...
static void test_reset_all(uint32_t is64) {
  enum index_type exceptions[] = {
      index_type_none,     index_type_seg_linkedit, index_type_seg_bitcode,
      index_type_symtab,   index_type_dysymtab,     index_type_dyldinfo,
      index_type_tlhint,   index_type_prebind,      index_type_codesig,
      index_type_splitseg, index_type_funcstarts,   index_type_dice,
      index_type_atominfo, index_type_csdir,        index_type_ldhint,
      index_type_trie,     index_type_chains,       index_type_encinfo,
  };
...
static void test_reset_32(void) { test_reset_all(0); }

static void test_reset_64(void) { test_reset_all(1); }

static int test_main(void) {
  int err = 0;

  if (!err)
    err = test_add("test reset_load_command_pointers 32-bit", test_reset_32);
  if (!err)
    err = test_add("test reset_load_command_pointers 64-bit", test_reset_64);
  if (!err)
    err = test_add("test removing code signature load command", test_reset_64);
