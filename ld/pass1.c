...
__private_extern__ char *standard_dirs[] = {"/lib/", "/usr/lib/",
                                            "/usr/local/lib/", NULL};
...
#ifdef __OPENSTEP__
    "/LocalLibrary/Frameworks/", "/NextLibrary/Frameworks/",
...
static void search_for_file(char *base_name, char **file_name, int *fd);
static void search_for_framework(char *name, char **file_name, int *fd);
static void search_paths_for_lname(const char *lname_argument, char **file_name,
                                   int *fd);
static void search_path_for_lname(const char *dir, const char *lname_argument,
                                  char **file_name, int *fd);
#endif /* !defined(RLD) */

static void pass1_fat(char *file_name, char *file_addr, unsigned long file_size,
                      enum bool base_name, enum bool dylib_only,
                      enum bool bundle_loader, enum bool force_weak);

static void pass1_archive(char *file_name, char *file_addr,
                          unsigned long file_size, enum bool base_name,
                          enum bool from_fat_file, enum bool bundle_loader,
                          enum bool force_weak);

static enum bool check_archive_arch(char *file_name, char *file_addr,
                                    unsigned long file_size);

static enum bool check_extend_format_1(char *name, struct ar_hdr *ar_hdr,
                                       unsigned long size_left,
                                       unsigned long *member_name_size);

static void pass1_object(char *file_name, char *file_addr,
                         unsigned long file_size, enum bool base_name,
                         enum bool from_fat_file, enum bool dylib_only,
                         enum bool bundle_loader, enum bool force_weak);

#ifndef RLD
static void load_init_dylib_module(struct dynamic_library *q);
static enum bool setup_sub_images(struct dynamic_library *p);
static void check_dylibs_for_definition(struct merged_symbol *merged_symbol,
                                        enum bool prebind_check,
                                        enum bool twolevel_namespace_check);
static enum bool
check_dylibs_for_reference(struct merged_symbol *merged_symbol);

static enum bool open_dylib(struct dynamic_library *p);

static enum bool set_sub_frameworks_ordinals(struct dynamic_library *umbrella);

static enum bool
set_sub_umbrella_sub_library_ordinal(struct dynamic_library *sub);

static void set_isub_image(struct dynamic_library *p,
                           struct dynamic_library *sub);

static int nlist_bsearch(const char *symbol_name, const struct nlist *symbol);
#endif /* !defined(RLD) */
...
static void check_cur_obj(enum bool dylib_only, enum bool bundle_loader);
 ...
 * prefix defined.
 */
__private_extern__ void pass1(char *name, enum bool lname, enum bool base_name,
                              enum bool framework_name, enum bool bundle_loader,
                              enum bool force_weak) {
  int fd;
  char *file_name;
#ifndef RLD
  ...
  if (lname) {
    ...
    if (fd == -1) {
      if (type != NULL)
        fatal("can't locate framework for: -framework %s,%s", name, type);
      else
  ...
  if (fat_header->magic == FAT_MAGIC)
...
#endif /* __LITTLE_ENDIAN__ */
    {
      ...
      pass1_object(file_name, file_addr, file_size, base_name, FALSE, FALSE,
                   bundle_loader, force_weak);
    }
#ifdef VM_SYNC_DEACTIVATE
  vm_msync(mach_task_self(), (vm_address_t)file_addr, (vm_size_t)file_size,
           VM_SYNC_DEACTIVATE);
...
static void search_paths_for_lname(const char *lname_argument, char **file_name,
  ...
  *fd = -1;
  ...
  for (i = 0; i < nld_library_paths; i++) {
    search_path_for_lname(ld_library_paths[i], lname_argument, file_name, fd);
    if (*fd != -1)
      return;
  }
  for (i = 0; standard_dirs[i] != NULL; i++) {
...
static void pass1_fat(char *file_name, char *file_addr, unsigned long file_size,
  ...
  if (arch_flag.cputype != 0) {
    best_fat_arch =
        cpusubtype_findbestarch(arch_flag.cputype, arch_flag.cpusubtype,
    ...
    if (best_fat_arch == NULL) {
      ...
      if (arch_flag.name != NULL) {
        ...
        prev_arch =
            get_arch_name_from_types(arch_flag.cputype, arch_flag.cpusubtype);
        if (arch_errors_fatal == TRUE) {
          error("fat file: %s does not contain an architecture "
    ...
    if (arch_size >= SARMAG && strncmp(arch_addr, ARMAG, SARMAG) == 0) {
      if (dylib_only == TRUE) {
        if (arch_flag.name != NULL) {
      ...
      pass1_object(file_name, arch_addr, arch_size, base_name, TRUE, dylib_only,
                   bundle_loader, force_weak);
    }
    goto pass1_fat_return;
  }
    ...
    if (arch_size >= SARMAG && strncmp(arch_addr, ARMAG, SARMAG) == 0) {
      if (dylib_only == TRUE) {
        error("fat file: %s (for architecture %s) is not a dynamic "
              "shared library",
              file_name,
              get_arch_name_from_types(fat_archs[0].cputype,
                                       fat_archs[0].cpusubtype));
        goto pass1_fat_return;
      }
      pass1_archive(file_name, arch_addr, arch_size, base_name, TRUE,
                    bundle_loader, force_weak);
  ...
  best_fat_arch =
      cpusubtype_findbestarch(host_arch_flag.cputype, host_arch_flag.cpusubtype,
                              fat_archs, fat_header->nfat_arch);
  if (best_fat_arch != NULL) {
    arch_addr = file_addr + best_fat_arch->offset;
    arch_size = best_fat_arch->size;
    if (arch_size >= SARMAG && strncmp(arch_addr, ARMAG, SARMAG) == 0) {
      if (dylib_only == TRUE) {
        error("fat file: %s (for architecture %s) is not a dynamic "
              "shared library",
              file_name,
              get_arch_name_from_types(best_fat_arch->cputype,
                                       best_fat_arch->cpusubtype));
        goto pass1_fat_return;
      }
      pass1_archive(file_name, arch_addr, arch_size, base_name, TRUE,
                    bundle_loader, force_weak);
  ...
  fatal("-arch flag must be specified (fat file: %s does not contain the "
        "host architecture or just one architecture)",
        file_name);
#endif /* !(defined(KLD) && defined(__STATIC__)) */
...
__private_extern__ void check_fat(char *file_name, unsigned long file_size,
                                  struct fat_header *fat_header,
                                  struct fat_arch *fat_archs, char *ar_name,
    ...
    if (fat_archs[i].offset + fat_archs[i].size > file_size) {
      if (ar_name != NULL)
        error("fat file: %s(%.*s) truncated or malformed (offset "
              ...
              file_name, (int)ar_name_size, ar_name, fat_archs[i].cputype,
              fat_archs[i].cpusubtype);
      else
        error("fat file: %s truncated or malformed (offset plus "
              "size of cputype (%d) cpusubtype (%d) extends past "
              "the end of the file)",
              file_name, fat_archs[i].cputype, fat_archs[i].cpusubtype);
      return;
    }
    if (fat_archs[i].cputype == 0) {
      if (ar_name != NULL)
        error("fat file: %s(%.*s) fat_archs %lu cputype is zero (a "
              "reserved value)",
              file_name, (int)ar_name_size, ar_name, i);
      else
    ...
    if (fat_archs[i].align > MAXSECTALIGN) {
      if (ar_name != NULL)
        error("fat file: %s(%.*s) align (2^%u) too large for "
              ...
              file_name, (int)ar_name_size, ar_name, fat_archs[i].align,
              fat_archs[i].cputype, fat_archs[i].cpusubtype, MAXSECTALIGN);
      else
        error("fat file: %s align (2^%u) too large for cputype "
              "(%d) cpusubtype (%d) (maximum 2^%d)",
              file_name, fat_archs[i].align, fat_archs[i].cputype,
    ...
    if (fat_archs[i].offset % (1 << fat_archs[i].align) != 0) {
      if (ar_name != NULL)
        error("fat file: %s(%.*s) offset: %u for cputype (%d) "
              ...
              "(2^%u)",
              file_name, (int)ar_name_size, ar_name, fat_archs[i].offset,
        ...
        error("fat file: %s offset: %u for cputype (%d) "
              "cpusubtype (%d)) not aligned on it's alignment "
              "(2^%u)",
              file_name, fat_archs[i].offset, fat_archs[i].cputype,
...
static void pass1_archive(char *file_name, char *file_addr,
  ...
  if (base_name)
    fatal("base file of incremental link (argument of -A): %s should't "
          ...
          file_name);
  if (bundle_loader)
    fatal("-bundle_loader argument: %s should't be an archive", file_name);
    ...
    while (offset < file_size) {
      if (offset + sizeof(struct ar_hdr) > file_size) {
        error("truncated or malformed archive: %s at offset %lu "
              ...
              "of the file, can't load from it)",
              file_name, offset);
        return;
      }
      ...
      if (ar_size >= sizeof(struct fat_header) &&
...
#endif
		{
                     ...
                     memcpy(fat_archs,
                            file_addr + offset + sizeof(struct fat_header),
                            fat_header->nfat_arch * sizeof(struct fat_arch));
                     ...
                     if (arch_flag.cputype != 0) {
                       best_fat_arch = cpusubtype_findbestarch(
                           arch_flag.cputype, arch_flag.cpusubtype, fat_archs,
                         ...
                         if (arch_flag.name != NULL) {
                           if (arch_errors_fatal == TRUE) {
                             error("fat file: %s(%.*s) does not contain "
                                   ...
                                   "specified -arch flag: %s",
                                   file_name, (int)i, ar_name, arch_flag.name);
                           } else
  ...
  if (strncmp(symdef_ar_hdr->ar_name, AR_EFMT1, sizeof(AR_EFMT1) - 1) == 0) {
    ...
    ar_name_size =
        strtoul(symdef_ar_hdr->ar_name + sizeof(AR_EFMT1) - 1, NULL, 10);
  } else {
  ...
  if (symdef_length < 2 * sizeof(long)) {
    error("size of table of contents for archive: %s too small to be "
          "a valid table of contents (can't load from it)",
          file_name);
    return;
  }
  if (offset + symdef_length > file_size) {
  ...
  if (nranlibs == 0) {
    warning("empty table of contents: %s (can't load from it)", file_name);
    return;
  }
  if (offset -
          (2 * sizeof(long) + ar_name_size + sizeof(struct ar_hdr) + SARMAG) >
      symdef_length) {
  ...
  for (i = 0; i < nranlibs; i++) {
    if (ranlibs[i].ran_un.ran_strx >= string_size) {
      error("malformed table of contents in: %s (ranlib struct %lu "
            "has bad string index, can't load from it)",
            file_name, i);
      return;
    }
  ...
  if (archive_ObjC == TRUE) {
    ...
    for (i = 0; i < nranlibs; i++) {
      ...
      if (strncmp(bsearch_strings + ranlibs[i].ran_un.ran_strx,
                  ".objc_class_name", sizeof(".objc_class_name") - 1) != 0 &&
      ...
      cur_obj = new_object_file();
      cur_obj->file_name = file_name;
      cur_obj->ar_hdr = (struct ar_hdr *)(file_addr + ranlibs[i].ran_off);
      if (strncmp(cur_obj->ar_hdr->ar_name, AR_EFMT1, sizeof(AR_EFMT1) - 1) ==
          0) {
        ar_name = file_addr + ranlibs[i].ran_off + sizeof(struct ar_hdr);
        ar_name_size =
            strtoul(cur_obj->ar_hdr->ar_name + sizeof(AR_EFMT1) - 1, NULL, 10);
        j = ar_name_size;
      } else {
      ...
      cur_obj->obj_addr =
          file_addr + ranlibs[i].ran_off + sizeof(struct ar_hdr) + ar_name_size;
      cur_obj->obj_size =
          strtol(cur_obj->ar_hdr->ar_size, NULL, 10) - ar_name_size;
  ...
  if (dynamic_libs != NULL) {
    ...
    if (strncmp(symdef_ar_name, SYMDEF_SORTED, sizeof(SYMDEF_SORTED) - 1) ==
        0) {
 ...
 */
      ...
      warning("archive library: %s appears after reference to "
              ...
              "that defines a symbol will ever be loaded",
              file_name);
#endif /* notdef */
    } else {
   ...
   */
  if (strncmp(symdef_ar_name, SYMDEF_SORTED, sizeof(SYMDEF_SORTED) - 1) == 0) {
    /*
    ...
    for (undefined = undefined_list.next; undefined != &undefined_list;
      ...
      ranlib = bsearch(undefined->merged_symbol->nlist.n_un.n_name, ranlibs,
      ...
      if (ranlib != NULL) {
        ...
        cur_obj->file_name = file_name;
#endif /* RLD */
        cur_obj->ar_hdr = (struct ar_hdr *)(file_addr + ranlib->ran_off);
        if (strncmp(cur_obj->ar_hdr->ar_name, AR_EFMT1, sizeof(AR_EFMT1) - 1) ==
            0) {
          ar_name = file_addr + ranlib->ran_off + sizeof(struct ar_hdr);
          ar_name_size = strtoul(
              cur_obj->ar_hdr->ar_name + sizeof(AR_EFMT1) - 1, NULL, 10);
          j = ar_name_size;
        } else {
          ar_name = cur_obj->ar_hdr->ar_name;
          ar_name_size = 0;
        ...
        cur_obj->ar_name_size = j;
        cur_obj->obj_addr =
            file_addr + ranlib->ran_off + sizeof(struct ar_hdr) + ar_name_size;
        cur_obj->obj_size =
            strtol(cur_obj->ar_hdr->ar_size, NULL, 10) - ar_name_size;
    ...
    while (member_loaded == TRUE && errors == 0) {
      ...
      for (i = 0; i < nranlibs; i++) {
        merged_symbol =
            lookup_symbol(bsearch_strings + ranlibs[i].ran_un.ran_strx);
...
__private_extern__ enum byte_sex get_toc_byte_sex(char *addr, uint32_t size) {
  ...
  while (p + sizeof(struct ar_hdr) + sizeof(uint32_t) < addr + size) {
    ...
    if (strncmp(ar_hdr->ar_name, AR_EFMT1, sizeof(AR_EFMT1) - 1) == 0)
      ar_name_size = strtoul(ar_hdr->ar_name + sizeof(AR_EFMT1) - 1, NULL, 10);
    else
      ar_name_size = 0;
    ...
    else if (magic == SWAP_INT(MH_MAGIC) || magic == SWAP_INT(MH_MAGIC_64))
  ...
  if (arch_flag.cputype != 0 && mixed_types == FALSE &&
    ...
    if (arch_flag.name != NULL)
      warning("%s archive's cputype (%d, architecture %s) does "
              ...
              "%s (can't load from it)",
              file_name, cputype, new_arch, arch_flag.cputype, arch_flag.name);
    else
      warning("%s archive's cputype (%d, architecture %s) does "
              "not match cputype (%d architecture %s) of objects "
              "files previously loaded (can't load from it)",
              file_name, cputype, new_arch, arch_flag.cputype, prev_arch);
    return (FALSE);
  }
  return (TRUE);
}
  ...
  *member_name_size = 0;
  ...
  if (isdigit(*p) == 0) {
    error("truncated or malformed archive: %s (ar_name: %.*s for "
          "archive extend format #1 starts with non-digit)",
 ...
 * when encountered.
 */
__private_extern__ void search_dynamic_libs(void) {
  ...
  for (p = dynamic_libs; p != NULL; p = p->next) {
    if (p->type == DYLIB && p->dl->cmd == LC_ID_DYLIB) {
      ...
      library_ordinal++;
      p->dependent_images = allocate(p->definition_obj->nload_dylibs *
  ...
  for (p = dynamic_libs; p != NULL; p = p->next) {
     ...
     */
    if (p->type == DYLIB &&
        (p->dl->cmd == LC_LOAD_DYLIB || p->dl->cmd == LC_LOAD_WEAK_DYLIB ||
         p->dl->cmd == LC_REEXPORT_DYLIB)) {
      if (open_dylib(p) == FALSE) {
        if (prebinding == TRUE) {
          warning("prebinding disabled because dependent "
                  "library: %s can't be searched",
                  p->dylib_file != NULL ? p->file_name : p->dylib_name);
     ...
     */
    if (removed == FALSE && p->type == DYLIB && p->dl->cmd == LC_ID_DYLIB) {
      mh = (struct mach_header *)p->definition_obj->obj_addr;
      if (prebinding == TRUE && (mh->flags & MH_PREBOUND) != MH_PREBOUND) {
      ...
      lc = (struct load_command *)((char *)p->definition_obj->obj_addr +
      ...
      if (p->type == DYLIB) {
        if (p->dylib_file != NULL)
          printf("  %s ordinal %lu (using file %s)\n", p->dylib_name,
                 p->definition_obj->library_ordinal, p->file_name);
        else {
          short_name =
              guess_short_name(p->dylib_name, &is_framework, &has_suffix);
  ...
  for (p = dynamic_libs; p != NULL; p = p->next) {
    ...
    if (p->umbrella_name == NULL) {
      umbrella_install_name = (char *)p->dl + p->dl->dylib.name.offset;
      short_name =
          guess_short_name(umbrella_install_name, &is_framework, &has_suffix);
  ...
  do {
    ...
    for (p = dynamic_libs; p != NULL; p = p->next) {
      if (p->type != DYLIB)
        continue;
      if (p->definition_obj->library_ordinal != 0 && p->umbrella_name != NULL)
    ...
    if (non_set_ordinals == TRUE) {
      for (p = dynamic_libs; p != NULL; p = p->next) {
        ...
        if (p->definition_obj->library_ordinal == 0 &&
            (p->umbrella_name != NULL || p->library_name != NULL)) {
          set_some_ordinals |= set_sub_umbrella_sub_library_ordinal(p);
        }
      }
    }
    ...
    for (p = dynamic_libs; p != NULL; p = p->next) {
      if (p->type == DYLIB) {
        ...
        for (i = 0; i < p->ndependent_images; i++) {
          ...
          short_name =
              guess_short_name(dep->dylib_name, &is_framework, &has_suffix);
          printf("      [%lu] %s\n", i,
                 short_name != NULL ? short_name : dep->dylib_name);
        }
   ...
   */
  if ((twolevel_namespace == TRUE) && (force_flat_namespace == FALSE) &&
      (filetype != MH_OBJECT)) {
  ...
  for (p = dynamic_libs; p != NULL; p = p->next) {
    if (p->type == DYLIB) {
      if (p->dl->cmd == LC_ID_DYLIB) {
        p->tocs =
            (struct dylib_table_of_contents *)(p->definition_obj->obj_addr +
                                               p->definition_obj->dysymtab
                                                   ->tocoff);
        p->strings =
            p->definition_obj->obj_addr + p->definition_obj->symtab->stroff;
        p->symbols = (struct nlist *)(p->definition_obj->obj_addr +
                                      p->definition_obj->symtab->symoff);
        if (p->definition_obj->swapped)
          swap_nlist(p->symbols, p->definition_obj->symtab->nsyms,
                     host_byte_sex);
        p->mods =
            (struct dylib_module *)(p->definition_obj->obj_addr +
        ...
        if (prebinding == TRUE && filetype == MH_EXECUTE) {
          ...
          p->pbdylib->name.offset = sizeof(struct prebound_dylib_command);
          strcpy(((char *)p->pbdylib) + sizeof(struct prebound_dylib_command),
                 p->dylib_name);
          p->pbdylib->nmodules = p->definition_obj->dysymtab->nmodtab;
    ...
    if (p->type == BUNDLE_LOADER) {
      ...
      p->symbols = (struct nlist *)(p->definition_obj->obj_addr +
                                    p->definition_obj->symtab->symoff);
      if (p->definition_obj->swapped)
        swap_nlist(p->symbols, p->definition_obj->symtab->nsyms, host_byte_sex);
    }
  }
  ...
  if (ld_trace_dylibs == TRUE) {
    for (p = dynamic_libs; p != NULL; p = p->next) {
      if (p->type == DYLIB) {
        char resolvedname[MAXPATHLEN];
        if (realpath(p->definition_obj->file_name, resolvedname) != NULL)
  ...
  for (undefined = undefined_list.next; undefined != &undefined_list;
    ...
    if (undefined->merged_symbol->twolevel_reference == TRUE) {
       ...
       */
      if (p->dl->cmd == LC_LOAD_DYLIB || p->dl->cmd == LC_LOAD_WEAK_DYLIB ||
          p->dl->cmd == LC_REEXPORT_DYLIB)
        goto undefined_twolevel_reference;
      bsearch_strings = q->strings;
      bsearch_symbols = q->symbols;
      toc = bsearch(undefined->merged_symbol->nlist.n_un.n_name, q->tocs,
      ...
      if (toc == NULL) {
        for (i = 0; toc == NULL && i < p->nsub_images; i++) {
          q = p->sub_images[i];
          if (q->dl->cmd == LC_LOAD_DYLIB || q->dl->cmd == LC_LOAD_WEAK_DYLIB ||
      ...
      if (toc != NULL) {
         ...
         */
        if (is_dylib_module_loaded(q->mods + toc->module_index) == FALSE) {
          cur_obj = new_object_file();
    ...
    for (p = dynamic_libs; p != NULL && found == FALSE; p = p->next) {
      ...
      case DYLIB:
        if (twolevel_namespace == TRUE && p->twolevel_searched == TRUE)
          break;
        /*
          ...
          *cur_obj = *(q->definition_obj);
          ...
          if (errors == 0 &&
            ...
            error("malformed table of contents in library: "
                  ...
                  cur_obj->file_name,
                  bsearch_strings + cur_obj->dylib_module->module_name,
                  undefined->merged_symbol->nlist.n_un.n_name);
          }
          if (twolevel_namespace == TRUE)
      ...
      case SORTED_ARCHIVE:
        ...
        ranlib = bsearch(undefined->merged_symbol->nlist.n_un.n_name,
                         p->ranlibs, p->nranlibs, sizeof(struct ranlib),
                         (int (*)(const void *, const void *))ranlib_bsearch);
        if (ranlib != NULL) {
          if (ld_trace_archives == TRUE &&
          ...
          cur_obj->ar_hdr = (struct ar_hdr *)(p->file_addr + ranlib->ran_off);
          if (strncmp(cur_obj->ar_hdr->ar_name, AR_EFMT1,
                      sizeof(AR_EFMT1) - 1) == 0) {
            ar_name = p->file_addr + ranlib->ran_off + sizeof(struct ar_hdr);
            ar_name_size = strtoul(
                cur_obj->ar_hdr->ar_name + sizeof(AR_EFMT1) - 1, NULL, 10);
            j = ar_name_size;
          } else {
          ...
          cur_obj->ar_name = ar_name;
          cur_obj->ar_name_size = j;
          cur_obj->obj_addr = p->file_addr + ranlib->ran_off +
                              sizeof(struct ar_hdr) + ar_name_size;
          cur_obj->obj_size =
              strtol(cur_obj->ar_hdr->ar_size, NULL, 10) - ar_name_size;
          if (ranlib->ran_off + sizeof(struct ar_hdr) + ar_name_size +
      ...
      case UNSORTED_ARCHIVE:
        for (i = 0; found == FALSE && i < p->nranlibs; i++) {
          if (strcmp(undefined->merged_symbol->nlist.n_un.n_name,
                     p->ranlib_strings + p->ranlibs[i].ran_un.ran_strx) == 0) {
            if (ld_trace_archives == TRUE &&
                p->ld_trace_archive_printed == FALSE) {
              char resolvedname[MAXPATHLEN];
              if (realpath(p->file_name, resolvedname) != NULL)
            ...
            cur_obj->file_name = p->file_name;
            cur_obj->ar_hdr =
                (struct ar_hdr *)(p->file_addr + p->ranlibs[i].ran_off);
            if (strncmp(cur_obj->ar_hdr->ar_name, AR_EFMT1,
                        sizeof(AR_EFMT1) - 1) == 0) {
              ar_name =
                  p->file_addr + p->ranlibs[i].ran_off + sizeof(struct ar_hdr);
              ar_name_size = strtoul(
                  cur_obj->ar_hdr->ar_name + sizeof(AR_EFMT1) - 1, NULL, 10);
              j = ar_name_size;
            } else {
            ...
            cur_obj->ar_name = ar_name;
            cur_obj->ar_name_size = j;
            cur_obj->obj_addr = p->file_addr + p->ranlibs[i].ran_off +
                                sizeof(struct ar_hdr) + ar_name_size;
            cur_obj->obj_size =
                strtol(cur_obj->ar_hdr->ar_size, NULL, 10) - ar_name_size;
            if (p->ranlibs[i].ran_off + sizeof(struct ar_hdr) + ar_name_size +
                    cur_obj->obj_size >
                p->file_size) {
              error("malformed library: %s (member %.*s "
                    "extends past the end of the file, can't "
      ...
      case BUNDLE_LOADER:
        bsearch_strings =
            p->definition_obj->obj_addr + p->definition_obj->symtab->stroff;
        extdef_symbols = (struct nlist *)(p->definition_obj->obj_addr +
                                          p->definition_obj->symtab->symoff) +
                         p->definition_obj->dysymtab->iextdefsym;
        extdef = bsearch(
            undefined->merged_symbol->nlist.n_un.n_name, extdef_symbols,
            p->definition_obj->dysymtab->nextdefsym, sizeof(struct nlist),
      ...
      for (i = 0; i < merged_symbol_list->used; i++) {
        ...
        for (p = dynamic_libs; p != NULL; p = p->next) {
          if (p->type == DYLIB) {
            ...
            bsearch_strings = q->strings;
            bsearch_symbols = q->symbols;
            toc = bsearch(merged_symbol->nlist.n_un.n_name, q->tocs,
                          q->definition_obj->dysymtab->ntoc,
                          sizeof(struct dylib_table_of_contents),
                          (int (*)(const void *, const void *))dylib_bsearch);
            if (toc == NULL && twolevel_namespace == TRUE) {
              q->twolevel_searched = TRUE;
              for (j = 0; toc == NULL && j < p->nsub_images; j++) {
                q = p->sub_images[j];
                q->twolevel_searched = TRUE;
                if (q->dl->cmd == LC_LOAD_DYLIB ||
                    q->dl->cmd == LC_LOAD_WEAK_DYLIB ||
                ...
                bsearch_symbols = q->symbols;
                toc =
                    bsearch(merged_symbol->nlist.n_un.n_name, q->tocs,
                            q->definition_obj->dysymtab->ntoc,
                            sizeof(struct dylib_table_of_contents),
                            (int (*)(const void *, const void *))dylib_bsearch);
              }
              ...
              if (merged_symbol->definition_object->obj_addr !=
                ...
                printf("symbol %s used from dynamic "
                       ...
                       (merged_symbol->definition_object->obj_addr +
                        merged_symbol->definition_object->symtab->stroff) +
                           merged_symbol->definition_object->dylib_module
                               ->module_name,
                       q->dylib_name,
                       bsearch_strings +
                           (q->mods + toc->module_index)->module_name);
              }
            }
          }
        }
...
static void load_init_dylib_module(struct dynamic_library *q) {
  if (q->definition_obj->rc != NULL &&
    ...
    if (is_dylib_module_loaded(q->mods + q->definition_obj->rc->init_module) ==
        FALSE) {
      cur_obj = new_object_file();
      *cur_obj = *(q->definition_obj);
      cur_obj->dylib_module = q->mods + q->definition_obj->rc->init_module;
      if (q->linked_modules != NULL)
        q->linked_modules[q->definition_obj->rc->init_module / 8] |=
            1 << q->definition_obj->rc->init_module % 8;
      if (whyload) {
        print_obj_name(cur_obj);
...
static enum bool setup_sub_images(struct dynamic_library *p) {
  ...
  p->sub_images = allocate(max_libraries * sizeof(struct dynamic_library *));
  n = 0;
 ...
 * is printed.
 */
__private_extern__ void prebinding_check_for_dylib_override_symbols(void) {
  unsigned long i;
  struct merged_symbol_list *merged_symbol_list;
  struct merged_symbol *merged_symbol;
...
twolevel_namespace_check_for_unused_dylib_symbols(void) {
  unsigned long i;
  struct merged_symbol_list *merged_symbol_list;
  struct merged_symbol *merged_symbol;
...
static void check_dylibs_for_definition(struct merged_symbol *merged_symbol,
  ...
  for (p = dynamic_libs; p != NULL; p = p->next) {
    if (p->type == DYLIB) {
      ...
      toc = bsearch(merged_symbol->nlist.n_un.n_name, p->tocs,
                    p->definition_obj->dysymtab->ntoc,
                    sizeof(struct dylib_table_of_contents),
                    (int (*)(const void *, const void *))dylib_bsearch);
      if (toc != NULL) {
        if (prebind_check == TRUE) {
          ...
          if (check_dylibs_for_reference(merged_symbol) == TRUE) {
            ...
            printf("%s(%s) definition of %s\n", p->definition_obj->file_name,
                   p->strings + p->mods[toc->module_index].module_name,
                   merged_symbol->nlist.n_un.n_name);
            prebinding = FALSE;
          }
        }
        if (twolevel_namespace_check == TRUE) {
           ...
           * flagged.
           */
          if (is_dylib_module_loaded(p->mods + toc->module_index) == TRUE)
            continue;
          if (printed_unused == FALSE) {
            if (multiply_defined_unused_flag == MULTIPLY_DEFINED_ERROR)
  ...
  for (p = dynamic_libs; p != NULL; p = p->next) {
    if (p->type == DYLIB) {
      ...
      if (toc != NULL) {
        ...
        symbol = bsearch(
            merged_symbol->nlist.n_un.n_name,
            bsearch_symbols + p->definition_obj->dysymtab->iundefsym,
            p->definition_obj->dysymtab->nundefsym, sizeof(struct nlist),
            (int (*)(const void *, const void *))nlist_bsearch);
        if (symbol == NULL)
          continue;
       ...
       */
      dylib_references =
          (struct dylib_reference *)(p->definition_obj->obj_addr +
      ...
      for (i = 0; i < p->definition_obj->dysymtab->nextrefsyms; i++) {
        if (dylib_references[i].isym == symbol_index &&
            (dylib_references[i].flags == REFERENCE_FLAG_UNDEFINED_NON_LAZY ||
...
static enum bool open_dylib(struct dynamic_library *p) {
  ...
  if (p->dylib_file != NULL)
    ...
    if (executable_path != NULL &&
      ...
      file_name = mkstr(executable_path,
                        p->dylib_name + sizeof("@executable_path") - 1, NULL);
    } else {
  ...
  if ((fd = open(file_name, O_RDONLY, 0)) == -1) {
    if (undefined_flag != UNDEFINED_SUPPRESS) {
      system_warning(
          "can't open dynamic library: %s referenced "
  ...
  if (sizeof(struct fat_header) > file_size) {
    error("truncated or malformed dynamic library file: %s (file size "
          "too small to be a dynamic library)",
          file_name);
    return (FALSE);
  }
  fat_header = (struct fat_header *)file_addr;
...
#endif /* __LITTLE_ENDIAN__ */
    {
      pass1_fat(file_name, file_addr, file_size, FALSE, TRUE, FALSE, FALSE);
    } else {
      pass1_object(file_name, file_addr, file_size, FALSE, FALSE, TRUE, FALSE,
                   FALSE);
    }
  if (errors)
    return (FALSE);
    ...
    if (lc->cmd == LC_ID_DYLIB) {
      ...
      if (strcmp(p->dylib_name, dylib_name) != 0) {
        error("wrong dynamic library: %s (the name in the "
              "LC_ID_DYLIB command (%s) is not %s)",
              file_name, dylib_name, p->dylib_name);
      }
#endif
      p->dl = dl;
 ...
 * for archive types).
 */
__private_extern__ struct dynamic_library *
add_dynamic_lib(enum library_type type, struct dylib_command *dl,
  ...
  if (type == DYLIB) {
     ...
     */
    for (p = dynamic_libs; p != NULL; p = p->next) {
      if (p->type == DYLIB && strcmp(p->dylib_name, dylib_name) == 0) {
        if (p->dl->cmd == LC_ID_DYLIB) {
          /*
  ...
  if (type == DYLIB) {
    ...
    if (next_root != NULL && *dylib_name == '/') {
      p->file_name = allocate(strlen(next_root) + strlen(dylib_name) + 1);
...
dylib_bsearch(const char *symbol_name,
              const struct dylib_table_of_contents *toc) {
  return (
      strcmp(symbol_name,
             bsearch_strings + bsearch_symbols[toc->symbol_index].n_un.n_strx));
}
 ...
 */
__private_extern__ void merge(enum bool dylib_only, enum bool bundle_loader,
                              enum bool force_weak) {
  unsigned long previous_errors;
...
static void check_cur_obj(enum bool dylib_only, enum bool bundle_loader) {
  ...
  if (mh->cputype != 0) {
    ...
    if (arch_flag.cputype) {
      if (arch_flag.cputype != mh->cputype) {
        new_arch = get_arch_name_from_types(mh->cputype, mh->cpusubtype);
        prev_arch =
            get_arch_name_from_types(arch_flag.cputype, arch_flag.cpusubtype);
        if (no_arch_warnings == TRUE)
          return;
      ...
      if (force_cpusubtype_ALL == FALSE) {
        new_cpusubtype = cpusubtype_combine(
            arch_flag.cputype, arch_flag.cpusubtype, mh->cpusubtype);
        if (new_cpusubtype == -1) {
          new_arch = get_arch_name_from_types(mh->cputype, mh->cpusubtype);
          prev_arch =
              get_arch_name_from_types(arch_flag.cputype, arch_flag.cpusubtype);
          ...
          if (arch_flag.name != NULL) {
            if (arch_errors_fatal == TRUE) {
              error_with_cur_obj("cpusubtype (%d, "
                                 ...
                                 "specified",
                                 mh->cpusubtype, new_arch, arch_flag.cpusubtype,
                                 arch_flag.name);
            } else
          ...
          } else if (mh->filetype != MH_DYLIB && bundle_loader == FALSE)
  ...
  if ((mh->flags & MH_DYLDLINK) != 0 &&
      (mh->filetype != MH_DYLIB && mh->filetype != MH_DYLIB_STUB &&
   ...
   */
  cur_obj->dylib =
      (enum bool)(mh->filetype == MH_DYLIB || mh->filetype == MH_DYLIB_STUB);
  dlid = NULL;
  dylib_id_name = NULL;
  ...
  for (i = 0; i < mh->ncmds; i++) {
    ...
    case LC_SEGMENT:
      ...
      check_size_offset(sg->filesize, sg->fileoff, sizeof(long), "filesize",
                        "fileoff", i);
      if (errors)
        return;
      /*
      ...
      if (sg->nsects == 0) {
         ...
         */
        cur_obj->section_maps = reallocate(
            cur_obj->section_maps,
            (cur_obj->nsection_maps + sg->nsects) * sizeof(struct section_map));
      ...
      for (j = 0; j < sg->nsects; j++) {
        ...
        if (mh->filetype != MH_OBJECT && strcmp(sg->segname, s->segname) != 0) {
          error_with_cur_obj("segment name %.16s of section %lu "
                             "(%.16s,%.16s) in load command %lu does not "
                             "match segment name %.16s",
                             s->segname, j, s->segname, s->sectname, i,
                             sg->segname);
          return;
        }
        ...
        if (section_type != S_REGULAR && section_type != S_ZEROFILL &&
            ...
            section_type != S_SYMBOL_STUBS && section_type != S_COALESCED &&
        ...
        if ((s->flags & S_ATTR_DEBUG) == S_ATTR_DEBUG &&
          ...
          error_with_cur_obj(
              "malformed object, debug section %lu"
        ...
        if (section_type == S_NON_LAZY_SYMBOL_POINTERS ||
          ...
          error_with_cur_obj("unsupported type of section %lu "
                             "(%.16s,%.16s) for %s in load command %lu",
                             j, s->segname, s->sectname, progname, i);
          return;
        }
#endif /* defined(SA_RLD) || (defined(KLD) && defined(__STATIC__)) */
        ...
        if (s->align > MAXSECTALIGN) {
          error_with_cur_obj("align (%u) of section %lu "
                             ...
                             "than maximum section alignment (%d)",
                             s->align, j, s->segname, s->sectname, i,
                             MAXSECTALIGN);
          return;
        }
        /* check the size and offset of the contents if it has any*/
        if (mh->filetype != MH_DYLIB_STUB && section_type != S_ZEROFILL) {
          check_size_offset_sect(s->size, s->offset, sizeof(char), "size",
                                 "offset", i, j, s->segname, s->sectname);
          if (errors)
            return;
        }
        /* check the relocation entries if it can have them */
        if (section_type == S_ZEROFILL || section_type == S_CSTRING_LITERALS ||
            section_type == S_4BYTE_LITERALS ||
            section_type == S_8BYTE_LITERALS ||
            section_type == S_NON_LAZY_SYMBOL_POINTERS) {
          if (s->nreloc != 0) {
            error_with_cur_obj("section %lu (%.16s,%.16s) in "
                               ...
                               "it shouldn't for its type (flags)",
                               j, s->segname, s->sectname, i);
            return;
          }
        } else {
          if (s->nreloc != 0) {
            ...
            check_size_offset_sect(s->nreloc * sizeof(struct relocation_info),
                                   s->reloff, sizeof(long),
        ...
        if (section_type == S_SYMBOL_STUBS && s->reserved2 == 0) {
          error_with_cur_obj(
              "symbol stub section %lu "
              "(%.16s,%.16s) in load command %lu, sizeof stub in "
              "reserved2 field is zero",
              j, s->segname, s->sectname, i);
          return;
        }
        s++;
    ...
    case LC_SYMTAB:
      ...
      check_size_offset(st->nsyms * sizeof(struct nlist), st->symoff,
                        sizeof(long), "nsyms * sizeof(struct nlist)", "symoff",
                        i);
      if (errors)
        return;
      check_size_offset(st->strsize, st->stroff, sizeof(long), "strsize",
                        "stroff", i);
      if (errors)
        return;
      cur_obj->symtab = st;
    ...
    case LC_DYSYMTAB:
      ...
      check_size_offset(dyst->ntoc * sizeof(struct dylib_table_of_contents),
                        dyst->tocoff, sizeof(long),
                        "ntoc * sizeof(struct dylib_table_of_contents)",
                        "tocoff", i);
      if (dyst->ntoc == 0 && cur_obj->dylib == TRUE && dyst->nextdefsym != 0)
      ...
      check_size_offset(dyst->nmodtab * sizeof(struct dylib_module),
                        ...
                        "nmodtab * sizeof(struct dylib_module)", "modtaboff",
                        i);
      if (errors)
        return;
      check_size_offset(dyst->nextrefsyms * sizeof(struct dylib_reference),
    ...
    case LC_SYMSEG:
      ...
      if (ss->size != 0) {
        warning_with_cur_obj(
            "contains obsolete LC_SYMSEG load "
    ...
    case LC_LOADFVMLIB:
      if (filetype == MH_FVMLIB || filetype == MH_DYLIB ||
        ...
        error_with_cur_obj(
            "LC_LOADFVMLIB load command in object "
            "file (should not be in an input file to the link "
      ...
      if (fl->fvmlib.name.offset >= fl->cmdsize) {
        error_with_cur_obj("name.offset of load command %lu extends"
                           " past the end of the load command",
                           i);
        return;
      }
    ...
    case LC_ID_DYLIB:
      if (filetype == MH_FVMLIB || filetype == MH_DYLINKER) {
      ...
      if (dlid != NULL) {
        error_with_cur_obj("malformed object (more than one "
                           "LC_ID_DYLIB load command in %s file)",
                           mh->filetype == MH_DYLIB ? "MH_DYLIB"
                                                    : "MH_DYLIB_STUB");
        return;
      }
    ...
    case LC_REEXPORT_DYLIB:
      ...
      if (dl->cmdsize < sizeof(struct dylib_command)) {
        error_with_cur_obj(
            "cmdsize of load command %lu incorrect "
            "for %s",
            i, l.cmd == LC_LOAD_DYLIB ? "LC_LOAD_DYLIB" : "LC_LOAD_WEAK_DYLIB");
        return;
      }
      if (dl->dylib.name.offset >= dl->cmdsize) {
        error_with_cur_obj("name.offset of load command %lu extends"
    ...
    case LC_SUB_UMBRELLA:
      ...
      if (usub->sub_umbrella.offset >= usub->cmdsize) {
        error_with_cur_obj("sub_umbrella.offset of load command "
                           "%lu extends past the end of the load command",
                           i);
        return;
      }
      sub_umbrella_name = (char *)usub + usub->sub_umbrella.offset;
    ...
    case LC_ID_DYLINKER:
      if (filetype == MH_FVMLIB || filetype == MH_DYLIB ||
          filetype == MH_DYLINKER) {
        error_with_cur_obj(
            "LC_ID_DYLINKER load command in object "
            "file (should not be in an input file to the link "
            "editor for the output file type %s)",
            filetype == MH_FVMLIB
                ? "MH_FVMLIB"
    ...
    case LC_LOAD_DYLINKER:
      if (filetype == MH_FVMLIB || filetype == MH_DYLIB ||
          filetype == MH_DYLINKER) {
        error_with_cur_obj(
            "LC_LOAD_DYLINKER load command in object"
            " file (should not be in an input file to the link "
            "editor for the output file type %s)",
            filetype == MH_FVMLIB
                ? "MH_FVMLIB"
    ...
    case LC_TWOLEVEL_HINTS:
      ...
      check_size_offset(hints->nhints * sizeof(struct twolevel_hint),
                        ...
                        "nhints * sizeof(struct twolevel_hint)", "offset", i);
      if (errors)
        return;
      break;
  ...
  if (cur_obj->symtab == NULL) {
    ...
    if (cur_obj->dysymtab == NULL) {
      ...
      if (dyst->nundefsym != 0 && dyst->iundefsym > st->nsyms)
        error_with_cur_obj("iundefsym in LC_DYSYMTAB load "
                           "command extends past the end of the symbol table");
      if (dyst->nundefsym != 0 && dyst->iundefsym + dyst->nundefsym > st->nsyms)
      ...
      if (dyst->ntoc != 0) {
        tocs = (struct dylib_table_of_contents *)(cur_obj->obj_addr +
                                                  dyst->tocoff);
        if (cur_obj->swapped)
          swap_dylib_table_of_contents(tocs, dyst->ntoc, host_byte_sex);
      ...
      if (dyst->nmodtab != 0) {
        mods = (struct dylib_module *)(cur_obj->obj_addr + dyst->modtaboff);
        if (cur_obj->swapped)
        ...
        for (i = 0; i < dyst->nmodtab; i++) {
          ...
          if (mods[i].iextdefsym > st->nsyms)
            error_with_cur_obj("iextdefsym field of module "
                               ...
                               "table",
                               i);
          if (mods[i].iextdefsym + mods[i].nextdefsym > st->nsyms)
          ...
          if (mods[i].ilocalsym > st->nsyms)
            error_with_cur_obj("ilocalsym field of module "
                               ...
                               "table",
                               i);
          if (mods[i].ilocalsym + mods[i].nlocalsym > st->nsyms)
      ...
      if (dyst->nextrefsyms != 0) {
        refs =
            (struct dylib_reference *)(cur_obj->obj_addr + dyst->extrefsymoff);
        if (cur_obj->swapped)
          swap_dylib_reference(refs, dyst->nextrefsyms, host_byte_sex);
        for (i = 0; i < dyst->nextrefsyms; i++) {
          if (refs[i].isym > st->nsyms)
            error_with_cur_obj("isym field of reference table "
                               "entry %lu past the end of the symbol table",
                               i);
        }
      }
      ...
      if (dyst->nindirectsyms != 0) {
        indirect_symtab =
            (uint32_t *)(cur_obj->obj_addr + dyst->indirectsymoff);
        if (cur_obj->swapped)
          swap_indirect_symbols(indirect_symtab, dyst->nindirectsyms,
                                host_byte_sex);
          ...
          if (indirect_symtab[i] != INDIRECT_SYMBOL_LOCAL &&
            ...
            if (indirect_symtab[i] > st->nsyms)
              error_with_cur_obj("indirect symbol table entry"
                                 " %lu past the end of the symbol table",
                                 i);
          }
        }
      }
  ...
  if ((mh->filetype == MH_DYLIB || mh->filetype == MH_DYLIB_STUB) &&
    ...
    error_with_cur_obj("malformed object (no LC_ID_DYLIB load command "
                       "in %s file)",
                       mh->filetype == MH_DYLIB ? "MH_DYLIB" : "MH_DYLIB_STUB");
    return;
  }
  /*
   * If this is a MH_DYLINKER file then a single LC_ID_DYLINKER command
  ...
  if (indirect_dylib == FALSE && sub != NULL &&
    ...
    short_name = guess_short_name(dylib_id_name, &is_framework, &has_suffix);
    /*
 ...
 */
__private_extern__ void
merge_base_program(char *basefile_name, struct mach_header *basefile_addr,
                   struct segment_command *seg_linkedit, struct nlist *symtab,
                   unsigned long nsyms, char *strtab, unsigned long strsize) {
  unsigned long i, j, section_type;
  struct mach_header *mh;
  struct load_command *lc, *load_commands;
  struct segment_command *sg;
  ...
  for (i = 0; i < mh->ncmds; i++) {
    ...
    if ((char *)lc + lc->cmdsize > (char *)load_commands + mh->sizeofcmds) {
      error_with_cur_obj("load command %lu extends past end of all "
                         "load commands",
                         i);
      return;
  ...
  if (seg_linkedit == NULL) {
    ...
    if (st->symoff < seg_linkedit->fileoff ||
      ...
      error_with_cur_obj("symbol table is not contained in " SEG_LINKEDIT
                         " segment");
      return;
    }
    if (st->stroff < seg_linkedit->fileoff ||
        st->stroff + st->strsize >
            seg_linkedit->fileoff + seg_linkedit->filesize) {
      error_with_cur_obj("string table is not contained in " SEG_LINKEDIT
                         " segment");
      return;
    }
    base_program_symtab = *st;
    base_program_symtab.symoff =
        (seg_linkedit->vmaddr + (st->symoff - seg_linkedit->fileoff)) -
        (long)cur_obj->obj_addr;
    base_program_symtab.stroff =
        (seg_linkedit->vmaddr + (st->stroff - seg_linkedit->fileoff)) -
        (long)cur_obj->obj_addr;
    cur_obj->symtab = &base_program_symtab;
  }
  /*
  ...
  if (size != 0) {
    if (offset % align != 0) {
      ...
      warning_with_cur_obj("%s of section %lu (%.16s,%.16s) in load "
                           "command %lu not aligned on %lu byte boundary",
                           offset_str, sect, segname, sectname, cmd, align);
#else  /* !defined(mc68000) */
    ...
    if (offset > cur_obj->obj_size) {
      error_with_cur_obj("%s of section %lu (%.16s,%.16s) in load "
                         ...
                         offset_str, sect, segname, sectname, cmd);
      return;
    }
    if (offset + size > cur_obj->obj_size) {
 ...
 */
static int symbol_address_compare(void *fail_p, const void *a_p,
                                  const void *b_p) {
...
static void read_dwarf_info(void) {
  ...
  if (!debug_info || !debug_abbrev || !cur_obj->symtab || debug_info->size == 0)
    return;
  ...
  if (read_comp_unit(
          (const uint8_t *)cur_obj->obj_addr + debug_info->offset,
          debug_info->size,
          ((const uint8_t *)cur_obj->obj_addr + debug_abbrev->offset),
          debug_abbrev->size, little_endian, &name, &comp_dir, &stmt_list) &&
      name) {
  ...
  if (stmt_list >= debug_line->size) {
    warning_with_cur_obj(
        "offset in DWARF debug_info for line number data is too large");
    free(sst);
    return;
  }
  ...
  for (i = 0; i < cur_obj->symtab->nsyms; i++) {
    ...
    if ((s->n_type & N_TYPE) != N_SECT || s->n_sect == NO_SECT)
      continue;
  ...
  cur_obj->dwarf_source_data =
      allocate((lastused + num_line_syms * 2 + 1) * sizeof(size_t));
  dwarf_source_i = 0;
  for (i = 0; i < cur_obj->symtab->nsyms; i++)
    if (symdata[i]) {
      ...
      for (j = symdata[i] - 1; symchunk->filedata[j % chunksize] != (size_t)-1;
           j++) {
