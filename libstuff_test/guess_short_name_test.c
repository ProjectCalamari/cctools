...
static void test_guess_short_name(void) {
  ...
  check("Frameworks/Foo.framework/Versions/A/Foo_profile", "Foo", "_profile",
        TRUE);
  check("Foo.framework/Versions/A/Foo_profile", "Foo", "_profile", TRUE);
  check("Frameworks/Versions/A/Foo.framework/Foo_debug", "Foo", "_debug", TRUE);
  ...
  check("Frameworks/Foo_network.framework/Foo_network", "Foo_network", NULL,
        TRUE);
  ...
  check("Foo_network.framework/Versions/A/Foo_network", "Foo_network", NULL,
        TRUE);

  // questionable actors
  //   configd_libSystem
  check("/lib/system/libsystem_configuration.dylib", "libsystem_configuration",
        NULL, FALSE);
  ...
  //   Sandbox_libs
  check("/usr/lib/system/libsystem_sandbox.dylib", "libsystem_sandbox", NULL,
        FALSE);
  check("/usr/lib/system/libsystem_sandbox_debug.dylib", "libsystem_sandbox",
        "_debug", FALSE);
  check("/usr/lib/system/libsystem_sandbox_profile.dylib", "libsystem_sandbox",
        "_profile", FALSE);

  // bad actors
  //   nanocom
  check("/usr/local/lib/nanocom/libnanocom_bell.dylib", "libnanocom_bell", NULL,
        FALSE);
  check("/usr/local/lib/nanocom/libnanocom_fastsim.dylib", "libnanocom_fastsim",
        NULL, FALSE);
  check("/usr/local/lib/nanocom/libnanocom_login.dylib", "libnanocom_login",
        NULL, FALSE);

  //   mDNSResponder
  check("/usr/lib/libdns_services.dylib", "libdns_services", NULL, FALSE);

  //   ngttp2
  check("/usr/lib/libapple_nghttp2.dylib", "libapple_nghttp2", NULL, FALSE);
}
