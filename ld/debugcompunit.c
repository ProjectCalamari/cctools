...
#define read_16(p)                                                             \
  (little_endian ? ((p)[1] << 8 | (p)[0]) : ((p)[0] << 8 | (p)[1]))
#define read_32(p)                                                             \
  (little_endian ? ((p)[3] << 24 | (p)[2] << 16 | (p)[1] << 8 | (p)[0])        \
                 : ((p)[0] << 24 | (p)[1] << 16 | (p)[2] << 8 | (p)[3]))
#define read_64(p)                                                             \
  (little_endian ? ((uint64_t)(p)[7] << 56 | (uint64_t)(p)[6] << 48 |          \
...
static void skip_leb128(const uint8_t **offset, const uint8_t *end) {
...
static uint64_t read_uleb128(const uint8_t **offset, const uint8_t *end) {
  uint64_t result = 0;
  int bit = 0;
...
static bool skip_form(const uint8_t **offset, const uint8_t *end, uint64_t form,
...
int read_comp_unit(const uint8_t *debug_info, size_t debug_info_size,
                   const uint8_t *debug_abbrev, size_t debug_abbrev_size,
                   int little_endian, const char **name, const char **comp_dir,
                   uint64_t *stmt_list) {
