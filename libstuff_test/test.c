...
int test_register_initializer(int order, test_initializer_func initializer) {
  if (s_ntests < 0xFFFFFFFF) {
    s_inits = (struct test_init *)reallocf(s_inits,
                                           sizeof(*s_inits) * (s_ninits + 1));
    if (s_inits) {
...
int test_add(const char *name, test_func test) {
  ...
  if (s_ntests < 0xFFFFFFFF) {
    s_tests =
        (struct test *)reallocf(s_tests, sizeof(*s_tests) * (s_ntests + 1));
    if (s_tests) {
...
#ifdef __cplusplus
    try { tests[i].testfunc(); }
    catch(int err) {
      test_printerr("uncaught exception: int %d (might be %s)", err,
                    strerror(err));
    }
    catch(const char *s) { test_printerr("uncaught exception: %s", s); }
    catch(std::string s) { test_printerr("uncaught exception: %s", s.c_str()); }
    catch(...) { test_printerr("uncaught exception!"); }
#else
    tests[i].testfunc();
#endif
    if (s_print_called)
...
void test_abort(void) { s_test_result = TEST_ABORT; }

__attribute__((format(printf, 1, 0))) static void
test_vprint(const char *__restrict fmt, va_list args) {
  assert(fmt);
...
int check_bool(const char *name, bool orig, bool copy) {
  int result = 0;
  if (orig != copy) {
    test_printerr("%s%s should be %d: %d", s_prefix ? s_prefix : "", name, orig,
                  copy);
    result = 1;
  }
  return result;
...
int check_uint32(const char *name, uint32_t orig, uint32_t copy) {
  int result = 0;
  if (orig != copy) {
    test_printerr("%s%s should be %u: %u", s_prefix ? s_prefix : "", name, orig,
                  copy);
    result = 1;
  }
  return result;
...
int check_uint64(const char *name, uint64_t orig, uint64_t copy) {
  int result = 0;
  if (orig != copy) {
    test_printerr("%s%s should be %llu: %llu", s_prefix ? s_prefix : "", name,
                  orig, copy);
    result = 1;
  }
  return result;
...
int check_pointer(const char *name, const void *orig, const void *copy) {
  int result = 0;
  if (orig != copy) {
    test_printerr("%s%s should be 0x%p: 0x%p", s_prefix ? s_prefix : "", name,
                  orig, copy);
    result = 1;
  }
  return result;
...
int check_string(const char *name, const char *orig, const char *copy) {
  ...
  if (NULL != orig && NULL == copy) {
    test_printerr("missing %s%s should be '%s'", s_prefix ? s_prefix : "", name,
                  orig);
    result = 1;
  } else if (NULL == orig && NULL != copy) {
    test_printerr("%s%s should be unset: '%s'", s_prefix ? s_prefix : "", name,
                  copy);
    result = 1;
  } else if (NULL != orig && NULL != copy && strcmp(copy, orig)) {
    test_printerr("%s%s should be '%s': '%s'", s_prefix ? s_prefix : "", name,
                  orig, copy);
    result = 1;
  }
  return result;
...
int check_null(const char *name, void *copy) {
  int result = 0;
  if (copy) {
    test_printerr("%s%s should be NULL: 0x%p", s_prefix ? s_prefix : "", name,
                  copy);
    result = 1;
  }
  return result;
