 ...
 * setting.
 */
__private_extern__ void put_macosx_deployment_target(char *target) {
...
get_macosx_deployment_target(struct macosx_deployment_target *value) {
  ...
  if (p != NULL) {
    if (command_line_macosx_deployment_target != NULL)
      ...
      warning("unknown MACOSX_DEPLOYMENT_TARGET environment "
              "variable value: %s ignored (using %s)",
              p, value->name);
  }
}
#endif /* !defined(RLD) */
