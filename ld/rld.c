...
getsectbynamefromheader(const struct mach_header *mhp, const char *segname,
                        const char *sectname);
#endif
 ...
 */
__private_extern__ unsigned long (*address_func)(
    unsigned long size, unsigned long headers_size) = NULL;

static enum strip_levels kld_requested_strip_level = STRIP_ALL;
...
void kld_set_byteorder(enum NXByteOrder order) {
  switch (order) {
  case NX_BigEndian:
...
static long internal_rld_load(
    NXStream *stream,
#endif /* KLD */
    struct mach_header **header_addr, const char *const *object_filenames,
    const char *output_filename, const char *file_name, const char *obj_addr,
    long obj_size);
...
#else  /* !defined(KLD) */
static long internal_rld_unload(NXStream *stream,
#endif /* KLD */
    enum bool internal_cleanup);

#if !defined(SA_RLD) && !defined(KLD)
static long internal_rld_load_basefile(NXStream *stream,
                                       const char *base_filename,
                                       char *base_addr, long base_size);
#endif /* !defined(SA_RLD) && !defined(KLD) */

#if defined(KLD)
static long internal_kld_load_basefile(const char *base_filename,
                                       char *base_addr, long base_size);
#endif /* defined(KLD) */
 ...
 */
long rld_load(NXStream *stream, struct mach_header **header_addr,
 ...
 */
long kld_load(struct mach_header **header_addr, const char *object_filename,
              const char *output_filename) {
  ...
  return (internal_kld_load(header_addr, object_filenames, output_filename,
                            NULL, NULL, 0));
}
 ...
 */
long kld_load_from_memory(struct mach_header **header_addr,
                          const char *object_name, char *object_addr,
                          long object_size, const char *output_filename) {
  return (internal_kld_load(header_addr, NULL, output_filename, object_name,
 ...
 * object_name, at address object_addr and is of size object_size.
 */
long rld_load_from_memory(NXStream *stream, struct mach_header **header_addr,
                          const char *object_name, char *object_addr,
                          long object_size, const char *output_filename) {
...
long kld_load_from_memory(struct mach_header **header_addr,
                          const char *object_name, char *object_addr,
                          long object_size) {
  return (internal_kld_load(header_addr, NULL, NULL, object_name, object_addr,
                            object_size));
}
#endif /* defined(KLD) && defined(__DYNAMIC__) */
...
internal_rld_load(
  ...
  if (base_obj == NULL) {
    ...
    if (sg != NULL)
      merge_base_program((*NXArgv_pointer)[0], _mh_execute_header_pointer, sg,
                         NULL, 0, NULL, 0);
    ...
    if (NXArgv_pointer == NULL)
      _dyld_lookup_and_bind("_NXArgv", (unsigned long *)&NXArgv_pointer, NULL);
    if (_mh_execute_header_pointer == NULL)
    ...
    if (sg != NULL)
      ...
      merge_base_program(kld_basefile_name,
                         (struct mach_header *)&_mh_execute_header, sg, NULL, 0,
                         NULL, 0);
#else
      merge_base_program(NXArgv[0], (struct mach_header *)&_mh_execute_header,
                         sg, NULL, 0, NULL, 0);
#endif /* KLD */
#else  /* defined(__DYNAMIC__) */
      merge_base_program((*NXArgv_pointer)[0], _mh_execute_header_pointer, sg,
  ...
  if (file_name == NULL) {
    for (i = 0; object_filenames[i] != NULL; i++)
      pass1((char *)object_filenames[i], FALSE, FALSE, FALSE, FALSE, FALSE);
  } else
#endif /* !defined(SA_RLD) && !(defined(KLD) && defined(__STATIC__)) */
  {
...
#if !defined(SA_RLD) && !(defined(KLD) && defined(__STATIC__))
  if (output_filename != NULL && output_filename != RLD_DEBUG_OUTPUT_FILENAME) {
    /*
    ...
    if (deallocate_size > 0) {
      if ((r = vm_deallocate(
               mach_task_self(),
               (vm_address_t)(output_addr + rnd(output_size, host_pagesize)),
      ...
      print("rld() vm_deallocate: addr = 0x%0x size = 0x%x\n",
            (unsigned int)(output_addr + rnd(output_size, host_pagesize)),
  ...
  if (rld_maintain_states == TRUE) {
    if (rld_nloaded_states + 1 >= rld_nallocated_states) {
      rld_loaded_state = reallocate(
          rld_loaded_state, sizeof(struct rld_loaded_state) *
    ...
    if (file_name == NULL) {
      ...
      for (i = 0; object_filenames[i] != NULL; i++) {
        if (object_filenames[i][0] != '/') {
          ...
          rld_loaded_state[rld_nloaded_states].object_filenames[i] =
              allocate(dir_len + 1 + strlen(object_filenames[i]) + 1);
          strcpy(rld_loaded_state[rld_nloaded_states].object_filenames[i], dir);
          strcat(rld_loaded_state[rld_nloaded_states].object_filenames[i], "/");
          strcat(rld_loaded_state[rld_nloaded_states].object_filenames[i],
                 object_filenames[i]);
        } else {
          rld_loaded_state[rld_nloaded_states].object_filenames[i] =
              allocate(strlen(object_filenames[i]) + 1);
          strcpy(rld_loaded_state[rld_nloaded_states].object_filenames[i],
                 object_filenames[i]);
        }
      }
    } else {
      ...
      strcpy(rld_loaded_state[rld_nloaded_states].object_filenames[0],
             file_name);
    }
  ...
  if (base_name == NULL) {
     ...
     */
    s = getsectbynamefromheader((struct mach_header *)output_addr, "__TEXT",
                                "__constructor");
    if (s != NULL) {
...
long kld_load_basefile(const char *base_filename) {
  return (internal_kld_load_basefile(base_filename, NULL, 0));
}
#endif /* !defined(__STATIC__) */

long kld_load_basefile_from_memory(const char *base_filename, char *base_addr,
                                   long base_size) {
  return (internal_kld_load_basefile(base_filename, base_addr, base_size));
}

#else  /* !defined(KLD) */
long rld_load_basefile(NXStream *stream, const char *base_filename) {
...
internal_rld_load_basefile(
    NXStream *stream,
#endif
    const char *base_filename, char *base_addr, long base_size) {
#if !(defined(KLD) && defined(__STATIC__))
  unsigned long size;
  char *addr;
  int fd;
  ...
  if (base_addr == NULL) {
    ...
    if (fat_header->magic == FAT_MAGIC)
      ...
      best_fat_arch = cpusubtype_findbestarch(host_arch_flag.cputype,
...
rld_load_basefile_error_return:
  if ((r = vm_deallocate(mach_task_self(), (vm_address_t)addr,
                         (vm_size_t)size)) != KERN_SUCCESS)
    mach_fatal(r, "can't vm_deallocate() memory for mapped file %s", base_name);
#ifdef RLD_VM_ALLOC_DEBUG
  print("rld() vm_deallocate: addr = 0x%0x size = 0x%x\n", (unsigned int)addr,
        (unsigned int)size);
#endif /* RLD_VM_ALLOC_DEBUG */
 ...
 * on that stream.
 */
long rld_unload(NXStream *stream) {
...
internal_kld_unload(
#else  /* !defined(KLD) */
internal_rld_unload(NXStream *stream,
#endif /* KLD */
    enum bool internal_cleanup) {
#if !defined(SA_RLD) && !(defined(KLD) && defined(__STATIC__))
  kern_return_t r;
  ...
  if (internal_cleanup == FALSE && rld_maintain_states == TRUE) {
    rld_nloaded_states -= 1;
    for (i = 0; i < rld_loaded_state[rld_nloaded_states].nobject_filenames;
         i++) {
      free(rld_loaded_state[rld_nloaded_states].object_filenames[i]);
    }
 ...
 */
long rld_write_symfile(NXStream *stream, const char *output_filename) {
  int fd;
   ...
   * the return code of the unlink() is ignored).
   */
  symbol_size = output_symtab_info.symtab_command.nsyms * sizeof(struct nlist) +
                output_symtab_info.symtab_command.strsize;
  (void)unlink(output_filename);
  if ((fd = open(output_filename, O_WRONLY | O_CREAT | O_TRUNC, 0666)) == -1) {
    system_error("can't create output file: %s", output_filename);
    return_value = 0;
    goto deallocate_and_return;
  } else {
...
static void rld_loaded_state_changed(void) {
  ...
  for (i = 0; i < rld_nloaded_states; i++) {
    print("state %lu\n\tnobject_filenames %lu\n\tobject_filenames 0x%x"
          "\n\theader_addr 0x%x\n",
          i, rld_loaded_state[i].nobject_filenames,
 ...
 */
static void rld_get_loaded_state(struct rld_loaded_state **s,
                                 unsigned long *n) {
  *s = rld_loaded_state;
  *n = rld_nloaded_states;
}
 ...
 * described above.
 */
void (*moninitrld(void (*m)(char *lowpc,
                            char *highpc)))(struct rld_loaded_state **s,
                                            unsigned long *n) {
  rld_monaddition = m;
 ...
 */
char *rld_get_current_header(void) {
  /*
    ...
    unsigned long strsize) /* sizeof the string table */
{
  ...
  if (base_obj == NULL) {
    if (symtab == NULL) {
      ...
      if (linkedit != NULL)
        merge_base_program(basefile_name, basefile_addr, linkedit, NULL, 0,
                           NULL, 0);
    } else {
      merge_base_program(basefile_name, basefile_addr, NULL, symtab, nsyms,
                         strtab, strsize);
    }
    if (target_byte_sex == UNKNOWN_BYTE_SEX)
      target_byte_sex = host_byte_sex;
 ...
 */
int sa_rld(char *basefile_name,               /* base file name */
           struct mach_header *basefile_addr, /* mach header of the base file */
           ...
           unsigned long malloc_len) /* length to use for same */
{
  return (sa_rld_internal(basefile_name, basefile_addr, object_name,
                          object_addr, object_size, workmem_addr, workmem_size,
                          error_buf_addr, error_buf_size, malloc_addr,
                          malloc_len, NULL, 0, NULL, 0));
}
    ...
    unsigned long strsize) /* sizeof the string table */
{
  return (sa_rld_internal(basefile_name, basefile_addr, object_name,
                          object_addr, object_size, workmem_addr, workmem_size,
                          error_buf_addr, error_buf_size, malloc_addr,
                          malloc_len, symtab, nsyms, strtab, strsize));
}
...
__private_extern__ void vprint(const char *format, va_list ap) {
  ...
  new = slvprintf(sa_rld_error_buf_addr, sa_rld_error_buf_size, format, ap);
 ...
 */
__private_extern__ struct mach_header *_NSGetMachExecuteHeader(void) {
