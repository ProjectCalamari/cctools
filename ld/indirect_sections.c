...
static unsigned long lookup_indirect_item(struct merged_symbol *merged_symbol,
                                          struct object_file *obj,
                                          unsigned long index,
                                          struct indirect_section_data *data,
                                          unsigned long stride, enum bool *new);
...
indirect_section_merge(struct indirect_section_data *data,
                       struct merged_section *ms, struct section *s,
                       struct section_map *section_map, enum bool redo_live) {
  ...
  if (s->reserved1 + nitems > cur_obj->dysymtab->nindirectsyms) {
    error_with_cur_obj(
        "malformed object (indirect symbol table entries"
  ...
  indirect_symtab =
      (unsigned long *)(cur_obj->obj_addr + cur_obj->dysymtab->indirectsymoff);
    ...
    if (index == INDIRECT_SYMBOL_LOCAL || index == INDIRECT_SYMBOL_ABS) {
      ...
      if (redo_live == FALSE || fine_relocs[i].live == TRUE) {
        fine_relocs[i].output_offset =
            lookup_indirect_item(NULL, cur_obj, index, data, stride, &new);
    ...
    if (merged_symbol == NULL) {
      ...
      if (merged_symbol->nlist.n_type == (N_EXT | N_UNDF) ||
        ...
        if ((output_for_dyld &&
             ...
             (((merged_symbol->nlist.n_desc & N_WEAK_DEF) != N_WEAK_DEF) ||
      ...
      if ((filetype == MH_DYLIB && multi_module_dylib == TRUE) ||
        ...
        if (redo_live == FALSE || fine_relocs[i].live == TRUE) {
          fine_relocs[i].output_offset =
              lookup_indirect_item(merged_symbol, NULL, 0, data, stride, &new);
        } else if (redo_live == TRUE && fine_relocs[i].live == FALSE) {
          new = FALSE;
        }
  ...
  if (section_type == S_LAZY_SYMBOL_POINTERS && s->nreloc != nitems) {
    error_with_cur_obj(
        "malformed object (lazy symbol pointer section "
  ...
  for (i = 0; i < s->nreloc; i++) {
    ...
    if (r_type == reloc_pair_r_type(arch_flag.cputype)) {
      error_with_cur_obj("malformed object (stray relocation PAIR "
                         "entry (%lu) in section (%.16s,%.16s))",
                         i, s->segname, s->sectname);
      continue;
    }
    /*
    ...
    if (r_address >= s->size) {
      error_with_cur_obj(
          "malformed object (r_address (0x%x) field "
    ...
    if (reloc_has_pair(arch_flag.cputype, r_type)) {
      if (i + 1 < s->nreloc) {
        ...
        if (cur_obj->swapped &&
          ...
          swap_relocation_info(&reloc, 1, host_byte_sex);
        if ((reloc.r_address & R_SCATTERED) != 0) {
          spair_reloc = (struct scattered_relocation_info *)&reloc;
    ...
    if (section_type == S_LAZY_SYMBOL_POINTERS) {
      ...
      if (r_pcrel != 0) {
        error_with_cur_obj(
            ...
            "(FALSE))",
            (unsigned int)r_pcrel, i, s->segname, s->sectname);
        continue;
      }
      if (r_type != 0) {
        error_with_cur_obj(
            ...
            "symbol pointer section (%.16s,%.16s) is not 0 "
            "(VANILLA))",
            (unsigned int)r_type, i, s->segname, s->sectname);
        continue;
      }
      if (r_scattered != 0) {
        error_with_cur_obj(
    ...
    if (output_for_dyld && r_extern) {
      ...
      if (r_symbolnum >= cur_obj->symtab->nsyms) {
        error_with_cur_obj(
            "r_symbolnum (%lu) field of external "
        ...
        if ((nlists[r_symbolnum].n_type & N_TYPE) == N_SECT &&
          ...
          if (nlists[r_symbolnum].n_type != (N_EXT | N_UNDF)) {
            error_with_cur_obj("r_symbolnum (%lu) field of "
                               ...
                               "symbol",
                               r_symbolnum, i, section_map->s->segname,
                               section_map->s->sectname);
            return;
          }
  ...
  if (dead_strip == FALSE || redo_live == TRUE) {
    ...
    discard_local_symbols_for_section(nsect, nlists, strings, s, section_map);
  }
}
...
static unsigned long lookup_indirect_item(struct merged_symbol *merged_symbol,
  ...
  if (data->hashtable == NULL) {
    ...
    memset(data->hashtable, '\0',
           sizeof(struct indirect_item_bucket *) * INDIRECT_SECTION_HASHSIZE);
  }
 ...
 */
__private_extern__ void
indirect_section_order(struct indirect_section_data *data,
  ...
  warning("section ordering for indirect sections not supported ("
          "-sectorder %s %s %s ignored)",
          ms->s.segname, ms->s.sectname, ms->order_filename);
  /*
 ...
 */
__private_extern__ void
indirect_section_reset_live(struct indirect_section_data *data,
                            struct merged_section *ms) {
  /*
 ...
 */
__private_extern__ enum bool indirect_live_ref(struct fine_reloc *fine_reloc,
  ...
  if (fine_reloc->local_symbol == TRUE) {
    ...
    if (fine_reloc->use_contents == FALSE) {
      ...
      nlists = (struct nlist *)(obj->obj_addr + obj->symtab->symoff);
      r_symbolnum = r_symbolnum_from_r_value(nlists[index].n_value, obj);
      r->map = &(obj->section_maps[r_symbolnum - 1]);
      r->fine_reloc = fine_reloc_for_input_offset(
          r->map, nlists[index].n_value - r->map->s->addr);
      r->obj = obj;
      ...
      if (fine_reloc->indirect_symbol_local == TRUE) {
        ...
        r->map = &(obj->section_maps[r_symbolnum - 1]);
        r->fine_reloc =
            fine_reloc_for_input_offset(r->map, value - r->map->s->addr);
        r->obj = obj;
        r->merged_symbol = NULL;
        return (TRUE);
      } else {
 ...
 */
__private_extern__ void
indirect_section_free(struct indirect_section_data *data) {
  unsigned long i;
  struct indirect_item_bucket *bp, *next_bp;
  struct indirect_item_block *indirect_item_block, *next_indirect_item_block;
  ...
  for (indirect_item_block = data->indirect_item_blocks; indirect_item_block;
...
legal_reference(struct section_map *from_map, unsigned long from_offset,
                struct section_map *to_map, unsigned long to_offset,
                unsigned long from_reloc_index, enum bool sectdiff_reloc) {
#ifndef SA_RLD
  ...
  if (from_section_type == S_COALESCED) {
    from_fine_reloc = fine_reloc_for_input_offset(from_map, from_offset);
    ...
    if (to_fine_reloc->use_contents == FALSE) {
      ...
      if (merged_symbol->defined_in_dylib == TRUE) {
        ...
        if (merged_symbols_relocated == FALSE)
          ...
          to_section_type =
              pass2_nsect_merged_symbol_section_type(merged_symbol);
      } else {
        if (sectdiff_reloc == TRUE && dynamic == TRUE) {
          if ((merged_symbol->nlist.n_type & N_TYPE) == N_ABS) {
            error_with_cur_obj("illegal reference for -dynamic "
  ...
  if (dynamic == TRUE && to_section_type == S_COALESCED &&
      ...
      fine_reloc_for_input_offset(to_map, to_offset)->local_symbol == FALSE) {
    error_with_cur_obj(
        "malformed object, illegal reference for "
  ...
  if (to_section_type == S_SYMBOL_STUBS) {
    ...
    if (from_section_type != S_LAZY_SYMBOL_POINTERS) {
      if ((to_offset % to_map->s->reserved2) != 0) {
         ...
         */
        if (from_section_type == S_SYMBOL_STUBS && from_map == to_map &&
        ...
        error_with_cur_obj(
            ...
            "(%lu))",
            from_map->s->segname, from_map->s->sectname, from_reloc_index);
        return (FALSE);
      } else
        return (TRUE);
    } else {
  ...
  if (to_section_type == S_LAZY_SYMBOL_POINTERS) {
    ...
    if (from_section_type != S_SYMBOL_STUBS) {
      ...
      if (indirect_symtab[from_map->s->reserved1 +
                          from_offset / from_map->s->reserved2] !=
 ...
 * file.
 */
__private_extern__ void output_indirect_symbols(void) {
  ...
  while (*p) {
    ...
    while (*content) {
      ...
      if ((ms->s.flags & SECTION_TYPE) == S_SYMBOL_STUBS ||
        ...
        for (q = &(data->indirect_item_blocks); *q;
             q = &(indirect_item_block->next)) {

          indirect_item_block = *q;
          for (i = 0; i < indirect_item_block->used; i++) {
            indirect_item = indirect_item_block->indirect_items + i;
            if (indirect_item->merged_symbol != NULL) {
               ...
               * INDIRECT_SYMBOL_ABS.
               */
              if ((ms->s.flags & SECTION_TYPE) == S_NON_LAZY_SYMBOL_POINTERS &&
                  (indirect_item->merged_symbol->nlist.n_type & N_PEXT) ==
                      N_PEXT &&
                  keep_private_externs == FALSE) {
                if ((indirect_item->merged_symbol->nlist.n_type & N_TYPE) ==
                    N_ABS)
                  indirect_symbols[nindirect_symbols++] = INDIRECT_SYMBOL_ABS;
                else
                  indirect_symbols[nindirect_symbols++] = INDIRECT_SYMBOL_LOCAL;
              } else {
                indirect_symbols[nindirect_symbols++] =
                    merged_symbol_output_index(indirect_item->merged_symbol);
              }
            } else {
              if (indirect_item->index == INDIRECT_SYMBOL_LOCAL ||
                  indirect_item->index == INDIRECT_SYMBOL_ABS) {
                indirect_symbols[nindirect_symbols++] = indirect_item->index;
              } else {
                indirect_symbols[nindirect_symbols++] =
                    local_symbol_output_index(indirect_item->obj,
                                              indirect_item->index);
              }
            }
          }
  ...
  if (host_byte_sex != target_byte_sex)
    swap_indirect_symbols(indirect_symbols, nindirect_symbols, target_byte_sex);
#ifndef RLD
