...
static unsigned long next_vmaddr(unsigned long vmaddr, unsigned long vmsize);
static int qsort_vmaddr(const struct merged_segment **msg1,
...
static void check_overlap(struct merged_segment *msg1,
                          struct merged_segment *msg2, enum bool prebind_check,
                          struct merged_segment *outputs_linkedit_segment);
static void
check_for_overlapping_segments(struct merged_segment *outputs_linkedit_segment);
 ...
 */
__private_extern__ void layout(void) {
#ifdef RLD
  ...
  memset(&hppa_frame_state, '\0', sizeof(struct hp_pa_frame_thread_state));
  memset(&hppa_integer_state, '\0', sizeof(struct hp_pa_integer_thread_state));
   ...
   */
  if (filetype == MH_EXECUTE || filetype == MH_BUNDLE || filetype == MH_DYLIB ||
   ...
   */
  if (nowarnings == FALSE && twolevel_namespace == TRUE &&
 ...
 */
__private_extern__ void layout_rld_symfile(void) {
  ...
  output_symtab_info.symtab_command.cmdsize = sizeof(struct symtab_command);
  output_symtab_info.symtab_command.nsyms = nmerged_symbols;
  output_symtab_info.symtab_command.strsize =
      rnd(merged_string_size + STRING_SIZE_OFFSET, sizeof(unsigned long));
  output_symtab_info.output_strpad = output_symtab_info.symtab_command.strsize -
  ...
  output_symtab_info.output_local_strsize =
      STRING_SIZE_OFFSET + merged_string_size;
  /*
  ...
  output_mach_header.sizeofcmds = output_symtab_info.symtab_command.cmdsize;
  ...
  offset += output_symtab_info.symtab_command.nsyms * sizeof(struct nlist);
  ...
  if ((r = vm_allocate(mach_task_self(), (vm_address_t *)&output_addr,
    ...
    mach_fatal(r,
               "can't vm_allocate() memory for output of size "
               "%lu",
               output_size);
...
static void layout_segments(void) {
  ...
  if (filetype != MH_FVMLIB && filetype != MH_DYLIB && filetype != MH_BUNDLE &&
    ...
    if ((stack_addr_specified == TRUE || stack_size_specified == TRUE) &&
        output_thread_info.thread_command.cmd == LC_UNIXTHREAD) {
      if (check_reserved_segment(SEG_UNIXSTACK,
                                 "segment " SEG_UNIXSTACK
                                 " reserved for the -stack_addr and "
                                 "-stack_size options")) {
        /*
        ...
        if (seg_spec != NULL) {
          ...
          if (seg_spec->prot_specified) {
            if ((seg_spec->maxprot & (VM_PROT_READ | VM_PROT_WRITE)) !=
                (VM_PROT_READ | VM_PROT_WRITE)) {
              error("specified maximum protection for "
                    "segment " SEG_UNIXSTACK " must include read "
                    " and write");
              seg_spec->maxprot |= (VM_PROT_READ | VM_PROT_WRITE);
            }
            if ((seg_spec->initprot & (VM_PROT_READ | VM_PROT_WRITE)) !=
                (VM_PROT_READ | VM_PROT_WRITE)) {
              error("specified initial protection for "
                    "segment " SEG_UNIXSTACK " must include read "
                    " and write");
              seg_spec->initprot |= (VM_PROT_READ | VM_PROT_WRITE);
            }
             ...
             */
            if ((stack_segment.sg.maxprot & VM_PROT_EXECUTE) == VM_PROT_EXECUTE)
              allow_stack_execute = TRUE;
          }
          seg_spec->processed = TRUE;
          stack_segment.prot_set = TRUE;
        } else {
          stack_segment.sg.maxprot = get_segprot_from_flag(&arch_flag);
        ...
        if (stack_addr_specified == TRUE) {
          if (stack_addr % segalign != 0)
            fatal("-stack_addr: 0x%x not a multiple of the "
                  ...
                  (unsigned int)stack_addr, (unsigned int)segalign);
        } else {
        ...
        if (stack_size_specified == TRUE) {
          if (stack_size % segalign != 0)
            fatal("-stack_size: 0x%x not a multiple of the "
                  "segment alignment (0x%x)",
                  (unsigned int)stack_size, (unsigned int)segalign);
        } else {
  ...
  if (filetype == MH_EXECUTE || filetype == MH_BUNDLE ||
      ...
      filetype == MH_DYLINKER) {
    if (check_reserved_segment(SEG_LINKEDIT,
      ...
      if (seglinkedit == TRUE) {
        ...
        if (nindirectsyms != 0)
          linkedit_segment.sg.filesize += nindirectsyms * sizeof(unsigned long);
        if (strip_level != STRIP_ALL)
          linkedit_segment.sg.filesize +=
              (nmerged_symbols - nstripped_merged_symbols + nlocal_symbols -
               nmerged_symbols_referenced_only_from_dylibs) *
                  sizeof(struct nlist) +
              rnd(merged_string_size + local_string_size + STRING_SIZE_OFFSET,
  ...
  if (filetype == MH_EXECUTE) {
    if (check_reserved_segment(SEG_PAGEZERO,
                               "segment " SEG_PAGEZERO
      ...
      for (i = 0; i < nsegment_specs; i++) {
        if (segment_specs[i].addr_specified && segment_specs[i].addr == 0 &&
      ...
      if (address_zero_specified == FALSE &&
        ...
        if (pagezero_size != 0)
          pagezero_segment.sg.vmsize = rnd(pagezero_size, segalign);
        else
  ...
  if (segs_read_only_addr_specified) {
    if (segs_read_only_addr % segalign != 0)
      fatal("-segs_read_only_addr: 0x%x not a multiple of the segment"
            " alignment (0x%x)",
            (unsigned int)segs_read_only_addr, (unsigned int)segalign);
    if (segs_read_write_addr_specified) {
      if (segs_read_write_addr % segalign != 0)
        fatal("-segs_read_write_addr: 0x%x not a multiple of the "
              "segment alignment (0x%x)",
              (unsigned int)segs_read_write_addr, (unsigned int)segalign);
    } else {
      segs_read_write_addr =
          segs_read_only_addr + get_shared_region_size_from_flag(&arch_flag);
    }
  }
    ...
    if (seg1addr_specified == TRUE) {
      if (seg1addr % segalign != 0)
        fatal("-seg1addr: 0x%x not a multiple of the segment "
              "alignment (0x%x)",
              (unsigned int)seg1addr, (unsigned int)segalign);
      ...
      if (first_msg->addr_set == FALSE) {
        if (filetype == MH_EXECUTE && pagezero_segment.addr_set == TRUE)
  ...
  while (mdl != NULL) {
    ...
    if (headerpad_max_install_names == TRUE) {
      if (mdl->dl->cmdsize - sizeof(struct dylib_command) < MAXPATHLEN)
        headerpad +=
            MAXPATHLEN - (mdl->dl->cmdsize - sizeof(struct dylib_command));
    }
    mdl = mdl->next;
  }
    ...
    for (dp = dynamic_libs; dp != NULL; dp = dp->next) {
      if (dp->type == DYLIB) {
        if (dp->pbdylib != NULL) {
           ...
           */
          if (headerpad_max_install_names == TRUE) {
            if (dp->pbdylib->cmdsize - sizeof(struct prebound_dylib_command) <
                MAXPATHLEN)
              headerpad += MAXPATHLEN - (dp->pbdylib->cmdsize -
  ...
  if (output_uuid_info.suppress != TRUE &&
      (output_uuid_info.emit == TRUE || arch_flag.cputype == CPU_TYPE_ARM)) {
  ...
  if (output_thread_info.thread_in_output == TRUE) {
    output_thread_info.thread_command.cmdsize =
        sizeof(struct thread_command) + 2 * sizeof(long);
    if (arch_flag.cputype == CPU_TYPE_MC680x0) {
      ...
      output_thread_info.thread_command.cmdsize +=
          sizeof(long) * M68K_THREAD_STATE_REGS_COUNT;
      ...
      output_thread_info.thread_command.cmdsize +=
          sizeof(long) * PPC_THREAD_STATE_COUNT;
      ...
      output_thread_info.thread_command.cmdsize +=
          sizeof(long) * M88K_THREAD_STATE_GRF_COUNT;
      ...
      output_thread_info.thread_command.cmdsize +=
          sizeof(long) * I860_THREAD_STATE_REGS_COUNT;
      ...
      output_thread_info.thread_command.cmdsize +=
          sizeof(long) * i386_THREAD_STATE_COUNT;
      ...
      output_thread_info.entry_point = (int *)&(hppa_frame_state.ts_pcoq_front);
      output_thread_info.state = &hppa_frame_state;
      output_thread_info.thread_command.cmdsize +=
          sizeof(long) * HPPA_FRAME_THREAD_STATE_COUNT;
      if (stack_addr_specified == TRUE) {
        output_thread_info.second_flavor = HPPA_INTEGER_THREAD_STATE;
        output_thread_info.second_count = HPPA_INTEGER_THREAD_STATE_COUNT;
        output_thread_info.stack_pointer = (int *)&(hppa_integer_state.ts_gr30);
        output_thread_info.second_state = &hppa_integer_state;
        output_thread_info.thread_command.cmdsize +=
            sizeof(long) * HPPA_INTEGER_THREAD_STATE_COUNT + 2 * sizeof(long);
      }
      ...
      output_thread_info.thread_command.cmdsize +=
          sizeof(long) * SPARC_THREAD_STATE_REGS_COUNT;
      ...
      output_thread_info.thread_command.cmdsize +=
          sizeof(long) * ARM_THREAD_STATE_COUNT;
    } else {
  ...
  if (filetype == MH_EXECUTE || filetype == MH_BUNDLE || filetype == MH_DYLIB) {
    if (first_msg != NULL) {
      ...
      if (*content) {
        ...
        if (errors == 0) {
          pad = ((rnd(size + rnd(headers_size, max_first_align), segalign) -
  ...
  while (*p) {
    ...
    if (msg != &pagezero_segment && msg != &linkedit_segment &&
        msg != &stack_segment) {
      if (msg == first_msg && (filetype == MH_EXECUTE ||
                               filetype == MH_BUNDLE || filetype == MH_FVMLIB ||
      ...
      while (*content) {
        ...
        if ((unsigned long)(1 << ms->s.align) > segalign)
          error("alignment (0x%x) of section (%.16s,%.16s) "
                "greater than the segment alignment (0x%x)",
                (unsigned int)(1 << ms->s.align), ms->s.segname, ms->s.sectname,
  ...
  if (strip_level != STRIP_ALL)
    allocate_size +=
        output_symtab_info.symtab_command.nsyms * sizeof(struct nlist) +
        output_symtab_info.symtab_command.strsize;
  ...
  if (allocate_size > sa_rld_output_size)
    fatal("not enough memory for output of size %lu (memory "
          "available %lu)",
          allocate_size, sa_rld_output_size);
    ...
    if (msg->addr_set == FALSE) {
      if (segs_read_only_addr_specified) {
        if ((msg->sg.initprot & VM_PROT_WRITE) == 0)
          msg->sg.vmaddr = next_vmaddr(segs_read_only_addr, msg->sg.vmsize);
        else
          msg->sg.vmaddr = next_vmaddr(segs_read_write_addr, msg->sg.vmsize);
      } else {
        msg->sg.vmaddr = next_vmaddr(first_msg->sg.vmaddr, msg->sg.vmsize);
      }
      msg->addr_set = TRUE;
    }
    if (msg != &pagezero_segment && msg != &linkedit_segment &&
   ...
   */
  if (filetype == MH_EXECUTE && first_msg != NULL &&
      first_msg != &linkedit_segment)
    define_link_editor_execute_symbols(first_msg->sg.vmaddr);
  if ((filetype == MH_BUNDLE || filetype == MH_DYLIB ||
       filetype == MH_DYLINKER) &&
  ...
  if (output_thread_info.thread_in_output == TRUE) {
    if (entry_point_name != NULL) {
      ...
      if (merged_symbol->name_len == 0 ||
        ...
        fatal("entry point symbol name: %s not defined", entry_point_name);
      *output_thread_info.entry_point = merged_symbol->nlist.n_value;
    } else {
      *output_thread_info.entry_point = first_msg->content_sections->s.addr;
    }
    /*
    ...
    if (arch_flag.cputype == CPU_TYPE_HPPA)
      hppa_frame_state.ts_pcoq_back = hppa_frame_state.ts_pcoq_front + 4;
    /*
  ...
  if (output_routines_info.routines_in_output == TRUE) {
    if (init_name != NULL) {
      ...
      if (merged_symbol->name_len == 0 ||
          merged_symbol->nlist.n_type == (N_EXT | N_UNDF))
        fatal("initialization routine symbol name: %s not defined", init_name);
...
static unsigned long next_vmaddr(unsigned long vmaddr, unsigned long vmsize) {
   ...
   */
  sorted_merged_segments =
      (struct merged_segment **)allocate(n * sizeof(struct merged_segment *));
  i = 0;
  for (msg = merged_segments; msg != NULL; msg = msg->next) {
    if (msg->addr_set == TRUE && msg->sg.vmsize != 0) {
    ...
    while (*content) {
      ...
      if (ms->contents_filename != NULL)
        print("section (%.16s,%.16s) created from file "
              "%s\n",
              ms->s.segname, ms->s.sectname, ms->contents_filename);
      content = &(ms->next);
    }
    /*
...
static void check_overlap(struct merged_segment *msg1,
  ...
  if (prebind_check == FALSE)
    error("%.16s segment (address = 0x%x size = 0x%x) of %s overlaps "
          ...
          (unsigned int)(msg1->sg.vmsize), msg1->filename, msg2->sg.segname,
          (unsigned int)(msg2->sg.vmaddr), (unsigned int)(msg2->sg.vmsize),
          msg2->filename);
  else {
    if ((segs_read_only_addr_specified &&
         ((msg1 == outputs_linkedit_segment && msg2->split_dylib == TRUE) ||
          (msg2 == outputs_linkedit_segment && msg1->split_dylib == TRUE))) ||
    ...
    warning("prebinding%s disabled because (%.16s segment (address = "
            ...
            not, msg1->sg.segname, (unsigned int)(msg1->sg.vmaddr),
            (unsigned int)(msg1->sg.vmsize), msg1->filename, msg2->sg.segname,
            (unsigned int)(msg2->sg.vmaddr), (unsigned int)(msg2->sg.vmsize),
            msg2->filename);
    if (prebind_allow_overlap == TRUE)
...
static void check_for_lazy_pointer_relocs_too_far(void) {
  ...
  while (*p && prebinding) {
    ...
    while (*content && prebinding) {
      ms = *content;
      if ((ms->s.flags & SECTION_TYPE) == S_LAZY_SYMBOL_POINTERS) {
...
static void print_load_map(void) {
  ...
  for (msg = merged_segments; msg; msg = msg->next) {
    print("%-16.16s %-16.16s 0x%08x 0x%08x\n", msg->sg.segname, "",
          (unsigned int)(msg->sg.vmaddr), (unsigned int)(msg->sg.vmsize));
    for (ms = msg->content_sections; ms; ms = ms->next) {
      print("%-16.16s %-16.16s 0x%08x 0x%08x", ms->s.segname, ms->s.sectname,
      ...
      if (ms->contents_filename)
        ...
        if (ms->order_load_maps) {
          ...
          for (i = 0; i < ms->norder_load_maps; i++) {
            if (dead_strip == FALSE ||
                ms->order_load_maps[i].load_order->fine_reloc->live == TRUE) {
              print("\t\t\t\t  0x%08x 0x%08x ",
      ...
      if (ms->order_load_maps) {
        for (i = 0; i < ms->norder_load_maps; i++) {
          ...
          /* old
             ...
             + ms->order_load_maps[i].section_map-> output_section->s.addr),
                                          (unsigned
             int)(ms->order_load_maps[i].size));
          */
          if (ms->order_load_maps[i].archive_name != NULL)
            print("%s:", ms->order_load_maps[i].archive_name);
          print("%s:%s\n", ms->order_load_maps[i].object_name,
        ...
        if (common_load_map.common_ms == ms) {
          ...
          for (i = 0; i < common_load_map.ncommon_symbols; i++) {
            print("\t\t\t\t  0x%08x 0x%08x symbol: %s\n",
                  (unsigned int)(common_symbol->merged_symbol->nlist.n_value),
      ...
      for (j = 0; j < object_file->nsection_maps; j++) {
        if (object_file->section_maps[j].output_section == ms &&
          ...
          if (object_file->section_maps[j].nfine_relocs != 0) {
            ...
            for (k = 0; k < object_file->section_maps[j].nfine_relocs; k++) {
              print("  (input address 0x%08x) ",
                    (unsigned int)(object_file->section_maps[j].s->addr +
                                   fine_relocs[k].input_offset));
              if ((object_file->section_maps[j].s->flags & SECTION_TYPE) ==
                      S_SYMBOL_STUBS ||
                  (object_file->section_maps[j].s->flags & SECTION_TYPE) ==
                ...
                if (fine_relocs[k].use_contents == FALSE)
                  print("(eliminated)    ");
                else if (dead_strip == TRUE && fine_relocs[k].live == FALSE)
                  print("(dead stripped) ");
                else
                  print("     0x%08x ",
                ...
                print(
                    "     0x%08x ",
                    (unsigned int)(ms->s.addr + fine_relocs[k].output_offset));
              print(
                  "0x%08x ",
                  (unsigned int)(k == (unsigned int)((object_file
                                                          ->section_maps[j]
                                                          .nfine_relocs) -
                                                     (unsigned int)1)
                                     ? (unsigned int)(object_file
                                                          ->section_maps[j]
                                                          .s->size) -
            ...
            print("\t\t\t\t  0x%08x 0x%08x ",
                  ...
                  (unsigned int)(object_file->section_maps[j].s->size));
            print_obj_name(object_file);
            print("\n");
          }
        }
 ...
 */
__private_extern__ void print_mach_header(void) {
 ...
 * debugging.
 */
__private_extern__ void print_symtab_info(void) {
 ...
 * debugging.
 */
__private_extern__ void print_thread_info(void) {
 ...
 */
static int get_stack_direction_from_flag(const struct arch_flag *flag) {
  if (flag->cputype == CPU_TYPE_MC680x0 || flag->cputype == CPU_TYPE_MC88000 ||
      flag->cputype == CPU_TYPE_POWERPC || flag->cputype == CPU_TYPE_I386 ||
      flag->cputype == CPU_TYPE_SPARC || flag->cputype == CPU_TYPE_I860 ||
      flag->cputype == CPU_TYPE_VEO || flag->cputype == CPU_TYPE_ARM)
    return (-1);
