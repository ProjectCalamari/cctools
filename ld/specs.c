 ...
 */
__private_extern__ struct segment_spec *create_segment_spec(char *segname) {
  unsigned long i;
  ...
  memset(&(segment_specs[nsegment_specs]), '\0', sizeof(struct segment_spec));
 ...
 */
__private_extern__ struct section_spec *
create_section_spec(struct segment_spec *seg_spec, char *sectname) {
  unsigned long i;
  ...
  seg_spec->section_specs =
      reallocate(seg_spec->section_specs,
                 (seg_spec->nsection_specs + 1) * sizeof(struct section_spec));
 ...
 */
__private_extern__ struct segment_spec *lookup_segment_spec(char *segname) {
  unsigned long i;
 ...
 */
__private_extern__ struct section_spec *lookup_section_spec(char *segname,
                                                            char *sectname) {
  unsigned long i, j;
  ...
  for (i = 0; i < nsegment_specs; i++) {
    if (strcmp(segment_specs[i].segname, segname) == 0) {
      for (j = 0; j < segment_specs[i].nsection_specs; j++) {
        if (strcmp(segment_specs[i].section_specs[j].sectname, sectname) == 0)
 ...
 * a file, etc).
 */
__private_extern__ void process_section_specs(void) {
  unsigned long i, j, k;
    ...
    for (j = i + 1; j < nsegment_specs; j++) {
      if (strncmp(segment_specs[i].segname, segment_specs[j].segname,
        ...
        error("segment names: %s and %s not unique to %lu "
              "characters\n",
              segment_specs[i].segname, segment_specs[j].segname,
              sizeof(s.segname));
    }
    for (j = 0; j < segment_specs[i].nsection_specs; j++) {
    ...
    for (j = 0; j < segment_specs[i].nsection_specs; j++) {
        ...
        if (sect_spec->contents_filename == NULL) {
          ...
          if (sect_spec->order_filename)
            warning("section (%s,%s) is to be created from "
                    ...
                    segment_specs[i].segname, sect_spec->sectname,
                    sect_spec->contents_filename, sect_spec->order_filename);
          /*
           ...
           */
          strncpy(s.segname, segment_specs[i].segname, sizeof(s.segname));
          strncpy(s.sectname, sect_spec->sectname, sizeof(s.segname));
          ...
          ms->s.size = rnd(sect_spec->file_size, 1 << ms->s.align);
          /*
      ...
      if (sect_spec->contents_filename != NULL)
        error("section (%.16s,%.16s) exist in the loaded object "
              ...
              ms->s.segname, ms->s.sectname, sect_spec->contents_filename);
      else {
        ...
        if (sect_spec->order_filename != NULL) {
          if (arch_flag.cputype == CPU_TYPE_I860)
            ...
            warning("for MH_DYLIB output files section (" SEG_OBJC
                    "," SECT_OBJC_MODULES ") can't be "
 ...
 * error.
 */
__private_extern__ void process_segment_specs(void) {
  unsigned long i;
  ...
  for (i = 0; i < nsegment_specs; i++) {
    ...
    if (msg != NULL) {
      if (segment_specs[i].addr_specified) {
        if (segment_specs[i].addr % segalign != 0) {
          error("address: 0x%x specified for segment: %s not a "
                ...
                (unsigned int)(segment_specs[i].addr), segment_specs[i].segname,
       ...
       */
      if (filetype == MH_OBJECT && (segment_specs[i].addr_specified ||
 ...
 */
__private_extern__ void print_segment_specs(void) {
  unsigned long i, j;
    ...
    for (j = 0; j < segment_specs[i].nsection_specs; j++) {
      ...
      if (segment_specs[i].section_specs[j].contents_filename != NULL) {
        ...
        print("\t    file_addr 0x%x\n",
              (unsigned int)(segment_specs[i].section_specs[j].file_addr));
        print("\t    file_size %lu\n",
      ...
      if (segment_specs[i].section_specs[j].order_filename != NULL) {
        ...
        print("\t    order_addr 0x%x\n",
              (unsigned int)(segment_specs[i].section_specs[j].order_addr));
        print("\t    order_size %lu\n",
...
__private_extern__ void print_prot(vm_prot_t prot) {
