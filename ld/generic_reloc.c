...
generic_reloc(char *contents, struct relocation_info *relocs,
              struct section_map *section_map, long pcrel_at_end_of_disp,
              struct live_refs *refs, unsigned long reloc_index) {
    ...
    if (r_type == GENERIC_RELOC_SECTDIFF ||
      ...
      if (i + 1 < section_map->s->nreloc) {
        ...
        if ((pair_reloc->r_address & R_SCATTERED) != 0) {
          ...
          pair_reloc = NULL;
          pair_r_type = (enum reloc_type_generic)spair_reloc->r_type;
  ...
  update_reloc:;
    ...
    if (save_reloc || output_for_dyld) {
      ...
      if (r_scattered == 0) {
        ...
        if (section_map->nfine_relocs == 0) {
          if (((sreloc->r_address + section_map->offset) & 0x00ffffff) !=
...
undef_bsearch(const unsigned long *index,
