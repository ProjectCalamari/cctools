...
static char *look_back_for_slash(char *name, char *p);
 ...
 */
__private_extern__ char *guess_short_name(char *name, enum bool *is_framework,
                                          char **return_suffix) {
...
guess_library:
  ...
  if (b == NULL) {
    ...
    c = strchr(name, '_');
    if (c != NULL && c != name &&
        (0 == strncmp("_debug", c, 6) || 0 == strncmp("_profile", c, 8))) {
      l = c - name;
      suffix = c;
      for (s = 0; suffix[s] != '.'; s++)
        ;
