...
#define DECLARE_VAR(var, type) static type *var##_pointer = 0
#define SETUP_VAR(var)                                                         \
  if (var##_pointer == 0) {                                                    \
    _dyld_lookup_and_bind(STRINGIFY(_##var), (uint32_t *)&var##_pointer, 0);   \
  }
...
const struct segment_command_64 *getsegbyname(char *segname) {
