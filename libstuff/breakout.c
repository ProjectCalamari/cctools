...
static void breakout_internal(char *filename, struct arch **archs,
                              uint32_t *narchs,
                              enum bool calculate_input_prebind_cksum,
                              struct ofile *ofile);
static void breakout_loop_through_archive(char *filename, struct arch *arch,
                                          struct ofile *ofile);
static void cksum_object(struct arch *arch,
                         enum bool calculate_input_prebind_cksum);
static struct arch *new_arch(struct arch **archs, uint32_t *narchs);
static struct member *new_member(struct arch *arch);
...
breakout_mem(void *membuf, uint32_t length, char *filename, struct arch **archs,
  ...
  *narchs = 0;
  ...
  breakout_internal(filename, archs, narchs, calculate_input_prebind_cksum,
                    ofile);
  ...
  if (ofile->file_type == OFILE_FAT && errors == 0) {
    ...
    do {
      ...
      if (ofile->arch_type == OFILE_ARCHIVE) {
        ...
        arch->lto = ofile->lto;
        if (ofile->fat_header->magic == FAT_MAGIC_64) {
          arch->unknown_addr = ofile->file_addr + arch->fat_arch64->offset;
          arch->unknown_size = arch->fat_arch64->size;
        } else {
          arch->unknown_addr = ofile->file_addr + arch->fat_arch->offset;
...
static void breakout_loop_through_archive(char *filename, struct arch *arch,
  ...
  if ((flag = ofile_first_member(ofile)) == TRUE && errors == 0) {
    ...
    while (flag == TRUE && errors == 0) {
      ...
      for (ar_name_size = ofile->member_name_size; ar_name_size > 1;
           ar_name_size--) {
      ...
      *ar_hdr = *(ofile->member_ar_hdr);
      ...
      memcpy(ar_hdr->ar_name, ar_name_buf, sizeof(ar_hdr->ar_name));
      sprintf(ar_size_buf, "%-*ld", (int)sizeof(ar_hdr->ar_size),
              (long unsigned int)size);
      memcpy(ar_hdr->ar_size, ar_size_buf, sizeof(ar_hdr->ar_size));
...
static void cksum_object(struct arch *arch,
  ...
  for (i = 0; i < ncmds && arch->object->cs == NULL; i++) {
   ...
   */
  if (calculate_input_prebind_cksum == FALSE || arch->object->cs == NULL ||
  ...
  if (arch->object->object_byte_sex != host_byte_sex) {
    if (arch->object->mh != NULL) {
      buf_size = sizeof(struct mach_header) + arch->object->mh->sizeofcmds;
      buf = allocate(buf_size);
      memcpy(buf, arch->object->mh, buf_size);
      if (swap_object_headers(arch->object->mh, arch->object->load_commands) ==
          FALSE)
        return;
    } else {
      buf_size = sizeof(struct mach_header_64) + arch->object->mh64->sizeofcmds;
      buf = allocate(buf_size);
...
static struct member *new_member(struct arch *arch) {
  ...
  arch->members =
      reallocate(arch->members, (arch->nmembers + 1) * sizeof(struct member));
