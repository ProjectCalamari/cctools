...
static int GetOpInfo(
    ...
    uint64_t
        OpSize, /* should always be passed as 4 for arm or 2 or 4 for thumb */
    uint64_t
        InstSize, /* should always be passed as 4 for arm or 2 or 4 for thumb */
  ...
  for (i = 0; i < nrelocs; i++) {
    ...
    if (r_address == sect_offset) {
      if (r_type == ARM_RELOC_HALF || r_type == ARM_RELOC_SECTDIFF ||
  ...
  offset = 0;
  if (reloc_found) {
    if (r_type == ARM_RELOC_HALF || r_type == ARM_RELOC_HALF_SECTDIFF) {
      if ((r_length & 0x1) == 1)
    ...
    if (r_scattered &&
        (r_type != ARM_RELOC_HALF && r_type != ARM_RELOC_HALF_SECTDIFF)) {
...
static int GetOpInfoOld(void *DisInfo, uint64_t Pc, uint64_t Offset,
                        uint64_t OpSize, int TagType, void *TagBuf) {
 ...
 */
static const char *
guess_cstring_pointer(const uint32_t value, const uint32_t ncmds,
  ...
  for (i = 0; i < ncmds; i++) {
    ...
    case LC_SEGMENT:
      ...
      for (j = 0; j < sg.nsects; j++) {
        ...
        if (section_type == S_CSTRING_LITERALS && value >= s.addr &&
            value < s.addr + s.size) {
 ...
 */
static enum bool get_literal_pool_value(bfd_vma pc, char const **name,
                                        uint32_t *pointer_value,
                                        struct disassemble_info *info) {
  int32_t reloc_found;
  uint32_t i, r_address, r_symbolnum, r_type, r_extern, r_length, r_value,
      r_scattered, pair_r_type, pair_r_value;
  uint32_t other_half;
  ...
  if (reloc_found && r_length == 2 &&
      (r_type == ARM_RELOC_SECTDIFF || r_type == ARM_RELOC_LOCAL_SECTDIFF)) {
    *name = guess_symbol(r_value, info->sorted_symbols, info->nsorted_symbols,
                         info->verbose);
  ...
  for (i = 0; i < ncmds; i++) {
    ...
    case LC_SEGMENT:
      ...
      for (j = 0; j < sg.nsects; j++) {
        ...
        if (swapped)
          swap_section(&s, 1, host_byte_sex);
        if (sect_addr == s.addr && pc >= s.addr && pc < s.addr + s.size &&
           ...
           */
          if (get_literal_pool_value(value, &name, &pointer_value, info) ==
              FALSE) {
            return (NULL);
          }
          /*
           ...
           * a cstring.
           */
          name = guess_cstring_pointer(pointer_value, ncmds, sizeofcmds,
                                       load_commands, object_byte_sex,
...
static const char *SymbolLookUp(void *DisInfo, uint64_t SymbolValue,
  ...
  if (*ReferenceType == LLVMDisassembler_ReferenceType_In_Branch) {
    *ReferenceName = guess_indirect_symbol(
        ...
        info->symbols, NULL, info->nsymbols, info->strings, info->strings_size);
    ...
    *ReferenceName = guess_literal_pointer(
        (uint32_t)SymbolValue, (uint32_t)ReferencePC, ReferenceType, info);
    if (*ReferenceName == NULL)
      *ReferenceType = LLVMDisassembler_ReferenceType_InOut_None;
  } else if (SymbolName != NULL && strncmp(SymbolName, "__Z", 3) == 0) {
    if (info->demangled_name != NULL)
 ...
 */
static uint32_t print_data_in_code(unsigned char *sect, uint32_t sect_left,
                                   uint32_t dice_left, uint16_t kind) {
  ...
  case DICE_KIND_DATA:
    if (left >= 4) {
      value = sect[3] << 24 | sect[2] << 16 | sect[1] << 8 | sect[0];
      if (!Xflag && !gflag && !no_show_raw_insn)
        printf("\t%08x", value);
      printf("\t.long %u\t@ ", value);
      size = 4;
    } else if (left >= 2) {
      value = sect[1] << 8 | sect[0];
      if (!Xflag && !gflag && !no_show_raw_insn)
        printf("\t    %04x", value);
...
static uint32_t read_up_to_word(char *sect, enum byte_sex object_byte_sex,
                                uint32_t len) {
  enum byte_sex host_byte_sex;
  enum bool swapped;
...
static void set_thumb_mode(uint32_t addr, uint32_t nsorted_symbols,
                           struct symbol *sorted_symbols, enum bool *in_thumb);
...
uint32_t arm_disassemble(
    char *sect, uint32_t left, uint32_t addr, uint32_t sect_addr,
    enum byte_sex object_byte_sex, struct relocation_info *relocs,
    uint32_t nrelocs, struct nlist *symbols, uint32_t nsymbols,
    ...
    uint32_t ncmds, uint32_t sizeofcmds, cpu_subtype_t cpusubtype,
    enum bool verbose, LLVMDisasmContextRef arm_dc,
    LLVMDisasmContextRef thumb_dc, char *object_addr, uint64_t object_size,
    struct data_in_code_entry *dices, uint32_t ndices, uint64_t seg_addr,
  ...
  if (ndices) {
    ...
    for (size_t i = 0; i < ndices; i++) {
      if (offset >= dices[i].offset &&
        ...
        bytes_consumed = print_data_in_code(
            (unsigned char *)sect, left,
            dices[i].offset + dices[i].length - offset, dices[i].kind);
  ...
  dst[4095] = '\0';
  bytes_consumed = (uint32_t)llvm_disasm_instruction(
      in_thumb ? thumb_dc : arm_dc, (uint8_t *)sect, left, addr, dst, 4095);
  ...
  while (high >= low) {
    if (sorted_symbols[mid].n_value == addr) {
      ...
      do {
        ...
        mid++;
        if (mid > nsorted_symbols || sorted_symbols[mid].n_value != addr) {
          *in_thumb = FALSE;
          return;
        }
      } while (1);
