 ...
 */
__private_extern__ int execute(char **argv, int verbose) {
  char *name, **p;
 ...
 */
__private_extern__ void add_execute_list(char *str) {
  ...
  if (runlist.next + 1 >= runlist.size) {
    runlist.strings =
        reallocate(runlist.strings, (runlist.size * 2) * sizeof(char **));
    runlist.size *= 2;
  }
 ...
 */
__private_extern__ void add_execute_list_with_prefix(char *str) {
 ...
 */
__private_extern__ char *cmd_with_prefix(char *str) {
  int i;
  char *p;
 ...
 */
__private_extern__ void reset_execute_list(void) { runlist.next = 0; }

/*
 * This routine calls execute() to run the command built up in the runlist
 * strings.
 */
__private_extern__ int execute_list(int verbose) {
