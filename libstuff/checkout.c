...
static void check_object(struct arch *arch, struct member *member,
                         struct object *object);

static void symbol_string_at_end(struct arch *arch, struct member *member,
                                 struct object *object);

static void dyld_order(struct arch *arch, struct member *member,
                       struct object *object);

static void order_error(struct arch *arch, struct member *member, char *reason);

__private_extern__ void checkout(struct arch *archs, uint32_t narchs) {
  uint32_t i, j;
...
static void check_object(struct arch *arch, struct member *member,
  ...
  for (i = 0; i < ncmds; i++) {
    if (lc->cmd == LC_SYMTAB) {
      ...
      if (object->lazyLoads == NULL) {
        object->lazyLoads = (struct LazyLoadCommandAndData *)malloc(
            sizeof(struct LazyLoadCommandAndData));
        object->nlazyLoads = 0;
      } else {
        object->lazyLoads = (struct LazyLoadCommandAndData *)realloc(
            object->lazyLoads,
            sizeof(struct LazyLoadCommandAndData) * (object->nlazyLoads + 1));
      }
      object->lazyLoads[object->nlazyLoads].lc =
      ...
      object->notes = (struct note_command **)realloc(
          object->notes, sizeof(struct note_command *) * (object->nnote + 1));
  ...
  if ((object->mh_filetype == MH_DYLIB ||
    ...
    fatal_arch(arch, member,
               ...
               "command in %s file): ",
               object->mh_filetype == MH_DYLIB ? "MH_DYLIB" : "MH_DYLIB_STUB");
  if (object->hints_cmd != NULL) {
  ...
  if (object->dyst == NULL) {
    ...
    } else if (flags & MH_DYLDLINK || object->mh_filetype == MH_KEXT_BUNDLE) {
      /*
...
static void dyld_order(struct arch *arch, struct member *member,
  ...
  if (object->mh != NULL) {
    ...
    if (object->seg_bitcode != NULL) {
      ...
      if (object->seg_bitcode->fileoff + object->seg_bitcode->filesize !=
          offset)
        fatal_arch(arch, member,
                   "the __LLVM segment not directly "
                   "before the " SEG_LINKEDIT " segment in: ");
      if (object->seg_bitcode->vmaddr + object->seg_bitcode->vmsize !=
          object->seg_linkedit->vmaddr)
        fatal_arch(arch, member,
                   "the __LLVM segment's vmaddr plus "
                   "vmsize not directly before the vmaddr of the " SEG_LINKEDIT
                   " segment in: ");
    }
  } else {
    if (object->seg_linkedit64 == NULL)
    ...
    if (object->seg_linkedit64->filesize != 0 &&
        object->seg_linkedit64->fileoff + object->seg_linkedit64->filesize !=
            object->object_size)
      fatal_arch(arch, member,
                 "the " SEG_LINKEDIT " segment "
                 "does not cover the end of the file (can't "
    ...
    if (object->seg_bitcode64 != NULL) {
      if (object->seg_bitcode64->filesize < sizeof(struct xar_header))
        fatal_arch(arch, member,
                   "the __LLVM segment too small "
                   "(less than sizeof(struct xar_header)) in: ");
      if (object->seg_bitcode64->fileoff + object->seg_bitcode64->filesize !=
          offset)
        fatal_arch(arch, member,
                   "the __LLVM segment not directly "
                   "before the " SEG_LINKEDIT " segment in: ");
      if (object->seg_bitcode64->vmaddr + object->seg_bitcode64->vmsize !=
  ...
  if (object->dyld_info != NULL) {
    ...
    if (object->dyld_info->export_size != 0)
      offset = object->dyld_info->export_off + object->dyld_info->export_size;
      ...
      offset = object->dyld_info->bind_off + object->dyld_info->bind_size;
    else if (object->dyld_info->rebase_size != 0)
      offset = object->dyld_info->rebase_off + object->dyld_info->rebase_size;
  }
  if (object->dyld_chained_fixups != NULL) {
  ...
  if (object->mh64 != NULL && (object->dyst->nindirectsyms % 2) != 0) {
  ...
  if (object->dyst->ntoc != 0) {
    ...
    if (object->dyst->tocoff == offset) {
      ...
      object->input_indirectsym_pad = (uint32_t)(rounded_offset - offset);
      rounded_offset +=
  ...
  if (object->st->strsize != 0) {
    if (object->st->stroff != offset && object->st->stroff != rounded_offset)
  ...
  if (offset != object->object_size && rounded_offset != object->object_size)
    order_error(arch, member,
                "link edit information does not fill the " SEG_LINKEDIT
                " segment");
}

static void order_error(struct arch *arch, struct member *member,
                        char *reason) {
  fatal_arch(arch, member,
...
static void symbol_string_at_end(struct arch *arch, struct member *member,
  ...
  if (object->st != NULL && object->st->nsyms != 0) {
    ...
    if (object->code_sig_cmd != NULL) {
      sigend = object->code_sig_cmd->dataoff + object->code_sig_cmd->datasize;
      if (sigend != end)
        fatal_arch(arch, member,
    ...
    if (object->dyst != NULL && object->dyst->nindirectsyms != 0 &&
      ...
      if (object->mh != NULL) {
        if (object->st->symoff + object->st->nsyms * sizeof(struct nlist) !=
            end)
          fatal_arch(arch, member,
        ...
        if (object->st->symoff + object->st->nsyms * sizeof(struct nlist_64) !=
            end)
          fatal_arch(arch, member,
                     "symbol table does not "
                     "directly preceed the indirect symbol table (can't "
    ...
    if (object->seg_linkedit != NULL &&
      ...
      if (object->seg_linkedit->fileoff + object->seg_linkedit->filesize !=
          object->object_size)
        fatal_arch(arch, member,
                   "the " SEG_LINKEDIT " segment "
