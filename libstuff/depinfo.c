...
void depinfo_add(struct depinfo *depinfo, uint8_t opcode, const char *string) {
  ...
  depinfo->deps =
      reallocf(depinfo->deps, sizeof(struct dependency) * (depinfo->ndep + 1));
  if (!depinfo->deps)
    fatal("out of memory");
...
struct depinfo *depinfo_read(const char *path, uint32_t flags) {
  ...
  if (readed != size) {
    if (-1 == readed)
      ...
      error("can't write %zu bytes from %s, read %zd bytes", size, path,
            readed);
    close(fd);
    return NULL;
  }
  ...
  for (int i = 0; i < depinfo->ndep; ++i) {
    ...
    if (wrote != size) {
      if (-1 == wrote)
        ...
        error("can't write %zu bytes to %s, wrote %zd bytes", size, temp,
              wrote);
      close(fd);
      unlink(temp);
      return -1;
