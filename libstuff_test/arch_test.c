...
static void sig_handler(int signo) { longjmp(env, 1); }
...
static void test_get_arch_name_if_known(void) {
  ...
  check_arch_if_known("armv8.1m.main", CPU_TYPE_ARM,
...
static void test_arch_flag_values(void) {
  ...
  check_arch_values("x86_64", LITTLE_ENDIAN_BYTE_SEX, 0x7fff5fc00000, 0x1000,
                    0);
  check_arch_values("x86_64h", LITTLE_ENDIAN_BYTE_SEX, 0x7fff5fc00000, 0x1000,
                    0);
  check_arch_values("ppc970-64", BIG_ENDIAN_BYTE_SEX, 0x7ffff00000000, 0x1000,
                    0);
...
static int test_main(void) {
  ...
  if (!err)
    err = test_add("test get_arch_from_flag", test_get_arch_from_flag);
  if (!err)
    err = test_add("test get_arch_name_from_types",
                   test_get_arch_name_from_types);
  if (!err)
    err = test_add("test get_arch_name_if_known", test_get_arch_name_if_known);
  if (!err)
    err = test_add("test get_arch_family_from_cputype",
                   test_get_arch_family_from_cputype);
  if (!err)
    err = test_add("test arch_flag values", test_arch_flag_values);
  if (!err)
    err = test_add("test shared region size", test_shared_region_size);
  if (!err)
    err = test_add("test force cpusubtypes", test_force_cpusubtypes);
