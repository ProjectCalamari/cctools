...
void swap_fat_arch(struct fat_arch *fat_archs, uint32_t nfat_arch,
                   enum NXByteOrder target_byte_sex) {
  uint32_t i;
...
void swap_fat_arch_64(struct fat_arch_64 *fat_archs64, uint32_t nfat_arch,
                      enum NXByteOrder target_byte_sex) {
  uint32_t i;
...
void swap_dysymtab_command(struct dysymtab_command *dyst,
                           enum NXByteOrder target_byte_sex) {
...
void swap_symseg_command(struct symseg_command *ss,
                         enum NXByteOrder target_byte_sex) {
...
void swap_fvmlib_command(struct fvmlib_command *fl,
                         enum NXByteOrder target_byte_sex) {
...
void swap_dylib_command(struct dylib_command *dl,
                        enum NXByteOrder target_byte_sex) {
...
void swap_sub_framework_command(struct sub_framework_command *sub,
                                enum NXByteOrder target_byte_sex) {
...
void swap_sub_umbrella_command(struct sub_umbrella_command *usub,
                               enum NXByteOrder target_byte_sex) {
...
void swap_sub_library_command(struct sub_library_command *lsub,
                              enum NXByteOrder target_byte_sex) {
...
void swap_sub_client_command(struct sub_client_command *csub,
                             enum NXByteOrder target_byte_sex) {
...
void swap_prebound_dylib_command(struct prebound_dylib_command *pbdylib,
                                 enum NXByteOrder target_byte_sex) {
  ...
  pbdylib->linked_modules.offset = OSSwapInt32(pbdylib->linked_modules.offset);
}

void swap_dylinker_command(struct dylinker_command *dyld,
                           enum NXByteOrder target_byte_sex) {
...
void swap_fvmfile_command(struct fvmfile_command *ff,
                          enum NXByteOrder target_byte_sex) {
...
void swap_thread_command(struct thread_command *ut,
                         enum NXByteOrder target_byte_sex) {
  ut->cmd = OSSwapInt32(ut->cmd);
  ut->cmdsize = OSSwapInt32(ut->cmdsize);
}

void swap_ident_command(struct ident_command *id_cmd,
                        enum NXByteOrder target_byte_sex) {
  id_cmd->cmd = OSSwapInt32(id_cmd->cmd);
  id_cmd->cmdsize = OSSwapInt32(id_cmd->cmdsize);
}

void swap_routines_command(struct routines_command *r_cmd,
                           enum NXByteOrder target_byte_sex) {
...
void swap_routines_command_64(struct routines_command_64 *r_cmd,
                              enum NXByteOrder target_byte_sex) {
  r_cmd->cmd = OSSwapInt32(r_cmd->cmd);
  r_cmd->cmdsize = OSSwapInt32(r_cmd->cmdsize);
...
void swap_twolevel_hints_command(struct twolevel_hints_command *hints_cmd,
                                 enum NXByteOrder target_byte_sex) {
...
void swap_twolevel_hint(struct twolevel_hint *hints, uint32_t nhints,
                        enum NXByteOrder target_byte_sex) {
...
void swap_prebind_cksum_command(struct prebind_cksum_command *cksum_cmd,
                                enum NXByteOrder target_byte_sex) {
...
void swap_uuid_command(struct uuid_command *uuid_cmd,
                       enum NXByteOrder target_byte_sex) {
  uuid_cmd->cmd = OSSwapInt32(uuid_cmd->cmd);
  uuid_cmd->cmdsize = OSSwapInt32(uuid_cmd->cmdsize);
}

void swap_linkedit_data_command(struct linkedit_data_command *ld,
                                enum NXByteOrder target_byte_sex) {
...
void swap_version_min_command(struct version_min_command *ver_cmd,
                              enum NXByteOrder target_byte_sex) {
...
void swap_build_version_command(struct build_version_command *bv,
                                enum NXByteOrder target_byte_sex) {
...
void swap_build_tool_version(struct build_tool_version *btv, uint32_t ntools,
                             enum NXByteOrder target_byte_sex) {
  uint32_t i;
...
void swap_rpath_command(struct rpath_command *rpath_cmd,
                        enum NXByteOrder target_byte_sex) {
...
void swap_encryption_command(struct encryption_info_command *ec,
                             enum NXByteOrder target_byte_sex) {
...
void swap_encryption_command_64(struct encryption_info_command_64 *ec,
                                enum NXByteOrder target_byte_sex) {
  ec->cmd = OSSwapInt32(ec->cmd);
  ec->cmdsize = OSSwapInt32(ec->cmdsize);
  ec->cryptoff = OSSwapInt32(ec->cryptoff);
...
void swap_linker_option_command(struct linker_option_command *lo,
                                enum NXByteOrder target_byte_sex) {
...
void swap_dyld_info_command(struct dyld_info_command *ed,
                            enum NXByteOrder target_byte_sex) {
...
void swap_entry_point_command(struct entry_point_command *ep,
                              enum NXByteOrder target_byte_sex) {
...
void swap_note_command(struct note_command *nc,
                       enum NXByteOrder target_byte_sex) {
...
void swap_fileset_entry_command(struct fileset_entry_command *lc,
                                enum NXByteOrder target_byte_sex) {
  lc->cmd = OSSwapInt32(lc->cmd);
  lc->cmdsize = OSSwapInt32(lc->cmdsize);
...
void swap_ranlib(struct ranlib *ranlibs, uint32_t nranlibs,
                 enum NXByteOrder target_byte_sex) {
  uint32_t i;
...
void swap_ranlib_64(struct ranlib_64 *ranlibs, uint64_t nranlibs,
                    enum NXByteOrder target_byte_sex) {
  uint64_t i;
...
void swap_relocation_info(struct relocation_info *relocs, uint32_t nrelocs,
                          enum NXByteOrder target_byte_sex) {
  uint32_t i;
  enum NXByteOrder host_byte_sex;
...
void swap_indirect_symbols(uint32_t *indirect_symbols,
...
void swap_dylib_reference(struct dylib_reference *refs, uint32_t nrefs,
                          enum NXByteOrder target_byte_sex) {
...
void swap_dylib_module(struct dylib_module *mods, uint32_t nmods,
                       enum NXByteOrder target_byte_sex) {
  uint32_t i;
...
void swap_dylib_table_of_contents(struct dylib_table_of_contents *tocs,
                                  uint32_t ntocs,
                                  enum NXByteOrder target_byte_sex) {
  uint32_t i;
