 ...
 */
__private_extern__ void coalesced_section_merge(void *data,
 ...
 */
#ifdef notdef
        if ((object_symbols[i].n_type & (N_EXT | N_PEXT)) == (N_EXT | N_PEXT)) {
  ...
  if (start_section == FALSE) {
    error_with_cur_obj("malformed object (section (%.16s,%.16s) no "
                       "symbol at start of coalesced section)",
                       s->segname, s->sectname);
    return;
  }
...
set_load_orders:
  ...
  for (i = 0; i < cur_obj->symtab->nsyms; i++) {
    if ((object_symbols[i].n_type & N_TYPE) == N_SECT &&
        object_symbols[i].n_sect == nsect &&
        (object_symbols[i].n_type & N_STAB) == 0) {
      load_orders[j].name = object_strings + object_symbols[i].n_un.n_strx;
      load_orders[j].value = object_symbols[i].n_value;
      /*
...
#ifdef DEBUG
  if (debug & (1 << 14))
    print_load_order(load_orders, count, ms, cur_obj, "names and values");
#endif /* DEBUG */
   ...
   */
  qsort(load_orders, count, sizeof(struct load_order),
        (int (*)(const void *, const void *))qsort_load_order_values);
  /*
   * Set the input offset and size fields.
   */
  for (i = 0; i < count - 1; i++) {
    load_orders[i].input_offset = load_orders[i].value - s->addr;
    load_orders[i].input_size = load_orders[i + 1].value - load_orders[i].value;
  }
  load_orders[i].input_offset = load_orders[i].value - s->addr;
  load_orders[i].input_size = s->addr + s->size - load_orders[i].value;
#ifdef DEBUG
  if (debug & (1 << 15))
    print_load_order(load_orders, count, ms, cur_obj, "sizes and offsets");
#endif /* DEBUG */
...
deal_with_contents:
    ...
    if (load_orders[i].global_coalesced_symbol == TRUE) {
      ...
      if (merged_symbol->definition_object == cur_obj) {
        ...
        if (redo_live == FALSE || fine_relocs[i].live == TRUE) {
          ...
          ms->s.size = align_to_input_mod(
              ms->s.size, fine_relocs[i].input_offset, ms->s.align);
      ...
      if (redo_live == FALSE || fine_relocs[i].live == TRUE) {
        /* align size before using it to assign output offset */
        ms->s.size = align_to_input_mod(ms->s.size, fine_relocs[i].input_offset,
                                        ms->s.align);
        fine_relocs[i].output_offset = ms->s.size;
        ms->s.size += load_orders[i].input_size;
      }
  ...
  for (i = 0; i < count; i++) {
    ...
    printf("\tuse_contents = %s\n",
           fine_relocs[i].use_contents == TRUE ? "TRUE" : "FALSE");
    printf("\tlocal_symbol = %s\n",
           fine_relocs[i].local_symbol == TRUE ? "TRUE" : "FALSE");
   ...
   * * of the fine_relocs just created determines how many relocation entries
   * will be in the output for this section of this object file.
   */
  ...
  for (i = 0; i < s->nreloc; i++) {
    ...
    if (output_for_dyld && r_extern) {
      ...
      if (undefined_map != NULL) {
         ...
         * the same symbol.
         */
        if ((object_symbols[r_symbolnum].n_type & N_TYPE) == N_SECT &&
            (cur_obj->section_maps[object_symbols[r_symbolnum].n_sect - 1]
                 .s->flags &
             SECTION_TYPE) == S_COALESCED) {
          merged_symbol = lookup_symbol(
              object_strings + object_symbols[r_symbolnum].n_un.n_strx);
          if (merged_symbol->name_len == 0) {
          ...
          if (object_symbols[r_symbolnum].n_type != (N_EXT | N_UNDF)) {
     ...
     */
    if (output_for_dyld &&
        ((arch_flag.cputype == CPU_TYPE_POWERPC && r_type == PPC_RELOC_JBSR) ||
   ...
   */
  if (dead_strip == FALSE)
    discard_local_symbols_for_section(nsect, object_symbols, object_strings, s,
                                      section_map);
...
__private_extern__ void coalesced_section_order(void *data,
                                                struct merged_section *ms) {
#ifndef RLD
  kern_return_t r;
#ifdef __MWERKS__
...
coalesced_section_reset_live(void *data, struct merged_section *ms) {
