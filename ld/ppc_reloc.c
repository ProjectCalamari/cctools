 ...
 */
__private_extern__ void ppc_reloc(char *contents,
  ...
  for (i = reloc_index; i < section_map->s->nreloc; i++) {
    ...
    if (r_extern) {
      ...
      if (((merged_symbol->nlist.n_type & N_TYPE) == N_UNDF) ||
        ...
        if (r_type == PPC_RELOC_VANILLA) {
          ...
          switch (r_type) {
          case PPC_RELOC_HI16:
            offset = ((instruction & 0xffff) << 16) | other_half;
            break;
          case PPC_RELOC_LO16:
            offset = (other_half << 16) | (instruction & 0xffff);
            break;
          case PPC_RELOC_HA16:
            if ((other_half & 0x00008000) != 0)
              offset =
                  ((instruction & 0xffff) << 16) + (0xffff0000 + other_half);
            else
              offset = ((instruction & 0xffff) << 16) + other_half;
            break;
          case PPC_RELOC_LO14:
            offset = (other_half << 16) | (instruction & 0xfffc);
            break;
          case PPC_RELOC_BR14:
      ...
      if (r_symbolnum == R_ABS) {
        ...
        if (local_map->nfine_relocs == 0 &&
          ...
          if (r_type == PPC_RELOC_SECTDIFF ||
             ...
             */
            if (refs != NULL) {
              fine_reloc_output_ref(local_map, r_value - local_map->s->addr,
                                    &(refs->ref1));
              fine_reloc_output_ref(
                  local_map, pair_r_value - local_map->s->addr, &(refs->ref2));
              return;
            }
          ...
          if (r_type == PPC_RELOC_VANILLA ||
            ...
            case PPC_RELOC_HI16_SECTDIFF:
              ...
              instruction =
                  (instruction & 0xffff0000) | ((value >> 16) & 0xffff);
              break;
            case PPC_RELOC_LO16:
            case PPC_RELOC_LO16_SECTDIFF:
              other_half = (value >> 16) & 0xffff;
              instruction = (instruction & 0xffff0000) | (value & 0xffff);
              break;
            case PPC_RELOC_HA16:
            case PPC_RELOC_HA16_SECTDIFF:
              other_half = value & 0xffff;
            ...
            case PPC_RELOC_LO14_SECTDIFF:
              if ((value & 0x3) != 0)
                error_with_cur_obj("relocation error "
                                   ...
                                   section_map->s->sectname);
              other_half = (value >> 16) & 0xffff;
              instruction = (instruction & 0xffff0003) | (value & 0xfffc);
              break;
            case PPC_RELOC_BR14:
               ...
               */
              if (r_length == 3 && (instruction & 0xfc000000) == 0x40000000 &&
    ...
    if (r_type == PPC_RELOC_VANILLA || r_type == PPC_RELOC_LOCAL_SECTDIFF ||
        r_type == PPC_RELOC_SECTDIFF) {
      switch (r_length) {
      case 0: /* byte */
        value += get_byte((char *)(contents + r_address));
     ...
     */
      ...
      case PPC_RELOC_HI16_SECTDIFF:
        ...
        immediate += value;
        instruction = (instruction & 0xffff0000) | ((immediate >> 16) & 0xffff);
      ...
      case PPC_RELOC_HA16_SECTDIFF:
        ...
        if ((immediate & 0x00008000) != 0)
          ...
          instruction =
              (instruction & 0xffff0000) | ((immediate >> 16) & 0xffff);
        other_half = immediate & 0xffff;
        break;
      case PPC_RELOC_LO14:
      case PPC_RELOC_LO14_SECTDIFF:
        ...
        if ((immediate & 0x3) != 0)
          error_with_cur_obj("relocation error for relocation "
                             ...
                             "value not a multiple of 4 bytes)",
                             i, section_map->s->segname,
                             section_map->s->sectname);
        instruction = (instruction & 0xffff0003) | (immediate & 0xfffc);
        other_half = (immediate >> 16) & 0xffff;
        break;
      case PPC_RELOC_BR14:
        br14_disp_sign = (instruction & 0x8000);
      ...
      case PPC_RELOC_BR24:
        ...
        if ((immediate & 0xfe000000) != 0xfe000000 &&
          ...
          error_with_cur_obj("relocation overflow for relocation "
                             ...
                             i, section_map->s->segname,
                             section_map->s->sectname);
        instruction = (instruction & 0xfc000003) | (immediate & 0x03fffffc);
        break;
      case PPC_RELOC_JBSR:
        value += other_half;
        other_half = value;
      ...
      if (pair_r_type == PPC_RELOC_PAIR) {
        if (pair_reloc != NULL) {
          ...
          if (r_type == PPC_RELOC_JBSR &&
            ...
            sreloc = (struct scattered_relocation_info *)pair_reloc;
            r_scattered = 1;
            sreloc->r_scattered = r_scattered;
            sreloc->r_address = 0;
            sreloc->r_pcrel = r_pcrel;
