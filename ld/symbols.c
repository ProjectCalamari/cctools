 ...
 * symbol table.
 */
static int qsort_by_module(const struct merged_symbol **ms1,
                           const struct merged_symbol **ms2);
static int qsort_by_name(const struct merged_symbol **ms1,
                         const struct merged_symbol **ms2);
static int qsort_by_undef_order(const struct merged_symbol **ms1,
                                const struct merged_symbol **ms2);
...
static struct undefined_list free_list = {NULL, &free_list, &free_list};
/*
...
static void setup_link_edit_symbols_object(void);
...
static void
mark_N_NO_DEAD_STRIP_local_symbols_in_section_live(struct merged_section *ms);
static void removed_dead_local_symbols_in_section(struct merged_section *ms);
static void remove_dead_N_GSYM_stabs(void);
static void setup_link_editor_symbol(char *symbol_name);
static void define_link_editor_dylib_symbol(unsigned long header_address,
                                            char *symbol_name);
static void exports_list_processing(char *symbol_name, struct nlist *symbol);
static char *find_stab_name_end(char *name);
static char *find_stab_type_end(char *name_end);
#endif /* !defined(RLD) */
static enum bool is_type_stab(unsigned char n_type, char *symbol_name);
...
#ifndef RLD
__private_extern__ struct symtab_command link_edit_common_symtab = {
    LC_SYMTAB,                     /* cmd */
...
__private_extern__ struct object_file link_edit_common_object = {
...
static struct merged_symbol *
enter_symbol(struct merged_symbol *hash_pointer, struct nlist *object_symbol,
             char *object_strings, struct object_file *definition_object);
...
static char *enter_string(char *symbol_name, unsigned long *len_ret);
...
static void define_link_editor_symbol(char *symbol_name, unsigned char type,
                                      ...
                                      unsigned long value);
static void remove_dead_N_GSYM_stabs_for_cur_obj(struct nlist *object_symbols,
                                                 char *object_strings);
#endif /* !defined(RLD) */
static unsigned long merged_symbol_string_index(char *symbol_name);
static struct string_block *get_string_block(char *symbol_name);
static void get_stroff_and_mtime_for_N_OSO(unsigned long *stroff_for_N_OSO,
                                           unsigned long *mtime);
 ...
 * error.
 */
static inline void check_symbol(struct nlist *symbol, char *strings,
                                unsigned long index) {
  ...
  case N_UNDF:
    ...
    if (symbol->n_type & N_PEXT && symbol->n_value == 0) {
      error_with_cur_obj("undefined symbol %lu (%s) can't be "
                         "private external symbol (N_PEXT)",
                         index,
                         symbol->n_un.n_strx == 0
                             ? "NULL name"
                             : strings + symbol->n_un.n_strx);
    ...
    if ((symbol->n_type & N_STAB) == 0 &&
        (((struct mach_header *)(cur_obj->obj_addr))->flags & MH_TWOLEVEL) ==
            MH_TWOLEVEL) {
  ...
  case N_ABS:
    if ((symbol->n_type & N_STAB) == 0 && symbol->n_sect != NO_SECT) {
      error_with_cur_obj("symbol %lu (%s) must have NO_SECT for "
                         "its n_sect field given its type",
                         index,
                         symbol->n_un.n_strx == 0
                             ? "NULL name"
                             : strings + symbol->n_un.n_strx);
  ...
  if ((unsigned long)(symbol->n_sect) > cur_obj->nsection_maps) {
    error_with_cur_obj(
        "symbol %lu (%s)'s n_sect field (%d) is "
        "greater than the number of sections in this object (%lu)",
        index,
        symbol->n_un.n_strx == 0 ? "NULL name" : strings + symbol->n_un.n_strx,
        symbol->n_sect, cur_obj->nsection_maps);
    return;
  }
   ...
   * indirect section.
   */
  if ((symbol->n_type & N_EXT) != 0 && (symbol->n_type & N_TYPE) == N_SECT) {
    section_type =
        (cur_obj->section_maps[symbol->n_sect - 1].s->flags) & SECTION_TYPE;
  ...
  if ((symbol->n_type & N_STAB) == 0 &&
    ...
    if ((symbol->n_type & N_EXT) == 0 && (symbol->n_type & N_PEXT) != N_PEXT) {
   ...
   */
  if ((symbol->n_type & N_TYPE) == N_SECT &&
      ((cur_obj->section_maps[symbol->n_sect - 1].s->flags) & S_ATTR_DEBUG) ==
          S_ATTR_DEBUG) {
    error_with_cur_obj(
        "malformed object, symbols not allowed in debug "
 ...
 * file.
 */
static inline void relocate_symbol(struct nlist *nlist,
                                   struct object_file *object_file) {
  ...
  if (section_map->nfine_relocs == 0)
    nlist->n_value += -section_map->s->addr +
                      section_map->output_section->s.addr + section_map->offset;
  else
    nlist->n_value = fine_reloc_output_offset(
                         section_map, nlist->n_value - section_map->s->addr) +
 ...
 */
static enum bool lookup_and_enter_include(char *include_file_name,
                                          unsigned long sum,
                                          unsigned long index,
  ...
  for (;;) {
    if (p->sum == sum && strcmp(p->include_file_name, include_file_name) == 0)
#ifdef OPTIMISTIC
      /*
...
#endif /* OPTIMISTIC */
      {
        ...
        if (debug & (1 << 24))
          printf("include file = %s in object file = %s has same "
                 ...
                 include_file_name, cur_obj->file_name, p->object_file_name);
#endif /* DEBUG */
        return (FALSE);
      }
    if (p->next == NULL) {
      ...
      for (;;) {
        if (debug & (1 << 23) &&
            ...
            q->sum != sum && sum != 0 && q->sum != 0 && next_eincl == FALSE) {
          if (debug & (1 << 23))
 ...
 */
static size_t count_dwarf_symbols(const struct nlist *sym, size_t i,
                                  const size_t *debug_ptr) {
  size_t cnt;
  ...
  if ((sym->n_type & (N_TYPE | N_STAB)) != N_SECT &&
      ((sym->n_type & (N_TYPE | N_STAB)) != N_UNDF || sym->n_value == 0))
    return 0;
 ...
 * DWARF symbols.
 */
static void maybe_remove_dwarf_symbol(struct merged_symbol *merged_symbol) {
 ...
 */
__private_extern__ void merge_symbols(void) {
  ...
  for (i = 0; i < cur_obj->symtab->nsyms; i++) {
    ...
    if ((object_symbols[i].n_type & N_EXT) == N_EXT &&
      ...
      exports_list_processing(object_strings + object_symbols[i].n_un.n_strx,
                              object_symbols + i);
#endif /* !defined(RLD) */
    /*
  ...
  if (cur_obj != base_obj) {
    ...
    if (cur_obj->nundefineds != 0)
      cur_obj->undefined_maps =
          allocate(object_undefineds * sizeof(struct undefined_map));
  }
  ...
  if (filetype == MH_DYLIB && multi_module_dylib == TRUE) {
    ...
    cur_obj->nrefsym =
        cur_obj->nundefineds + cur_obj->nextdefsym + cur_obj->nprivatesym;
    cur_obj->irefsym = output_dysymtab_info.dysymtab_command.nextrefsyms;
    output_dysymtab_info.dysymtab_command.nextrefsyms += cur_obj->nrefsym;
    if (cur_obj->nrefsym != 0)
      cur_obj->reference_maps =
          allocate(cur_obj->nrefsym * sizeof(struct reference_map));
  }
#endif /* !defined(RLD) */
   ...
   */
  if (sect_object_symbols.specified && strip_level != STRIP_ALL &&
    ...
    if (sect_object_symbols.ms == NULL)
      sect_object_symbols.ms = lookup_merged_section(
          sect_object_symbols.segname, sect_object_symbols.sectname);
    if (sect_object_symbols.ms != NULL) {
        ...
        for (i = 0; i < cur_obj->nsection_maps; i++) {
          if (sect_object_symbols.ms ==
            ...
            if (cur_obj->ar_hdr == NULL)
              local_string_size += strlen(cur_obj->file_name) + 1;
            else
  ...
  for (i = 0; i < cur_obj->symtab->nsyms; i++) {
    ...
    if (object_symbols[i].n_type & N_EXT) {
      ...
      if (ntrace_syms != 0) {
        for (j = 0; j < ntrace_syms; j++) {
          if (strcmp(trace_syms[j],
            ...
            trace_object_symbol(&(object_symbols[i]), object_strings);
            break;
          }
        }
      }
      ...
      hash_pointer =
          lookup_symbol(object_strings + object_symbols[i].n_un.n_strx);
      if (hash_pointer->name_len == 0) {
        ...
        if (cur_obj != base_obj ||
          ...
          merged_symbol = enter_symbol(hash_pointer, &(object_symbols[i]),
      ...
      /* the symbol has been seen so merge it */
        ...
        if (merged_symbol->referenced_in_non_dylib == FALSE) {
          merged_symbol->nlist.n_un.n_name = enter_string(
              object_strings + object_symbols[i].n_un.n_strx, NULL);
          merged_symbol->referenced_in_non_dylib = TRUE;
          if (merged_symbol->non_dylib_referenced_obj == NULL)
            merged_symbol->non_dylib_referenced_obj = cur_obj;
        }
        ...
        if (object_symbols[i].n_type == (N_EXT | N_UNDF) &&
          ...
          if ((merged_symbol->nlist.n_type == (N_EXT | N_UNDF) &&
            ...
            if (merged_symbol->seen_undef == FALSE) {
               ...
               * setting.
               */
              if (((merged_symbol->nlist.n_desc & N_WEAK_REF) == N_WEAK_REF &&
                   (object_symbols[i].n_desc & N_WEAK_REF) != N_WEAK_REF) ||
                  ((merged_symbol->nlist.n_desc & N_WEAK_REF) != N_WEAK_REF &&
                   (object_symbols[i].n_desc & N_WEAK_REF) == N_WEAK_REF)) {
                if (weak_reference_mismatches == WEAK_REFS_MISMATCH_ERROR)
                  merged_symbol->weak_reference_mismatch = TRUE;
                else if (weak_reference_mismatches == WEAK_REFS_MISMATCH_WEAK)
                  merged_symbol->nlist.n_desc |= N_WEAK_REF;
         ...
         */
          ...
          if ((merged_symbol->nlist.n_type & N_EXT) == N_EXT &&
            ...
            if (merged_symbol->nlist.n_value != 0) {
              ...
              if (object_symbols[i].n_value > merged_symbol->nlist.n_value) {
                merged_symbol->nlist.n_value = object_symbols[i].n_value;
         ...
         */
           ...
           */
          n_desc |= (merged_symbol->nlist.n_desc & REFERENCED_DYNAMICALLY);
          /*
           ...
           * information.
           */
          if ((object_symbols[i].n_desc & N_WEAK_DEF) == N_WEAK_DEF)
            n_desc |= (merged_symbol->nlist.n_desc & N_WEAK_REF);
          merged_symbol->nlist.n_desc = object_symbols[i].n_desc | n_desc;
          if (merged_symbol->nlist.n_type == (N_EXT | N_INDR))
            enter_indr_symbol(merged_symbol, &(object_symbols[i]),
                              object_strings, cur_obj);
          else
            merged_symbol->nlist.n_value = object_symbols[i].n_value;
          merged_symbol->definition_object = cur_obj;
         ...
         */
        else if ((object_symbols[i].n_desc & N_WEAK_DEF) == N_WEAK_DEF &&
         ...
         * symbol is used.
         */
        else if ((merged_symbol->nlist.n_desc & N_WEAK_DEF) == N_WEAK_DEF ||
          ...
          if (filetype == MH_DYLIB && multi_module_dylib == TRUE &&
             ...
             */
            for (j = 0; j < merged_symbol->definition_object->nrefsym; j++) {
              if (merged_symbol->definition_object->reference_maps[j]
                      .merged_symbol == merged_symbol) {
                if (object_symbols[i].n_type & N_PEXT)
                  merged_symbol->definition_object->reference_maps[j].flags =
           ...
           */
          if (merged_symbol->nlist.n_desc & REFERENCED_DYNAMICALLY)
            merged_symbol->nlist.n_desc =
                object_symbols[i].n_desc | REFERENCED_DYNAMICALLY
                /*
         ...
         */
          ...
          multiply_defined(merged_symbol, &(object_symbols[i]), object_strings);
       ...
       */
      if (filetype == MH_DYLIB && multi_module_dylib == TRUE) {
        cur_obj->reference_maps[nrefsym].merged_symbol = merged_symbol;
        /*
        ...
        if (discarded_coalesced_symbol == TRUE) {
          ...
          cur_obj->reference_maps[nrefsym].flags = REFERENCE_FLAG_DEFINED;
        nrefsym++;
      }
#endif /* !defined(RLD) */
    } else if (cur_obj != base_obj || strip_base_symbols == FALSE) {
#ifndef RLD
      if (dead_strip && (object_symbols[i].n_type & N_STAB) == 0 &&
          (object_symbols[i].n_type & N_TYPE) == N_SECT &&
      ...
      if (strip_level != STRIP_DUP_INCLS &&
          is_output_local_symbol(
              object_symbols[i].n_type, object_symbols[i].n_sect,
        ...
        nlocal_symbols++;
        local_string_size +=
            object_symbols[i].n_un.n_strx == 0 ? 0 : output_strlen + 1;
      }
    }
    ...
    if (cur_obj->dwarf_name && !discarded_coalesced_symbol &&
      ...
      size_t n = count_dwarf_symbols(object_symbols + i, i, debug_ptr);
      cur_obj->nlocalsym += n;
      nlocal_symbols += n;
    }
  }
  ...
  if (strip_level == STRIP_DUP_INCLS) {
    ...
    for (i = 0; i < cur_obj->symtab->nsyms; i++) {
      ...
      localsym_block->input_N_BINCL_n_value = object_symbols[i].n_value;
      ...
      for (j = i + 1; j < cur_obj->symtab->nsyms; j++) {
        if (object_symbols[j].n_type == N_EINCL) {
          ...
          if (cur_localsym_block->count != 0) {
            temp_localsym_block = allocate(sizeof(struct localsym_block));
            memset(temp_localsym_block, '\0', sizeof(struct localsym_block));
            temp_localsym_block->state = PARSE_SYMBOLS;

            /* insert it after the current block */
            temp_localsym_block->next = cur_localsym_block->next;
          ...
          if ((object_symbols[j].n_type & N_STAB) != 0) {
            ...
            if (localsym_block->input_N_BINCL_n_value == 0 &&
              ...
              stab_string = object_strings + object_symbols[j].n_un.n_strx;
              for (; *stab_string != '\0'; stab_string++) {
                sum += *stab_string;
                if (*stab_string == '(') {
                  /* skip the file number */
                  stab_string++;
                  while (isdigit((unsigned char)*stab_string))
                    stab_string++;
                  stab_string--;
                } else if (*stab_string == '.' && stab_string[1] != '\0' &&
                           stab_string[1] == '_') {
      ...
      if (j == cur_obj->symtab->nsyms || no_exclusion == TRUE) {
        ...
        while (temp_localsym_block != NULL) {
          ...
          for (k = temp_localsym_block->index;
               k < temp_localsym_block->index + temp_localsym_block->count;
               k++) {
            /*
             ...
             */
            (void)is_output_local_symbol(
                object_symbols[k].n_type, object_symbols[k].n_sect,
                object_symbols[k].n_desc, cur_obj,
                object_symbols[k].n_un.n_strx == 0
                    ? ""
                    : object_strings + object_symbols[k].n_un.n_strx,
                &output_strlen);
            local_string_size +=
                object_symbols[k].n_un.n_strx == 0 ? 0 : output_strlen + 1;
          }
        ...
        i = i + localsym_block->count - 1; /* the loop will do i++ */
        localsym_block = localsym_block->next;
      } else {
        /*
         ...
         */
        include_file_name = object_strings + object_symbols[i].n_un.n_strx;
...
static void exports_list_processing(char *symbol_name, struct nlist *symbol) {
  ...
  if (save_symbols != NULL) {
    sp = bsearch(symbol_name, save_symbols, nsave_symbols,
                 ...
                 (int (*)(const void *, const void *))symbol_list_bsearch);
    if (sp != NULL) {
      sp->seen = TRUE;
    } else {
 ...
 * symbol for it.
 */
__private_extern__ struct merged_symbol *
command_line_symbol(char *symbol_name) {
  unsigned long i;
  struct merged_symbol *hash_pointer, *merged_symbol;
  ...
  if (ntrace_syms != 0) {
    for (i = 0; i < ntrace_syms; i++) {
      if (strcmp(trace_syms[i], symbol_name) == 0) {
        trace_symbol(symbol_name, &(undefined_symbol), command_line_object,
  ...
  if (hash_pointer->name_len == 0) {
     ...
     */
    merged_symbol = enter_symbol(hash_pointer, &(undefined_symbol), symbol_name,
                                 command_line_object);
    if (filetype == MH_DYLIB && multi_module_dylib == TRUE) {
      command_line_object->reference_maps = reallocate(
          command_line_object->reference_maps,
          (command_line_object->nrefsym + 1) * sizeof(struct reference_map));
      command_line_object->reference_maps[command_line_object->nrefsym].flags =
          REFERENCE_FLAG_UNDEFINED_NON_LAZY;
      command_line_object->reference_maps[command_line_object->nrefsym]
          .merged_symbol = merged_symbol;
  ...
  /* the symbol has been seen so just use it */
     ...
     */
    if (merged_symbol->referenced_in_non_dylib == FALSE)
      merged_symbol->nlist.n_un.n_name = enter_string(symbol_name, NULL);
  }
  merged_symbol->referenced_in_non_dylib = TRUE;
  if (merged_symbol->non_dylib_referenced_obj == NULL)
 ...
 */
__private_extern__ void command_line_indr_symbol(char *symbol_name,
                                                 char *indr_symbol_name) {
  unsigned long i, j;
  ...
  if (ntrace_syms != 0) {
    for (i = 0; i < ntrace_syms; i++) {
      if (strcmp(trace_syms[i], symbol_name) == 0) {
        trace_symbol(symbol_name, &(indr_symbol), command_line_object,
                     indr_symbol_name);
        was_traced = TRUE;
        break;
      }
      ...
      if (i == nmultiple_defs) {
        if (allow_multiply_defined_symbols == TRUE)
          ...
          error("multiple definitions of symbol %s",
                merged_symbol->nlist.n_un.n_name);
        multiple_defs =
            reallocate(multiple_defs, (nmultiple_defs + 1) * sizeof(char *));
  ...
  if (hash_pointer->name_len == 0) {
    ...
    for (i = 0; i < ntrace_syms; i++) {
      if (strcmp(trace_syms[i], indr_symbol_name) == 0) {
        trace_symbol(indr_symbol_name, &(undefined_symbol), command_line_object,
                     "error in trace_symbol()");
        break;
      }
    }
    ...
    if (merged_indr_symbol->non_dylib_referenced_obj == NULL)
      merged_indr_symbol->non_dylib_referenced_obj = command_line_object;
  } else {
  ...
  if (filetype == MH_DYLIB && multi_module_dylib == TRUE) {
    ...
    command_line_object->reference_maps = reallocate(
        command_line_object->reference_maps,
        (command_line_object->nrefsym + 2) * sizeof(struct reference_map));
    command_line_object->reference_maps[command_line_object->nrefsym + 0]
        .flags = REFERENCE_FLAG_DEFINED;
    command_line_object->reference_maps[command_line_object->nrefsym + 0]
        .merged_symbol = merged_symbol;
    command_line_object->reference_maps[command_line_object->nrefsym + 1]
        .flags = REFERENCE_FLAG_UNDEFINED_NON_LAZY;
    command_line_object->reference_maps[command_line_object->nrefsym + 1]
        .merged_symbol = merged_indr_symbol;
    command_line_object->irefsym =
        output_dysymtab_info.dysymtab_command.nextrefsyms;
...
merge_dylib_module_symbols(struct dynamic_library *dynamic_library) {
  ...
  symbols = (struct nlist *)(cur_obj->obj_addr + cur_obj->symtab->symoff);
  ...
  for (i = 0; i < cur_obj->dylib_module->nextdefsym; i++) {
    ...
    if (hash_pointer->name_len == 0) {
       ...
       */
      merged_symbol =
          enter_symbol(hash_pointer, &(pbud_symbol), symbol_name, cur_obj);
    } else {
      merged_symbol = hash_pointer;
      /*
      ...
      if ((merged_symbol->nlist.n_desc & N_WEAK_DEF) == N_WEAK_DEF ||
        ...
        if (filetype == MH_DYLIB && multi_module_dylib == TRUE &&
           ...
           * assume non-lazy to be safe.
           */
          for (k = 0; k < merged_symbol->definition_object->nrefsym; k++) {
            if (merged_symbol->definition_object->reference_maps[k]
                    .merged_symbol == merged_symbol) {
              if (symbols[k].n_type & N_PEXT)
                merged_symbol->definition_object->reference_maps[k].flags =
                    REFERENCE_FLAG_PRIVATE_UNDEFINED_NON_LAZY;
              else
                merged_symbol->definition_object->reference_maps[k].flags =
                    REFERENCE_FLAG_UNDEFINED_NON_LAZY;
      ...
      if ((((merged_symbol->nlist.n_type & N_TYPE) == N_SECT &&
          ...
          ((cur_obj->section_maps[symbols[j].n_sect - 1].s->flags) &
           SECTION_TYPE) == S_COALESCED) {
        continue;
      }
      /*
      ...
      if (merged_symbol->nlist.n_type != (N_UNDF | N_EXT)) {
        ...
        for (k = 0; k < nmultiple_defs; k++) {
          if (strcmp(multiple_defs[k], merged_symbol->nlist.n_un.n_name) == 0)
            break;
        }
        for (l = 0; l < ntrace_syms; l++) {
          if (strcmp(trace_syms[l], merged_symbol->nlist.n_un.n_name) == 0)
            break;
        }
        if (k == nmultiple_defs) {
          if (allow_multiply_defined_symbols == TRUE) {
                    ...
                    merged_symbol->nlist.n_un.n_name);
                     ...
                     (force_flat_namespace == FALSE &&
                      ((((struct mach_header *)(cur_obj->obj_addr))->flags &
                       ...
                       (merged_symbol->defined_in_dylib == TRUE &&
                        (((struct mach_header
                               *)(merged_symbol->definition_object->obj_addr))
                             ->flags &
                         MH_TWOLEVEL) == MH_TWOLEVEL)))) {
            if (multiply_defined_flag == MULTIPLY_DEFINED_WARNING) {
              warning("multiple definitions of symbol %s",
                      merged_symbol->nlist.n_un.n_name);
              if (nowarnings == TRUE)
                continue;
            } else if (multiply_defined_flag == MULTIPLY_DEFINED_ERROR) {
              error("multiple definitions of symbol %s",
                    merged_symbol->nlist.n_un.n_name);
            } else if (multiply_defined_flag == MULTIPLY_DEFINED_SUPPRESS)
              continue;
          } else {
            error("multiple definitions of symbol %s",
                  merged_symbol->nlist.n_un.n_name);
  ...
  use_symbol_definition_from_this_dylib:
     ...
     */
    if (save_reloc == FALSE && twolevel_namespace == TRUE &&
        merged_symbol->referenced_in_non_dylib == TRUE &&
        dynamic_library->definition_obj->library_ordinal == 0 &&
      ...
      error_with_cur_obj("illegal reference to symbol: %s defined in "
                         "indirectly referenced dynamic library %s",
                         symbol_name,
                         dynamic_library->dylib_file != NULL
                             ? dynamic_library->file_name
                             : dynamic_library->dylib_name);
      cur_obj = obj;
    ...
    if (twolevel_namespace == TRUE) {
      ...
      if (merged_symbol->itoc == 0) {
        ...
        toc = bsearch(merged_symbol->nlist.n_un.n_name, dynamic_library->tocs,
  ...
  if (Yflag && cur_obj->dylib_module->nrefsym != 0) {
    ...
    cur_obj->undefined_maps =
        allocate(nundefineds * sizeof(struct undefined_map));
  ...
  for (i = 0; i < cur_obj->dylib_module->nrefsym; i++) {
    ...
    if (flags == REFERENCE_FLAG_UNDEFINED_NON_LAZY ||
        ...
        for (k = 0; k < ntrace_syms; k++) {
          if (strcmp(trace_syms[k], symbol_name) == 0) {
            if (force_flat_namespace == TRUE ||
                (((struct mach_header *)(cur_obj->obj_addr))->flags &
                 MH_TWOLEVEL) != MH_TWOLEVEL) {
              trace_symbol(symbol_name, &(undefined_symbol), cur_obj,
                           "error in trace_symbol()");
            } else {
              print_obj_name(cur_obj);
              library_ordinal =
                  GET_LIBRARY_ORDINAL(symbols[refs[j].isym].n_desc);
      ...
      if (force_flat_namespace == TRUE)
        ...
        if ((((struct mach_header *)(cur_obj->obj_addr))->flags &
             MH_TWOLEVEL) == MH_TWOLEVEL) {
      ...
      if (Yflag) {
        cur_obj->undefined_maps[nundefineds++].merged_symbol = merged_symbol;
      }
    }
  }
    ...
    if (flags == REFERENCE_FLAG_PRIVATE_UNDEFINED_NON_LAZY ||
      ...
      if (k >= obj->dysymtab->nmodtab) {
        error_with_cur_obj("isym field (%u) of reference table "
                           ...
                           "symbols for any module",
                           refs[j].isym, j);
        return;
      }
      module_index = k;
      if (is_dylib_module_loaded(dynamic_library->mods + module_index) ==
          FALSE) {

        cur_obj = new_object_file();
        *cur_obj = *(dynamic_library->definition_obj);
        cur_obj->dylib_module = dynamic_library->mods + module_index;
 ...
 */
__private_extern__ void
merge_bundle_loader_symbols(struct dynamic_library *dynamic_library) {
...
is_output_local_symbol(unsigned char n_type, unsigned char n_sect,
                       unsigned char n_desc, struct object_file *obj,
                       char *symbol_name, unsigned long *output_strlen) {
#ifndef RLD
  char *end;
#endif /* !defined(RLD) */
  ...
  *output_strlen = 0;
  ...
  case STRIP_NONGLOBALS:
    return (FALSE);
  case STRIP_DEBUG:
    if (n_type & N_STAB || (*symbol_name == 'L' && (n_type & N_STAB) == 0) ||
        (save_reloc == FALSE && (n_type & N_TYPE) == N_SECT &&
         (obj->section_maps[n_sect - 1].s->flags & S_ATTR_STRIP_STATIC_SYMS) ==
             S_ATTR_STRIP_STATIC_SYMS))
      return (FALSE);
    else {
      *output_strlen = strlen(symbol_name);
  ...
  case STRIP_MIN_DEBUG:
    ...
    if (n_type & N_STAB) {
      ...
      case N_GSYM:
      case N_FUN:
        end = find_stab_type_end(find_stab_name_end(symbol_name));
        if (end != NULL)
...
static char *find_stab_name_end(char *name) {
  ...
  for (first_colon = strchr(name, ':');
       first_colon != NULL && first_colon[1] == ':' && first_colon[2] != '\0';
 ...
 */
__private_extern__ struct merged_symbol *lookup_symbol(char *symbol_name) {
  struct merged_symbol_chunk *p, *q;
  struct merged_symbol *sym;
  unsigned long hash_index, i, name_len;

  hash_index = hash_string(symbol_name, &name_len) % SYMBOL_LIST_HASH_SIZE;
  if (merged_symbol_root == NULL) {
    ...
    merged_symbol_root->list = allocate(sizeof(struct merged_symbol_list));
    memset(merged_symbol_root->list, 0, sizeof(struct merged_symbol_list));
 ...
 */
__private_extern__ void hash_instrument(void) {
  ...
  n = 0;
  u = 0;
  for (merged_symbol_list =
           merged_symbol_root == NULL ? NULL : merged_symbol_root->list;
  ...
  print("Number of used pointers in the lists = %lu (%.2f%%)\n", u,
        ((double)u) / ((double)(SYMBOL_LIST_HASH_SIZE * n)) * 100.0);
 ...
 */
static void add_to_symbol_list(struct merged_symbol *merged_symbol) {
  ...
  for (merged_symbol_list =
    ...
    if (merged_symbol_list->used != SYMBOL_LIST_HASH_SIZE) {
      merged_symbol_list->symbols[merged_symbol_list->used] = merged_symbol;
...
enter_symbol(struct merged_symbol *hash_pointer, struct nlist *object_symbol,
  ...
  if (cur_obj == base_obj && base_name == NULL)
    merged_symbol->nlist.n_un.n_name =
        object_strings + object_symbol->n_un.n_strx;
  else
#endif
    merged_symbol->nlist.n_un.n_name = enter_string(
        object_strings + object_symbol->n_un.n_strx, &merged_symbol->name_len);
  merged_symbol->definition_object = definition_object;
  add_to_symbol_list(merged_symbol);

  if (object_symbol->n_type == (N_UNDF | N_EXT) && object_symbol->n_value == 0)
    add_to_undefined_list(merged_symbol);
...
static void enter_indr_symbol(struct merged_symbol *merged_symbol,
  ...
  if (hash_pointer->name_len != 0) {
    ...
    if (definition_object != NULL && definition_object->dylib_module != NULL)
...
#ifdef RLD
    if (cur_obj == base_obj && base_name == NULL)
      indr_symbol->nlist.n_un.n_name = object_strings + object_symbol->n_value;
    else
#endif
      indr_symbol->nlist.n_un.n_name =
          enter_string(object_strings + object_symbol->n_value, NULL);
...
static char *enter_string(char *symbol_name, unsigned long *len_ret) {
  ...
  for (p = &(merged_string_blocks); *p; p = &(string_block->next)) {
    ...
    if ((strip_base_symbols == FALSE || string_block->base_strings == FALSE) &&
        string_block->dylib_strings == FALSE)
 ...
 */
__private_extern__ void
delete_from_undefined_list(struct undefined_list *undefined) {
 ...
 */
__private_extern__ void
trace_merged_symbol(struct merged_symbol *merged_symbol) {
  char *indr_symbol_name;

  if (merged_symbol->nlist.n_type == (N_INDR | N_EXT))
    indr_symbol_name = ((struct merged_symbol *)(merged_symbol->nlist.n_value))
                           ->nlist.n_un.n_name;
  else
    indr_symbol_name = "error in trace_symbol()";
 ...
 */
__private_extern__ void free_pass1_symbol_data(void) { free_undefined_list(); }
#endif /* !defined(RLD) */

/*
 * free_undefined_list() free's up the memory for the undefined list.
 */
__private_extern__ void free_undefined_list(void) {
 ...
 */
__private_extern__ void define_common_symbols(void) {
...
#ifdef RLD
  *(sets[cur_set].link_edit_common_object) = link_edit_common_object;
  sets[cur_set].link_edit_common_object->set_num = cur_set;
  sets[cur_set].link_edit_common_object->section_maps =
      sets[cur_set].link_edit_section_maps;
  *(sets[cur_set].link_edit_section_maps) = link_edit_section_maps;
  sets[cur_set].link_edit_section_maps->s =
      sets[cur_set].link_edit_common_section;
  *(sets[cur_set].link_edit_common_section) = link_edit_common_section;
#endif /* RLD */
  ...
  if (ms != NULL && (ms->s.flags & SECTION_TYPE) != S_ZEROFILL) {
      ...
      for (i = 0; i < object_list->used; i++) {
        ...
        for (j = 0; j < object_file->nsection_maps; j++) {
          ...
          if (strcmp(s->segname, SEG_DATA) == 0 &&
            ...
            print_obj_name(object_file);
            print("contains section (" SEG_DATA "," SECT_COMMON ")\n");
          }
        }
      }
    }
     ...
     */
    if (sect_spec != NULL && sect_spec->order_filename != NULL && ms != NULL) {
      ...
      if ((merged_symbol->nlist.n_type & N_EXT) == N_EXT &&
        ...
        if (filetype == MH_FVMLIB)
          error("common symbols not allowed with MH_FVMLIB "
                "output format");
        /*
         * If the output format is multi module MH_DYLIB then
         * * commons are not allowed because each symbol can only be defined in
         * at most one module.
         */
        if (filetype == MH_DYLIB && multi_module_dylib == TRUE)
  ...
  if ((sect_spec != NULL && sect_spec->order_filename != NULL) ||
    ...
    memset(link_edit_common_object.obj_addr, '\0',
           link_edit_common_object.obj_size);
    link_edit_common_object_mach_header =
        (struct mach_header *)link_edit_common_object.obj_addr;
    link_edit_common_object_mach_header->magic = MH_MAGIC;
    link_edit_common_object_mach_header->filetype = MH_OBJECT;
    link_edit_common_object_mach_header->flags = MH_SUBSECTIONS_VIA_SYMBOLS;
    ...
    common_load_map.common_symbols = allocate(common_load_map.ncommon_symbols *
  ...
  for (merged_symbol_list =
    ...
    for (i = 0; i < merged_symbol_list->used; i++) {
      ...
      if ((merged_symbol->nlist.n_type & N_EXT) == N_EXT &&
        ...
        align = 0;
        while ((unsigned long)(1 << align) < common_size && align < ms->s.align)
          align++;
        /* round the address of the section to this alignment */
#ifdef RLD
        sets[cur_set].link_edit_common_section->size =
            rnd(sets[cur_set].link_edit_common_section->size, 1 << align);
#else
        link_edit_common_section.size =
            rnd(link_edit_common_section.size, 1 << align);
#endif /* RLD */
        /*
         ...
         */
        merged_symbol->nlist.n_type =
            N_SECT | N_EXT | (merged_symbol->nlist.n_type & N_PEXT);
        ...
        merged_symbol->definition_object = &link_edit_common_object;
        /* Create the space for this symbol */
        ...
        if ((sect_spec != NULL && sect_spec->order_filename != NULL) ||
          ...
          strcpy(common_names + n_strx, merged_symbol->nlist.n_un.n_name);
          common_nlist++;
   ...
   */
#ifdef RLD
  sets[cur_set].link_edit_common_object->section_maps[0].output_section = ms;
  ms->s.size = rnd(ms->s.size, 1 << ms->s.align);
  sets[cur_set].link_edit_common_object->section_maps[0].offset = ms->s.size;
 ...
 */
__private_extern__ void define_undefined_symbols_a_way(void) {
  unsigned long i;
  struct merged_symbol_list *merged_symbol_list;
  struct merged_symbol *merged_symbol;
  ...
  for (merged_symbol_list =
    ...
    for (i = 0; i < merged_symbol_list->used; i++) {
      ...
      if (merged_symbol->nlist.n_type == (N_EXT | N_UNDF) &&
          merged_symbol->nlist.n_value == 0) {
        if (dynamic == TRUE && filetype != MH_EXECUTE &&
            merged_segments != NULL) {
          define_link_editor_symbol(merged_symbol->nlist.n_un.n_name,
 ...
 * symbol live.
 */
__private_extern__ void mark_globals_live(void) {
  unsigned long i;
  struct merged_symbol_list *merged_symbol_list;
  struct merged_symbol *merged_symbol;
  ...
  for (merged_symbol_list =
    ...
    for (i = 0; i < merged_symbol_list->used; i++) {
      ...
      if ((merged_symbol->nlist.n_desc & REFERENCED_DYNAMICALLY) ==
          ...
          (merged_symbol->nlist.n_desc & N_NO_DEAD_STRIP) == N_NO_DEAD_STRIP ||
 ...
 */
__private_extern__ void mark_N_NO_DEAD_STRIP_local_symbols_live(void) {
 ...
 */
__private_extern__ void set_fine_relocs_for_merged_symbols(void) {
  unsigned long i;
  struct merged_symbol_list *merged_symbol_list;
  struct merged_symbol *merged_symbol;
  ...
  for (merged_symbol_list =
    ...
    for (i = 0; i < merged_symbol_list->used; i++) {
       ...
       */
      merged_symbol->fine_reloc =
          get_fine_reloc_for_merged_symbol(merged_symbol, NULL);
    }
  }
}
 ...
 */
__private_extern__ void count_live_symbols(void) {
    ...
    for (i = 0; i < merged_symbol_list->used; i++) {
      ...
      if (merged_symbol->live == TRUE) {
        ...
        */
         ...
         */
        else if (filetype == MH_DYLIB && multi_module_dylib == TRUE)
          merged_symbol->definition_object->nextdefsym--;
      }
    }
  }
      ...
      for (i = 0; i < object_list->used; i++) {
        ...
        for (j = 0; j < obj->nrefsym; j++) {
          if (obj->reference_maps[j].merged_symbol->live) {
            obj->reference_maps[nrefsym] = obj->reference_maps[j];
            nrefsym++;
          }
        }
        obj->nrefsym = nrefsym;
        if (nrefsym != 0)
          obj->irefsym = output_dysymtab_info.dysymtab_command.nextrefsyms;
        output_dysymtab_info.dysymtab_command.nextrefsyms += nrefsym;
      }
    }
  }
...
static void removed_dead_local_symbols_in_section(struct merged_section *ms) {
  ...
  for (q = &objects; *q; q = &(object_list->next)) {
    ...
    for (i = 0; i < object_list->used; i++) {
      ...
      discard_local_symbols_for_section(j + 1, object_symbols, object_strings,
    ...
    for (i = 0; i < object_list->used; i++) {
      ...
      if (cur_obj->symtab != NULL) {
        object_symbols =
            (struct nlist *)(cur_obj->obj_addr + cur_obj->symtab->symoff);
        object_strings = (char *)(cur_obj->obj_addr + cur_obj->symtab->stroff);
        remove_dead_N_GSYM_stabs_for_cur_obj(object_symbols, object_strings);
      }
    }
  }
}
 ...
 */
__private_extern__ void
define_link_editor_execute_symbols(unsigned long header_address) {
  struct merged_symbol *merged_symbol;
 ...
 */
__private_extern__ void setup_link_editor_symbols(void) {
  if (filetype == MH_EXECUTE) {
...
static void setup_link_editor_symbol(char *symbol_name) {
   ...
   */
  if (filetype == MH_BUNDLE && strcmp(symbol_name, "___dso_handle") == 0 &&
  ...
  if (filetype == MH_EXECUTE) {
    define_link_editor_symbol(
        symbol_name, N_EXT | N_ABS, NO_SECT,
  ...
  link_edit_symbols_object->reference_maps[n].flags =
      REFERENCE_FLAG_PRIVATE_DEFINED;
  link_edit_symbols_object->reference_maps[n].merged_symbol = merged_symbol;
    ...
    while (*p) {
      ...
      for (j = 0; j < 2; j++) {
        ...
        while (*q) {
          ...
          strncpy(sections[i].sectname, ms->s.sectname, sizeof(ms->s.sectname));
          strncpy(sections[i].segname, ms->s.segname, sizeof(ms->s.segname));
  ...
  if (nsects > 0)
    define_link_editor_symbol(
        symbol_name, N_SECT | N_PEXT | N_EXT, 1,
        merged_symbol->nlist.n_desc & REFERENCED_DYNAMICALLY, 0);
  else
    define_link_editor_symbol(
        symbol_name, N_ABS | N_PEXT | N_EXT, NO_SECT,
        merged_symbol->nlist.n_desc & REFERENCED_DYNAMICALLY, 0);
}
 ...
 */
__private_extern__ void
define_link_editor_dylib_symbols(unsigned long header_address) {
  struct merged_symbol *merged_symbol;
  struct merged_symbol_list *merged_symbol_list;
  unsigned long i;
      ...
      for (i = 0; i < merged_symbol_list->used; i++) {
        ...
        if (merged_symbol->define_a_way == 1) {
          if (merged_symbol->nlist.n_sect == NO_SECT)
            ...
            merged_symbol->nlist.n_value =
                header_address - link_edit_symbols_object->section_maps[0]
 ...
 */
__private_extern__ void define_link_editor_preload_symbols(enum bool setup) {
  ...
  while (*p) {
    ...
    for (j = 0; j < 2; j++) {
      ...
      while (*q) {
        ...
        strncpy(symbol_name, ms->s.segname, sizeof(ms->s.segname));
        strncat(symbol_name, ms->s.sectname, sizeof(ms->s.sectname));
        strcat(symbol_name, "__begin");
        if (setup == TRUE)
          define_link_editor_symbol(symbol_name, N_EXT | N_SECT, i + 1, 0, 0);
    ...
    strcat(symbol_name, "__end");
    if (setup)
      define_link_editor_symbol(symbol_name, N_EXT | N_SECT, first_section, 0,
                                0);
    else {
      merged_symbol = lookup_symbol(symbol_name);
      if (merged_symbol->name_len != 0)
 ...
 */
__private_extern__ void reduce_indr_symbols(void) {
  ...
  for (merged_symbol_list =
    ...
    for (i = 0; i < merged_symbol_list->used; i++) {
      ...
      if (merged_symbol->nlist.n_type == (N_EXT | N_INDR) &&
        ...
        indr_depth = 1;
        indr_symbol = (struct merged_symbol *)(merged_symbol->nlist.n_value);
        while (indr_symbol->nlist.n_type == (N_EXT | N_INDR) &&
          ...
          if (j == indr_depth) {
            ...
            indr_symbol = (struct merged_symbol *)(indr_symbol->nlist.n_value);
          } else {
        ...
        if (from_dylibs != 0 && not_from_dylibs != 0 &&
          ...
          for (j = 0; j < indr_depth; j++) {
            if (indr_symbols[j]->defined_in_dylib == FALSE) {
              ...
              indr_symbol_pairs = reallocate(indr_symbol_pairs,
                                             sizeof(struct indr_symbol_pair) *
                                                 (nindr_symbol_pairs + 1));
              indr_symbol_pair = indr_symbol_pairs + nindr_symbol_pairs;
              nindr_symbol_pairs++;
              indr_symbol_pair->merged_symbol = merged_symbol;
              if (k < indr_depth && indr_symbols[k]->defined_in_dylib)
                indr_symbol_pair->indr_symbol = indr_symbols[k];
              else
                indr_symbol_pair->indr_symbol = indr_symbol;
            }
          }
        }
        if (indr_symbol->nlist.n_type != (N_EXT | N_INDR)) {
          for (j = 0; j < indr_depth; j++) {
            indr_symbols[j]->nlist.n_value = (unsigned long)indr_symbol;
            /*
            ...
            if ((indr_symbol->nlist.n_type & N_PEXT) == N_PEXT) {
              indr_symbols[j]->definition_object->nprivatesym++;
              nmerged_private_symbols++;
            }
          }
        }
 ...
 * symbols.
 */
__private_extern__ void layout_merged_symbols(void) {
  unsigned long i;
  struct merged_symbol_list *merged_symbol_list;
  struct merged_symbol *merged_symbol;
   ...
   */
  if (noundefs == TRUE && (define_comldsyms == TRUE || commons_exist == FALSE))
...
__private_extern__ void discard_local_symbols_for_section(
    unsigned long nsect, struct nlist *object_symbols, char *object_strings,
    struct section *s, struct section_map *section_map) {
  ...
  for (i = 0; i < cur_obj->symtab->nsyms; i++) {
    ...
    if ((object_symbols[i].n_type & N_EXT) == 0 &&
       ...
       */
      if (fine_reloc_offset_in_output(section_map, object_symbols[i].n_value -
                                                       s->addr) == FALSE &&
        ...
        *next_localsym_block = localsym_block;
        ...
        fine_reloc = fine_reloc_for_input_offset(
            section_map, object_symbols[i].n_value - s->addr);
          ...
          for (j = i + 1; j < cur_obj->symtab->nsyms; j++) {
            ...
            if (fine_reloc !=
                fine_reloc_for_input_offset(
                    section_map, object_symbols[j].n_value - s->addr))
              break;
          }
          /*
       ...
       */
        ...
        fine_reloc = fine_reloc_for_input_offset(
            section_map, object_symbols[i].n_value - s->addr);
        for (j = i + 1; j < cur_obj->symtab->nsyms; j++) {
          if (localsym_block != NULL && j >= localsym_block->index)
            break;
          if (object_symbols[j].n_type == N_ENSYM)
            break;
          if ((object_symbols[j].n_type & N_EXT) == N_EXT)
          ...
          if (object_symbols[j].n_sect != nsect)
            break;
          if (fine_reloc !=
              fine_reloc_for_input_offset(section_map,
         ...
         */
        if (j < cur_obj->symtab->nsyms &&
            (localsym_block == NULL || j < localsym_block->index) &&
            object_symbols[j].n_type == N_ENSYM &&
            object_symbols[j].n_sect == nsect) {
          i = j;
        }
       ...
       */
               ...
               fine_reloc_offset_in_output(
                   section_map, object_symbols[i].n_value - s->addr) == FALSE) {
...
static void remove_dead_N_GSYM_stabs_for_cur_obj(struct nlist *object_symbols,
  ...
  for (i = 0; i < cur_obj->symtab->nsyms; i++) {
    ...
    if (cur_obj->dwarf_name && object_symbols[i].n_type & N_EXT &&
        object_symbols[i].n_un.n_strx != 0) {
      size_t cnt = count_dwarf_symbols(object_symbols + i, i, debug_ptr);
      if (cnt == 0)
      ...
      hash_pointer =
          lookup_symbol(object_strings + object_symbols[i].n_un.n_strx);
      if (hash_pointer->name_len == 0 || hash_pointer->live ||
          (object_symbols[i].n_sect != NO_SECT &&
    ...
    if (object_symbols[i].n_type == N_GSYM &&
      ...
      if (merged_symbol->live == FALSE) {
        nlocal_symbols--;
        cur_obj->nlocalsym--;
        local_string_size -= output_strlen == 0 ? 0 : output_strlen + 1;
        /*
         * Create a block for this symbol which will cause it to
         * be discarded for the output.
         */
 ...
 */
static void record_bincl(unsigned long sum, const char *path,
  ...
  if (bincl_entries == NULL) {
    ...
    bincl_entries = allocate(sizeof(struct bincl_entry) * bincl_entry_count);
  }
  if (bincl_entries_used == bincl_entry_count) {
    bincl_entry_count *= 2;
    tmp = allocate(sizeof(struct bincl_entry) * bincl_entry_count);
    memcpy(tmp, bincl_entries, sizeof(struct bincl_entry) * bincl_entries_used);
    free(bincl_entries);
    bincl_entries = tmp;
  }
 ...
 */
static unsigned long find_bincl(unsigned long sum, const char *path) {
  unsigned long i;
 ...
 */
static void add_dwarf_map_entry(struct nlist **nlist,
                                unsigned long *output_nsyms, int32_t n_strx,
 ...
 */
static void add_dwarf_map_for_sym(const struct nlist *sym, size_t i,
                                  const size_t *debug_ptr, uint32_t n_strx,
                                  uint16_t old_sect, struct nlist **nlist,
                                  unsigned long *output_nsyms) {
  size_t j;
  ...
  /* If S_ATTR_STRIP_STATIC_SYMS is set on this symbol's section,
     we don't need a debug symbol for this symbol.  */
  if ((sym->n_type & (N_TYPE | N_STAB)) == N_SECT && old_sect != NO_SECT &&
      (cur_obj->section_maps[old_sect - 1].s->flags & S_ATTR_STRIP_STATIC_SYMS))
    return;
  ...
  if (!debug_ptr || *debug_ptr != i) {
    if (sym->n_type & N_EXT)
      ...
      add_dwarf_map_entry(nlist, output_nsyms, sym->n_un.n_strx, N_STSYM,
                          sym->n_sect, 0, sym->n_value);
    return;
  }
  add_dwarf_map_entry(nlist, output_nsyms, 0, N_BNSYM, sym->n_sect, 0,
                      sym->n_value);
  add_dwarf_map_entry(nlist, output_nsyms, n_strx, N_FUN, sym->n_sect, 0,
                      sym->n_value);
  ...
  add_dwarf_map_entry(nlist, output_nsyms, 0, N_ENSYM, sym->n_sect, 0,
                      sym->n_value);
}
 ...
 * get relocated.
 */
__private_extern__ void output_local_symbols(void) {
  ...
  if (sect_object_symbols.ms != NULL) {
     ...
     */
    for (i = 0; i < cur_obj->nsection_maps; i++) {
      if (sect_object_symbols.ms == cur_obj->section_maps[i].output_section) {
        /* make the nlist entry in the output file */
        nlist->n_value = cur_obj->section_maps[i].output_section->s.addr +
                         cur_obj->section_maps[i].offset;
        nlist->n_sect = cur_obj->section_maps[i].output_section->output_sectnum;
        nlist->n_type = N_SECT;
        nlist->n_desc = 0;

        nlist->n_un.n_strx = output_symtab_info.output_local_strsize;
        string = output_addr + output_symtab_info.symtab_command.stroff +
                 output_symtab_info.output_local_strsize;
        if (cur_obj->ar_hdr == NULL) {
          ...
          output_symtab_info.output_local_strsize += cur_obj->ar_name_size + 1;
        }
        nlist++;
        output_nsyms++;
        break;
  ...
  if (cur_obj->dwarf_name) {
    ...
    char *strbase = (output_addr + output_symtab_info.symtab_command.stroff);

    if (cur_obj->dwarf_comp_dir) {
      size_t len = strlen(cur_obj->dwarf_comp_dir);
      add_dwarf_map_entry(&nlist, &output_nsyms,
                          output_symtab_info.output_local_strsize, N_SO, 0, 0,
                          0);
  ...
  for (i = 0; i < cur_obj->symtab->nsyms; i++) {
      ...
      if (localsym_block->state == EXCLUDED_INCLUDE ||
        ...
        *nlist = object_symbols[i];
        ...
        if (object_symbols[i].n_un.n_strx != 0) {
          ...
          nlist->n_un.n_strx = find_bincl(
              nlist->n_value, object_strings + object_symbols[i].n_un.n_strx);
          ...
          if (doCopy) {
            ...
            string = output_addr + output_symtab_info.symtab_command.stroff +
                     output_symtab_info.output_local_strsize;
            strcpy(string, object_strings + object_symbols[i].n_un.n_strx);
            output_symtab_info.output_local_strsize +=
                strlen(object_strings + object_symbols[i].n_un.n_strx) + 1;
          }
        }
        output_nsyms++;
        nlist++;
    ...
    if (cur_obj->dwarf_name
      ...
      if (hash_pointer->name_len != 0 && (!dead_strip || hash_pointer->live) &&
          (object_symbols[i].n_sect == NO_SECT ||
           hash_pointer->definition_object == cur_obj)) {
        unsigned long symbol_index;
        symbol_index =
            (output_symtab_info.output_merged_strsize +
             (merged_symbol_string_index(hash_pointer->nlist.n_un.n_name)));
        add_dwarf_map_for_sym(&hash_pointer->nlist, i, debug_ptr, symbol_index,
                              object_symbols[i].n_sect, &nlist, &output_nsyms);
      }
    }
    ...
    if ((object_symbols[i].n_type & N_EXT) == 0 &&
      ...
      *nlist = object_symbols[i];
       ...
       */
      if (save_reloc == FALSE && (nlist->n_type & N_TYPE) == N_SECT &&
      ...
      copy_string_to_output_file:
        ...
        if (object_symbols[i].n_un.n_strx != 0) {
           ...
           * the trailing '\0'.
           */
          strncpy(string, object_strings + object_symbols[i].n_un.n_strx,
                  output_strlen);
          output_symtab_info.output_local_strsize += output_strlen + 1;
        }
      }
      output_nsyms++;
      nlist++;
      ...
      /* Add the DWARF map entry/entries for this local symbol,
         if necessary.  */
      if (cur_obj->dwarf_name)
        add_dwarf_map_for_sym(nlist - 1, i, debug_ptr, nlist[-1].n_un.n_strx,
                              object_symbols[i].n_sect, &nlist, &output_nsyms);
    }
  }
  ...
  output_flush(flush_string_offset,
               output_symtab_info.output_local_strsize - start_string_size);
#endif /* !defined(RLD) */
  /*
...
static void get_stroff_and_mtime_for_N_OSO(unsigned long *stroff_for_N_OSO,
  ...
  *stroff_for_N_OSO = output_symtab_info.output_local_strsize;
  ...
  output_symtab_info.output_local_strsize += cur_obj->resolved_path_len + 1;
  /*
...
local_symbol_output_index(struct object_file *obj, unsigned long index) {
  ...
  /* setup pointers to the symbol table and string table */
  object_symbols = (struct nlist *)(obj->obj_addr + obj->symtab->symoff);
  ...
  if (sect_object_symbols.ms != NULL) {
    ...
    for (i = 0; i < obj->nsection_maps; i++) {
      if (sect_object_symbols.ms == obj->section_maps[i].output_section) {
        output_nsyms++;
        break;
      }
    }
 ...
 * string block.
 */
__private_extern__ void set_merged_string_block_indexes(void) {
  unsigned long index;
  ...
  for (q = &(merged_string_blocks); *q; q = &(string_block->next)) {
    ...
    if (string_block->set_num != cur_set)
      continue;
#endif /* RLD */
    string_block->index = index, index += string_block->used;
  }
}
 ...
 * the output.
 */
__private_extern__ void output_merged_symbols(void) {
  unsigned long i, j, flush_symbol_offset, flush_string_offset,
      start_string_size;
  struct merged_symbol_list *merged_symbol_list;
    ...
    for (i = 0; i < merged_symbol_list->used; i++) {
      ...
      if (merged_symbol->nlist.n_type == (N_EXT | N_INDR)) {
        ...
        if (indr_symbol->nlist.n_type != (N_EXT | N_UNDF) &&
          ...
          merged_symbol->nlist.n_value = indr_symbol->nlist.n_value;
        } else {
          merged_symbol->nlist.n_value =
              output_symtab_info.output_merged_strsize +
              merged_symbol_string_index(indr_symbol->nlist.n_un.n_name);
        }
      }
    }
  }
      ...
      for (i = 0; i < merged_symbol_list->used; i++) {
        ...
        if ((merged_symbol->nlist.n_type & N_PEXT) == N_PEXT &&
          ...
          *nlist = merged_symbol->nlist;
          ...
          nlist->n_un.n_strx =
              output_symtab_info.output_merged_strsize +
              merged_symbol_string_index(merged_symbol->nlist.n_un.n_name);
  ...
  flush_symbol_offset =
      ...
      output_dysymtab_info.dysymtab_command.iextdefsym * sizeof(struct nlist);
  nlist = (struct nlist *)(output_addr + flush_symbol_offset);
  for (i = 0; i < output_dysymtab_info.dysymtab_command.nextdefsym; i++) {
    ...
    *nlist = merged_symbol->nlist;
    ...
    if (nlist->n_sect != NO_SECT)
      nlist->n_sect =
          merged_symbol->definition_object->section_maps[nlist->n_sect - 1]
              .output_section->output_sectnum;
#endif
    nlist++;
  }
  ...
  flush_symbol_offset =
      output_symtab_info.symtab_command.symoff +
      output_dysymtab_info.dysymtab_command.iundefsym * sizeof(struct nlist);
  nlist = (struct nlist *)(output_addr + flush_symbol_offset);
...
#ifndef RLD
  output_flush(flush_string_offset,
               output_symtab_info.output_merged_strsize - start_string_size);
#endif /* !defined(RLD) */
   ...
   */
  hint =
      (struct twolevel_hint *)(output_addr +
    ...
    if (host_byte_sex != target_byte_sex) {
      hint = (struct twolevel_hint *)(output_addr +
                                      output_hints_info.twolevel_hints_command
                                          .offset);
      swap_twolevel_hint(hint, output_dysymtab_info.dysymtab_command.nundefsym,
                         target_byte_sex);
    }
#ifndef RLD
 ...
 */
__private_extern__ void output_rld_symfile_merged_symbols(void) {
  struct nlist *nlist;
  struct merged_symbol_list *merged_symbol_list;
  struct merged_symbol *merged_symbol;
  ...
  nlist =
      (struct nlist *)(output_addr + output_symtab_info.symtab_command.symoff);
  for (merged_symbol_list =
           merged_symbol_root == NULL ? NULL : merged_symbol_root->list;
       merged_symbol_list != NULL;
       merged_symbol_list = merged_symbol_list->next) {
 ...
 * output file.
 */
__private_extern__ void process_undefineds(void) {
      ...
      if (merged_symbol->nlist.n_type == (N_EXT | N_UNDF) &&
        ...
        if (prebinding == TRUE) {
          if (ld_trace_prebinding_disabled == TRUE)
            print("[Logging for XBS] prebinding"
                  ...
                  "symbols\n",
                  final_output != NULL ? final_output : outputfile);
        ...
        if (nundef_syms != 0) {
          for (j = 0; j < nundef_syms; j++) {
            if (strcmp(undef_syms[j], merged_symbol->nlist.n_un.n_name) == 0) {
              allowed_undef = TRUE;
              break;
            }
          }
        ...
        if (allowed_undef == FALSE)
          noundefs = FALSE;
        if (save_reloc == FALSE && (undefined_flag == UNDEFINED_ERROR ||
          ...
          if (allowed_undef == FALSE || prebound_undef == TRUE) {
            if (printed_undef == FALSE) {
              if (undefined_flag == UNDEFINED_WARNING)
                ...
                if (allowed_undef == TRUE && prebound_undef == TRUE)
                  errors_save = errors;
              ...
              printed_undef = TRUE;
            } else if (errors == 0 && undefined_flag == UNDEFINED_ERROR &&
                       allowed_undef == FALSE) {
              errors = 1;
            }
            print("%s\n", merged_symbol->nlist.n_un.n_name);
        ...
        if (save_reloc == FALSE && twolevel_namespace == TRUE &&
            merged_symbol->defined_in_dylib == TRUE &&
            merged_symbol->referenced_in_non_dylib == TRUE &&
            merged_symbol->definition_library->definition_obj
                    ->library_ordinal == 0 &&
            merged_symbol->definition_library->indirect_twolevel_ref_flagged ==
                FALSE) {
          obj = cur_obj;
          cur_obj = merged_symbol->non_dylib_referenced_obj;
          error_with_cur_obj(
              "illegal reference to symbol: %s "
              "defined in indirectly referenced dynamic library "
              "%s",
              merged_symbol->nlist.n_un.n_name,
              merged_symbol->definition_library->dylib_file != NULL
      ...
      for (i = 0; i < merged_symbol_list->used; i++) {
        ...
        if (merged_symbol->weak_reference_mismatch == TRUE &&
            ((merged_symbol->nlist.n_type == (N_EXT | N_UNDF) &&
             ...
             (merged_symbol->nlist.n_type & N_TYPE) == N_PBUD) &&
            (merged_symbol->defined_in_dylib == FALSE ||
             merged_symbol->definition_library->force_weak_dylib == FALSE)) {
          ...
          for (q = &objects; *q; q = &(object_list->next)) {
            ...
            for (j = 0; j < object_list->used; j++) {
              ...
              for (k = 0; k < cur_obj->nundefineds; k++) {
                if (merged_symbol == cur_obj->undefined_maps[k].merged_symbol) {
                  print_obj_name(cur_obj);
                  object_symbols = (struct nlist *)(cur_obj->obj_addr +
                                                    cur_obj->symtab->symoff);
                  if ((object_symbols[cur_obj->undefined_maps[k].index].n_desc &
   ...
   */
  for (undefined = undefined_list.next; undefined != &undefined_list;
    ...
    if (undefined->merged_symbol->twolevel_reference == TRUE) {
       ...
       */
      library_ordinal =
          GET_LIBRARY_ORDINAL(undefined->merged_symbol->nlist.n_desc);
      if (library_ordinal == SELF_LIBRARY_ORDINAL)
      ...
      reported = FALSE;
      for (prevs = undefined_list.next; prevs != undefined;
           prevs = prevs->next) {
        if (prevs->merged_symbol->twolevel_reference == FALSE)
          continue;
        library_ordinal =
            GET_LIBRARY_ORDINAL(prevs->merged_symbol->nlist.n_desc);
        if (library_ordinal == SELF_LIBRARY_ORDINAL)
    ...
    for (q = &objects; *q; q = &(object_list->next)) {
      ...
      for (i = 0; i < object_list->used; i++) {
        ...
        for (j = 0; j < cur_obj->nundefineds; j++) {
          merged_symbol = cur_obj->undefined_maps[j].merged_symbol;
          ...
          if (merged_symbol->nlist.n_type == (N_EXT | N_UNDF) &&
            ...
            print_obj_name(cur_obj);
            print("%sreference to undefined %s",
                  merged_symbol->nlist.n_desc & N_WEAK_REF ? "weak " : "",
                  merged_symbol->nlist.n_un.n_name);
#ifndef RLD
            library_ordinal = GET_LIBRARY_ORDINAL(merged_symbol->nlist.n_desc);
 ...
 */
__private_extern__ void reset_prebound_undefines(void) {
  unsigned long i;
  struct merged_symbol_list *merged_symbol_list;
  struct merged_symbol *merged_symbol, *indr_symbol;
 ...
 *	    Sorted by name
 */
__private_extern__ void assign_output_symbol_indexes(void) {
   ...
   */
  else if (strip_level == STRIP_NONGLOBALS && keep_private_externs == FALSE &&
      ...
      if (keep_private_externs == FALSE) {
        ...
        if (strip_level != STRIP_DYNAMIC_EXECUTABLE &&
          ...
          */
          ...
          nstripped_merged_private_symbols += cur_obj->nprivatesym;
          /*
          print("assign_output_symbol_indexes() adding cur_obj->nprivatesym %lu
          to nstripped_merged_symbols\n", cur_obj->nprivatesym);
          */
        }
      }
    }
  ...
  if ((keep_private_externs == TRUE && index != nlocal_symbols) ||
      (keep_private_externs == FALSE &&
       index != nlocal_symbols + nmerged_private_symbols -
                    nstripped_merged_private_symbols))
  ...
  for (merged_symbol_list =
    ...
    for (i = 0; i < merged_symbol_list->used; i++) {
       ...
       */
      if (strip_level != STRIP_DYNAMIC_EXECUTABLE && dead_strip == TRUE &&
      ...
      if ((merged_symbol->nlist.n_type & N_EXT) == N_EXT &&
        ...
        if (dead_strip == FALSE || merged_symbol->live == TRUE) {
          ...
          /*
          printf("assign_output_symbol_indexes() nstripped_merged_symbols
          incremented for undefined %s\n", merged_symbol->nlist.n_un.n_name);
          */
          nstripped_merged_symbols++;
        }
      } else {
   ...
   * symbols.
   */
  undefsyms_order = allocate(nundefsym * sizeof(struct merged_symbol *));
  extdefsyms_order = allocate(nextdefsym * sizeof(struct merged_symbol *));
  /*
        ...
        if (keep_private_externs == TRUE || n_pext == 0) {
          if (strip_level != STRIP_DYNAMIC_EXECUTABLE ||
            ...
            if ((merged_symbol->nlist.n_type & N_TYPE) == N_ABS)
              ...
              merged_symbol->output_index = INDIRECT_SYMBOL_LOCAL;
          }
        }
      }
    }
...
static int qsort_by_module(const struct merged_symbol **ms1,
                           const struct merged_symbol **ms2) {
  return ((int)((*ms1)->definition_object) - (int)((*ms2)->definition_object));
}
 ...
 */
__private_extern__ unsigned long
merged_symbol_output_index(struct merged_symbol *merged_symbol) {
 ...
 */
__private_extern__ void layout_dylib_tables(void) {
        ...
        for (j = 0; j < cur_obj->nrefsym; j++) {
          ...
          if (merged_symbol->nlist.n_type & N_PEXT) {
            ...
            if (flags == REFERENCE_FLAG_PRIVATE_UNDEFINED_NON_LAZY)
              ...
              cur_obj->reference_maps[j].flags = REFERENCE_FLAG_UNDEFINED_LAZY;
          }
        }
      }
    }
    ...
    for (q = &objects; *q; q = &(object_list->next)) {
      ...
      for (i = 0; i < object_list->used; i++) {
        ...
        if (cur_obj->bundle_loader == TRUE)
          continue;
        cur_obj->imodtab = output_dysymtab_info.dysymtab_command.nmodtab;
 ...
 *	The table of contents
 */
__private_extern__ void output_dylib_tables(void) {
    ...
    for (merged_symbol_list =
      ...
      for (i = 0; i < merged_symbol_list->used; i++) {
        ...
        if (merged_symbol->nlist.n_type == (N_EXT | N_UNDF) ||
          ...
          ref->flags = merged_symbol->nlist.n_desc & REFERENCE_TYPE;
          ref++;
  ...
  output_flush(flush_offset, output_dysymtab_info.dysymtab_command.nextrefsyms *
  ...
  if (multi_module_dylib == TRUE) {
    ...
    for (q = &objects; *q; q = &(object_list->next)) {
      ...
      for (i = 0; i < object_list->used; i++) {
        ...
        if (cur_obj->objc_module_info != NULL) {
          ...
          mod->objc_module_info_size = cur_obj->objc_module_info->s->size;
        } else {
    ...
    mod->iextdefsym = output_dysymtab_info.dysymtab_command.iextdefsym;
    mod->nextdefsym = output_dysymtab_info.dysymtab_command.nextdefsym;
    mod->irefsym = 0;
    mod->nrefsym = output_dysymtab_info.dysymtab_command.nextrefsyms;
    mod->ilocalsym = output_dysymtab_info.dysymtab_command.ilocalsym;
    mod->nlocalsym = output_dysymtab_info.dysymtab_command.nlocalsym;
    mod->iextrel = 0;
    mod->nextrel = output_dysymtab_info.dysymtab_command.nextrel;
    mod->iinit_iterm = 0;
  ...
  if (host_byte_sex != target_byte_sex) {
    swap_dylib_module(mods, output_dysymtab_info.dysymtab_command.nmodtab,
                      target_byte_sex);
  }
  output_flush(flush_offset, output_dysymtab_info.dysymtab_command.nmodtab *
  ...
  tocs = (struct dylib_table_of_contents *)(output_addr + flush_offset);
  toc = tocs;
  for (i = 0; i < ntoc; i++) {
  ...
  output_flush(flush_offset, ntoc * sizeof(struct dylib_table_of_contents));
  free(toc_order);
}
 ...
 */
__private_extern__ void clear_read_only_reloc_flags(void) {
  unsigned long i;
  struct merged_symbol_list *merged_symbol_list;
  struct merged_symbol *merged_symbol;
 ...
 */
__private_extern__ void flag_read_only_reloc(struct section *s,
 ...
 * zero if it exist.
 */
__private_extern__ void free_multiple_defs(void) {
  if (nmultiple_defs != 0) {
    free(multiple_defs);
    multiple_defs = NULL;
    nmultiple_defs = 0;
 ...
 */
__private_extern__ void remove_merged_symbols(void) {
  long i;
  unsigned long j;
  ...
  for (string_block = merged_string_blocks; string_block != NULL;
 ...
 */
__private_extern__ void print_symbol_list(char *string, enum bool input_based) {
  struct merged_symbol_list *merged_symbol_list;
  struct merged_symbol_chunk *p;
  unsigned long i, j;
  struct nlist *nlist;
  ...
  for (merged_symbol_list =
    ...
    for (i = 0; i < merged_symbol_list->used; i++) {
      ...
      case N_INDR:
        print("N_INDR for %s\n",
              ((struct merged_symbol *)(nlist->n_value))->nlist.n_un.n_name);
        break;
      default:
        print("unknown 0x%x\n", (unsigned int)(nlist->n_type));
        break;
      ...
      maps = merged_symbol_list->symbols[i]->definition_object->section_maps;
      if (nlist->n_sect == NO_SECT)
        print("NO_SECT\n");
      else {
        if (input_based == TRUE)
      ...
      print_obj_name(merged_symbol_list->merged_symbols[i].definition_object);
      print("\n");
      print("    set_num %d\n",
            merged_symbol_list->merged_symbols[i].definition_object->set_num);
#endif
    }
  }
 ...
 */
__private_extern__ struct section *get_output_section(unsigned long sect) {
  struct merged_segment **p, *msg;
 ...
 */
__private_extern__ void print_undefined_list(void) {
