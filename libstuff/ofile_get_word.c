    ...
    if (lc->cmd == LC_SEGMENT) {
      ...
      for (j = 0; j < sg->nsects; j++) {
        if (addr >= s->addr && addr < s->addr + s->size) {
          if (s->flags == S_ZEROFILL || s->flags == S_THREAD_LOCAL_ZEROFILL)
            *word = 0;
          else {
            if (s->offset > ofile->object_size ||
              ...
              memcpy(word, (ofile->object_addr + (s->offset + addr - s->addr)),
                     sizeof(uint32_t));
      ...
      for (j = 0; j < sg64->nsects; j++) {
        if (addr >= s64->addr && addr < s64->addr + s64->size) {
          if (s64->flags == S_ZEROFILL || s64->flags == S_THREAD_LOCAL_ZEROFILL)
            *word = 0;
          else {
            if (s64->offset > ofile->object_size ||
              ...
              memcpy(word,
                     (ofile->object_addr + (s64->offset + addr - s64->addr)),
                     sizeof(uint32_t));
              if (ofile->object_byte_sex != get_host_byte_sex())
                *word = SWAP_INT(*word);
            }
