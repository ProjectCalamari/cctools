 ...
 */
static int set_OpInfo_from_reloc(uint32_t r_symbolnum, uint32_t r_type,
...
static int GetOpInfo(
  ...
  if (Offset != 0 || OpSize != 4 || TagType != 1 || dis_info->verbose == FALSE)
    return (0);
  ...
  for (i = 0; i < dis_info->nrelocs; i++) {
    if (dis_info->relocs[i].r_address == sect_offset) {
      ...
      if (dis_info->relocs[i].r_extern)
        return (set_OpInfo_from_reloc(dis_info->relocs[i].r_symbolnum,
                                      dis_info->relocs[i].r_type, dis_info,
                                      op_info));
    }
  }
  ...
  for (i = 0; i < dis_info->next_relocs; i++) {
    if (dis_info->ext_relocs[i].r_address == Pc) {
      ...
      return (set_OpInfo_from_reloc(dis_info->ext_relocs[i].r_symbolnum,
        ...
        if ((strncmp(s64.sectname, "__objc_selrefs", 16) == 0 ||
          ...
          if (object_offset < object_size) {
            ...
            if (strncmp(s64.sectname, "__objc_selrefs", 16) == 0)
              ...
              *msgref = TRUE;
              memcpy(&pointer_value64, object_addr + object_offset + 8,
                     sizeof(uint64_t));
              if (swapped)
                pointer_value64 = SWAP_LONG_LONG(pointer_value64);
      ...
      for (j = 0; j < sg.nsects; j++) {
        ...
        if ((strncmp(s.sectname, "__objc_selrefs", 16) == 0 ||
          ...
          if (object_offset < object_size) {
            ...
            if (strncmp(s.sectname, "__objc_selrefs", 16) == 0)
              *selref = TRUE;
            else if (strncmp(s.sectname, "__objc_classrefs", 16) == 0 ||
                     strncmp(s.sectname, "__objc_superrefs", 16) == 0)
              *classref = TRUE;
            else if (strncmp(s.sectname, "__objc_msgrefs", 16) == 0 &&
                     value + 4 < s.addr + s.size) {
              *msgref = TRUE;
              memcpy(&pointer_value, object_addr + object_offset + 4,
                     sizeof(uint32_t));
              if (swapped)
                pointer_value = SWAP_INT(pointer_value);
            } else if (strncmp(s.sectname, "__cfstring", 16) == 0)
              *cfstring = TRUE;
    ...
    *reference_type = LLVMDisassembler_ReferenceType_Out_Objc_Class_Ref;
    if (info->cputype == CPU_TYPE_ARM64) {
      name = get_objc2_64bit_class_name(
          ...
          info->strings_size, CPU_TYPE_ARM64);
      if (name != NULL)
    ...
    if (strcmp(*ReferenceName, "_objc_msgSend") == 0) {
      ...
      if (info->selector_name != NULL) {
        ...
        if (info->class_name != NULL) {
          ...
          *ReferenceName = info->method;
          ...
          info->method = allocate(7 + strlen(info->selector_name));
      ...
      *ReferenceType = LLVMDisassembler_ReferenceType_Out_Objc_Message;
      if (info->selector_name != NULL) {
        if (info->method != NULL)
          free(info->method);
        info->method = allocate(15 + strlen(info->selector_name));
...
static const char *SymbolLookUp(void *DisInfo, uint64_t SymbolValue,
   ...
   */
  else if (*ReferenceType == LLVMDisassembler_ReferenceType_In_ARM64_ADDXri &&
    ...
    SymbolValue = (info->adrp_addr & 0xfffffffffffff000LL) + (adrp_imm << 12) +
                  addxri_imm;

    *ReferenceName = guess_literal_pointer(SymbolValue, ReferenceType, info);
    if (*ReferenceName == NULL)
      *ReferenceType = LLVMDisassembler_ReferenceType_InOut_None;
  }
  /*
   ...
   */
  else if (*ReferenceType == LLVMDisassembler_ReferenceType_In_ARM64_LDRXui &&
           ReferencePC - 4 == info->adrp_addr &&
           (info->adrp_inst & 0x9f000000) == 0x90000000 &&
           (info->adrp_inst & 0x1f) == ((SymbolValue >> 5) & 0x1f)) {
   ...
   */
  else if (*ReferenceType == LLVMDisassembler_ReferenceType_In_ARM64_LDRXl ||
...
LLVMDisasmContextRef create_arm64_llvm_disassembler(cpu_subtype_t cpusubtype) {
  ...
  LLVMOpInfoCallback OpInfo = llvm_disasm_new_getopinfo_abi()
                                  ? GetOpInfo
                                  : (LLVMOpInfoCallback)GetOpInfoOld;
  dc =
#ifdef STATIC_LLVM
      LLVMCreateDisasm
#else
...
uint32_t arm64_disassemble(
    ...
    enum byte_sex object_byte_sex, struct relocation_info *relocs,
    ...
    char *object_addr, uint64_t object_size, enum bool verbose,
    LLVMDisasmContextRef dc) {
  enum byte_sex host_byte_sex;
  enum bool swapped;
  uint32_t opcode;
