...
static int get_lto_cputype(struct arch_flag *arch_flag,
 ...
 */
__private_extern__ int is_llvm_bitcode_from_memory(char *addr, uint32_t size,
                                                   struct arch_flag *arch_flag,
                                                   void **pmod) /* maybe NULL */
{
  ...
  if (tried_to_load_lto == 0) {
    ...
    lto_is_object = dlsym(lto_handle, "lto_module_is_object_file_in_memory");
    lto_create = dlsym(lto_handle, "lto_module_create_from_memory");
    lto_create_local = dlsym(lto_handle, "lto_module_create_in_local_context");
    lto_dispose = dlsym(lto_handle, "lto_module_dispose");
    lto_get_target = dlsym(lto_handle, "lto_module_get_target_triple");
    lto_get_num_symbols = dlsym(lto_handle, "lto_module_get_num_symbols");
    lto_get_sym_attr = dlsym(lto_handle, "lto_module_get_symbol_attribute");
    ...
    if (lto_is_object == NULL || lto_create == NULL || lto_dispose == NULL ||
        lto_get_target == NULL || lto_get_num_symbols == NULL ||
        lto_get_sym_attr == NULL || lto_error == NULL ||
 ...
 */
__private_extern__ uint32_t lto_get_nsyms(void *mod) {
 ...
 */
__private_extern__ int lto_toc_symbol(void *mod, uint32_t symbol_index,
                                      int commons_in_toc) {
  ...
  if ((attr & LTO_SYMBOL_DEFINITION_MASK) == LTO_SYMBOL_DEFINITION_REGULAR ||
      (attr & LTO_SYMBOL_DEFINITION_MASK) == LTO_SYMBOL_DEFINITION_WEAK)
    return (1);
  if ((attr & LTO_SYMBOL_DEFINITION_MASK) == LTO_SYMBOL_DEFINITION_TENTATIVE &&
      commons_in_toc)
    return (1);
  return (0);
}
 ...
 * and rodata.
 */
__private_extern__ void lto_get_nlist_64(struct nlist_64 *nl, void *mod,
  ...
  default:
    if (attr & LTO_SYMBOL_SCOPE_MASK) {
      ...
      if (name) {
        warning("unknown scope for symbol '%s': 0x%x", name,
                attr & LTO_SYMBOL_SCOPE_MASK);
      } else {
        warning("unknown scope for symbol at index %d: 0x%x", symbol_index,
                attr & LTO_SYMBOL_SCOPE_MASK);
      }
    }
  }
  ...
  if ((attr & LTO_SYMBOL_DEFINITION_MASK) == LTO_SYMBOL_DEFINITION_UNDEFINED) {
    nl->n_type |= N_EXT;
    nl->n_type |= N_UNDF;
  } else
 ...
 */
__private_extern__ const char *lto_symbol_name(void *mod,
                                               uint32_t symbol_index) {
  return (lto_get_sym_name(mod, symbol_index));
}

__private_extern__ void lto_free(void *mod) { lto_dispose(mod); }
