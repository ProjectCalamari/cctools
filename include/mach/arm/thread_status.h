...
#define VALID_THREAD_STATE_FLAVOR(x)                                           \
  ((x == ARM_THREAD_STATE) || (x == ARM_VFP_STATE) ||                          \
   ...
   (x == THREAD_STATE_NONE) || (x == ARM_NEON_STATE) ||                        \
   (x == ARM_DEBUG_STATE32) || (x == ARM_THREAD_STATE64) ||                    \
   (x == ARM_EXCEPTION_STATE64) || (x == ARM_NEON_STATE64) ||                  \
   (x == ARM_DEBUG_STATE64))
...
#define ARM_THREAD_STATE_COUNT                                                 \
  ((mach_msg_type_number_t)(sizeof(arm_thread_state_t) / sizeof(uint32_t)))
...
#define ARM_VFP_STATE_COUNT                                                    \
  ((mach_msg_type_number_t)(sizeof(arm_vfp_state_t) / sizeof(uint32_t)))

#define ARM_EXCEPTION_STATE_COUNT                                              \
  ((mach_msg_type_number_t)(sizeof(arm_exception_state_t) / sizeof(uint32_t)))

#define ARM_EXCEPTION_STATE64_COUNT                                            \
  ((mach_msg_type_number_t)(sizeof(arm_exception_state64_t) / sizeof(uint32_t)))

#define ARM_DEBUG_STATE_COUNT                                                  \
  ((mach_msg_type_number_t)(sizeof(arm_debug_state_t) / sizeof(uint32_t)))
 ...
 */
static inline arm_saved_state32_t *saved_state32(arm_saved_state_t *iss) {
  return iss;
}

static inline boolean_t is_saved_state32(arm_saved_state_t *iss __unused) {
  return TRUE;
}
...
#define ARM_SAVED_STATE32_COUNT                                                \
  ((mach_msg_type_number_t)(sizeof(arm_saved_state_t) / sizeof(unsigned int)))

#elif defined(__arm64__)

#include <arm64/proc_reg.h>
#include <kern/assert.h>
#define CAST_ASSERT_SAFE(type, val)                                            \
  (assert((val) == ((type)(val))), (type)(val))
...
#define ARM_SAVED_STATE32_COUNT                                                \
  ((mach_msg_type_number_t)(sizeof(arm_saved_state32_t) / sizeof(unsigned int)))
...
#define ARM_SAVED_STATE64_COUNT                                                \
  ((mach_msg_type_number_t)(sizeof(arm_saved_state64_t) / sizeof(unsigned int)))
...
static inline boolean_t is_saved_state32(arm_saved_state_t *iss) {
  return (iss->flavor == ARM_SAVED_STATE32);
}

static inline boolean_t is_saved_state64(arm_saved_state_t *iss) {
...
static inline arm_saved_state64_t *saved_state64(arm_saved_state_t *iss) {
  return &iss->ss_64;
}

static inline register_t get_saved_state_pc(arm_saved_state_t *iss) {
  return (is_saved_state32(iss) ? saved_state32(iss)->pc
                                : saved_state64(iss)->pc);
}

static inline void set_saved_state_pc(arm_saved_state_t *iss, register_t pc) {
...
static inline register_t get_saved_state_sp(arm_saved_state_t *iss) {
  return (is_saved_state32(iss) ? saved_state32(iss)->sp
                                : saved_state64(iss)->sp);
}

static inline void set_saved_state_sp(arm_saved_state_t *iss, register_t sp) {
  if (is_saved_state32(iss)) {
...
static inline register_t get_saved_state_lr(arm_saved_state_t *iss) {
  return (is_saved_state32(iss) ? saved_state32(iss)->lr
                                : saved_state64(iss)->lr);
}

static inline void set_saved_state_lr(arm_saved_state_t *iss, register_t lr) {
  if (is_saved_state32(iss)) {
...
static inline register_t get_saved_state_fp(arm_saved_state_t *iss) {
  return (is_saved_state32(iss) ? saved_state32(iss)->r[7]
                                : saved_state64(iss)->fp);
}

static inline void set_saved_state_fp(arm_saved_state_t *iss, register_t fp) {
  if (is_saved_state32(iss)) {
...
static inline int check_saved_state_reglimit(arm_saved_state_t *iss,
                                             unsigned reg) {
  return (is_saved_state32(iss) ? (reg < ARM_SAVED_STATE32_COUNT)
                                : (reg < ARM_SAVED_STATE64_COUNT));
}

static inline register_t get_saved_state_reg(arm_saved_state_t *iss,
                                             unsigned reg) {
  if (!check_saved_state_reglimit(iss, reg))
    return 0;

  return (is_saved_state32(iss) ? (saved_state32(iss)->r[reg])
                                : (saved_state64(iss)->x[reg]));
}

static inline void set_saved_state_reg(arm_saved_state_t *iss, unsigned reg,
                                       register_t value) {
  if (!check_saved_state_reglimit(iss, reg))
    return;
...
static inline uint32_t get_saved_state_cpsr(arm_saved_state_t *iss) {
  return (is_saved_state32(iss) ? saved_state32(iss)->cpsr
                                : saved_state64(iss)->cpsr);
}

static inline void set_saved_state_cpsr(arm_saved_state_t *iss, uint32_t cpsr) {
  if (is_saved_state32(iss)) {
...
static inline register_t get_saved_state_far(arm_saved_state_t *iss) {
  return (is_saved_state32(iss) ? saved_state32(iss)->far
                                : saved_state64(iss)->far);
}

static inline void set_saved_state_far(arm_saved_state_t *iss, register_t far) {
  if (is_saved_state32(iss)) {
...
static inline uint32_t get_saved_state_esr(arm_saved_state_t *iss) {
  return (is_saved_state32(iss) ? saved_state32(iss)->esr
                                : saved_state64(iss)->esr);
}

static inline void set_saved_state_esr(arm_saved_state_t *iss, uint32_t esr) {
  if (is_saved_state32(iss)) {
...
static inline uint32_t get_saved_state_exc(arm_saved_state_t *iss) {
  return (is_saved_state32(iss) ? saved_state32(iss)->exception
                                : saved_state64(iss)->exception);
}

static inline void set_saved_state_exc(arm_saved_state_t *iss, uint32_t exc) {
  if (is_saved_state32(iss)) {
...
static inline int get_saved_state_svc_number(arm_saved_state_t *iss) {
  return (is_saved_state32(iss)
              ? (int)saved_state32(iss)->r[12]
              : (int)saved_state64(iss)
                    ->x[ARM64_SYSCALL_CODE_REG_NUM]); /* Only first word counts
                                                         here */
}
...
#define ARM_LEGACY_DEBUG_STATE_COUNT                                           \
  ((mach_msg_type_number_t)(sizeof(arm_legacy_debug_state_t) /                 \
                            sizeof(uint32_t)))

#define ARM_DEBUG_STATE32_COUNT                                                \
  ((mach_msg_type_number_t)(sizeof(arm_debug_state32_t) / sizeof(uint32_t)))
...
#define ARM_NEON_STATE_COUNT                                                   \
  ((mach_msg_type_number_t)(sizeof(arm_neon_state_t) / sizeof(uint32_t)))
#define ARM_NEON_STATE64_COUNT                                                 \
  ((mach_msg_type_number_t)(sizeof(arm_neon_state64_t) / sizeof(uint32_t)))
...
static inline boolean_t is_neon_saved_state32(arm_neon_saved_state_t *state) {
  return (state->flavor == ARM_NEON_SAVED_STATE32);
}

static inline boolean_t is_neon_saved_state64(arm_neon_saved_state_t *state) {
