 ...
 */
__private_extern__ void pass2(void) {
   ...
   */
  if (get_unix_standard_mode() == TRUE && access(outputfile, F_OK) == 0 &&
  ...
  if (fchmod(fd, mode) == -1)
    system_fatal("can't set execution permissions output file: %s", outputfile);
  ...
  if ((r = vm_allocate(mach_task_self(), (vm_address_t *)&output_addr,
    ...
    mach_fatal(r,
               "can't vm_allocate() buffer for output file of size "
               "%lu",
               output_size);
      ...
      if (cur_obj != base_obj) {
        for (j = 0; j < cur_obj->nsection_maps; j++) {
...
#endif /* RLD */
          {
            ...
            if (section_type == S_REGULAR || section_type == S_SYMBOL_STUBS ||
 ...
 */
__private_extern__ void pass2_rld_symfile(void) {
  /*
   * Copy the merged symbol table into the output file.
   */
 ...
 */
__private_extern__ void create_output_sections_array(void) {
  ...
  output_sections = (struct merged_section **)allocate(
      nsects * sizeof(struct merged_section *));
  ...
  while (*p) {
    ...
    while (*content) {
      ...
      if (ms->s.nreloc != 0) {
        ...
        offset = ms->s.reloff + ms->s.nreloc * sizeof(struct relocation_info);
      }
      content = &(ms->next);
    }
    p = &(msg->next);
  ...
  if (output_for_dyld) {
    if (strip_level != STRIP_ALL) {
      ...
      if (output_symtab_info.symtab_command.symoff != offset)
        output_flush(offset, output_symtab_info.symtab_command.symoff - offset);
 ...
 */
__private_extern__ void output_flush(unsigned long offset, unsigned long size) {
  ...
  */
  ...
  if (offset + size > output_size)
    fatal("internal error: output_flush(offset = %lu, size = %lu) out "
          "of range for output_size = %lu",
          offset, size, output_size);
  ...
  if (before != NULL && before->offset + before->size == offset) {
    ...
    if (after != NULL && offset + size == after->offset) {
      ...
      if (after->written_size == 0)
        write_size =
            trnc(after->offset + after->size - write_offset, host_pagesize);
      else
        write_size = trnc(after->written_offset - write_offset, host_pagesize);
      if (write_size != 0) {
  ...
  if (write_size != 0) {
    ...
    if (write(fd, output_addr + write_offset, write_size) != (int)write_size)
      system_fatal("can't write to output file");
    if ((r = vm_deallocate(mach_task_self(),
                           (vm_address_t)(output_addr + write_offset),
...
static void output_headers(void) {
  ...
  while (*p) {
    ...
    while (*content) {
      ms = *content;
      memcpy(output_addr + header_offset, &(ms->s), sizeof(struct section));
  ...
  while (mdl != NULL) {
    ...
    if (mdl->output_id == FALSE) {
       ...
       */
      some_symbols_referenced = mdl->dynamic_library->some_symbols_referenced;
      some_non_weak_refs = mdl->dynamic_library->some_non_weak_refs;
      for (i = 0; i < mdl->dynamic_library->nsub_images; i++) {
        if (mdl->dynamic_library->sub_images[i]->some_symbols_referenced ==
            TRUE) {
          some_symbols_referenced = TRUE;
          if (mdl->dynamic_library->sub_images[i]->some_non_weak_refs == TRUE)
            some_non_weak_refs = TRUE;
        }
      }
      if ((some_symbols_referenced == TRUE && some_non_weak_refs == FALSE) ||
  ...
  /* next the symbol table load command */
  memcpy(output_addr + header_offset, &(output_symtab_info.symtab_command),
         output_symtab_info.symtab_command.cmdsize);
  ...
  if (output_uuid_info.uuid_command.cmdsize != 0) {
    memcpy(output_addr + header_offset, &(output_uuid_info.uuid_command),
  ...
  if (output_thread_info.thread_in_output == TRUE) {
    ...
    memcpy(output_addr + header_offset, &(output_thread_info.thread_command),
    ...
    if (output_thread_info.second_count != 0) {
      ...
      memcpy(output_addr + header_offset, &(output_thread_info.second_flavor),
             sizeof(long));
      header_offset += sizeof(long);
      /* the count of longs of the second thread state */
      memcpy(output_addr + header_offset, &(output_thread_info.second_count),
             sizeof(long));
      header_offset += sizeof(long);
      /* the second thread state */
      memcpy(output_addr + header_offset, output_thread_info.second_state,
  ...
  if (host_byte_sex != target_byte_sex)
    if (swap_object_headers(mh, lc) == FALSE)
      fatal("internal error: swap_object_headers() failed in "
            "output_headers()");
#ifndef RLD
  output_flush(0, sizeof(struct mach_header) + output_mach_header.sizeofcmds);
#endif /* !defined(RLD) */
}
