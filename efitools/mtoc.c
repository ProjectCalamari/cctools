...
static void process_arch(struct arch *archs, uint32_t narchs);
static void process_32bit_arch(struct arch *arch);
static void process_64bit_arch(struct arch *arch);
static void layout_output(struct ofile *ofile);
static void create_output(struct ofile *ofile, char *out);
static void create_ms_dos_stub(struct ms_dos_stub *p);
static void usage(void);

static void create_32bit_symbol_table(struct arch *arch);
static void create_64bit_symbol_table(struct arch *arch);
...
static void create_base_reloc(struct arch *arch);
static void gather_base_reloc_info(uint32_t addr,
                                   struct relocation_info *relocs,
                                   uint32_t nreloc, cpu_type_t cpu_type,
                                   uint32_t length, int macho_reloc_type,
                                   int base_reloc_type);
static void add_base_reloc(uint64_t addr, uint32_t type);
static void make_base_relocs(void);
static int cmp_base_relocs(struct base_reloc *x1, struct base_reloc *x2);
static uint32_t checksum(unsigned char *buf);
static void string_to_uuid(char *string, uint8_t *uuid);

static void create_debug(struct arch *arch);
static void set_debug_addrs_and_offsets(void);
    ...
    if (strcmp(argv[i], "-subsystem") == 0) {
      ...
      if (section_alignment > MAXALIGN)
        fatal("argument to -section_alignment: %x (hex) must "
              ...
              section_alignment, (unsigned int)MAXALIGN);
      section_alignment_specified = TRUE;
      if (align_specified == TRUE && section_alignment != file_alignment)
      ...
      if (file_alignment > MAXALIGN)
        fatal("argument to -file_alignment: %x (hex) must "
              "equal to or less than %x (hex)",
              file_alignment, (unsigned int)MAXALIGN);
      ...
      if (sscanf(argv[i + 1], "%u.%u", &majorVersion, &minorVersion) != 2) {
...
static void process_arch(struct arch *archs, uint32_t narchs) {
   ...
   */
  if (narchs != 1)
    fatal("input file: %s must only have one architecture", archs->file_name);
  ...
  if (archs->object->mh != NULL) {
    ...
    if (archs->object->mh64->filetype == MH_DYLIB ||
      ...
      if (entry_point == NULL && archs->object->mh64->filetype == MH_DYLIB)
      ...
      if (strcmp(sg->segname, SEG_TEXT) == 0)
        ...
        for (j = 0; j < sg->nsects; j++, s++) {
          if (strcmp(s->sectname, SECT_COMMON) == 0 ||
            ...
            nscns++;
          } else if (s->size != 0 && strcmp(s->sectname, SECT_DATA) != 0)
            fatal("input file: %s contains Mach-O section "
                  "(%.16s,%.16s) unsupported for conversion "
                  "to a pecoff file",
                  arch->file_name, s->segname, s->sectname);
        }
...
/* i386 thread states on older releases */
...
#endif /* i386_THREAD_STATE == -1 */
          {
            i386_thread_state_t *cpu = (i386_thread_state_t *)state;
            entry = cpu->eip;
        ...
        case CPU_TYPE_ARM:
          ...
          case ARM_THREAD_STATE: {
            arm_thread_state_t *cpu = (arm_thread_state_t *)state;
            entry = cpu->__pc;
    ...
    if (lc->cmd == LC_SEGMENT) {
      ...
      if (strcmp(sg->segname, SEG_TEXT) == 0) {
        ...
        scnhdrs[j].s_flags = EFI_IMAGE_SCN_MEM_EXECUTE |
                             EFI_IMAGE_SCN_MEM_READ | EFI_IMAGE_SCN_CNT_CODE;
        ...
        scnhdrs[j].s_lnnoptr = 0;
        scnhdrs[j].s_nlnno = 0;
        scnhdrs[j].s_flags = EFI_IMAGE_SCN_MEM_READ | EFI_IMAGE_SCN_MEM_WRITE |
        ...
        for (i = 0; i < sg->nsects; i++, s++) {
          ...
          scnhdrs[j].s_lnnoptr = 0;
          scnhdrs[j].s_nlnno = 0;
          scnhdrs[j].s_flags = IMAGE_SCN_MEM_READ | IMAGE_SCN_MEM_WRITE |
...
static void process_64bit_arch(struct arch *arch) {
  ...
  for (i = 0; i < arch->object->mh64->ncmds; i++) {
    if (lc->cmd == LC_SEGMENT_64) {
      ...
      for (i = 0; i < sg64->nsects; i++, s64++) {
        ...
        section_names_size += strlen("LC_SEGMENT.") + strlen(s64->segname) + 1 +
      ...
      while (state < p) {
        ...
        case CPU_TYPE_X86_64:
          ...
          case x86_THREAD_STATE64: {
            x86_thread_state64_t *cpu64 = (x86_thread_state64_t *)state;
            /*
        ...
        case CPU_TYPE_ARM64:
          ...
          case ARM_THREAD_STATE64: {
            arm_thread_state64_t *cpu64 = (arm_thread_state64_t *)state;
            /*
             * The aouthdr_64 struct only allows for a
             * 32-bit entry point.
             */
  ...
  for (i = 0; i < arch->object->mh64->ncmds; i++) {
    if (lc->cmd == LC_SEGMENT_64) {
      ...
      if (strcmp(sg64->segname, SEG_TEXT) == 0) {
        ...
        scnhdrs[j].s_size = (uint32_t)rnd64(sg64->filesize, file_alignment);
        scnhdrs[j].s_relptr = 0;
        scnhdrs[j].s_lnnoptr = 0;
        scnhdrs[j].s_nlnno = 0;
        scnhdrs[j].s_flags = EFI_IMAGE_SCN_MEM_EXECUTE |
      ...
      for (i = 0; i < sg64->nsects; i++, s64++) {
        ...
        *section_name = '\0'; /* start of next section name */
        ...
        scnhdrs[j].s_nlnno = 0;
        scnhdrs[j].s_flags =
            IMAGE_SCN_MEM_EXECUTE | IMAGE_SCN_CNT_CODE | IMAGE_SCN_MEM_WRITE;
  ...
  if (reloc_size != 0) {
    ...
    scnhdrs[j].s_flags = EFI_IMAGE_SCN_MEM_READ |
                         EFI_IMAGE_SCN_CNT_INITIALIZED_DATA |
                         EFI_IMAGE_SCN_MEM_DISCARDABLE |
                         EFI_IMAGE_SCN_CNT_CODE | EFI_IMAGE_SCN_MEM_EXECUTE;
    reloc_scnhdr = scnhdrs + j;
    scn_contents[j] = reloc_contents;
    j++;
    debug_addr = reloc_addr + reloc_scnhdr->s_size;
...
static void layout_output(struct ofile *ofile) {
   ...
   * in the output file.
   */
  header_size = sizeof(struct ms_dos_stub) + sizeof(signature) +
                sizeof(struct filehdr) + nscns * sizeof(struct scnhdr);
  if (ofile->mh != NULL)
  ...
  filehdr.f_flags =
      ...
      EFI_IMAGE_FILE_32BIT_MACHINE | EFI_IMAGE_FILE_DEBUG_STRIPPED;
  if (ofile->mh64 != NULL)
...
static void create_output(struct ofile *ofile, char *out) {
  ...
  if (buf == NULL)
    fatal("Can't allocate buffer for output file (size = %u)", output_size);
  ...
  for (i = 0; i < nscns; i++) {
    if ((scnhdrs[i].s_flags & EFI_IMAGE_SCN_CNT_UNINITIALIZED_DATA) == 0) {
      memcpy(buf + scnhdrs[i].s_scnptr, scn_contents[i],
#ifndef HACK_TO_MATCH_TEST_CASE
             scnhdrs[i].s_size);
#else
             scnhdrs[i].s_vsize);
...
static void create_32bit_symbol_table(struct arch *arch) {
...
#ifdef HACK_TO_MATCH_TEST_CASE
  uint32_t j, n_sect, bss_n_sect, common_n_sect, bss_addr, common_addr, size;
  struct load_command *lc;
  struct segment_command *sg;
  struct section *s;
#endif /* HACK_TO_MATCH_TEST_CASE */
  ...
  for (i = 0; i < st->nsyms; i++) {
    ...
    if ((syms[i].n_type & N_TYPE) == N_UNDF) {
      if (found_undef == FALSE) {
        error("input file: %s contains undefined symbols:", arch->file_name);
      }
      found_undef = TRUE;
  ...
  for (i = 0; i < st->nsyms; i++) {
    if ((syms[i].n_type & N_STAB) == 0 && (syms[i].n_type & N_TYPE) == N_SECT &&
        (syms[i].n_sect == bss_n_sect || syms[i].n_sect == common_n_sect)) {
      nsyments++;
      if (syms[i].n_un.n_strx != 0) {
    ...
    if ((syms[i].n_type & N_STAB) == 0 && (syms[i].n_type & N_TYPE) == N_SECT &&
      ...
      if (syms[i].n_un.n_strx != 0) {
        ...
        if (size > EFI_IMAGE_SIZEOF_SHORT_NAME) {
          ...
          strncpy(syments[j].name.short_name, strs + syms[i].n_un.n_strx,
...
static void create_64bit_symbol_table(struct arch *arch) {
  ...
  if (entry_point != NULL) {
    for (i = 0; i < st->nsyms; i++) {
      if ((syms64[i].n_type & N_STAB) == 0 && syms64[i].n_un.n_strx != 0 &&
    ...
    if (i == st->nsyms)
      fatal("can't find symbol for -e %s in input file: %s", entry_point,
            arch->file_name);
  }
...
static void create_base_reloc(struct arch *arch) {
  ...
  for (i = 0; i < ncmds; i++) {
    if (lc->cmd == LC_SEGMENT) {
      ...
      s = (struct section *)((char *)sg + sizeof(struct segment_command));
      for (j = 0; j < sg->nsects; j++) {
        relocs = (struct relocation_info *)(object_addr + s[j].reloff);
        if (swapped)
          swap_relocation_info(relocs, s[j].nreloc, host_byte_sex);
        if (arch->object->mh_cputype == CPU_TYPE_I386)
          gather_base_reloc_info(s[j].addr, relocs, s[j].nreloc, CPU_TYPE_I386,
                                 2, GENERIC_RELOC_VANILLA,
                                 IMAGE_REL_BASED_HIGHLOW);
        else if (arch->object->mh_cputype == CPU_TYPE_ARM)
          gather_base_reloc_info(s[j].addr, relocs, s[j].nreloc, CPU_TYPE_ARM,
                                 2, GENERIC_RELOC_VANILLA,
                                 IMAGE_REL_BASED_HIGHLOW);
        if ((s[j].flags & SECTION_TYPE) == S_NON_LAZY_SYMBOL_POINTERS) {
          for (addr = s[j].addr; addr < s[j].addr + s[j].size; addr += 4) {
      ...
      for (j = 0; j < sg64->nsects; j++) {
        relocs = (struct relocation_info *)(object_addr + s64[j].reloff);
        if (swapped)
          swap_relocation_info(relocs, s64[j].nreloc, host_byte_sex);
        if (arch->object->mh_cputype == CPU_TYPE_X86_64)
          gather_base_reloc_info((uint32_t)s64[j].addr, relocs, s64[j].nreloc,
          ...
          gather_base_reloc_info((uint32_t)s64[j].addr, relocs, s64[j].nreloc,
                                 ...
                                 IMAGE_REL_BASED_DIR64);
        if ((s64[j].flags & SECTION_TYPE) == S_NON_LAZY_SYMBOL_POINTERS) {
          for (addr = s64[j].addr; addr < s64[j].addr + s64[j].size;
               addr += 8) {
  ...
  if (dyst != NULL && dyst->nlocrel != 0) {
    relocs = (struct relocation_info *)(object_addr + dyst->locreloff);
    if (swapped)
...
static void gather_base_reloc_info(uint32_t addr,
  ...
  for (i = 0; i < nreloc; i++) {
    ...
    if (r_extern == 0 && r_pcrel == 0 && r_length == length &&
        r_type == macho_reloc_type)
...
static void add_base_reloc(uint64_t addr, uint32_t type) {
  ...
  if (!max) {
    max = 128;
    base_relocs = (struct base_reloc *)malloc(max * sizeof(struct base_reloc));
  }
  if (nbase_reloc >= max) {
    new_base_relocs = malloc(2 * max * sizeof(struct base_reloc));
    memcpy(new_base_relocs, base_relocs, max * sizeof(struct base_reloc));
    max *= 2;
    free(base_relocs);
...
static void create_debug(struct arch *arch) {
  ...
  dde->SizeOfData =
      sizeof(struct mtoc_debug_info) + (uint32_t)strlen(debug_filename) + 1;
  /*
 ...
 */
static void set_debug_addrs_and_offsets(void) {
  dde->AddressOfRawData =
      debug_scnhdr->s_vaddr + sizeof(struct debug_directory_entry);
  dde->PointerToRawData =
      debug_scnhdr->s_scnptr + sizeof(struct debug_directory_entry);
  if (swapped) {
...
static void string_to_uuid(char *string, uint8_t *uuid) {
  ...
  count =
      sscanf(string, UUID_FORMAT_STRING, &uuid[0], &uuid[1], &uuid[2], &uuid[3],
