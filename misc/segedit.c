...
static struct mach_header_64 *mhp64; /* pointer to the input file's mach header
                                        for 64-bit files */
...
static void extract_sections(void);
static void extract_section(char *segname, char *sectname, uint32_t flags,
                            uint32_t offset, uint32_t size);
static void replace_sections(void);
static void search_for_replace_section(char *segname, char *sectname,
                                       uint32_t seg_flags, uint32_t sect_flags,
                                       uint32_t offset, uint32_t size);
static int cmp_qsort(const struct rep_seg *seg1, const struct rep_seg *seg2);
...
int main(int argc, char *argv[], char *envp[]) {
  ...
  if (replaces != NULL && (output == NULL || strncmp("-", output, 2) == 0))
  ...
  if (magic == FAT_MAGIC || magic == FAT_MAGIC_64)
    ...
    if (magic == SWAP_INT(FAT_MAGIC) || magic == SWAP_INT(FAT_MAGIC_64))
      ...
      fatal("file: %s is a fat file (%s only operates on Mach-O files, "
            "use lipo(1) on it to get a Mach-O file)",
            input, progname);
  ...
  if (magic == SWAP_INT(MH_MAGIC) || magic == MH_MAGIC) {
    ...
    if (mhp->sizeofcmds + sizeof(struct mach_header) > input_size)
      fatal("truncated or malformed object (load commands would "
            "extend past the end of the file) in: %s",
            input);
  ...
  for (i = 0; i < mh_ncmds; i++) {
    ...
    if (l.cmdsize % sizeof(uint32_t) != 0)
      error("load command %u size not a multiple of "
            "sizeof(uint32_t) in: %s",
            i, input);
    if (l.cmdsize <= 0)
      fatal("load command %u size is less than or equal to zero "
            "in: %s",
            i, input);
    if ((char *)lcp + l.cmdsize > (char *)load_commands + mh_sizeofcmds)
    ...
    case LC_DYLD_EXPORTS_TRIE: {
      struct linkedit_data_command *ldcp = (struct linkedit_data_command *)lcp;
      if (swapped)
...
static void extract_sections(void) {
  ...
  for (i = 0; i < mh_ncmds; i++) {
    if (lcp->cmd == LC_SEGMENT) {
      ...
      for (j = 0; j < sgp->nsects; j++) {
        extract_section(sp->segname, sp->sectname, sp->flags, sp->offset,
                        sp->size);
        sp++;
      }
      ...
      for (j = 0; j < sgp64->nsects; j++) {
        extract_section(sp64->segname, sp64->sectname, sp64->flags,
                        (uint32_t)sp64->offset, (uint32_t)sp64->size);
        sp64++;
      }
    }
  ...
  while (ep != NULL) {
    if (ep->found == 0) {
      error("section (%s,%s) not found in: %s", ep->segname, ep->sectname,
            input);
    }
    ep = ep->next;
  }
...
static void extract_section(char *segname, char *sectname, uint32_t flags,
  ...
  ep = extracts;
  while (ep != NULL) {
    if (ep->found == 0 && strncmp(ep->segname, segname, 16) == 0 &&
      ...
      if (offset + size > input_size)
        fatal("truncated or malformed object (section "
              ...
              "end of the file) in: %s",
              segname, sectname, input);
      if (0 == strncmp("-", ep->filename, 2)) {
        printf("%s", (char *)input_addr + offset);
      } else {
        if ((fd = open(ep->filename, O_WRONLY | O_CREAT | O_TRUNC, 0666)) == -1)
          system_fatal("can't create: %s", ep->filename);
        if (write64(fd, (char *)input_addr + offset, size) != (ssize_t)size)
...
static void replace_sections(void) {
  ...
  for (i = 0; i < mh_ncmds; i++) {
    ...
    case LC_SEGMENT:
      ...
      for (j = 0; j < sgp->nsects; j++) {
        ...
        search_for_replace_section(sp->segname, sp->sectname, sgp->flags,
                                   sp->flags, sp->offset, sp->size);
        sp++;
      }
      break;
    case LC_SEGMENT_64:
      ...
      if (strcmp(sgp64->segname, SEG_LINKEDIT) != 0) {
        if (sgp64->flags & SG_NORELOC) {
          if (sgp64->filesize != 0) {
            if (sgp64->fileoff + sgp64->filesize > high_noreloc_seg)
              high_noreloc_seg = (uint32_t)(sgp64->fileoff + sgp64->filesize);
          ...
          if (sgp64->filesize != 0 &&
            ...
            high_reloc_seg = (uint32_t)(sgp64->fileoff + sgp64->filesize);
        }
      } else {
      ...
      for (j = 0; j < sgp64->nsects; j++) {
        ...
        search_for_replace_section(sp64->segname, sp64->sectname, sgp64->flags,
                                   sp64->flags, sp64->offset,
                                   (uint32_t)sp64->size);
        sp64++;
      }
      break;
    ...
    case LC_DYSYMTAB:
      ...
      if (dstp->extrefsymoff != 0 && dstp->extrefsymoff < low_linkedit)
        low_linkedit = dstp->extrefsymoff;
      if (dstp->indirectsymoff != 0 && dstp->indirectsymoff < low_linkedit)
    ...
    case LC_DYLD_INFO_ONLY: {
      struct dyld_info_command *dicp = (struct dyld_info_command *)lcp;
      no_seg_resize = TRUE;
      if (dicp->rebase_off != 0 && dicp->rebase_off < low_linkedit)
        low_linkedit = dicp->rebase_off;
      if (dicp->bind_off != 0 && dicp->bind_off < low_linkedit)
        low_linkedit = dicp->bind_off;
      if (dicp->weak_bind_off != 0 && dicp->weak_bind_off < low_linkedit)
        low_linkedit = dicp->weak_bind_off;
      if (dicp->lazy_bind_off != 0 && dicp->lazy_bind_off < low_linkedit)
  ...
  while (rp != NULL) {
    if (rp->found == 0) {
      error("section (%s,%s) not found in: %s", rp->segname, rp->sectname,
            input);
    } else {
      if (stat(rp->filename, &stat_buf) == -1) {
        system_error("Can't stat file: %s to replace section "
                     "(%s,%s) with",
                     rp->filename, rp->segname, rp->sectname);
      }
  ...
  if (high_reloc_seg > low_noreloc_seg || high_reloc_seg > low_linkedit ||
   ...
   */
  segalign = get_seg_align(mhp, mhp64, load_commands, FALSE, input_size, input);
  pagesize = 1 << segalign;
  ...
  for (i = 0; i < nsegs; i++) {
    if (segs[i].sgp != NULL) { /* process the 32-bit file */
      ...
      if (segs[i].sgp->filesize != 0) {
        ...
        if (strcmp(segs[i].sgp->segname, SEG_LINKEDIT) != 0 || i != nsegs - 1) {
          if (no_seg_resize == FALSE) {
      ...
      if (segs[i].sgp->flags & SG_NORELOC) {
        ...
        for (j = 0; j < segs[i].sgp->nsects; j++) {
          ...
          rp = replaces;
          while (rp != NULL) {
            if (strncmp(rp->segname, sp->segname, sizeof(sp->segname)) == 0 &&
                strncmp(rp->sectname, sp->sectname, sizeof(sp->sectname)) ==
                    0) {
              sects[k + j].rp = rp;
              segs[i].modified = 1;
        ...
        if (strcmp(segs[i].sgp->segname, SEG_LINKEDIT) != 0 || i != nsegs - 1) {
          if (segs[i].sgp->filesize != rnd(oldsectsize, pagesize))
            fatal("contents of input file: %s not in a "
                  "format that the specified sections can "
                  "be replaced by this program",
                  input);
          segs[i].padsize = segs[i].sgp->filesize - newsectsize;
          if (no_seg_resize == FALSE) {
            segs[i].sgp->filesize = rnd32(newsectsize, pagesize);
            segs[i].sgp->vmsize = rnd32(newsectsize, pagesize);
          } else {
            if (segs[i].sgp->filesize != rnd(newsectsize, pagesize)) {
              fatal("input file: %s segment %.16s "
                    "cannot be resized (filesize %u => "
                    "%llu)",
                    input, segs[i].sgp->segname, segs[i].sgp->filesize,
                    rnd(newsectsize, pagesize));
            }
            if (segs[i].sgp->vmsize != rnd(newsectsize, pagesize)) {
              fatal("input file: %s segment %.16s "
                    "cannot be resized (vmsize %u => "
                    "%llu)",
                    input, segs[i].sgp->segname, segs[i].sgp->vmsize,
                    rnd(newsectsize, pagesize));
            }
          }
        }
      ...
      if (segs[i].sgp64->flags & SG_NORELOC) {
        ...
        for (j = 0; j < segs[i].sgp64->nsects; j++) {
          ...
          rp = replaces;
          while (rp != NULL) {
            if (strncmp(rp->segname, sp64->segname, sizeof(sp64->segname)) ==
                    0 &&
                strncmp(rp->sectname, sp64->sectname, sizeof(sp64->sectname)) ==
                    0) {
              sects[k + j].rp = rp;
              segs[i].modified = 1;
              sp64->size = rnd(rp->size, 1 << sp64->align);
              break;
            }
            rp = rp->next;
          }
        ...
        if (strcmp(segs[i].sgp64->segname, SEG_LINKEDIT) != 0 ||
            i != nsegs - 1) {
          if (segs[i].sgp64->filesize != rnd(oldsectsize, pagesize))
            fatal("contents of input file: %s not in a "
                  "format that the specified sections can "
                  "be replaced by this program",
                  input);
          segs[i].padsize = (uint32_t)(segs[i].sgp64->filesize - newsectsize);
          if (no_seg_resize == FALSE) {
            segs[i].sgp64->filesize = rnd(newsectsize, pagesize);
            segs[i].sgp64->vmsize = rnd(newsectsize, pagesize);
          } else {
            if (segs[i].sgp64->filesize != rnd(newsectsize, pagesize)) {
              fatal("input file: %s segment %.16s "
                    "cannot be resized (filesize %llu => "
                    "%llu)",
                    input, segs[i].sgp64->segname, segs[i].sgp64->filesize,
                    rnd(newsectsize, pagesize));
            }
            if (segs[i].sgp64->vmsize != rnd(newsectsize, pagesize)) {
              fatal("input file: %s segment %.16s "
                    "cannot be resized (vmsize %llu => "
                    "%llu)",
                    input, segs[i].sgp64->segname, segs[i].sgp64->vmsize,
                    rnd(newsectsize, pagesize));
            }
          }
        }
      }
      if (strcmp(segs[i].sgp64->segname, SEG_LINKEDIT) != 0 || i != nsegs - 1) {
        oldoffset += segs[i].filesize;
   ...
   * the link edit info.
   */
  if ((outfd = open(output, O_CREAT | O_WRONLY | O_TRUNC, input_mode)) == -1)
  ...
  for (i = 0; i < nsegs; i++) {
    if (segs[i].modified) {
        ...
        for (j = 0; j < segs[i].sgp->nsects; j++) {
          ...
          if (rp != NULL) {
            ...
            for (l = rp->size + 1; l < sp->size; l++)
              *((char *)sect_addr + l) = '\0';
            if (write64(outfd, (char *)sect_addr, sp->size) != sp->size)
            ...
            if (close(sectfd) == -1)
              system_error("can't close file: %s to replace "
                           "section (%s,%s) with",
                           rp->filename, rp->segname, rp->sectname);
            if ((r = vm_deallocate(mach_task_self(), (vm_address_t)sect_addr,
                                   rp->size)) != KERN_SUCCESS)
              mach_fatal(r,
                         "Can't deallocate memory for "
                         "mapped file: %s",
                         rp->filename);
          } else {
            ...
            if (sects[k + j].offset + sp->size > input_size)
              fatal("truncated or malformed object file: %s "
                    ...
                    "end of the file)",
                    input, sp->segname, sp->sectname);
            if (write64(outfd, (char *)input_addr + sects[k + j].offset,
                        sp->size) != sp->size)
        ...
        for (j = 0; j < segs[i].sgp64->nsects; j++) {
          ...
          if (rp != NULL) {
            ...
            for (l = rp->size + 1; l < sp64->size; l++)
              *((char *)sect_addr + l) = '\0';
            if (write64(outfd, (char *)sect_addr, sp64->size) != sp64->size)
              system_fatal("can't write new section contents "
                           "for section (%s,%s) to output file: %s",
                           rp->segname, rp->sectname, output);
            if (close(sectfd) == -1)
            ...
            if (sects[k + j].offset + sp64->size > input_size)
              fatal("truncated or malformed object file: %s "
                    "(section (%.16s,%.16s) extends past the "
                    "end of the file)",
                    input, sp64->segname, sp64->sectname);
            if (write64(outfd, (char *)input_addr + sects[k + j].offset,
                        sp64->size) != sp64->size)
              system_fatal("can't write section contents for "
                           "section (%s,%s) to output file: %s",
      ...
      if (segs[i].sgp != NULL) {
        ...
        if (strcmp(segs[i].sgp64->segname, SEG_LINKEDIT) != 0 ||
            i != nsegs - 1) {
          if (segs[i].fileoff + segs[i].sgp64->filesize > input_size)
            fatal("truncated or malformed object file: %s "
                  "(segment: %s extends past the end of "
                  "the file)",
                  input, segs[i].sgp64->segname);
...
static void search_for_replace_section(char *segname, char *sectname,
  ...
  rp = replaces;
  while (rp != NULL) {
    if (rp->found == 0 && strncmp(rp->segname, segname, 16) == 0 &&
        strncmp(rp->sectname, sectname, 16) == 0) {
      if (sect_flags == S_ZEROFILL || sect_flags == S_THREAD_LOCAL_ZEROFILL) {
