...
swap_base_relocation_block_header(struct base_relocation_block_header *h,
...
__private_extern__ void
swap_base_relocation_entry(struct base_relocation_entry *b, uint32_t n,
                           enum byte_sex target_byte_sex) {
  uint32_t i;
...
__private_extern__ void swap_ms_dos_stub(struct ms_dos_stub *m,
                                         enum byte_sex target_byte_sex) {
  int i;
...
__private_extern__ void swap_filehdr(struct filehdr *f,
                                     enum byte_sex target_byte_sex) {
...
__private_extern__ void swap_aouthdr(struct aouthdr *a,
                                     enum byte_sex target_byte_sex) {
  int i;
  ...
  a->MajorOperatingSystemVersion = SWAP_SHORT(a->MajorOperatingSystemVersion);
  a->MinorOperatingSystemVersion = SWAP_SHORT(a->MinorOperatingSystemVersion);
...
__private_extern__ void swap_aouthdr_64(struct aouthdr_64 *a,
                                        enum byte_sex target_byte_sex) {
  int i;
...
__private_extern__ void swap_scnhdr(struct scnhdr *s, uint32_t n,
                                    enum byte_sex target_byte_sex) {
  uint32_t i;
...
__private_extern__ void swap_syment(struct syment *s, uint32_t n,
                                    enum byte_sex target_byte_sex) {
  uint32_t i;

  for (i = 0; i < n; i++) {
    if (s[i].name.long_name.zeros == 0)
      s[i].name.long_name.strtable_offset =
          SWAP_INT(s[i].name.long_name.strtable_offset);
...
__private_extern__ void
swap_debug_directory_entry(struct debug_directory_entry *d,
                           enum byte_sex target_byte_sex) {
...
__private_extern__ void swap_mtoc_debug_info(struct mtoc_debug_info *m,
                                             enum byte_sex target_byte_sex) {
