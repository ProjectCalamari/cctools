...
int main(int argc, const char *argv[]) {
  ...
  while (argc > 0) {
    if (read_options && *argv && '-' == **argv) {
      if (0 == strcmp("-arch", *argv)) {
      ...
      } else if (0 == strcmp("-r", *argv) || 0 == strcmp("-replace", *argv)) {
        ...
        gOptions.removeItems =
            reallocf(gOptions.removeItems, sizeof(*gOptions.removeItems) *
        ...
        gOptions.setItems =
            reallocf(gOptions.setItems,
                     sizeof(*gOptions.setItems) * (gOptions.numSetItems + 1));
        struct set_item *item = &gOptions.setItems[gOptions.numSetItems++];
        memset(item, 0, sizeof(*item));
        item->type = kAddTool;
      ...
      else if (0 == strcmp("-show", *argv) || 0 == strcmp("-show-all", *argv)) {
        if (gOptions.command)
          usage("option %s cannot be used with other commands", *argv);
  ...
  for (uint32_t iitem = 0; iitem < gOptions.numSetItems; ++iitem) {
    ...
    for (uint32_t jitem = 0; jitem < iitem; ++jitem) {
      ...
      if (gOptions.setItems[iitem].platform ==
              gOptions.setItems[jitem].platform &&
          gOptions.setItems[iitem].tool == gOptions.setItems[jitem].tool) {
        uint32_t plid = gOptions.setItems[iitem].platform;
        const char *plname = platform_name_for_id(plid);
  ...
  if (0 == res && gOptions.narch) {
    for (uint32_t iopt = 0; iopt < gOptions.narch; ++iopt) {
      ...
      for (uint32_t iarch = 0; false == found && iarch < file.nfat_arch;
        ...
        if (a->cputype == b->cputype && (a->cpusubtype & ~CPU_SUBTYPE_MASK) ==
                                            (b->cpusubtype & ~CPU_SUBTYPE_MASK))
          found = true;
      }
      if (false == found) {
        fprintf(stderr,
                "%s error: %s file does not contain "
                "architecture: %s\n",
                gProgramName, gOptions.inPath, a->name);
        res = -1;
      }
    }
  }
...
int command_remove(struct file *fb) {
  ...
  for (uint32_t iitem = 0; iitem < gOptions.numRemoveItems; ++iitem) {
    ...
    if (!found) {
      for (uint32_t jitem = 0; !found && jitem < gOptions.numRemoveItems;
           ++jitem) {
    ...
    if (!found) {
      ...
      if (plname)
        fprintf(stderr,
                "%s error: no build verion load command "
    ...
    if (bv) {
      ...
      for (uint32_t itool = 0; itool < bv->ntools; ++itool) {
        struct build_tool_version *bt =
            (struct build_tool_version *)(((unsigned char *)(bv + 1)) +
                                          (itool *
  ...
  for (uint32_t icmd = 0; icmd < lcmds->count; ++icmd) {
    ...
    if (lc->cmd == LC_SEGMENT) {
      ...
      for (uint32_t isect = 0; isect < sg->nsects; ++isect) {
        struct section *sc = (struct section *)(((unsigned char *)(sg + 1)) +
                                                isect * sizeof(*sc));
      ...
      for (uint32_t isect = 0; isect < sg->nsects; ++isect) {
        struct section_64 *sc =
            (struct section_64 *)(((unsigned char *)(sg + 1)) +
                                  isect * sizeof(*sc));
        uint32_t type = sc->flags & SECTION_TYPE;
        if (type == S_ZEROFILL)
          continue;
        ...
        if (sc->offset < sectoffset)
          sectoffset = sc->offset;
               ...
               lc->cmd == LC_VERSION_MIN_IPHONEOS ||
               lc->cmd == LC_VERSION_MIN_WATCHOS ||
               lc->cmd == LC_VERSION_MIN_TVOS || lc->cmd == LC_BUILD_VERSION) {
      ...
      for (uint32_t iitem = 0; !modify && iitem < gOptions.numRemoveItems;
           ++iitem) {
      ...
      for (uint32_t iitem = 0; !modify && iitem < nitem; ++iitem) {
        if (items[iitem].type == kSetBuild && items[iitem].platform == platform)
          modify = true;
      }
  ...
  if (totalcmdspace < sizeofnewcmds) {
    if (fb->nfat_arch > 1 || gOptions.narch) {
      ...
      archInfo = NXGetArchInfoFromCpuType(fb->mh.cputype, fb->mh.cpusubtype);
      if (archInfo)
        fprintf(stderr,
                "%s error: %s (%s) not enough space to "
                "hold load commands\n",
                gProgramName, gOptions.inPath, archInfo->name);
      else
        fprintf(stderr,
                "%s error: %s (%u, %u) not enough space to "
                "hold load commands\n",
                gProgramName, gOptions.inPath, fb->mh.cputype,
                fb->mh.cpusubtype);
    } else {
      fprintf(stderr,
              "%s error: %s not enough space to hold load "
              "commands\n",
              gProgramName, gOptions.inPath);
    }
  ...
  for (uint32_t icmd = 0; icmd < lcmds->count; ++icmd) {
    ...
    if (lc->cmd == LC_VERSION_MIN_MACOSX ||
      ...
      for (uint32_t iitem = 0; copy && iitem < gOptions.numRemoveItems;
           ++iitem) {
        if (gOptions.removeItems[iitem].type == kRemoveBuild &&
            gOptions.removeItems[iitem].platform == platform)
          copy = false;
  ...
  for (uint32_t iitem = 0; iitem < nitem; ++iitem) {
    if (items[iitem].type == kSetBuild) {
      struct build_version_command *bv = (struct build_version_command *)q;
      ...
      struct build_tool_version *tools = (struct build_tool_version *)q;
      ...
      for (uint32_t itool = iitem + 1; itool < nitem; ++itool) {
        ...
        if (items[itool].platform != bv->platform)
          continue;
        struct build_tool_version *bt = (struct build_tool_version *)q;
...
int command_set(struct file *fb) {
  ...
  for (uint32_t iitem = 0; iitem < gOptions.numSetItems; ++iitem) {
      ...
      for (uint32_t jitem = 0; !found && jitem < nitem; ++jitem) {
        if (platform == items[jitem].platform &&
          ...
          if (items[jitem].versmin) {
            fprintf(stderr,
                    "%s error: version min load commands "
    ...
    if (!found) {
      ...
      if (plname)
        fprintf(stderr,
                "%s error: no build verion load command found "
                "for platform %s\n",
                gProgramName, plname);
      else
        fprintf(stderr,
    ...
    if (bv) {
      ...
      if (!gOptions.replace) {
        for (uint32_t itool = 0; itool < bv->ntools; ++itool) {
          ...
          found = false;
          for (uint32_t jitem = 0; !found && jitem < gOptions.numSetItems;
               ++jitem) {
  ...
  // rdar://139215820 (install_name_tool/vtool should reserve sufficient space
  ...
  for (uint32_t iitem = 0; iitem < nitem; ++iitem) {
    if (items[iitem].type == kSetBuild) {
      if (items[iitem].versmin) {
        struct version_min_command *vmc = (struct version_min_command *)q;
    ...
    if (items[iitem].type == kSetSource) {
      struct source_version_command *sv = (struct source_version_command *)q;
...
int command_show(struct file *fb) {
  ...
  for (uint32_t icmd = 0; icmd < lcmds->count; ++icmd) {
    ...
    // display all build and source version load commands
      ...
      if (gOptions.show == kShowAll || gOptions.show == kShowSource) {
        struct source_version_command *sv = (struct source_version_command *)lc;
        printf("Load command %u\n", icmd);
  ...
  if (magic == MH_MAGIC || magic == MH_CIGAM || magic == MH_MAGIC_64 ||
    ...
    for (uint32_t i = 0; i < fb->nfat_arch; ++i) {
      if (fb->len < fb->fat_archs[i].offset + fb->fat_archs[i].size) {
        fprintf(stderr,
                "%s error: %s file #%d for cputype (%u, %u) "
                "extends beyond file boundaries (%zu < %u + %u)\n",
                gProgramName, path, i, fb->fat_archs[i].cputype,
                fb->fat_archs[i].cpusubtype, fb->len, fb->fat_archs[i].offset,
                fb->fat_archs[i].size);
        free(fb->buf);
        free(fb->fat_archs);
        return -1;
...
int file_write(const char *path, struct file *fb) {
  ...
  for (uint32_t iarch = 0;
       0 == res && !(warn && resign) && iarch < fb->nfat_arch; ++iarch) {
    // prepare the mach-o for reading
    ...
    for (uint32_t icmd = 0; !(warn && resign) && icmd < lcmds->count; ++icmd) {
      ...
      if (lc->cmd == LC_CODE_SIGNATURE) {
        ...
        if (codedir_is_linker_signed(dataaddr, datasize)) {
          ...
          if (codedir_create_object(path, fileaddr, filesize, &codedirs[iarch]))
            res = -1;
        } else
#endif /* CODEDIRECTORY_SUPPORT */
          warn = true;
  ...
  if (0 == res && warn) {
    fprintf(stderr,
            "%s warning: code signature will be invalid "
            "for %s\n",
            gProgramName, path);
  }
...
struct lcmds *lcmds_alloc(struct file *fb) {
  ...
  for (uint32_t icmd = 0; icmd < fb->mh.ncmds; ++icmd) {
    ...
    if (fb->mh.sizeofcmds < p - fb->lcs + sizeof(struct load_command)) {
      fprintf(stderr,
              "%s error: load command %d extends beyond "
              "range\n",
              gProgramName, icmd);
      lcmds_free(lcmds);
      return NULL;
    ...
    if (fb->mh.sizeofcmds < p - fb->lcs + lc.cmdsize) {
      fprintf(stderr,
              "%s error: load command %d (0x%X) extends "
              "beyond range\n",
    ...
    if (LC_SEGMENT == lc.cmd) {
      ...
      struct segment_command *sg = (struct segment_command *)lcmds->items[icmd];
      if (fb->swap) {
...
void usage(const char *__restrict format, ...) {
  ...
  fprintf(stderr, "usage: %s [-arch <arch>] ... <show_command> <file>\n",
          basename);
  fprintf(stderr,
          "       %s [-arch <arch>] ... <set_command> ... [-replace] [-output "
          "<output>] "
          "<file>\n",
          basename);
  fprintf(stderr,
          "       %s [-arch <arch>] ... <remove_command> ... [-output "
          "<output>] <file>\n",
          basename);
  ...
  if (gOptions.command == kCommandHelp) {
    ...
    fprintf(stderr, "  platform and tool can also be specified by number\n");
  }
  exit(EXIT_FAILURE);
}
