...
static struct merged_dylib *lookup_merged_dylib(struct dylib_command *dl);
 ...
 */
__private_extern__ void create_dylib_id_command(void) {
  char *name;
  ...
  cmdsize = sizeof(struct dylib_command) + rnd(strlen(name) + 1, sizeof(long));
  dl = allocate(cmdsize);
  memset(dl, '\0', cmdsize);
  dl->cmd = LC_ID_DYLIB;
  dl->cmdsize = cmdsize;
 ...
 */
__private_extern__ void merge_dylibs(enum bool force_weak) {
  unsigned long i;
  ...
  for (i = 0; i < mh->ncmds; i++) {
    if (lc->cmd == LC_ID_DYLIB || lc->cmd == LC_LOAD_DYLIB ||
      ...
      if (merged_dylinker == NULL) {
        ...
        memset(merged_dylinker, '\0', sizeof(struct merged_dylinker));
 ...
 */
__private_extern__ void create_dylinker_id_command(void) {
  char *name;
  unsigned long cmdsize;
  struct dylinker_command *dyld;
  ...
  cmdsize =
      sizeof(struct dylinker_command) + rnd(strlen(name) + 1, sizeof(long));
 ...
 */
__private_extern__ void create_sub_framework_command(void) {
  char *name;
  unsigned long cmdsize;
 ...
 */
__private_extern__ unsigned long create_sub_umbrella_commands(void) {
  unsigned long i;
  ...
  sizeofcmds = 0;
  merged_sub_umbrellas =
      allocate(sizeof(struct merged_sub_umbrella) * nsub_umbrellas);
  for (i = 0; i < nsub_umbrellas; i++) {
    ...
    while (*p) {
      ...
      umbrella_framework_name =
          guess_short_name(mdl->dylib_name, &is_framework, &has_suffix);
 ...
 * It returns the total size of the load commands it creates.
 */
__private_extern__ unsigned long create_sub_library_commands(void) {
  unsigned long i;
  ...
  sizeofcmds = 0;
  merged_sub_librarys =
      allocate(sizeof(struct merged_sub_library) * nsub_librarys);
  for (i = 0; i < nsub_librarys; i++) {
    ...
    while (*p) {
      ...
      library_name =
          guess_short_name(mdl->dylib_name, &is_framework, &has_suffix);
      if (library_name != NULL && is_framework == FALSE &&
 ...
 * It returns the total size of the load commands it creates.
 */
__private_extern__ unsigned long create_sub_client_commands(void) {
  unsigned long i;
  char *name;
  unsigned long cmdsize, sizeofcmds;
  ...
  sizeofcmds = 0;
  merged_sub_clients =
      allocate(sizeof(struct merged_sub_client) * nallowable_clients);
  for (i = 0; i < nallowable_clients; i++) {
    name = allowable_clients[i];
    cmdsize =
        sizeof(struct sub_client_command) + rnd(strlen(name) + 1, sizeof(long));
    sub = allocate(cmdsize);
    memset(sub, '\0', cmdsize);
 ...
 */
__private_extern__ void add_dylib_segment(struct segment_command *sg,
                                          char *dylib_name,
