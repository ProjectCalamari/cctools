...
static void check_for_dylib(struct ofile *ofile, char *arch_name, void *cookie);

char *get_symfile_for_dylib(char *install_name, char *release_name,
...
char *get_dstfile_for_dylib(char *install_name, char *release_name,
                            enum bool *found_project, enum bool disablewarnings,
                            enum bool no_error_if_missing) {
  const char *dstroot;
  ...
  if (*install_name == '/') {
    ...
    ofile_process(image_file_name, NULL, 0, TRUE, TRUE, TRUE, FALSE,
                  check_for_dylib, &block);
...
char *find_dylib_in_root(char *install_name, const char *root) {
#ifndef __OPENSTEP__
  ...
  while ((ftsent = fts_read(fts)) != NULL) {
    ...
    if (S_ISREG(ftsent->fts_statp->st_mode) &&
       ...
       */
      block.check_result = TRUE;
      ofile_process(ftsent->fts_path, NULL, 0, TRUE, TRUE, TRUE, FALSE,
                    check_for_dylib, &block);
      if (block.check_result == TRUE) {
...
static void check_for_dylib(struct ofile *ofile, char *arch_name,
                            void *cookie) {
  uint32_t i;
  struct check_block *block;
...
#ifdef BIG_DEBUG
  printf("In check_for_dylib() ofile->file_name = %s", ofile->file_name);
  if (arch_name != NULL)
    printf(" arch_name = %s\n", arch_name);
  else
    printf("\n");
