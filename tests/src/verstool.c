...
int main(int argc, const char *argv[]) {
  ...
  if (0 == strcmp("list", *argv)) {
    ...
    while (argc > 1) {
      ...
      gOptions.ves =
          reallocf(gOptions.ves, sizeof(*gOptions.ves) * (gOptions.nve + 1));
...
static int cmd_list(const struct file *fb) {
  ...
  for (uint32_t icmd = 0; icmd < fb->mh.ncmds; ++icmd) {
    ...
    if (fb->mh.sizeofcmds < p - fb->lcs + lc.cmdsize) {
      fprintf(stderr, "%s error: load command %d (%x) extends beyond range\n",
    ...
    if (lc.cmd == LC_VERSION_MIN_MACOSX || lc.cmd == LC_VERSION_MIN_IPHONEOS ||
      ...
      // version name
      const char *name = platform_name_for_id(platform_id_for_lccmd(lc.cmd));
      if (!name)
        name = "(unknown)";
      printf("%s ", name);
...
static int cmd_write(const struct file *fb) {
  ...
  for (uint32_t icmd = 0; icmd < fb->mh.ncmds; ++icmd) {
    ...
    if (lc.cmd == LC_SEGMENT) {
      ...
      if (fb->swap) {
        ...
        swap_section((struct section *)(p + sizeof(sg)), sg.nsects,
                     NXHostByteOrder());
      }
    ...
    if (lc.cmd == LC_SEGMENT_64) {
      ...
      if (fb->swap) {
        ...
        swap_section_64((struct section_64 *)(p + sizeof(sg)), sg.nsects,
                        NXHostByteOrder());
      }
  ...
  for (uint32_t i = 0; i < gOptions.nve; ++i) {
    ...
    if (ve.type == kVersMin) {
      ...
      if (lccmd == 0) {
        ...
        if (name) {
          fprintf(stderr,
                  "%s error: version min load command not "
                  "supported for %s\n",
                  gProgramName, name);
        } else {
  ...
  if (write_handle_error(fd, &(fb->buf[sectoffset]), fb->len - sectoffset,
                         tmppath)) {
    close(fd);
    free(tmppath);
    return -1;
...
static void usage(void) {
  ...
  fprintf(stderr,
          "%s <input> write [(vm|bv) <platform> <minos> <sdk>] ... "
          "<output>\n",
          spaces);
