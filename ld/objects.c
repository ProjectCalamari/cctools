 ...
 */
__private_extern__ struct object_file *new_object_file(void) {
 ...
 */
__private_extern__ unsigned long object_index(struct object_file *obj) {
 ...
 * object file list.
 */
__private_extern__ struct object_file *
add_last_object_file(struct object_file *new_object) {
...
remove_last_object_file(struct object_file *last_object) {
  struct object_list *object_list, **p;
  struct object_file *object_file;
 ...
 */
__private_extern__ void print_obj_name(struct object_file *obj) {
  char *strings;

  if (obj->ar_hdr != NULL) {
    print("%s(%.*s) ", obj->file_name, (int)obj->ar_name_size, obj->ar_name);
  } else if (obj->dylib_module != NULL) {
    strings = obj->obj_addr + obj->symtab->stroff;
    print("%s(%s) ", obj->file_name, strings + obj->dylib_module->module_name);
  } else
    print("%s ", obj->file_name);
}

__private_extern__ unsigned long size_ar_name(struct ar_hdr *ar_hdr) {
  long i;
 ...
 */
__private_extern__ void set_obj_resolved_path(struct object_file *obj) {
  ...
  if (obj->resolved_path == NULL) {
    ...
    if (obj->ar_hdr != NULL) {
      obj->resolved_path_len = strlen(resolved_path) + obj->ar_name_size + 2;
 ...
 */
__private_extern__ void print_whatsloaded(void) {
  unsigned long i;
  struct object_list *object_list, **p;
  ...
  for (p = &objects; *p; p = &(object_list->next)) {
    ...
    for (i = 0; i < object_list->used; i++) {
      ...
      if (obj->dylinker)
        continue;
      if (obj->ar_hdr) {
        print("%s(%.*s)\n", obj->file_name, (int)obj->ar_name_size,
              obj->ar_name);
      } else if (obj->dylib_module != NULL) {
        strings = obj->obj_addr + obj->symtab->stroff;
        print("%s(%s)\n", obj->file_name,
...
is_dylib_module_loaded(struct dylib_module *dylib_module) {
  unsigned long i;
  struct object_list *object_list, **p;
  struct object_file *obj;
...
fine_reloc_output_offset(struct section_map *map, unsigned long input_offset) {
  ...
  if (((section_type == S_LAZY_SYMBOL_POINTERS ||
    ...
    return (map->output_section->s.size + input_offset -
            fine_reloc->input_offset);

  return (fine_reloc->output_offset + input_offset - fine_reloc->input_offset);
}
 ...
 */
__private_extern__ unsigned long
fine_reloc_output_address(struct section_map *map, unsigned long input_offset,
      ...
      if (section_map->nfine_relocs == 0)
        ...
        return (section_map->output_section->s.addr +
                fine_reloc_output_offset(
                    section_map, nlists[index].n_value - section_map->s->addr));
    } else if ((map->s->flags & SECTION_TYPE) == S_LAZY_SYMBOL_POINTERS ||
               (map->s->flags & SECTION_TYPE) == S_NON_LAZY_SYMBOL_POINTERS) {
      return (output_base_address + fine_reloc->output_offset + input_offset -
              fine_reloc->input_offset);
 ...
 */
__private_extern__ void fine_reloc_output_ref(struct section_map *map,
 ...
 */
__private_extern__ enum bool
fine_reloc_offset_in_output(struct section_map *map,
                            unsigned long input_offset) {
  struct fine_reloc *fine_reloc;
  unsigned long section_type;
 ...
 */
__private_extern__ enum bool
fine_reloc_offset_in_output_for_output_offset(struct section_map *map,
 ...
 */
__private_extern__ unsigned long
fine_reloc_output_sectnum(struct section_map *map, unsigned long input_offset) {
  struct fine_reloc *fine_reloc;
  struct merged_symbol *merged_symbol;
  unsigned long index;
  ...
  if (fine_reloc->local_symbol == TRUE) {
    if ((map->s->flags & SECTION_TYPE) == S_COALESCED) {
      ...
      index = fine_reloc->output_offset;
    } else if ((map->s->flags & SECTION_TYPE) == S_NON_LAZY_SYMBOL_POINTERS ||
      ...
      index =
          indirect_symtab[map->s->reserved1 + (fine_reloc->input_offset / 4)];
    } else {
 ...
 */
__private_extern__ enum bool fine_reloc_arm(struct section_map *map,
                                            unsigned long input_offset) {
  struct fine_reloc *fine_reloc;
  struct merged_symbol *merged_symbol;
 ...
 * instuction.
 */
__private_extern__ enum bool fine_reloc_thumb(struct section_map *map,
                                              unsigned long input_offset) {
  struct fine_reloc *fine_reloc;
  struct merged_symbol *merged_symbol;
 ...
 */
__private_extern__ enum bool fine_reloc_local(struct section_map *map,
                                              unsigned long input_offset) {
  struct fine_reloc *fine_reloc;
  struct merged_symbol *merged_symbol;
...
fine_reloc_for_input_offset(struct section_map *map,
                            unsigned long input_offset) {
  int l = 0;
 ...
 */
__private_extern__ void clean_objects(void) {
  unsigned long i, j;
  struct object_list *object_list, **p;
  struct object_file *object_file;
#ifndef SA_RLD
  ...
  for (p = &objects; *p; p = &(object_list->next)) {
    ...
    for (i = 0; i < object_list->used; i++) {
      ...
      if (object_file->set_num != cur_set)
        continue;
      if (object_file->ar_hdr == NULL && object_file->from_fat_file == FALSE &&
          object_file->obj_size != 0 && object_file->user_obj_addr == FALSE) {
#ifndef SA_RLD
        if ((r = vm_deallocate(mach_task_self(),
                               (vm_address_t)object_file->obj_addr,
                               object_file->obj_size)) != KERN_SUCCESS)
          mach_fatal(r,
                     "can't vm_deallocate() memory for "
                     "mapped file %s",
 ...
 */
__private_extern__ void remove_objects(void) {
  ...
  for (object_list = objects; object_list != NULL;
 ...
 */
__private_extern__ void print_object_list(void) {
  unsigned long i, j, k;
  struct object_list *object_list, **p;
  struct object_file *object_file;
    ...
    for (i = 0; i < object_list->used; i++) {
      ...
      for (j = 0; j < object_file->nundefineds; j++) {
        print("\t    (%lu,%s)\n", object_file->undefined_maps[j].index,
              object_file->undefined_maps[j].merged_symbol->nlist.n_un.n_name);
      }
#ifdef RLD
      print("\tset_num = %d\n", object_file->set_num);
#endif /* RLD */
 ...
 */
__private_extern__ void print_fine_relocs(struct fine_reloc *fine_relocs,
