...
static void write_on_input_safely(struct arch *archs, uint32_t narchs,
                                  char *input, unsigned short mode);

static void update_load_commands(struct arch *arch, uint32_t *header_size);
...
int main(int argc, char **argv, char **envp) {
  ...
  for (i = 1; i < argc; i++) {
    if (strcmp(argv[i], "-id") == 0) {
      ...
      changes = reallocate(changes, sizeof(struct changes) * (nchanges + 1));
      ...
      for (j = 0; j < nrpaths; j++) {
        if (strcmp(rpaths[j].old, argv[i + 1]) == 0) {
          if (strcmp(rpaths[j].new, argv[i + 2]) == 0) {
            error("\"-rpath %s %s\" specified more than once", argv[i + 1],
                  argv[i + 2]);
            usage();
          }
          error("can't specify both \"-rpath %s %s\" and "
                "\"-rpath %s %s\"",
                rpaths[j].old, rpaths[j].new, argv[i + 1], argv[i + 2]);
          usage();
        }
      ...
      for (j = 0; j < nadd_rpaths; j++) {
        if (strcmp(add_rpaths[j].new, argv[i + 1]) == 0 ||
          ...
          error("can't specify both \"-add_rpath %s\" "
                "and \"-rpath %s %s\"",
                add_rpaths[j].new, argv[i + 1], argv[i + 2]);
          usage();
        }
      }
      for (j = 0; j < ndelete_rpaths; j++) {
        if (strcmp(delete_rpaths[j].old, argv[i + 1]) == 0 ||
          ...
          error("can't specify both \"-delete_rpath %s\" "
                "and \"-rpath %s %s\"",
                delete_rpaths[j].old, argv[i + 1], argv[i + 2]);
          usage();
        }
      }
      rpaths = reallocate(rpaths, sizeof(struct rpaths) * (nrpaths + 1));
      ...
      for (j = 0; j < nrpaths; j++) {
        if (strcmp(rpaths[j].old, argv[i + 1]) == 0 ||
          ...
          error("can't specify both \"-rpath %s %s\" and "
                "\"-add_rpath %s\"",
                rpaths[j].old, rpaths[j].new, argv[i + 1]);
          usage();
        }
      }
      for (j = 0; j < ndelete_rpaths; j++) {
        if (strcmp(delete_rpaths[j].old, argv[i + 1]) == 0) {
          error("can't specify both \"-delete_rpath %s\" "
                "and \"-add_rpath %s\"",
                delete_rpaths[j].old, argv[i + 1]);
          usage();
        }
      }
      add_rpaths =
          reallocate(add_rpaths, sizeof(struct add_rpaths) * (nadd_rpaths + 1));
      ...
      for (j = 0; j < nadd_rpaths; j++) {
        if (strcmp(add_rpaths[j].new, argv[i + 1]) == 0) {
          error("can't specify both \"-add_rpath %s\" "
                "and \"-delete_rpath %s\"",
                add_rpaths[j].new, argv[i + 1]);
          usage();
        }
      }
      delete_rpaths = reallocate(delete_rpaths, sizeof(struct delete_rpaths) *
                                                    (ndelete_rpaths + 1));
  ...
  for (i = 0; i < narchs; i++) {
    if (archs[i].type == OFILE_Mach_O) {
      ...
      if (object->code_sig_cmd != NULL) {
        ...
        char *dataaddr =
            (char *)(object->object_addr + object->code_sig_cmd->dataoff);
        if (codedir_is_linker_signed(dataaddr, datasize)) {
          codedir_create_object(output ? output : input, object->object_addr,
  ...
  if (output != NULL)
    writeout(archs, narchs, output, stat_buf.st_mode & 0777, TRUE, FALSE, FALSE,
             FALSE, FALSE, NULL);
  else
...
static void write_on_input_safely(struct arch *archs, uint32_t narchs,
  ...
  for (i = 0; i < narchs; i++) {
    ...
    if (archs[i].object->mh != NULL) {
      ...
      lc = (struct load_command *)archs[i].object->object_addr + sizeof(mh);
    } else {
      mh64 = archs[i].object->mh64;
      headers_size = sizeof(*mh64) + mh64->sizeofcmds;
      lc = (struct load_command *)archs[i].object->object_addr + sizeof(mh64);
    }
  ...
  writeout(archs, narchs, tmppath, mode, TRUE, FALSE, FALSE, FALSE, FALSE,
           NULL);
...
static void update_load_commands(struct arch *arch, uint32_t *header_size) {
      ...
      for (j = 0; j < nadd_rpaths; j++) {
        if (strcmp(add_rpaths[j].new, path1) == 0) {
          error("for: %s (for architecture %s) option "
                ...
                "already has LC_RPATH for: %s",
                arch->file_name, arch_name, add_rpaths[j].new, path1);
        }
      }
      for (j = 0; j < nrpaths; j++) {
  ...
  for (i = 0; i < ndelete_rpaths; i++) {
    if (delete_rpaths[i].found == FALSE) {
      error("no LC_RPATH load command with path: %s found in: "
            ...
            "\"-delete_rpath %s\"",
            delete_rpaths[i].old, arch->file_name, arch_name,
            delete_rpaths[i].old);
    }
    delete_rpaths[i].found = FALSE;
  }
  for (i = 0; i < nrpaths; i++) {
    if (rpaths[i].found == FALSE) {
      error("no LC_RPATH load command with path: %s found in: "
            "%s (for architecture %s), required for specified option "
            "\"-rpath %s %s\"",
            rpaths[i].old, arch->file_name, arch_name, rpaths[i].old,
            rpaths[i].new);
    }
    rpaths[i].found = FALSE;
  }
  ...
  for (i = 0; i < nadd_rpaths; i++) {
    new_size =
        rnd32(sizeof(struct rpath_command) + (int)strlen(add_rpaths[i].new) + 1,
              cmd_round);
      ...
      for (j = 0; j < nchanges; j++) {
        if (strcmp(changes[j].old, dylib_name1) == 0) {
          ...
          dl_load2->cmdsize = sizeof(struct dylib_command) +
                              rnd32((int)strlen(changes[j].new) + 1, cmd_round);
          dl_load2->dylib.name.offset = sizeof(struct dylib_command);
          dylib_name2 = (char *)dl_load2 + dl_load2->dylib.name.offset;
    ...
    case LC_PREBOUND_DYLIB:
      ...
      for (j = 0; j < nchanges; j++) {
        if (strcmp(changes[j].old, dylib_name1) == 0) {
          ...
          pbdylib2->name.offset = sizeof(struct prebound_dylib_command);
          dylib_name2 = (char *)pbdylib2 + pbdylib2->name.offset;
          strcpy(dylib_name2, changes[j].new);

          pbdylib2->linked_modules.offset =
              sizeof(struct prebound_dylib_command) +
              rnd32((int)strlen(changes[j].new) + 1, cmd_round);
          linked_modules1 = (char *)pbdylib1 + pbdylib1->linked_modules.offset;
          linked_modules2 = (char *)pbdylib2 + pbdylib2->linked_modules.offset;
          memcpy(linked_modules2, linked_modules1, linked_modules_size);
          break;
        }
      }
      if (j >= nchanges) {
   ...
   */
  if (arch->object->mh != NULL) {
    *header_size = sizeof(struct mach_header) + arch->object->mh->sizeofcmds;
    ...
    *header_size =
        sizeof(struct mach_header_64) + arch->object->mh64->sizeofcmds;
