...
static void setup_code_signature(struct arch *arch, struct member *member,
                                 struct object *object);

static struct linkedit_data_command *
add_code_sig_load_command(struct arch *arch, char *arch_name,
                          struct member *member, struct object *object);
...
int main(int argc, char **argv, char **envp) {
  ...
  for (i = 1; i < argc; i++) {
    if (strcmp(argv[i], "-i") == 0) {
      ...
      if (i + 2 == argc) {
        ...
        if (*endp != '\0')
          fatal("size for '-a %s %s' not a proper number", argv[i + 1],
                argv[i + 2]);
        if ((arch_signs[narch_signs].datasize % 16) != 0)
          fatal("size for '-a %s %s' not a multiple of 16", argv[i + 1],
                argv[i + 2]);
      ...
      if (i + 3 == argc) {
        ...
        if (*endp != '\0')
          fatal("cputype for '-A %s %s %s' not a proper number", argv[i + 1],
                argv[i + 2], argv[i + 3]);
        ...
        if (*endp != '\0')
          fatal("size for '-A %s %s %s' not a proper number", argv[i + 1],
                argv[i + 2], argv[i + 3]);
        if ((arch_signs[narch_signs].datasize % 16) != 0)
          fatal("size for '-A %s %s %s' not a multiple of 16", argv[i + 1],
                argv[i + 2], argv[i + 3]);
  ...
  if (input == NULL || output == NULL || (narch_signs == 0 && rflag == FALSE))
    usage();
  ...
  for (i = 0; i < narch_signs; i++) {
    if (arch_signs[i].found == FALSE)
      fatal("input file: %s does not contain a matching architecture "
            "for specified '-a %s %u' option",
            input, arch_signs[i].arch_flag.name, arch_signs[i].datasize);
  }

  writeout(archs, narchs, output, 0777, TRUE, FALSE, FALSE, FALSE, FALSE, NULL);
    ...
    if (archs[i].type == OFILE_ARCHIVE) {
      ...
      for (j = 0; j < archs[i].nmembers; j++) {
        ...
        if (archs[i].members[j].member_long_name == TRUE) {
          size = rnd32(archs[i].members[j].member_name_size, sizeof(long));
...
static void setup_code_signature(struct arch *arch, struct member *member,
  ...
  if (object->st != NULL && object->st->nsyms != 0) {
    ...
    if (object->mh != NULL) {
      object->input_sym_info_size =
          object->st->nsyms * sizeof(struct nlist) + object->st->strsize;
    } else {
      object->input_sym_info_size =
          object->st->nsyms * sizeof(struct nlist_64) + object->st->strsize;
    }
  ...
  if (rflag || (i < narch_signs && arch_signs[i].datasize == 0)) {
    ...
    if (object->code_sig_cmd != NULL) {
      ...
      if (object->seg_linkedit != NULL) {
        ...
        object->seg_linkedit64->filesize -= old_align_delta;
  ...
  if (object->code_sig_cmd != NULL) {
    ...
    if (object->seg_linkedit != NULL) {
      ...
      object->seg_linkedit64->filesize -= old_align_delta;
      object->seg_linkedit64->filesize -= object->code_sig_cmd->datasize;
      object->seg_linkedit64->filesize += align_delta;
    ...
    object->output_sym_info_size +=
        align_delta + object->code_sig_cmd->datasize;
  }
  /*
   ...
   */
    ...
    object->code_sig_cmd = add_code_sig_load_command(
        arch, arch_signs[i].arch_flag.name, member, object);
    object->code_sig_cmd->datasize = arch_signs[i].datasize;
    if (object->seg_linkedit != NULL)
      linkedit_end =
          object->seg_linkedit->fileoff + object->seg_linkedit->filesize;
    else if (object->seg_linkedit64 != NULL)
      ...
      fatal("can't allocate code signature data for: %s (for "
            "architecture %s) because file does not have a " SEG_LINKEDIT
            " segment",
            arch->file_name, arch_signs[i].arch_flag.name);
    ...
    if (object->st != NULL && pflag) {
      object->code_sig_cmd->dataoff = rnd32(linkedit_end, segalign);
    ...
    object->output_sym_info_size = align_delta + object->output_sym_info_size;
...
add_code_sig_load_command(struct arch *arch, char *arch_name,
    ...
    if (lc->cmd == LC_SEGMENT) {
      ...
      if (sg->nsects != 0) {
        for (j = 0; j < sg->nsects; j++) {
          if (s->size != 0 && (s->flags & S_ZEROFILL) != S_ZEROFILL &&
              (s->flags & S_THREAD_LOCAL_ZEROFILL) != S_THREAD_LOCAL_ZEROFILL &&
      ...
      if (sg64->nsects != 0) {
        for (j = 0; j < sg64->nsects; j++) {
          if (s64->size != 0 && (s64->flags & S_ZEROFILL) != S_ZEROFILL &&
  ...
  if (sizeofcmds + sizeof(struct linkedit_data_command) + sizeof_mach_header >
      low_fileoff) {
    if (member)
      fatal("can't allocate code signature data for: %s(%s) (for "
            ...
            "-headerpad value)",
            arch->file_name, member->member_name, arch_name);
    else
      fatal("can't allocate code signature data for: %s (for "
            "architecture %s) because larger updated load commands "
            "do not fit (the program must be relinked using a larger "
   ...
   */
  code_sig = (struct linkedit_data_command *)((char *)object->load_commands +
                                              sizeofcmds);
  ...
  if (object->mh != NULL) {
    object->mh->sizeofcmds = sizeofcmds + sizeof(struct linkedit_data_command);
...
// codesign_allocate use libstuff.a which calls some functions from
// libcodirectory.dylib The code paths codesign_allocate takes in libstuff.a
// never use libcodirectory.dylib but it must be linked.  MRM copies the
// codesign_allocate tool to a new location, which then won't run because
// libcodirectory.dylib does not exist. This is a hack so that codesign_allocate
// does not need to link with libcodirectory.dylib
